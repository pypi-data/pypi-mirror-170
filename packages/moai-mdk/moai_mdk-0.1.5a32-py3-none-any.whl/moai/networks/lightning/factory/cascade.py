...
class Cascade(minet.FeedForward):
    def __init__(self,
        ...
        self.mods = torch.nn.ModuleDict(OrderedDict(
        ...
        for name, module in self.mods.items():
            ...
            mod_in = list(zip(*[mirtp.force_list(configuration[name][prop]) for prop in params if configuration[name][prop] is not None]))
            ...
            mod_out = mirtp.split_as(mirtp.resolve_io_config(configuration[name]['out']), mod_in)
            ...
            for keys in mod_in: #TODO: check if feedforward to ignore filler
                self.mod_fwds[name].append(lambda td,
                    ...
                    args=params.keys(), 
                    mod=module,
                    filler=next(get_mod_filler):
                        filler(td, mod(**dict(zip(args,
                            list(
