...
class Distance(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if 'vector' in mode:
            self.distance_func = functools.partial(torch.linalg.vector_norm, keepdim=True, dim=dim)
        elif 'matrix' in mode:
            self.distance_func = functools.partial(torch.linalg.matrix_norm, keepdim=True, dim=dim)
        self.order = order
