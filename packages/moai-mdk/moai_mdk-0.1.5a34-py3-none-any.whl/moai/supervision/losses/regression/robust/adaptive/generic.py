...
def loss(
  ...
  """
  if approximate: # an approximate and faster form of the loss but inaccurate when x and alpha are near zero.
    ...
    loss_neginf = -torch.expm1(-0.5 * squared_scaled_x) # The loss when alpha == -infinity.    
    loss_posinf = util.expm1_safe(0.5 * squared_scaled_x) # The loss when alpha == +infinity.
    # The loss when not in one of the above special cases.
    machine_epsilon = torch.tensor(np.finfo(np.float32).eps).to(residual)    
    beta_safe = torch.max(machine_epsilon, torch.abs(alpha - 2.)) # Clamp |2-alpha| to be >= machine epsilon so that it's safe to divide by.    
    alpha_safe = torch.where( # Clamp |alpha| to be >= machine epsilon so that it's safe to divide by.
        alpha >= 0, 
        torch.ones_like(alpha),
        -torch.ones_like(alpha)) * torch.max(machine_epsilon, torch.abs(alpha)
    )
