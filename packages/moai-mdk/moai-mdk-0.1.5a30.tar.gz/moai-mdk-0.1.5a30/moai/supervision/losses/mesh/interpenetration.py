...
class Interpenetration(torch.nn.Module):
    def __init__(self,
        ...
        ignore_part_pairs:  typing.Sequence[str]=["9,16", "9,17", "6,16", "6,17", "1,2", "12,22"],
    ):
        super(Interpenetration, self).__init__()
        if not torch.cuda.is_available():
            log.error('The interpenetration loss cannot be used without any CUDA devices.')
        ...
        if part_segmentation:
            if not os.path.exists(part_segmentation):
                log.error(f"Part segmentation file ({part_segmentation}) given to interpenetration loss does not exist.")            
            ...
            self.filter_faces = FilterFaces(# Create the module used to filter invalid collision pairs
    ...
    def forward(self, 
        ...
        return self.pen_distance(triangles, collision_idxs) \
