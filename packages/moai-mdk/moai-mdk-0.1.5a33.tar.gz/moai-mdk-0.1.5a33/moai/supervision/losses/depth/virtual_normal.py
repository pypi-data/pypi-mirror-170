...
class VirtualNormal(torch.nn.Module):
    ...
    def form_pw_groups(self, 
        p123:       typing.Dict[str, torch.Tensor],
        pw:         torch.Tensor
    ) -> torch.Tensor:
        """
    ...
    def filter_mask(self, 
        ...
        pw_diff = torch.cat([pw12[:, :, :, np.newaxis], pw13[:, :, :, np.newaxis], pw23[:, :, :, np.newaxis]], 3)  # [b, n, 3, 3]
    ...
    def select_points_groups(self, 
        ...
        mask, pw_groups_gt = self.filter_mask(p123, pw_gt,
            delta_cos=0.867, delta_diff_x=0.005, 
            delta_diff_y=0.005, delta_diff_z=0.005
        ) # [b, n, 3, 3]
