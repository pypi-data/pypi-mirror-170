...
class Blend2d(Callable):
    ...
    def __init__(self,
        ...
        self.formats = [ensure_choices(log, "output format", ext, Blend2d.__FORMATS__) for ext in extensions]
        self.mode = mode
        self.scale = scale
        self.index = 0
    ...
    def __call__(self, 
        ...
        for l, r, b, t, c, f in zip(self.left, self.right, self.blending, 
            self.transforms, self.colormaps, self.formats):
                ...
                bs = self._save_color(self.folder, 
                    left * b + (1.0 - b) * self.colorize_map[c](
                        self.transform_map[t](tensors[r])
                    ), n, self.index, f, self.scale
                )
