...
def plot_time_frequency_responses(dataframe: pd.DataFrame = None,
    ...
    if ylabel is None:
        fig_out.supylabel('Amplitude [{:}]'.format(_row['y_unit']))
    else:
        fig_out.supylabel(ylabel)
    if _domain == 'time':
        fig_out.supxlabel('Time [{:}]'.format(_row['x_unit']))
    if _domain == 'frequency':
        fig_out.supxlabel('Frequency [{:}]'.format(_row['x_unit']))
...
def plot_topographic_maps(dataframe: pd.DataFrame = None,
    ...
    min_sub = sub_groups.potentials.apply(lambda x: np.ma.mean(x, axis=2)).apply(np.min).min()
