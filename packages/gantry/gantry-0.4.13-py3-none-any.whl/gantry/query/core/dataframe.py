...
def _raw_query_cached_request(
    ...
    return api_client.request(
        "POST", f"/api/v1/{resource}/query", json=json.loads(dumped_json_data)
    )
...
class GantrySeries(GantryQueryFrame):
    ...
    def _aggregate_query(self, attribute, num_points: int = 1, **attribute_kwargs):
        ...
        response = _raw_query_cached_request(
            self.api_client, json.dumps(data, default=lambda x: x.isoformat()), resource=resource
        )
        try:
            ...
            raise GantryException(
                "Aggregation query [" + attribute + "] failed on API with this error: " + str(e)
            )
