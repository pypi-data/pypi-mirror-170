...
@pytest.mark.parametrize("status_code", [200, 400, 401, 404, 500])
...
@mock.patch("backoff._sync._call_handlers")
def test_backoff_for_send_batch_as_raw_events(mock_handler):
    with patch("gantry.logger.stores.APIClient") as mock_client:
        instance = mock_client.return_value
        instance.request.side_effect = [requests.Timeout() for __ in range(BATCH_UPLOAD_MAX_TRIES)]
        ...
        try:
            ...
            pass
        assert mock_handler.call_count == BATCH_UPLOAD_MAX_TRIES
