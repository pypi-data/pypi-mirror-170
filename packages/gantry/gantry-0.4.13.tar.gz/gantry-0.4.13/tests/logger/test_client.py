...
def test_log_predictions_as_batch(
    mock_batch_success, mock_uuid, version, sort, method, init_gantry_with_api
):
    ...
    with responses.RequestsMock() as resp:
        ...
        resp.add(
            resp.PUT,
            "http://test_url",
            headers={"Content-Type": "application/json", "ETag": "working_etag"},
        )
        resp.add(
            resp.POST,
        ...
        if method == "log_predictions":
            ...
            init_gantry_with_api.log_records(
                "simple_test",
                version,
                inputs=inputs,
                outputs=outputs,
...
@mock.patch("gantry.logger.event_builder.uuid.uuid4")
def test_batch_completion_fail(mock_uuid, init_gantry_with_api):
    size = 20
