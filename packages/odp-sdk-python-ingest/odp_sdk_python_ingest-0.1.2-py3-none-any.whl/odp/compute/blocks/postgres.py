import asyncio
import logging
from typing import cast, Optional, Tuple, Union, Coroutine
...
class Postgres(Block):
    ...
    validate_connection: bool = Field(
        description="If true, connection will be validated when the block is initialized",
        default=True,
    )
    ...
    def block_initialization(self) -> None:
        ...
        if len(path_segments) > 1:
            raise ValueError(
                "A database connection string expects the path to be the database name, so can only have a single part/segment"
            )
