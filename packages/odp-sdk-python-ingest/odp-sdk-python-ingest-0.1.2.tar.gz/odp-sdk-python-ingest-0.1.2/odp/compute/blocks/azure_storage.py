...
class AzureStorage(Block):
    ...
    See [Azure Docs](https://learn.microsoft.com/en-us/python/api/azure-storage-blob/azure.storage.blob.containerclient?view=azure-python)
    for more details.
    """

    _block_type_name = "Azure Storage"
    _logo_url = "https://images.ctfassets.net/gm98wzqotmnx/6AiQ6HRIft8TspZH7AfyZg/39fd82bdbb186db85560f688746c8cdd/azure.png?h=250"  # type: ignore
    ...
    assert_exists: bool = Field(
        description="If true, the block initialization will check whether the container exists, and raise and assertion error if it doesn't",
        default=True,
    )
    ...
    def block_initialization(self) -> None:
        ...
        if self.assert_exists and not client.exists():
            raise AssertionError(
                "The container '{}' does not exist".format(client.container_name)
            )
    ...
    def blob_client(
        ...
        if blob_url:
            return BlobClient.from_blob_url(
                blob_url=blob_url, credential=self._get_credential(), snapshot=snapshot
            )
