...
def run(
    ...
    run_pipeline(pipeline_path, fn, param, verbose, debug)


def run_pipeline(
    pipeline_path: str, fn: str, parameters: Dict[str, str], verbose: bool, debug: bool
) -> None:
    ...
    inspect.signature(flow.fn).parameters

    flow_parameters = set(inspect.signature(flow.fn).parameters)

    for param, value in parameters.items():

        if not param in flow_parameters:
            raise ValueError(
                f"The parameter '{param}' is not defined in the flow '{flow.name}'"
            )
