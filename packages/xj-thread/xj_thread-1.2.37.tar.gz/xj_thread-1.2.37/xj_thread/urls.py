...
urlpatterns = [
    ...
    re_path(r'^category_tree/?(?P<category_value>\w+)?/?$', ThreadCategoryTreeAPIView.as_view(),
            name='thread_category_tree'),
    re_path(r'^classify_list/?$', ClassifyListAPIView.as_view(), name='classify_list'),  # 分类列表
    re_path(r'^classify_tree/?(?P<classify_value>\w+)?/?$', ThreadClassifyTreeAPIView.as_view(),
    ...
    re_path(r'^list/?(?P<category_value>\w+)?/?$', ThreadListAPIView.as_view(), name='list'),  # 信息列表/新增
