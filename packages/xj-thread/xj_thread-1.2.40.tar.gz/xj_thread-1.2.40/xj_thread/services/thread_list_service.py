...
class ThreadListService:
    ...
    def list(params):
        ...
        try:
            ...
            if is_admin:
                ...
                has_auth = False
                if conditions["user_id__not_in"] == []: # 这种情况等价于所有人可见
                    has_auth = True
                ...
                if has_auth:  # token 验证成功
                    ...
                    # print("query:", thread_set.query)
                    thread_set = thread_set.exclude(category_id__need_auth=True)
                    # 边界写法错误，need_auth是别名，不能使用别名做为索引条件，会导致布尔类型判断null/0失作用
                    # thread_set = thread_set.filter(Q(**conditions) & ~Q(need_auth=True))
                    thread_set = thread_set.filter(**conditions)
    ...
    def search(id_list, need_map=False):
        ...
        # 开始按过滤条件
        try:
            thread_set = thread_set.annotate(thread_category_value=F("category_id__value")) \
                .annotate(need_auth=F("category_id__need_auth")) \
