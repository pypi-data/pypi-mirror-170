...
class DataBaseWorker:
    def __init__(
        ...
        self.logger.info("try to connect to {}, host {}...".format(self.db, self.cred["host"]))
        self.conn = self.engine.connect()
        self.logger.info("connecting to {}, host {} is successful".format(self.db, self.cred["host"]))

    def __del__(self):
        if self.conn is not None:
            self.logger.info("Connecting to {}, host {} is closed".format(self.db, self.cred["host"]))
            self.conn.close()
            self.engine.dispose()
