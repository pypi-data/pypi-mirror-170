...
def _safe_modulo(num: numpy.typing.NDArray,
                 denom: float,
                 decimals: int
                 ) -> numpy.typing.NDArray:
    """"""
    # The difference between longitude values read from the input file might be
    # very small values: having such values might cause underflow errors when
    # calling the numpy implementation of modulo.
    # As a workaround, since we know that longitude values are not meant to
    ...
    _values = numpy.mod(UFLOW_FIX * numpy.round(num, decimals=decimals),
                        UFLOW_FIX * denom)
    values: numpy.typing.NDArray = _values / UFLOW_FIX
    return values
...
def _fix_swath_longitudes(_lon: numpy.typing.NDArray,
    ...
    lon = numpy.mod(_lon + 180.0, 360.0) - 180.0

    # Find acrosstrack main direction
    ac0_dlon = lon[0, 1:] - lon[0, :-1]
    ac0_dlon = _safe_modulo(ac0_dlon, 360.0, decimals)
    ac0_dlon = numpy.mod(ac0_dlon + 180.0, 360.0) - 180.0
    ac0_dlon_trend = numpy.sum(numpy.sign(ac0_dlon))
    ...
    for i in range(0, nac):
        ...
        al_dlon = _safe_modulo(al_dlon, 360.0, decimals)
