...
from localstack_ext.bootstrap.pods.pods_api_types import DEFAULT_MERGE_STRATEGY,GetStatusResponse,GetStatusVerboseResponse,MergeStrategy,NotifyOperationType,PodMeta,StateMergeRequest,StateResetRequest,StatusGetRequest,StatusNotifyEventRequest
...
class CloudPodsManager(metaclass=ABCMeta):
	...
	def restart_container():
		LOG.info('Restarting LocalStack instance with updated persistence state - this may take some time ...');B={'action':'restart'};A=f"{config.get_edge_url()}/health"
...
class CloudPodsVersionManager(CloudPodsManager):
	...
	def pull(A,inject=_B,merge_strategy=_A):
		...
		if inject:LOG.info('Injecting the cloud pod state into the running instance');A.inject(version=-1,merge_strategy=C)
	...
	def deploy_pod_into_instance(pod_name,pod_version,pod_path,merge_strategy):
		...
		try:
			I=load_file(A,mode='rb');G=get_pods_endpoint();J={_D:pod_name,'pod_version':pod_version,'merge_strategy':merge_strategy};C=requests.post(G,data=I,params=J,timeout=POD_INJECT_CLI_TIMEOUT)
	...
	def inject(A,version=-1,merge_strategy=_A):
		...
		try:
			...
			if not B and C<=Version.DEFAULT_INITIAL_VERSION_NUMBER:B=new_tmp_file();F=A.pods_api.get_state_archive_from_state_files();save_file(B,F);E=_B
	...
	def list_pods(C,only_local):
		A={A:{PodLocation.LOCAL.value}for A in C.pods_api.list_locally_available_pods()}
		if only_local:return A
		A=A or defaultdict(set);D=C.remote.list_pods()
		for B in D or[]:E=B.get(_D)if isinstance(B,dict)else B;A.setdefault(E,set()).add(PodLocation.REMOTE.value)
		return A
	@property
	def remote(self):
...
def delete_pod(pod_name,remote=_B):A=get_pods_manager(pods_name=pod_name);B=A.delete(remote=remote);return B
def register_remote(pod_name,ci_pod=_A,**C):A=get_pods_manager(pods_name=pod_name);B=A.register_remote(ci_pod=ci_pod);return B
...
def push_state(pod_name,comment=_A,register=_C,services=_A,**F):
	D=comment;C=pod_name;B=services
	if B is _A:B=[]
	A=get_pods_manager(pods_name=C)
	if not A.pods_api.config_context.is_initialized():A.init()
	A.pods_api.set_pod_context(pod_name=C);A.push(comment=D,services=B);E=_B;notify_status_event(StatusNotifyEventRequest(operation=NotifyOperationType.PUSH,pod_meta=PodMeta(pod_name=C,pod_version=A.pods_api.get_max_version_no()-1),services=B or _A,message=D))
	if register:E&=A.register_remote()
	return E
def get_pods_endpoint():A=config.get_edge_url();return f"{A}{API_PATH_PODS}"
def pull_state(pod_name,inject=_B,merge_strategy=_A,**C):
	A=pod_name
	if not A:raise Exception('Need to specify a pod name')
	B=get_pods_manager(pods_name=A);B.pull(inject=inject,merge_strategy=merge_strategy);print('Done.')
...
def export_pod(pod_name,target_dir=_A):B=pod_name;A=target_dir;A=A or'.';D=os.path.join(A,'cloudpods.yml');C=os.path.join(A,f"{B}.zip");E=get_pods_manager(pods_name=B);F=E.pods_api.config_context.get_pod_root_dir();G={'pods':[{'name':B,'location':C}]};H=yaml.dump(G);save_file(D,H);create_zip_file_python(base_dir=F,zip_file=C);LOG.info('Successfully exported pods content to %s and %s',D,C)
