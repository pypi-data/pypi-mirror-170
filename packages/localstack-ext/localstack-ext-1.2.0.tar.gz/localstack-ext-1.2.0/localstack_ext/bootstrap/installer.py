...
_K='plpython3'
_J='postgres11'
_I='Error while installing package %s.'
_H='mosquitto'
_G='redis-server'
_F='/tmp/timescaledb'
_E='git'
...
from localstack.utils.platform import in_docker,is_debian
...
@lru_cache()
def is_redhat():return'rhel'in load_file('/etc/os-release','')
...
class PackageInstaller(ABC):
	...
	def install(A,raise_on_error=_B):
		try:
			...
			elif A.logger.isEnabledFor(logging.DEBUG):A.logger.exception(_I,A.log_package_name)
...
class DebianPackageInstaller(PackageInstaller,ABC):
	...
	def _install_os_packages(B,packages):
		...
		with INSTALL_LOCK:C=B._download_os_packages(A);D=B.cmd_prefix(C)+[_C]+A;run(D)
	def _download_os_packages(C,packages):
		...
		with INSTALL_LOCK:
			...
			try:E=C.cmd_prefix(B)+['-d',_C]+A;run(E)
...
class RedHatPackageInstaller(PackageInstaller,ABC):
	...
	def _install_os_packages(B,packages):
		A=packages;A=A if isinstance(A,List)else[A]
		with INSTALL_LOCK:run(['dnf',_C,'-y']+A)
...
class DebianPostgres11Installer(DebianPackageInstaller):
	def __init__(A):super(DebianPostgres11Installer,A).__init__(_J)
...
class RedHatPostgres11Installer(RedHatPackageInstaller):
	def __init__(A):super(RedHatPostgres11Installer,A).__init__(_J)
	def _check_if_available(A):return is_command_available('psql')
...
class DebianPlPythonInstaller(DebianPackageInstaller):
	def __init__(A):super(DebianPlPythonInstaller,A).__init__(_K)
...
class RedHatPlPythonInstaller(RedHatPackageInstaller):
	def __init__(A):super(RedHatPlPythonInstaller,A).__init__(_K)
...
postgres_installer=OSSpecificPackageInstaller(debian_installers=MultiPackageInstaller(_L,[DebianPostgres11Installer(),DebianPlPythonInstaller()]),redhat_installers=MultiPackageInstaller(_L,[RedHatPostgres11Installer(),RedHatPlPythonInstaller()]))
class DebianMariaDBInstaller(DebianPackageInstaller):
	def __init__(A):super(DebianMariaDBInstaller,A).__init__(_M)
	def _check_if_available(A):return is_command_available(_N)
...
class RedHatMariaDBInstaller(RedHatPackageInstaller):
	def __init__(A):super(RedHatMariaDBInstaller,A).__init__(_M)
	def _check_if_available(A):return is_command_available(_N)
...
class DebianTimescaleDBInstaller(DebianPackageInstaller):
	def __init__(A):super(DebianTimescaleDBInstaller,A).__init__(_O)
	def _check_if_available(A):return os.path.exists(f"{DEBIAN_POSTGRES_LIB_FOLDER}/timescaledb.so")
	def _install_package(D):C='build';B='make';D._install_os_packages([_D,'cmake',_D,_E]);A=_F;E=_P;run([_E,'clone','https://github.com/timescale/timescaledb.git'],cwd='/tmp');run([_E,'checkout',E],cwd=A);run([os.path.join(A,'bootstrap'),'-DREGRESS_CHECKS=OFF'],cwd=A);run([B],cwd=os.path.join(A,C));run([B,_C],cwd=os.path.join(A,C));rm_rf(A)
class RedHatTimescaleDBInstaller(RedHatPackageInstaller):
	def __init__(A):super(RedHatTimescaleDBInstaller,A).__init__(_O)
	def _check_if_available(A):return os.path.exists(f"{REDHAT_POSTGRES_LIB_FOLDER}/timescaledb.so")
	def _install_package(A):A._install_os_packages([_D,'cmake',_D,_E,'redhat-rpm-config']);B=_F;C=_P;run('cd /tmp; git clone https://github.com/timescale/timescaledb.git');run('cd %s; git checkout %s; ./bootstrap -DREGRESS_CHECKS=OFF; cd build; make; make install'%(B,C));rm_rf(_F)
...
class DebianMosquittoInstaller(DebianPackageInstaller):
	def __init__(A):super(DebianMosquittoInstaller,A).__init__(_Q)
	def _check_if_available(A):return is_command_available(_H)
	def _install_package(A):A._install_os_packages(_H)
class RedHatMosquittoInstaller(RedHatPackageInstaller):
	def __init__(A):super(RedHatMosquittoInstaller,A).__init__(_Q)
	def _check_if_available(A):return is_command_available(_H)
