...
class PyJAMAS(QtCore.QObject):
    ...
    def replacePolyline(self, index: int, coordinates: list, z: int, paint: bool = True,
        ...
        if pushundo:
            self.undo_stack.push(
                {'changetype': undo_modes.POLYLINE_DELETED, 'frame': z, 'index': index, 'details': old_poly})
            self.undo_stack.push(
                {'changetype': undo_modes.POLYLINE_ADDED, 'frame': z, 'index': index,
                 'details': None})
