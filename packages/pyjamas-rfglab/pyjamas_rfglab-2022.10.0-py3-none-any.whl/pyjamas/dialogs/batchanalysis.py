...
class BatchMeasureDialog(object):
    ...
    normalization_modes: List[str] = ['none', 'photobleaching (divide by image mean)',
                                      'image background and photobleaching (subtract image mode and divide by image mean)',
                                      'sample background and photobleaching (autodetect sample, subtract sample mode and divide by sample mean)',
                                      'file (load additional image, subtract image mode and divide by image sample)']
