...
tst_driver_meas_P_UL = 31.5e3
tst_driver_meas_P_LL = 26.7e3
tst_driver_meas_P_UR = 26.6e3
tst_driver_meas_P_LR = 31.6e3
tst_driver_DC_gain_VpV_HV = 40
tst_driver_DC_gain_VpV_LV = 1.881
anti_imaging_rate_string = 16k
anti_imaging_method = biquad
...
'''
        ...
        self.known_correction = np.array(
            [0.08050209475040816-1.6513476841862913j,
             5.520955244198903-5.39432597631647j,
             42.86576723694163-12.95206706562494j,
             286.2976701235751-29.870138626022648j,
             1989.2449225789353+25.625179916953805j,
             13324.518167327555-256.1410730974964j,
             83043.27111119543+8600.650901100558j,
             550117.5295480754+134754.87628064284j,
             3020136.055721985+2211463.562530411j,
             -2232612.4359687367+21856221.21387424j])
    ...
    def test_deltal_ext_pcal_corrrection(self):
        calcs = pydarm.calcs.CALCSModel(self.model_string)
        deltal_ext_pcal = calcs.deltal_ext_pcal_corrrection(self.frequencies)
        for n in range(len(self.frequencies)):
            self.assertAlmostEqual(
