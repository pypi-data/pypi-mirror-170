...
def get_apr(date, pct_change, volume_df = None, current_apr=None):
    ...
    ratio = volume_df.loc[date].volumeToken1 / volume_df.iloc[-1].volumeToken1
...
def get_swapping_cost(swapping_size_proportion =None, swapping_cost_map=None):
    ...
    try:
        ...
        print(e)
    return cost
def LS_IL_boundaries_with_signal(data_df = None, plotHtml = False, me_pair = ('ETH', 'BTC'), bb_periods=[5, 15, 30, 55], stdNbrs=[2., 3., 5.], distance_to_bound_percs = [0,5,10,15,20,30,40],current_apr_ethbtc=None, bypass_signal=True, bypass_stoploss=True, swapping_cost_map = {1:1e-4, 10:0.01, 50:0.05, 75:0.075, 100:0.1}):
    ssj = me_pair[0]
    ssj_against = me_pair[1]
    objectives = []
        ...
        for stdNbr in stdNbrs:
            for distance_to_bound_perc in distance_to_bound_percs:
                ...
                for i, row in df.iterrows():
                    ...
                    apr = get_apr(row['DateStr'],  row['pct_change'], volume_df=data_df.copy(), current_apr_ethbtc=current_apr_ethbtc)
                ...
                rebalancing_df = stages_df[stages_df['rebalancing']]
                if len(rebalancing_df)>0:
                    ...
                    starting_number_of_bitcoins = 10
                    ...
                    for ii, row in rebal_data_df.iterrows():
                        if ii == 0 :
                            ### btc at initial date
                            previous_btc = starting_number_of_bitcoins
                        ...
                        if deltaBTC >0: #### we have more btcs than eth
                            ...
                            def get_eth_matching_proportion(btc_amount=None, current_price=None, lower_bound=None, upper_bound=None):
                            ...
                            new_eth, _ = get_eth_matching_proportion(btc_amount = after_withdrawal_btc, current_price=newly_initial_price, lower_bound=new_real_lower_bound, upper_bound = new_real_upper_bound)
                            ...
                            def get_btc_matching_proportion(eth_amount=None, current_price=None, lower_bound=None, upper_bound=None):
                            ...
                            new_btc, _ = get_btc_matching_proportion(eth_amount = after_withdrawal_eth, current_price=newly_initial_price, lower_bound=new_real_lower_bound, upper_bound = new_real_upper_bound)
                            pool_ratio_price = new_btc/new_eth
                            print(f'new ratio {pool_ratio_price}')
                ...
                rebal_data_df[['previous_btcs','previous_ethers','previous_liquidities','new_btcs','new_ethers','new_liquidities']]
                ...
                all_df = pd.merge(df.copy(),  stages_rebal_df.copy(), how = 'left', left_index=True, right_index=True)

                all_df['swapping_cost'] = all_df['swapping_cost'].fillna(0.)

                all_df['new_btcs'] = all_df['new_btcs'].ffill()
                all_df['new_ethers'] = all_df['new_ethers'].ffill()
                all_df['new_liquidities'] = all_df['new_liquidities'].ffill()
                all_df['swapping_sizes'] = all_df['swapping_sizes'].ffill()

                all_df['new_btcs'] = all_df['new_btcs'].bfill()
                all_df['new_ethers'] = all_df['new_ethers'].bfill()
                all_df['new_liquidities'] = all_df['new_liquidities'].bfill()
                all_df['swapping_sizes'] = all_df['swapping_sizes'].bfill()
                ...
                all_df['mark_to_market_loss_v3'] = 100. * (1.+all_df['curILv3'].values)
                all_df['mark_to_market_loss_v2'] = 100. * (1.+all_df['curILv2'].values)

                all_df['mark_to_market_lp_value'] = 100. * (1.+all_df['curlp_position_profit'].values)
                ...
                if plotHtml:
                    fig = realtime_plotting_utility.plot_multiple_time_series(
                        ...
                        put_on_same_scale=False,
                        title=f'swapping costs over time{ssj}{ssj_against}')
                    fig.show()
                    fig = realtime_plotting_utility.plot_multiple_time_series(
                    ...
                    fig = realtime_plotting_utility.plot_multiple_time_series(
                        ...
                        put_on_same_scale=False,
                        title=f'swapping sizes over time{ssj}{ssj_against}')
                    fig.show()
                    fig = realtime_plotting_utility.plot_multiple_time_series(
                        data_df=all_df[['new_btcs', 'new_ethers', 'new_liquidities']].copy(), logy=False, split=False,
                        put_on_same_scale=False,
                        title=f'assets over time{ssj}{ssj_against}')
                    fig.show()
                    fig = realtime_plotting_utility.plot_multiple_time_series(
                        data_df=all_df[['new_btcs']].copy(), logy=False, split=False,
                        put_on_same_scale=False,
                        title=f'BTCs amount over time{ssj}{ssj_against}')
                    fig.show()
                    fig = realtime_plotting_utility.plot_multiple_time_series(
                        data_df=all_df[['new_ethers']].copy(), logy=False, split=False,
                        put_on_same_scale=False,
                        title=f'ethers amount over time{ssj}{ssj_against}')
                    fig.show()
                    fig = realtime_plotting_utility.plot_multiple_time_series(
                        data_df=all_df[['new_liquidities']].copy(), logy=False, split=False,
                        put_on_same_scale=False,
                        title=f'liquidities over time{ssj}{ssj_against}')
                    fig.show()
                    fig = realtime_plotting_utility.plot_multiple_time_series(
                    ...
                    fig = realtime_plotting_utility.plot_multiple_time_series(
                        ...
                        put_on_same_scale=False,
                        title=f'v3 IL over time{ssj}{ssj_against}')
                    fig.show()
                    fig = realtime_plotting_utility.plot_multiple_time_series(
                        data_df=all_df[['curlp_value_first']].copy(), logy=False, split=False,
                        put_on_same_scale=False,
                        title=f'LP position value over time{ssj}{ssj_against}')
                    fig.show()
                    fig = realtime_plotting_utility.plot_multiple_time_series(
                    ...
                    fig1 = realtime_plotting_utility.plot_multiple_time_series(
                        ...
                        data_df=all_df[['gain_and_value_LP_LS', 'gain_and_value_LP_LO']].copy(),
                        logy=False, split=False,
                        put_on_same_scale=False,
                        title=f'v3 liquidity providing {ssj}{ssj_against}')
                    fig1.show()
                ...
                kpi_df = riskmetrics.get_kpi(all_df[[f'gain_and_value_LP_LS']])
                obf_data= {
                    'bb_period': bb_period,
                    'stdNbr': stdNbr,
                    'distance_to_bound_perc': distance_to_bound_perc,
                ...
                obf_data.update(kpi_df.to_dict()['gain_and_value_LP_LS'])
