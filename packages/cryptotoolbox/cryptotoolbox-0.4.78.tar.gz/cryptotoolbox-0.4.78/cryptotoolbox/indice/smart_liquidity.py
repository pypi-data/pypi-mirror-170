    ...
    for bb_period in bb_periods:
        for stdNbr in stdNbrs:
            for distance_to_bound_perc in distance_to_bound_percs:
                ...
                all_df['gain_and_value_LP_LO_mark_to_market'] = 100. * np.cumprod(1 + all_df['mark_to_market_lp_value_returns'].values + all_df['apr'].values - all_df['swapping_cost'].values)
                ...
                if plotHtml:
                    ...
                    fig1 = realtime_plotting_utility.plot_multiple_time_series(
                        #                    data_df=all_df[['loss_v2', 'loss_v3', 'gain_and_loss_v3','gain_and_loss_LS']].copy(),
                        data_df=all_df[['gain_and_value_LP_LO_mark_to_market', 'gain_and_value_LP_LO']].copy(),
                        logy=False, split=False,
                        put_on_same_scale=False,
                    ...
                    fig1 = realtime_plotting_utility.plot_multiple_time_series(
                        #                    data_df=all_df[['loss_v2', 'loss_v3', 'gain_and_loss_v3','gain_and_loss_LS']].copy(),
                        data_df=all_df[['gain_and_value_LP_LO']].copy(),
                        logy=False, split=False,
                        put_on_same_scale=False,
                        title=f'Absolute performance v3 liquidity providing {ssj} {ssj_against}')
                    fig1.show()
                ...
                kpi_df = riskmetrics.get_kpi(all_df[[f'gain_and_value_LP_LO_mark_to_market']])
                obf_data= {
                    'bb_period': bb_period,
                    'stdNbr': stdNbr,
                    'distance_to_bound_perc': distance_to_bound_perc,
                ...
                obf_data.update(kpi_df.to_dict()['gain_and_value_LP_LO_mark_to_market'])
