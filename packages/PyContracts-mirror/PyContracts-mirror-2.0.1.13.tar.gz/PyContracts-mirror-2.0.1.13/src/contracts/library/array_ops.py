...
jnp_types = {
    "jnp_int8": jnp.int8,  # Byte (-128 to 127)
    "jnp_int16": jnp.int16,  # Integer (-32768 to 32767)
    "jnp_int32": jnp.int32,  # Integer (-2147483648 to 2147483647)
    "jnp_int64": jnp.int64,  # Integer (9223372036854775808 to 9223372036854775807)
    "jnp_uint8": jnp.uint8,  # Unsigned integer (0 to 255)
    "jnp_uint16": jnp.uint16,  # Unsigned integer (0 to 65535)
    "jnp_uint32": jnp.uint32,  # Unsigned integer (0 to 4294967295)
    "jnp_uint64": jnp.uint64,  # Unsigned integer (0 to 18446744073709551615)
    "jnp_float16": jnp.float16,  # Half precision float: sign bit, 5 bits exponent, 10 bits mantissa
    "jnp_float32": jnp.float32,  # Single precision float: sign bit, 8 bits exponent, 23 bits mantissa
    "jnp_float64": jnp.float64,  # Double precision float: sign bit, 11 bits exponent, 52 bits mantissa
    "jnp_complex64": jnp.complex64,  # Complex number, represented by two 32-bit floats (real and imaginary components)
    "jnp_complex128": jnp.complex128,
}

for k, t in np_types.items():
    add_contract(Keyword(k).setParseAction(CheckType.parse_action(t)))
    add_keyword(k)
for k, t in jnp_types.items():
    add_contract(Keyword(k).setParseAction(CheckType.parse_action(t)))
    add_keyword(k)
