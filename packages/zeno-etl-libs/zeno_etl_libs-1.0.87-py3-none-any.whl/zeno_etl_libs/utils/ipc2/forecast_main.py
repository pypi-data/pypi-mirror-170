...
def ipc_forecast(store_id, reset_date, type_list, schema, db, logger):
    ...
    sales['avg_sales_value'].fillna(drug_sp['avg_sales_value'].mean)
    sales_daily['avg_sales_value'].fillna(drug_sp['avg_sales_value'].mean)
    ...
    weekly_fcst['model'] = 'LGBM_seg_quantity'
