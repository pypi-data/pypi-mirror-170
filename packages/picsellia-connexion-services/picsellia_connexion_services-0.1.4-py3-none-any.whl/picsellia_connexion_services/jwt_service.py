...
class JwtServiceConnexion(AbstractServiceConnexion):
    ...
    def wrapped_request(f):
        def decorated(self, *args, regenerate_jwt=True, **kwargs):
            try:
                resp: Response = f(self, *args, **kwargs)
            except UnauthorizedError as e:
                raise e
            except Exception as e:
                raise NetworkError(f"Something went wrong while executing a request '{f.__name__}'. Error: {e}")
    ...
    def generate_jwt(self):
        ...
        try:
            ...
            raise NetworkError(f"Server is not responding, please check your host. Exception: {e}")
