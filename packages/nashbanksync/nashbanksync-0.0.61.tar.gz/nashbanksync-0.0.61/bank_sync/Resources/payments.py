...
class Payments(Resource):
    ...
    def serialize(self, payload=None, operation=None):
        ...
        default_reference=payload.get("transfer", {}).get("reference", get_code(length=14))
    ...
    def standardize_callback(self, callback_data={}, forward=True):
        ...
        callback = Callbacks.objects.filter(reference=callback_data.get("reference", ""))
        ...
        if callback_data.get("bank_id", -1)==super().COOP:
            ...
            if callback_data.get("type", -1)in[super().IFT,super().MOBILE_WALLET]:
                ...
                if len(callback):
                    ...
                    response["account_reference"] = callback_data.get("reference", "")
