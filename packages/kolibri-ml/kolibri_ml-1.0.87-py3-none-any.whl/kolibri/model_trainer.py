...
try:
    ...
    pass
from kolibri.experiment_tracking import DefaultLogger
import os
import pandas as pd
...
class ModelTrainer(object):
    ...
    def __init__(self, params, skip_validation=False):
        ...
        self.experiment_logger=DefaultLogger
...
#            self.y_pred=list(predictions[:,0])
            ...
            elif predictions is not None and (len(predictions.shape)==1 or predictions.shape[1]==1):
                performance_data=pd.DataFrame(X)
                performance_data['class']=y
