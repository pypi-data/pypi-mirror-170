...
def element_dict(
    ...
    if with_metadata:
        ...
        element_md = element_metadata(database_path, item.id)
    ...
    if with_classes:
        classes = element_classes(database_path, item.id)
        if len(classes):
...
def run(
    ...
    md_columns = None
    if with_metadata:
        md_columns = metadata_columns(database_path, parent, type, recursive)
