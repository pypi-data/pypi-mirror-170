...
Classification = namedtuple(
    "Classification", ["class_name", "class_id", "classification_confidence"]
)
...
def element_metadata(database_path: str, element_id: str) -> Optional[List[Metadata]]:
    """
...
def element_classes(
    ...
    cursor.execute(
        """
        SELECT class_name, id, confidence FROM classification
        WHERE element_id = ?;
        """,
        (element_id,),
    )
