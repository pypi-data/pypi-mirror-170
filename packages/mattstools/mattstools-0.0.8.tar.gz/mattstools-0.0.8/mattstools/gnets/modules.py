"""
Custom layers, blocks, and network components for working on graphs.
These modules are designed to be combined and chained together
...
class GraphVectorGenerator(nn.Module):
    """A simple graph generator which takes in a singe vector.
    vector to graph
...
class FullGraphVectorGenerator(nn.Module):
    ...
    2)  Passes this through a GVG to get a graph output
