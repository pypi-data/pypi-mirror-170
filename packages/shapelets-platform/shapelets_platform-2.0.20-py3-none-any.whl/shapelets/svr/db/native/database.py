...
class SandBoxSettings:
    ...
    @staticmethod
    def from_settings(settings: DatabaseSettings) -> SandBoxSettings:
        ...
        if settings.temp_directory is not None:
            ...
            settings.temp_directory = str(tmp_dir)
        ...
        return SandBoxSettings(settings.db_schema, opt, path)
...
class Connection:
    ...
    """
    A sandbox connection.
    
    Connections are thread dependent and they should not be shared 
    among threads.
    """
    def __init__(self, connection: sn.SandBoxConnection) -> None:
...
class ConnectionWrapper(ScopedConnection):
    ...
    def get_connection(self) -> Connection:
        ...
        if self.__refCount == 1:
            self.__dbConnection = this.sandbox.connect()
...
def initialize_db(settings: DatabaseSettings = DatabaseSettings()):
    ...
    """
    this.settings = SandBoxSettings.from_settings(settings)
    this.sandbox = sn.SandBox(this.settings.path, False, this.settings.options)
...
def connect() -> ScopedConnection:
    ...
    """
    if this.settings is None or this.sandbox is None:
