...
class SQLTranslator(ASTTranslator):
            ...
            if monad.aggregated:
                ...
                if monad.nogroup:
                    if isinstance(monad, ListMonad):
                        ...
                        throw(NotImplementedError, 'Too complex aggregation, expressions cannot be combined: %s' % ast2src(node))
            return monad
    ...
    def __init__(translator, tree, parent_translator, code_key=None, filter_num=None, vartypes=None, left_join=False, optimize=None):
        ASTTranslator.__init__(translator, tree)
        translator.id = next(translator_counter)
        local.translators.append(translator)
        try:
            translator.init(tree, parent_translator, code_key, filter_num, vartypes, left_join, optimize)
        finally:
            assert local.translators
            local.translators.pop()

    def init(translator, tree, parent_translator, code_key=None, filter_num=None, vartypes=None, left_join=False, optimize=None):
        ...
        for i, generator in enumerate(tree.generators):
            ...
            if monad:  # Lambda was encountered inside generator
                ...
                varkey = translator.filter_num, node.src, translator.code_key
                leftJoin = node.src.startswith('left(')
                if (leftJoin):
                    ds = node.src.replace('(', '_')
                    ds = ds.replace(')', '')
                    x = ds.split("_")
                    value = x[1]
                    node.src = value
...
class Monad(object, metaclass=MonadMeta):
    ...
    def aggregate(monad, func_name, distinct=None, sep=None):
        if isinstance(distinct, NumericAttrMonad):
            a = 2
        else:
        ...
        if distinct is None:
            distinct = getattr(monad, 'forced_distinct', False) and func_name in ('SUM', 'GENERIC')
...
class FuncGenericMonad(FuncMonad):
    ...
    def call(self, x, distinct=None, **kwargs):
        custom_func = kwargs['custom_func']
        new_method = 'GENERIC_' + custom_func

        # return x.aggregate('GENERIC', distinct)
        return x.aggregate(new_method, distinct)
