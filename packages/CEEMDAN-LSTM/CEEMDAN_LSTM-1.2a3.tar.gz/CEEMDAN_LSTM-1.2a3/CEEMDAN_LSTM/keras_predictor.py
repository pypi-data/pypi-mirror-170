...
class keras_predictor:
    ...
    def __init__(self, PATH=None, FORECAST_HORIZONS=30, FORECAST_LENGTH=30, KERAS_MODEL='GRU', DECOM_MODE='CEEMDAN', INTE_LIST=None, REDECOM_LIST={'co-imf0':'vmd'}, 
        ...
        KERAS_MODEL        - the Keras model, eg. 'GRU', 'LSTM', 'DNN', 'BPNN', model = Sequential(), .h file of str, list, dict, pd.DataFrame to load
                           - ex. KERAS_MODEL = {'co-imf0':'co_imf0_model.h5', 'co-imf1':'co_imf1_model.h5'}
        ...
        self.REDECOM_LIST = dict(REDECOM_LIST)
        ...
        try: KERAS_MODEL = pd.DataFrame(KERAS_MODEL, index=[0])
            ...
            if not isinstance(KERAS_MODEL, Sequential): # Check KERAS_MODEL
                if type(KERAS_MODEL) == str: 
                    if '.h5' not in self.KERAS_MODEL: self.KERAS_MODEL = KERAS_MODEL.upper()
        ...
        if REDECOM_LIST is not None:# Check REDECOM_LIST and Input REDECOM_MODE
            REDECOM_MODE = ''
            try:
                REDECOM_LIST = pd.DataFrame(REDECOM_LIST, index=[0]) 
    ...
    # ------------------------------------------------------
    # 1.1 Build model
    def build_model(self, trainset_shape, model_name):
        """
        Build Keras model, eg. 'GRU', 'LSTM', 'DNN', 'BPNN', or model = Sequential().
        """
        if self.TRANSFER is not None and os.path.exists(str(self.PATH)+self.TRANSFER): 
            print('Transfer Learning for Keras model:', self.PATH+self.TRANSFER)
            return load_model(self.PATH+self.TRANSFER) # load user's saving custom model
        elif isinstance(self.KERAS_MODEL, Sequential): 
            return self.KERAS_MODEL  
        elif isinstance(self.KERAS_MODEL, pd.DataFrame):
            if self.PATH is None: raise ValueError("Please set PATH to load model:", self.KERAS_MODEL)
            model_name = self.PATH + self.KERAS_MODEL[model_name]
            if os.path.exists(model_name): 
                print('Load Keras model:', model_name)
                return load_model(model_name) # load user's saving custom model
            else: raise ValueError("File does not exist:", model_name)
        elif '.h5' in str(self.KERAS_MODEL):
            if self.PATH is None: raise ValueError("Please set PATH to load model:", self.KERAS_MODEL)
            if os.path.exists(self.PATH+self.KERAS_MODEL): 
                print('Load Keras model:', self.PATH+self.KERAS_MODEL)
                return load_model(self.PATH+self.KERAS_MODEL) # load user's saving custom model
            else: raise ValueError("File does not exist:", self.PATH+self.KERAS_MODEL)
    ...
    def keras_predict(self, data=None, show_model=False, save_model=False, fitting_set=None, **kwargs): # GRU forecasting function
        """
        Forecast by Keras
        ...
        **kwargs           - any parameters of model.fit()
        ...
        if save_model and self.PATH is not None:
            if self.TRANSFER is not None: self.TRANSFER = data.name
        ...
        # load the best one to predict
        if self.TRANSFER is not None: load_model(self.PATH+self.TRANSFER) 
    ...
    def single_keras_predict(self, data=None, show_data=False, show_model=False, save_model=False, plot_result=False, save_result=False, **kwargs):
        """
        ...
        save_model      - save Keras model in .h5 file
        ...
        **kwargs        - any parameters of self.keras_predict()
        ...
        data = check_dataset(data, show_data, self.DECOM_MODE, self.REDECOM_LIST)
        data.name = change_name(predictor_name+'_model.h5')
        self.TARGET = data['target']

        # Forecast
        start = time.time()
        df_result = self.keras_predict(data=data, show_model=show_model, save_model=save_model, **kwargs)
        end = time.time()
    ...
    def ensemble_keras_predict(self, data=None, show_data=False, show_model=False, save_model=False, plot_result=False, save_result=False, **kwargs):
        """
        ...
        **kwargs        - any parameters of self.keras_predict()       
        ...
        # Forecast
        df_result = self.keras_predict(data=df_redecom, show_model=show_model, save_model=save_model, **kwargs)
        end = time.time()
    ...
    def respective_keras_predict(self, data=None, show_data=False, save_model=False, show_model=False, plot_result=False, save_result=False, **kwargs):
        """
        ...
        **kwargs        - any parameters of self.keras_predict()
        ...
        for imf in df_redecom.columns.difference(['target']):
            ...
            time1 = time.time()
            df_result = self.keras_predict(data=df_redecom[imf], show_model=show_model, save_model=save_model, **kwargs)
            time2 = time.time()
    ...
    def hybrid_keras_predict(self, data=None, show_data=False, show_model=False, save_model=False, plot_result=False, save_result=False, **kwargs):
        """
        ...
        **kwargs        - any parameters of self.keras_predict()
        ...
        for df in df_redecom_list+df_rest_list: # both ensemble (matrix-input) and respective (vector-input) method 
            ...
            time1 = time.time()
            df_result = self.keras_predict(data=df, show_model=show_model, save_model=save_model, **kwargs) # the ensemble method with matrix input 
            time2 = time.time()
        ...
        if not self.NEXT_DAY: 
            if self.FIT_METHOD == 'ensemble': # fitting method
                ...
                time1 = time.time()
                df_result = self.keras_predict(data=df_redecom, show_model=show_model, save_model=save_model, fitting_set=fitting_set, **kwargs) # the ensemble method with matrix input 
                time2 = time.time()
    ...
    def multiple_keras_predict(self, data=None, run_times=10, predict_method=None, save_each_model=False, save_each_result=False, **kwargs):
        """
        ...
        save_each_model    - save each run's model
        ...
        **kwargs           - any parameters of self.keras_predict()
        ...
        for i in range(run_times):
            ...
            with self.HiddenPrints():
                if predict_method == 'Single': df_result = self.single_keras_predict(data=data, save_model=save_each_model, save_result=save_each_result, **kwargs)
                if predict_method == 'Ensemble': df_result = self.ensemble_keras_predict(data=data, save_model=save_each_model, save_result=save_each_result, **kwargs) 
                if predict_method == 'Respective': df_result = self.respective_keras_predict(data=data, save_model=save_each_model, save_result=save_each_result, **kwargs)
                if predict_method == 'Hybrid': df_result = self.hybrid_keras_predict(data=data, save_model=save_each_model, save_result=save_each_result, **kwargs)
            time2 = time.time()
    ...
    def rolling_keras_predict(self, data=None, predict_method=None, transfer_learning=False, **kwargs):
        """
        ...
        **kwargs           - any parameters of self.keras_predict()
        ...
        if transfer_learning: self.TRANSFER = True
        ...
        for i in range(self.FORECAST_LENGTH):
            if self.TRANSFER is not None and os.path.exists(str(self.PATH)+str(self.TRANSFER)): 
                print('Transfer Learning for Keras model:', self.PATH+self.TRANSFER)
            print('Run %d is running...'%i, end='')
            time1 = time.time()
            """
            # with self.HiddenPrints():
            """
            self.TARGET = data[:-self.FORECAST_LENGTH+i]
            if predict_method == 'Single': df_result = self.single_keras_predict(data=self.TARGET, save_model=transfer_learning, **kwargs)
            if predict_method == 'Ensemble': df_result = self.ensemble_keras_predict(data=self.TARGET, save_model=transfer_learning, **kwargs) 
            if predict_method == 'Respective': df_result = self.respective_keras_predict(data=self.TARGET, save_model=transfer_learning, **kwargs)
            if predict_method == 'Hybrid': df_result = self.hybrid_keras_predict(data=self.TARGET, save_model=transfer_learning, **kwargs)

            time2 = time.time()
            print('taking time: %.3fs'%(time2-time1))
            df_result['Run'] = i
            df_result.name = predictor_name+' Running Log'
            print(df_result)
