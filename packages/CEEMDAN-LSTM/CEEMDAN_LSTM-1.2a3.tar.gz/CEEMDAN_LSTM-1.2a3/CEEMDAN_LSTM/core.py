...
def help():
    ...
    print("cl.single_keras_predict(self, data=None, show_data=False, show_model=False, save_model=False, plot_result=False, save_result=False, **kwargs)")
    print("cl.ensemble_keras_predict(self, data=None, show_data=False, show_model=False, save_model=False, plot_result=False, save_result=False, **kwargs)")
    print("cl.respective_keras_predict(self, data=None, show_data=False, show_model=False, save_model=False, plot_result=False, save_result=False, **kwargs)")
    print("cl.hybrid_keras_predict(self, data=None, show_data=False, show_model=False, save_model=False, plot_result=False, save_result=False, **kwargs)")
...
def check_dataset(data, show_data=False, decom_mode=None, redecom_list=None):
    ...
    if 'target' not in check_data.columns:
        if len(check_data.columns) == 1: 
            check_data.columns=['target']
            if 'vmd' in decom_mode.lower() or 'vmd' in [x.lower() for x in redecom_list.values()]: 
                if len(check_data)%2: check_data = check_data.sort_index()[1:]
        elif decom_mode.lower() in ['emd', 'eemd', 'ceemdan']: check_data['target'] = check_data.sum(axis=1).values
        else:
...
def name_predictor(now, name, module, model, decom_mode=None, redecom_mode=None, next_pred=False):
    ...
    if type(model) == str and '.h5' not in str(model): name = name + model.upper()  # forecasting model
...
def plot_save_result(data, name=None, plot=True, save=True, path=None, type=None): 
    ...
    if isinstance(data, tuple):
        ...
        elif 'Next-day' or 'Rolling' in data.name: # save Next-day forecasting result
            name = name[:9]+'000000_'
            if save:
                log_name = LOG_PATH + name + change_name(data.name)+'.csv'
