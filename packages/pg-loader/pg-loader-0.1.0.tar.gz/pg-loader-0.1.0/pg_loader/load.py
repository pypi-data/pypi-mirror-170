...
class PgLoader:
    ...
    async def source_loader(
        ...
        **kwargs,
        ...
        Args:
            ...
            kwargs: If `db_loader` argument is not provided, PgLoader kwargs may be provided and a new `PgLoader` will be initialized.

        Raises:
            RuntimeError: If the loader's table does not contain column `row_id_column`.
        """
        self = await cls.create(**kwargs)
        self.source = source
    ...
    def get_sources_table(
        ...
        Args:
            table (Union[PgLoader, sa.Table, DeclarativeMeta]): The entity to make the sources table for.
