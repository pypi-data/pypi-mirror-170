...
from pybabelnet.resources import WordNetSynsetID, BabelSynsetID
...
class RESTfulPacket:
    ...
    def __init__(self, call_type, rf_key):
        ...
        if not rf_key:
            raise ValueError(
                "Please define the parameter RESTFUL_KEY in " + "pybabelnet_conf.yml"
            )
        ...
        if response.status_code != requests.codes.ok:
            if response.status_code == requests.codes.not_found:
                raise RuntimeError(
                    ...
                    "Please check your connection or the parameter RESTFUL_KEY in pybabelnet_conf.yml."
                )
            else:
