...
from pybabelnet import _utils
from pybabelnet._impl import BabelNetIndexField, BabelNetIndexImageField
from pybabelnet.conf import _config
from pybabelnet.data import phonetics
from pybabelnet.data.category import BabelCategory
from pybabelnet.data.domain import BabelDomain
from pybabelnet.data.example import BabelExample
from pybabelnet.data.frame import VerbAtlasFrameID
from pybabelnet.data.gloss import BabelGloss
from pybabelnet.data.image import BabelImage, BabelImageComparator
from pybabelnet.data.license import BabelLicense
...
from pybabelnet.data.tokens import *
from pybabelnet.indices.index import BNIndex
from pybabelnet.iterators.iter_impl import *
from pybabelnet.iterators.abstract_iter import *
from pybabelnet.language import Language
from pybabelnet.resources import (
...
from pybabelnet.sense import WordNetSense, BabelSense
from pybabelnet.synset import SynsetType, _OfflineBabelSynset, BabelSynset
from pybabelnet.tag_utils import get_tag
from pybabelnet.versions import BabelVersion, WordNetVersion
...
class OfflineIndex(BNIndex):
    ...
    def _synset_from_all_documents(
        ...
        language_simple_lemma = doc.getValues(BabelNetIndexField.LANGUAGE_LEMMA.name)
        ...
        for k in range(len(lemmas)):
            ...
            simple_lemma = _utils.simple_lemma_to_string(lemma)
            audio = audios.get(str(lemma_language) + ":" + simple_lemma.lower(), None)

            transcription = transcripts.get(
                str(lemma_language) + ":" + simple_lemma.lower(), None
            )
            ...
            for j in range(0, len(sense_offset_triples), 3):
                ...
                if lemma_language is Language.EN:
                    b_key_sense = simple_lemma.lower().strip() in key_senses
                ...
                if (
                    ...
                    ln = language_simple_lemma[k].split(":")[0]
                    lem = language_simple_lemma[k].replace(ln + ":", "").strip()
