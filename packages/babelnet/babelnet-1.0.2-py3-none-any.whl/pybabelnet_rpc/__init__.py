...
def print_config(tcp: int, ipc: str):
    ...
    print("To use PyBabelNet in RPC mode, add",
          "this line" if (len(conf) == 1) else "one of these lines",
          "in your pybabelnet_conf.yml file")
    for c in conf:
        print(c)
...
def check_bn(bn_path):
    if 'PYBABELNET_SKIP_CHECK' in os.environ:
        return True
...
def start_server_interactive(print_only=None):
    if print_only is None:
        print_only = 'PYBABELNET_PRINT_CMD' in os.environ
    not_win = not is_win()
...
def start_server(bn_path: str, doc_port: int = None, tcp_port: int = None, ipc_path: str = None, print_only=False):
    ...
    if print_only:
        ...
        if ipc_path is not None:
            cmd += f'-v "{ipc_path}:/pybabelnet_ipc" '
        ...
        if ipc_path is not None:
            ...
            volumes.append(f"{ipc_path}:/pybabelnet_ipc")
    ...
    if doc_port is not None:
        ...
        print()
        print("PyBabelNet documentation",
...
def main(prog=None):
    ...
    if cmd == 'start':
        ...
        if interactive:
            ...
            not_win = not is_win()
            parser = argparse.ArgumentParser(prog=f"{prog} start", description='Starts the PyBabelNet RPC server')
            ...
            start_server(args.bn, None if args.no_doc else args.doc, args.tcp if MODE_TCP in modes else None,
                         args.ipc if MODE_IPC in modes else None, args.print)
    elif cmd == 'stop':
        import docker

        parser = argparse.ArgumentParser(prog=f"{prog} stop", description='Stops the PyBabelNet RPC server')
...
Controls the PyBabelNet RCP server in Docker 
