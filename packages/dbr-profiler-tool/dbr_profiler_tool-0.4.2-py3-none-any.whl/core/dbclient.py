...
class dbclient:
    ...
    def __init__(self, inp_configs):
        ...
        if self._raw_token.strip() == '':
            raise Exception('Pass valid Token')
    ...
    def _update_token(self):
        # if self._raw_token=='': #do it first time
        #     self._raw_token = self.get_secret_value(self._workspace_pat_scope, self._workspace_pat_token)
    ...
    def test_connection(self, master_acct=False):
        if master_acct: #master acct may use a different credential
            ...
            results = requests.get(self._url + f'/api/2.0/accounts/{self._account_id}/workspaces', headers=self._token,
    ...
    @staticmethod
    def secretk(data: str) -> str:
        s = str(b64e(bytes(data, 'utf-8')), 'utf-8')
        s = s + 'Yg$aBDeF'
        return s

    @staticmethod
    def unsecretk(obscured: str) -> str:
        appkey = 'Yg$aBDeF'
        obscured = obscured[:len(obscured)-len(appkey)]
        s = str(b64d(bytes(obscured, 'utf-8')), 'utf_8')
        return s
 
    @staticmethod
    def getDecryptedJsonKey(obscured: str, key) -> str:
        appkey = 'Yg$aBDeF'
        obscured = obscured[:len(obscured)-len(appkey)]
        s = str(b64d(bytes(obscured, 'utf-8')), 'utf_8')
