...
class SliderBoxV(wx.BoxSizer):
  ...
  def __init__(self, parent, text, init, themax, handler, display=False, themin=0):
    ...
    if display:		# Display the slider value when it is thumb'd
      ...
      charw = self.text_ctrl.GetCharWidth()
      ...
      w2 += charw
      self.width = max(w1, w2, w3, sw)
      self.text_ctrl.SetSizeHints(self.width, -1, self.width)
      self.slider.SetSizeHints(self.width, -1, self.width)
...
class WrapIndicator(WrapControl):
  def __init__(self, button, text, text_color, on_color):
    self.button = button
    self.text = text
    self.on_brush = wx.Brush(on_color)
    WrapControl.__init__(self)
    self.Add(button, 1, flag=wx.ALIGN_CENTER_VERTICAL)
    self.light = QuiskPushbutton(button.GetParent(), None, "", text_color=text_color)
    self.Add(self.light, 0, flag=wx.ALIGN_CENTER_VERTICAL|wx.LEFT, border=2)
  def TurnOn(self, on):
    if on:
      self.light.SetLabel(self.text)
      self.light.up_brush = self.on_brush
    else:
      self.light.SetLabel("")
      self.light.up_brush = self.button.up_brush
    self.light.Refresh()
