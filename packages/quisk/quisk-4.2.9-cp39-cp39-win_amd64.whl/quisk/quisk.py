...
class ConfigConfig(wx.ScrolledWindow):
  def __init__(self, parent, width):
    ...
    self.btn_decimation = wx.Choice(self, -1, choices=lst, size=(x2, y2))
    if lst:
      self.Bind(wx.EVT_CHOICE, application.OnBtnDecimation, self.btn_decimation)
      self.btn_decimation.SetSelection(index)
    t.SetPosition((tab0, self.y))
    self.btn_decimation.SetPosition((tab1, self.y - self.offset))
    self.y += self.dy
...
class App(wx.App):
  ...
  def MakeButtons(self, frame, gbs):
    ...
    self.split_menu.AppendCheckItem('Lock Tx Frequency', self.OnMenuSplitLock, False)
