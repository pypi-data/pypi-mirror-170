...
class HiveHeating:
    ...
    def getMinTemperature(self, device: dict):
    ...
    def getMaxTemperature(self, device: dict):
    ...
    def getCurrentTemperature(self, device: dict):
    ...
    def getTargetTemperature(self, device: dict):
    ...
    def getState(self, device: dict):
        ...
        try:
            current_temp = self.getCurrentTemperature(device)
            target_temp = self.getTargetTemperature(device)
    ...
    def getCurrentOperation(self, device: dict):
    ...
    def getBoostStatus(self, device: dict):
    ...
    def getBoostTime(self, device: dict):
        ...
        Returns:
            str: Boost time.
        """
        if self.getBoostStatus(device) == "ON":
            state = None
    ...
    def getHeatOnDemand(self, device):
    ...
    def setTargetTemperature(self, device: dict, new_temp: str):
    ...
    def setMode(self, device: dict, new_mode: str):
        """Set heating mode.
    ...
    def setBoostOn(self, device: dict, mins: str, temp: float):
        ...
        Returns:
            boolean: True/False if successful
        """
        if mins > 0 and temp >= self.getMinTemperature(device):
            if temp <= self.getMaxTemperature(device):
                self.session.hiveRefreshTokens()
                final = False
    ...
    def setBoostOff(self, device: dict):
    ...
    def setHeatOnDemand(self, device: dict, state: str):
...
class Climate(HiveHeating):
    ...
    def getClimate(self, device: dict):
        """Get heating data.
        ...
        if device["deviceData"]["online"]:
            ...
            dev_data = {
                ...
                "min_temp": self.getMinTemperature(device),
                "max_temp": self.getMaxTemperature(device),
                "status": {
                    "current_temperature": self.getCurrentTemperature(device),
                    "target_temperature": self.getTargetTemperature(device),
                    "action": self.getCurrentOperation(device),
                    "mode": self.getMode(device),
                    "boost": self.getBoostStatus(device),
                },
    ...
    def getScheduleNowNextLater(self, device: dict):
    ...
    def minmaxTemperature(self, device: dict):
        """Min/Max Temp.
