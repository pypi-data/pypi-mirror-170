...
class ExcelExport:
    ...
    def __init__(self, input_dict, excel_templet_path=None, excel_templet_title="Sheet1", save_path=None, ):
        ...
        self.wb = load_workbook(filename=excel_templet_path) if excel_templet_path and os.path.exists(excel_templet_path) else None  # 直接解析模板
        # body解析
        self.parsed_body_data, err = BodyParser(work_book_obj=self.wb).parse_body(sheet_name=excel_templet_title)
        if err:
            self.err_msg = err
        self.sheetnames = self.wb.sheetnames if self.wb else None  # 共工作簿列表
