...
RB.HeaderView = Backbone.View.extend({
  ...
  _ensureSingleton() {
    if (RB.HeaderView.instance !== null) {
      console.warn('There are two instances of RB.HeaderView on the ' + 'page. Make sure only one RB.PageView is ' + 'instantiated and registered through ' + 'RB.PageManager.setupPage().');
    } else {
  ...
  render() {
    ...
    this._$mobileMenuMask = $('<div id="mobile_menu_mask"/>').on('click touchstart', this._closeMobileMenu.bind(this)).insertAfter(options.$pageSidebar || $('#page-sidebar'));
      ...
      formatItem: data => {
        ...
        if (data.username) {
          ...
          if (data.summary.length < this.SEARCH_SUMMARY_TRIM_LEN) {
            ...
            s = data.summary.substring(0, this.SEARCH_SUMMARY_TRIM_LEN);
          }
