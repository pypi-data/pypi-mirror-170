...
<% if (detail) { %><p><%= detail %><% } %>`),
  _interdiffTemplate: _.template(gettext("Changes between revision <%- revision %> and <%- interdiffRevision %>")),
  _latestTemplate: _.template(gettext("Diff Revision <%- revision %> (Latest)")),
  ...
  _onSelectChanged(ev) {
    ev.preventDefault();
    ev.stopPropagation();
    this.trigger('revisionSelected', [this.model.get('revision'), this.model.get('latestRevision')]);
  }

});

//# sourceMappingURL=diffRevisionLabelView.js.map