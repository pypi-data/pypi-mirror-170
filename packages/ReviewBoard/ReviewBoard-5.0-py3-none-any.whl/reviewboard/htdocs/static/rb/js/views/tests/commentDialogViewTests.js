  ...
  describe('Instances', function () {
    ...
    describe('Height', function () {
      ...
      it('When canEdit=true', function () {
        ...
        dlg.render();
        dlg.open();
        expect(dlg.$el.height()).toBe(RB.CommentDialogView.prototype.DIALOG_TOTAL_HEIGHT);
      });
      it('When canEdit=false', function () {
        editor.set('canEdit', false);
        dlg.render();
        dlg.open();
        expect(dlg.$el.height()).toBe(RB.CommentDialogView.prototype.DIALOG_NON_EDITABLE_HEIGHT);
      });
    });
    ...
    describe('In Draft indicator', function () {
      it('Shown when hasDraft=true', function () {
        ...
        dlg.render();
        expect(dlg.$el.find('p[class="draft-warning"]').length).toBe(1);
      });
      it('Hidden when hasDraft=false', function () {
        ...
        dlg.render();
        expect(dlg.$el.find('p[class="draft-warning"]').length).toBe(0);
      });
    });
  });
});

//# sourceMappingURL=commentDialogViewTests.js.map