...
RB.ReviewRequestPage.ReviewReplyEditor = Backbone.Model.extend({
  ...
  async save() {
    ...
    if (contextType === 'body_top') {
      ...
      if (!obj) {
        ...
        console.assert(ReplyClass, "Unexpected context type '%s'", contextType);
        obj = new ReplyClass({
    ...
    if (text) {
      ...
      await obj.save({
        attrs: [valueAttr, richTextAttr, 'forceTextType', 'includeTextTypes', 'replyToID']
      });
      this.set({
        hasDraft: true,
  ...
  async resetStateIfEmpty() {
    ...
    if (!replyObject || replyObject.isNew()) {
      ...
      if (contextType === 'body_top' || contextType === 'body_bottom') {
...
//# sourceMappingURL=reviewReplyEditorModel.js.map