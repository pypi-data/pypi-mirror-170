...
RB.ScrollManagerView = Backbone.View.extend({
   ...
   */
  initialize() {
    this.scrollYOffset = 0; // This is used so that unit tests can create a dummy window.
  ...
  _scheduleUpdateScrollPos() {
    if (!this._updateScrollPosScheduled) {
      ...
      this.window.requestAnimationFrame(this._updateScrollPos.bind(this));
    }
  },
    ...
    if (elInfos.length > 0) {
      ...
      for (let i = 0; i < elInfos.length; i++) {
        ...
        if (elInfo.newOffset.top + elInfo.newHeight < scrollY) {
          scrollY += elInfo.newHeight - elInfo.oldHeight + (elInfo.newOffset.top - elInfo.oldOffset.top);
        }
      }
    }
...
//# sourceMappingURL=scrollManagerView.js.map