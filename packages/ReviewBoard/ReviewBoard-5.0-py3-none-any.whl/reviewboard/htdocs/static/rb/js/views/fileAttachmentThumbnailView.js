...
RB.FileAttachmentThumbnail = Backbone.View.extend({
  ...
  template: _.template(`<div class="file">
 ...
 <div class="file-caption-container">
  ...
  render() {
    ...
    this.$el.hover(this._onHoverIn.bind(this), this._onHoverOut.bind(this));
   ...
   */
  showCommentDlg() {
    console.assert(!this.model.get('reviewURL'), 'showCommentDlg can only be called if the file ' + 'attachment does not have a review UI');
  ...
  _createDraftComment(commentID, text) {
    ...
    this._draftComment = review.createFileAttachmentComment(commentID, this.model.id);

    if (text) {
      this._draftComment.set('text', text);
    }

    this.listenTo(this._draftComment, 'saved', () => this.trigger('commentSaved', this._draftComment));
  },
  ...
  _renderContents() {
    ...
    }, this.model.attributes))).addClass(this.className);
  },
   ...
   */
  _renderThumbnail() {
    this._$thumbnailContainer.html(this.thumbnailContainerTemplate(this.model.attributes));

    Djblets.enableRetinaImages(this._$thumbnailContainer); // Disable tabbing to any <a> elements inside the thumbnail.
  ...
  _onCaptionChanged() {
    ...
    if (caption) {
      this._$caption.text(caption).removeClass('empty-caption');
    } else {
      this._$caption.text(gettext("No caption")).addClass('empty-caption');
    }
  },
  ...
  _onHoverIn() {
    ...
    const actionsRight = this._$file.offset().left + this._$file.outerWidth() + actionsWidth;
    ...
    if (actionsRight > $(window).width()) {
      this._$actionsContainer.css('left', -actionsWidth).addClass('left');
    } else {
      this._$actionsContainer.css('left', '100%').addClass('right');
    }
  ...
  _onHoverOut() {
    ...
    this._$actionsContainer.removeClass('left').removeClass('right');
...
//# sourceMappingURL=fileAttachmentThumbnailView.js.map