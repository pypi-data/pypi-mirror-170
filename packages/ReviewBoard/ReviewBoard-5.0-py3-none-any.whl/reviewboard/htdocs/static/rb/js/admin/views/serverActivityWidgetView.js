...
RB.Admin.ServerActivityWidgetView = RB.Admin.WidgetView.extend({
  ...
  renderWidget() {
    ...
    this._$tooltip = $();
    this._$chart = $('<div class="rb-c-admin-server-activity-widget__chart">').appendTo(this.$content).bind('plothover', (ev, pos, item) => {
      if (item) {
        if (previousHoverPoint !== item.dataIndex) {
          ...
          this._$tooltip = $('<div class="rb-c-admin-server-activity-widget-tooltip">').text(`${item.series.label} on ${x}: ${y}`).appendTo('body').css({
    ...
    const options = {
      ...
      grid: {
        markings: axes => {
          ...
          const markings = [];
          const d = new Date(axes.xaxis.min); // Go to the first Saturday.
...
//# sourceMappingURL=serverActivityWidgetView.js.map