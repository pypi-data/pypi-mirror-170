...
RB.ReviewRequestPage.IssueSummaryTableView = Backbone.View.extend({
 ...
 <td colspan="5"><em><%- text %></em></td>
   ...
   */
  render() {
    const $issueSummaryTable = this.$el.children('.rb-c-issue-summary-table');
    ...
    this._$reviewerHeader = this._$thead.find(`tr :nth-child(${this.COLUMN_REVIEWER})`);
    ...
    if (this.statusFilterState === null) {
      ...
      this._$currentTab = this.$('.rb-c-tabs__tab' + `[data-issue-state=${this.statusFilterState}]`).addClass('-is-active');
   ...
   */
  _applyFilters() {
    const sel = this.stateToSelectorMap[this.statusFilterState] + this.reviewerToSelectorMap[this.reviewerFilterState];
  ...
  _buildMaps() {
    ...
    _.each(this._getIssueRows(), issueEl => {
      ...
      if (!_.has(this.reviewerToSelectorMap, reviewer)) {
        this.reviewerToSelectorMap[reviewer] = `[data-reviewer="${reviewer}"]`;

        this._$reviewerFilter.append($('<option>').text(reviewer).val(reviewer));
      }
    });
  },
  ...
  _onIssueStatusChanged(comment, oldStatus, timestamp) {
    ...
    $entry.removeClass(`-is-${oldStatus}`).addClass(`-is-${newStatus}`).find('.rb-icon').removeClass(this.statusIconsMap[oldStatus]).addClass(this.statusIconsMap[newStatus]);
    ...
    const $oldCounter = this._$tabs.children(`[data-issue-state=${oldStatus}]`).find('.rb-c-issue-summary-table__counter');

    const $newCounter = this._$tabs.children(`[data-issue-state=${newStatus}]`).find('.rb-c-issue-summary-table__counter');
