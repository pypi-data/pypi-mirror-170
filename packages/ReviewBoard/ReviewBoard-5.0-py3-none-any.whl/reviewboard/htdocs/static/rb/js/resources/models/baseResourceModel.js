...
RB.BaseResource = Backbone.Model.extend({
  ...
  async ready() {
    ...
    if (_.isFunction(options.success) || _.isFunction(options.error) || _.isFunction(options.complete) || _.isFunction(options.ready)) {
      console.warn('RB.BaseResource.ready was called using callbacks. ' + 'Callers should be updated to use promises instead.');
      return RB.promiseToCallbacks(options, context, newOptions => this.ready());
    }

    const success = _.isFunction(options.ready) ? _.bind(options.ready, context) : undefined;
    const error = _.isFunction(options.error) ? _.bind(options.error, context) : undefined;
    const parentObject = this.get('parentObject');
  ...
  async ensureCreated() {
    ...
    if (_.isFunction(options.success) || _.isFunction(options.error) || _.isFunction(options.complete)) {
      console.warn('RB.BaseResource.ensureCreated was called using ' + 'callbacks. Callers should be updated to use ' + 'promises instead.');
      return RB.promiseToCallbacks(options, context, newOptions => this.ensureCreated());
    }
  ...
  fetch: async function () {
    ...
    if (_.isFunction(options.success) || _.isFunction(options.error) || _.isFunction(options.complete)) {
      console.warn('RB.BaseResource.fetch was called using callbacks. ' + 'Callers should be updated to use promises instead.');
  ...
  async save() {
    ...
    if (_.isFunction(options.success) || _.isFunction(options.error) || _.isFunction(options.complete)) {
      console.warn('RB.BaseResource.save was called using callbacks. ' + 'Callers should be updated to use promises instead.');
  ...
  _saveWithFiles(files, fileReaders, options) {
    ...
    for (let [key, file, reader] of _.zip(this.payloadFileKeys, files, fileReaders)) {
      if (!file || !reader) {
        continue;
      }
  ...
  async destroy() {
    ...
    if (_.isFunction(options.success) || _.isFunction(options.error) || _.isFunction(options.complete)) {
      console.warn('RB.BaseResource.destroy was called using ' + 'callbacks. Callers should be updated to use ' + 'promises instead.');
