...
RB.DiffFragmentQueueView = Backbone.View.extend({
    ...
    _.each(this._queue, queuedLoads => {
      $.funcQueue(queueName).add(() => {
        ...
        for (let i = 0; i < queuedLoads.length; i++) {
          ...
          const onFragmentRendered = _.isFunction(queuedLoad.onFragmentRendered) ? queuedLoad.onFragmentRendered : null;
          ...
          if (this._saved.hasOwnProperty(commentID)) {
            ...
            if (view) {
              ...
              view = this._renderFragment(container, commentID, html);
            }
   ...
   */
  _loadDiff(commentIDs) {
    let options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    const containerPrefix = this._containerPrefix;
    const queryArgs = [];
    const onFragmentRendered = _.isFunction(options.onFragmentRendered) ? options.onFragmentRendered : null;
      ...
      success: arrayBuffer => {
        ...
        const onFragmentLoaded = (commentID, html) => {
          ...
          if ($container.length === 0) {
            ...
            const view = this._renderFragment($(`#${containerPrefix}_${commentID}`), commentID, html);
        ...
        while (!done) {
          const parsed = this._parseDiffFragmentFromPayload(arrayBuffer, dataView, pos);
  ...
  _parseDiffFragmentFromPayload(arrayBuffer, dataView, pos) {
    ...
    return {
      ...
      load(cb) {
        RB.DataUtils.readBlobAsString(new Blob([arrayBuffer.slice(htmlStart, htmlStart + htmlLen)]), html => cb(commentID, html));
      }
