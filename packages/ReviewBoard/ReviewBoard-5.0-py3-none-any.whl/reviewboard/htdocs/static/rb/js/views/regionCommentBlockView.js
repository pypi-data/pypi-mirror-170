...
RB.RegionCommentBlockView = RB.AbstractCommentBlockView.extend({
  ...
  delegateEvents() {
    ...
    this.listenTo(this.model, 'change:x change:y change:width change:height', this._updateBounds);
  ...
  _moveTo(left, top) {
    ...
    const newLeft = this._moveState.initialBounds.left + left - this._moveState.initialCursor.left;
    const newTop = this._moveState.initialBounds.top + top - this._moveState.initialCursor.top;
    this.model.set({
  ...
  _resizeTo(left, top) {
    ...
    const newWidth = this._moveState.initialBounds.width + left - this._moveState.initialCursor.left;
    const newHeight = this._moveState.initialBounds.height + top - this._moveState.initialCursor.top;
    this.model.set({
  ...
  renderContent() {
    ...
    if (this.model.canUpdateBounds()) {
      ...
      this._$resizeIcon = $('<div class="resize-icon" />').appendTo(this.$el);
    }
   ...
   */
  _updateBounds() {
    this.$el.move(this.model.get('x') * this._scale, this.model.get('y') * this._scale, 'absolute').width(this.model.get('width') * this._scale).height(this.model.get('height') * this._scale);
  },
...
//# sourceMappingURL=regionCommentBlockView.js.map