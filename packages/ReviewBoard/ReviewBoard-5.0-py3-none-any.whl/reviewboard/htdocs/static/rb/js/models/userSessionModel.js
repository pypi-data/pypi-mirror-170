...
(function () {
  ...
  const StoredItems = RB.BaseResource.extend({
    ...
    addImmediately(obj) {
      ...
      if (_.isFunction(options.success) || _.isFunction(options.error) || _.isFunction(options.complete)) {
        console.warn('StoredItems.addImmediately was called using ' + 'callbacks. Callers should be updated to use ' + 'promises instead.');
        return RB.promiseToCallbacks(options, context, newOptions => this.addImmediately(obj));
      }
    ...
    removeImmediately(obj) {
      ...
      if (_.isFunction(options.success) || _.isFunction(options.error) || _.isFunction(options.complete)) {
        console.warn('StoredItems.removeImmediately was called using ' + 'callbacks. Callers should be updated to use ' + 'promises instead.');
        return RB.promiseToCallbacks(options, context, newOptions => this.removeImmediately(obj));
      }
  ...
  RB.UserSession = Backbone.Model.extend({
     ...
     */
    create(options) {
      console.assert(!RB.UserSession.instance, 'UserSession.create can only be called once.');
