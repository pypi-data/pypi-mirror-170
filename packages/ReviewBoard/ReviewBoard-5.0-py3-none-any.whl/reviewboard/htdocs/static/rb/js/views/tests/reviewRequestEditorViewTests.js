...
'</div>`);
  ...
  describe('Actions bar', function () {
    ...
    it('ReviewRequestActionHooks', function () {
      var MyExtension, extension, $action;
      ...
      $action = $('<a href="#" id="my-action" />').appendTo(view.$('.actions'));
      view.render();
      $action.click();
  ...
  describe('Fields', function () {
    ...
    function runSavingTest(richText, textType, supportsRichTextEV) {
      ...
      it('', function () {
        ...
        expect(reviewRequest.draft.save.calls.argsFor(0)[0].data).toEqual(expectedData);
      });
    }
      ...
      function closeDescriptionTests(options) {
        ...
        describe('Saves', function () {
          function testSave(richText, textType, setRichText) {
            ...
            expect(reviewRequest.save.calls.argsFor(0)[0].data).toEqual(expectedData);
          }
    ...
    describe('Reviewers', function () {
      describe('Groups', function () {
        ...
        it('Formatting', function () {
          ...
          expect($field.html()).toBe('<a href="/groups/group1/">group1</a>, ' + '<a href="/groups/group2/">group2</a>');
        });
        editCountTests();
        securityTests();
      });
    ...
    describe('Custom fields', function () {
      beforeEach(function () {
        saveSpyFunc = options => {
          expect(options.data['extra_data.' + jsonFieldName]).toBe('My Value');
          return Promise.resolve();
        };
      });
      setupFieldTests({
    ...
    describe('Custom checkbox field', function () {
      beforeEach(function () {
        ...
        saveSpyFunc = options => {
          expect(options.data['extra_data.checkbox']).toBe(true);
          return Promise.resolve();
        };
  ...
  describe('File attachments', function () {
    it('Rendering when added', function () {
      spyOn(RB.FileAttachmentThumbnail.prototype, 'render').and.callThrough();
      expect($filesContainer.find('.file-container').length).toBe(0);
      view.render();
      editor.createFileAttachment();
      expect(RB.FileAttachmentThumbnail.prototype.render).toHaveBeenCalled();
  ...
  describe('Screenshots', function () {
    describe('Importing on render', function () {
      ...
      it('With screenshots', function () {
        ...
        spyOn(RB.ScreenshotThumbnail.prototype, 'render').and.callThrough();
        view.render();
        expect(RB.ScreenshotThumbnail.prototype.render).toHaveBeenCalled();
    ...
    describe('Events', function () {
      ...
      describe('beginEdit', function () {
        it('Increment edit count', function () {
          expect(editor.get('editCount')).toBe(0);
          $thumbnail.find('.screenshot-caption .edit').inlineEditor('startEdit');
          expect(editor.get('editCount')).toBe(1);
        });
      });
      describe('endEdit', function () {
        describe('Decrement edit count', function () {
          ...
          beforeEach(function () {
            expect(editor.get('editCount')).toBe(0);
            $caption = $thumbnail.find('.screenshot-caption .edit').inlineEditor('startEdit');
          });
          it('On cancel', function () {
...
//# sourceMappingURL=reviewRequestEditorViewTests.js.map