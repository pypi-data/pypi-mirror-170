...
suite('rb/newReviewRequest/views/PostCommitView', function () {
  ...
  beforeEach(function () {
    ...
    spyOn(repository.branches, 'sync').and.callFake((method, collection, options) => {
      options.success({
        stat: 'ok',
        branches: [{
          name: 'master',
    ...
    spyOn(repository, 'getCommits').and.callFake(options => {
      ...
      spyOn(commits, 'sync').and.callFake((method, collection, options) => {
        options.success({
          stat: 'ok',
          commits: [{
            authorName: 'Author 1',
  ...
  describe('Error handling', function () {
    describe('Branches', function () {
      ...
      beforeEach(async function () {
        spyOn(repository.branches, 'fetch').and.callFake(() => returnError ? Promise.reject(new BackboneError(model, {
          errorText
        }, {})) : Promise.resolve());
        returnError = true;
        spyOn(RB.PostCommitView.prototype, '_showLoadError').and.callThrough();
        await view._loadBranches();
      });
      it('UI state', function () {
        expect(repository.branches.fetch).toHaveBeenCalled();
        expect(view._showLoadError).toHaveBeenCalledWith('branches', errorText);
        ...
        expect(view._$error.find('.error-text').text().strip()).toBe('Oh no');
    ...
    describe('Commits', function () {
      ...
      beforeEach(async function () {
        view.render();
        spyOn(RB.RepositoryCommits.prototype, 'fetch').and.callFake(() => returnError ? Promise.reject(new BackboneError(model, {
          errorText
        }, {})) : Promise.resolve());
        returnError = true;
        spyOn(RB.PostCommitView.prototype, '_showLoadError').and.callThrough();
      ...
      it('UI state', function () {
        ...
        expect(view._showLoadError).toHaveBeenCalledWith('commits', errorText);
...
//# sourceMappingURL=postCommitViewTests.js.map