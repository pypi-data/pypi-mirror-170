...
suite('rb/diffviewer/views/DiffCommitListView', function () {
  function testRows($rows, options) {
    ...
    const linkIndex = (options.haveHistory ? 1 : 0) + (options.haveInterCommitDiffControls ? 2 : 0);
    const valueStartIndex = linkIndex + (options.haveExpandCollapse ? 1 : 0);
    ...
    for (let i = 0; i < $rows.length; i++) {
      ...
      if (options.haveInterCommitDiffControls) {
        ...
        expect($baseSelector.prop('checked')).toEqual(!!rowOptions.baseSelected);
        expect($baseSelector.prop('disabled')).toEqual(!!rowOptions.baseDisabled);
        ...
        expect($tipSelector.prop('checked')).toEqual(rowOptions.tipSelected ? true : false);
        expect($tipSelector.prop('disabled')).toEqual(!!rowOptions.tipDisabled);
      }
      ...
      if (options.haveExpandCollapse) {
        ...
        if (rowOptions.haveExpandCollapse) {
          ...
          if (rowOptions.expanded) {
            ...
            expect($span.attr('title')).toEqual(gettext("Collapse commit message."));
          } else {
            expect($span.attr('class')).toEqual('fa fa-plus');
            expect($span.attr('title')).toEqual(gettext("Expand commit message."));
          }
        }
      }
  ...
  let $container;
  beforeEach(function () {
    $container = $('<div class="diff-commit-list" />').appendTo($testsScratch);
  });
  afterEach(function () {
    view.remove();
    view = null;
  ...
  describe('Render', function () {
    ...
    it('With base and tip commit IDs set', function () {
      ...
      model.set({
        'baseCommitID': commits.findWhere({
          commitID: 'r2'
        }).id,
        'tipCommitID': commits.findWhere({
          commitID: 'r3'
        }).id
      });
      view = new RB.DiffCommitListView({
  ...
  describe('Event handlers', function () {
    ...
    it('Expand/collapse', function () {
      ...
      const $links = $table.find('a'); // Expand first row.
      ...
      testRows($table.find('tbody tr'), {
        ...
        rowOptions: [{
          haveExpandCollapse: true,
          expanded: true,
          values: ['Long commit message\n\nThis is a long message.', 'Example Author']
        }, {
          haveExpandCollapse: true,
          expanded: false,
          values: ['Super long', 'Example Author']
    ...
    it('Select base/tip', function () {
      ...
      expect($tipSelectors.length).toEqual(4); // Select the interval (r4, r4].
      ...
      expect(model.get('tipCommitID')).toEqual(null); // Select the interval (r1, r4].
      ...
      expect(model.get('tipCommitID')).toEqual(null); // Select the interval (r1, r1].
      ...
      expect($tipSelectors.eq(3).prop('disabled')).toEqual(false);
      expect(model.get('baseCommitID')).toEqual(null);
      expect(model.get('tipCommitID')).toEqual(1); // Select the interval (r3, r3].
...
//# sourceMappingURL=diffCommitListViewTests.js.map