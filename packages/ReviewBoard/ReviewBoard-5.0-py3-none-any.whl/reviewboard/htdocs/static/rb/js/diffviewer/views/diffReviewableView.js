...
RB.DiffReviewableView = RB.AbstractReviewableView.extend({
  ...
  _hideRemovedCommentBlockViews() {
    ...
    _.sortBy(this._hiddenCommentBlockViews, commentBlockView => commentBlockView.model.get('beginLineNum'));
  },
  ...
  async _expandOrCollapse($btn, expanding) {
    ...
    const scrollOffsetTop = $scrollAnchor.offset().top - this._$window.scrollTop();
    /*
    ...
    if (tbodyID !== undefined) {
      ...
      if (newEl !== null) {
        ...
        this._$window.scrollTop($scrollAnchor.offset().top - scrollOffsetTop);
      }
    }
    /* Recompute the set of buttons for later use. */


    this._centered.setElements(new Map(Array.prototype.map.call(this.$('.diff-collapse-btn'), el => [el, {
  ...
  _precalculateContentWidths() {
    ...
    if (!this.$el.hasClass('diff-error') && this._$revisionRow.length > 0) {
      ...
      this._colReservedWidths = $cells.eq(0).outerWidth() + cellPadding + containerExtents;
      this._numColumns = $cells.length;

      if (this._numColumns === 4) {
        /* There's a left-hand side and a right-hand side. */
        this._colReservedWidths += $cells.eq(2).outerWidth() + cellPadding;
      }
      ...
      this._filenameReservedWidths = containerExtents + 2 * this._numFilenameColumns;
    } else {
      this._colReservedWidths = 0;
      this._filenameReservedWidths = 0;
      this._numColumns = 0;
  ...
  _onShowDeletedClicked(e) {
    ...
    $(e.target).parent().html('<span class="fa fa-spinner fa-pulse"></span>');
...
//# sourceMappingURL=diffReviewableView.js.map