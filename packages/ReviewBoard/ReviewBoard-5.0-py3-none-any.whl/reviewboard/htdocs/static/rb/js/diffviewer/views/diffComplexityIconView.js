...
RB.DiffComplexityIconView = Backbone.View.extend({
  ...
  render() {
    ...
    const innerRadius = 0.5 * (this.totalLines === null ? 1 : (this.totalLines - numTotal) / this.totalLines);
    const iconColors = RB.DiffComplexityIconView.getIconColors();
    this.$el.width(this.ICON_SIZE).height(this.ICON_SIZE).plot([{
  ...
  getIconColors() {
    if (!this._iconColors) {
      this._iconColors = {};
      const $iconColor = $('<div/>').hide().appendTo(document.body);
...
//# sourceMappingURL=diffComplexityIconView.js.map