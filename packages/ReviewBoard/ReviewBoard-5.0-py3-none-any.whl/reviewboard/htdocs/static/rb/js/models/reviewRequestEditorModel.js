...
RB.ReviewRequestEditor = Backbone.Model.extend({
   ...
   */
  initialize() {
    const reviewRequest = this.get('reviewRequest'); // Set up file attachments.
    ...
    fileAttachments.each(this._onFileAttachmentOrScreenshotAdded.bind(this)); // Set up screenshots.
    ...
    screenshots.each(this._onFileAttachmentOrScreenshotAdded.bind(this)); // Connect to other signals.
   ...
   */
  setDraftField: function (fieldName, value) {
    let options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    let context = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : undefined;

    if (_.isFunction(options.success) || _.isFunction(options.error) || _.isFunction(options.complete)) {
      console.warn('RB.ReviewRequestEditor.setDraftField was called ' + 'using callbacks. Callers should be updated to ' + 'use promises instead.');
      return RB.promiseToCallbacks(options, context, newOptions => this.setDraftField(fieldName, value, newOptions));
    }
    ...
    return reviewRequest.draft.save({
      data
    }).then(() => {
