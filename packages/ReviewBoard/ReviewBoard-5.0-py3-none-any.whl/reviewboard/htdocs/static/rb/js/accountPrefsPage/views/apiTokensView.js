...
(function () {
  ...
  const APITokenItem = RB.Config.ResourceListItem.extend({
    ...
    syncAttrs: ['expired', 'expires', 'id', 'invalidReason', 'invalidDate', 'lastUsed', 'note', 'policy', 'tokenValue', 'valid'],
  ...
  const APITokenItemView = Djblets.Config.ListItemView.extend({
    ...
    template: _.template(gettext("<div class=\"rb-c-config-api-tokens__main\"> <div class=\"rb-c-config-api-tokens__value\"> <input readonly=\"readonly\" value=\"<%- tokenValue %>\"> </div> <span class=\"fa fa-clipboard js-copy-token\" title=\"Copy to clipboard\"></span> </div> <div class=\"rb-c-config-api-tokens__info\"> <% if (expired) { %> <p class=\"rb-c-config-api-tokens__token-state -is-expired\"> Expired <time class=\"timesince\" datetime=\"<%= expires %>\"></time>. </p> <% } else if (valid) { %> <% if (lastUsed) { %> <p class=\"rb-c-config-api-tokens__usage -has-last-used\"> Last used <time class=\"timesince\" datetime=\"<%= lastUsed %>\"></time>. </p> <% } else { %> <p class=\"rb-c-config-api-tokens__usage\">Never used.</p> <% } %> <% if (expires) { %> <p class=\"rb-c-config-api-tokens__token-state -has-expires\"> <span>Expires <%= expiresTimeHTML %>.</span> </p> <% } else { %> <p class=\"rb-c-config-api-tokens__token-state\"> <span>Never expires.</span> </p> <% } %> <% } else { %> <p class=\"rb-c-config-api-tokens__token-state -is-invalid\"> Invalidated <time class=\"timesince\" datetime=\"<%= invalidDate %>\"></time>: <%= invalidReason %> </p> <% } %> </div> <div class=\"rb-c-config-api-tokens__actions\"></div> <span class=\"rb-c-config-api-tokens__note\"></span>")),
    events: {
    ...
    render() {
      ...
      this._$expires = this._$tokenState.not('.is-expired, .is-invalid').find('span');
