...
(function () {
  ...
  const ImageDifferenceDiffView = BaseImageView.extend({
    ...
    template: _.template(['<div class="image-container">', ' <canvas></canvas>', '</div>'].join('')),
    ...
    onImagesLoaded() {
      ...
      const origImageData = origContext.getImageData(0, 0, this._maxWidth, this._maxHeight);
      const origPixels = origImageData.data;
      const modifiedPixels = modifiedContext.getImageData(0, 0, this._maxWidth, this._maxHeight).data;
  ...
  const ImageOnionDiffView = BaseImageView.extend({
    ...
    template: _.template(['<div class="image-containers">', ' <div class="orig-image">', '  <img title="<%- caption %>" src="<%- diffAgainstImageURL %>" />', ' </div>', ' <div class="modified-image">', '  <img title="<%- caption %>" src="<%- imageURL %>" />', ' </div>', '</div>', '<div class="image-slider"></div>'].join('')),
    ...
    _resize() {
      ...
      this._$origImage.parent().width(origW).height(origH);

      this._$modifiedImage.parent().width(newW).height(newH);
    ...
    getInitialSize() {
      return {
        width: Math.max(this._$origImage.data('initial-width'), this._$modifiedImage.data('initial-width')),
        height: Math.max(this._$origImage.data('initial-height'), this._$modifiedImage.data('initial-height'))
      };
    }
  ...
  const ImageSplitDiffView = BaseImageView.extend({
    mode: 'split',
    name: gettext("Split"),
    template: _.template(['<div class="image-containers">', ' <div class="image-diff-split-container-orig">', '  <div class="orig-image">', '   <img title="<%- caption %>" src="<%- diffAgainstImageURL %>" />', '  </div>', ' </div>', ' <div class="image-diff-split-container-modified">', '  <div class="modified-image">', '   <img title="<%- caption %>" src="<%- imageURL %>" />', '  </div>', ' </div>', '</div>', '<div class="image-slider"></div>'].join('')),
    ...
    render() {
      ...
      this._$modifiedSplitContainer = this.$('.image-diff-split-container-modified');
      this._$slider = this.$('.image-slider').slider({
    ...
    setSplitPercentage(percentage) {
      ...
      this._$modifiedSplitContainer.outerWidth(this._maxWidth * (1.0 - percentage));
    },
    ...
    _resize() {
      ...
      this._$modifiedImage.parent().outerWidth(newW).height(newH);

      this._$origSplitContainer.outerWidth(this._maxWidth).height(maxOuterH);

      this._$modifiedSplitContainer.outerWidth(this._maxWidth).height(maxOuterH);

      this.$('.image-containers').width(this._maxWidth).height(maxH);
  ...
  const ImageTwoUpDiffView = BaseImageView.extend({
    mode: 'two-up',
    name: gettext("Two-Up"),
    template: _.template(['<div class="image-container image-container-orig">', ' <div class="orig-image">', '  <img title="<%- caption %>" src="<%- diffAgainstImageURL %>" />', ' </div>', '</div>', '<div class="image-container image-container-modified">', ' <div class="modified-image">', '  <img title="<%- caption %>" src="<%- imageURL %>" />', ' </div>', '</div>'].join('')),
  ...
  RB.ImageReviewableView = RB.FileAttachmentReviewableView.extend({
    ...
    modeItemTemplate: _.template('<li><a class="image-diff-mode" href="#" data-mode="<%- mode %>"><%- name %></a></li>'),
    captionTableTemplate: _.template('<table><tr><%= items %></tr></table>'),
    captionItemTemplate: _.template(['<td>', ' <h1 class="caption">', '  <%- caption %>', ' </h1>', '</td>'].join('')),
    errorTemplate: _.template(['<div class="review-ui-error">', ' <div class="rb-icon rb-icon-warning"></div>', ' <%- errorStr %>', '</div>'].join('')),
    ANIM_SPEED_MS: 200,
     ...
     */
    initialize() {
      RB.FileAttachmentReviewableView.prototype.initialize.apply(this, arguments);
      ...
      this.on('commentBlockViewAdded', commentBlockView => {
        commentBlockView.setSelectionRegionSizeFunc(() => _.pick(this._imageView.getSelectionRegion(), 'width', 'height'));
        ...
        this.listenTo(commentBlockView, 'removing', () => {
          this._commentBlockViews = _.without(this._commentBlockViews, commentBlockView);
        });
      });
      this.listenTo(this.model, 'change:scale', (model, scale) => {
        this._commentBlockViews.forEach(view => view.setScale(scale));

        this.$('.image-resolution-menu-current').text(scalingFactors.get(scale));
        /*
    ...
    renderContent() {
      ...
      this._$selectionArea = $('<div/>').addClass('selection-container').hide().proxyTouchEvents();
      this._$selectionRect = $('<div/>').addClass('selection-rect').prependTo(this._$selectionArea).proxyTouchEvents().hide();
      this.$el
      /*
      ...
      .hover(() => {
        ...
        if (this._$selectionRect.is(':hidden') && !this.commentDlg) {
          this._$selectionArea.hide();
        }
      });
      const $wrapper = $('<div class="image-content" />').append(this._$selectionArea);
      ...
      if (this.model.get('diffTypeMismatch')) {
        ...
        $wrapper.append(this._$imageDiffs).appendTo(this.$el);
        ...
        $wrapper.append(this._imageView.$el).appendTo(this.$el);
      ...
      if (this.model.get('numRevisions') > 1) {
        ...
        if (!this.renderedInline) {
          if (hasDiff) {
            const captionItems = [this.captionItemTemplate({
        ...
        if (!this.renderedInline) {
          ...
          $('<h1 class="caption" />').text(this.model.get('caption')).appendTo($header);
        }
      }

      const $resolutionMenu = $(['<li class="image-resolution-menu has-menu">', ' <a href="#" class="menu-header">', '  <span class="fa fa-search-plus"></span>', '  <span class="image-resolution-menu-current">100%</span>', '  <span class="rb-icon rb-icon-dropdown-arrow">', ' </a>', ' <ul class="menu" />', '</li>'].join(''));
      const $menu = $resolutionMenu.find('.menu');
      scalingFactors.forEach((text, scale) => {
        $(`<li class="menu-item" data-image-scale="${scale}" />`).text(text).appendTo($menu);
      });
    ...
    _onRevisionSelected(revisions) {
      ...
      const [base, tip] = revisions; // Ignore clicks on No Diff Label
    ...
    _addDiffMode(ViewClass) {
      ...
      $selector.appendTo(this._$modeBar).addClass('selected');
      const selectorWidth = $selector.outerWidth(true);
      $selector.removeClass('selected').width(selectorWidth);
    ...
    _setDiffMode(mode) {
      ...
      if (this._imageView) {
        this._diffModeSelectors[this._imageView.mode].removeClass('selected');
        ...
        this._imageView.$el.fadeOut(this.ANIM_SPEED_MS, () => this._showDiffMode(newView, true));
      } else {
        this._showDiffMode(newView);
      }

      this._diffModeSelectors[newView.mode].addClass('selected');
    },
    ...
    _onMouseDown(e) {
      if (e.which === 1 && !this.commentDlg && !$(e.target).hasClass('selection-flag')) {
        ...
        this._activeSelection.beginX = e.pageX - Math.floor(offset.left) - 1;
        this._activeSelection.beginY = e.pageY - Math.floor(offset.top) - 1;
        const updateData = {
        ...
        this._$selectionRect.css(updateData).data(updateData).show();
    ...
    _onMouseMove(e) {
      if (!this.commentDlg && this._$selectionRect.is(':visible')) {
        ...
        this._$selectionRect.css(updateData).data(updateData);
    ...
    _adjustPos() {
      ...
      this._$selectionArea.width(region.width).height(region.height).css({
        left: region.left,
        top: region.top
      });
...
//# sourceMappingURL=imageReviewableView.js.map