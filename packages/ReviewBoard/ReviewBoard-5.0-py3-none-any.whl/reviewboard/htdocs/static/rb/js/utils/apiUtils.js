  ...
  return {
    send(headers, completeCB) {
      ...
      xhr.addEventListener('load', () => {
        ...
        completeCB(xhr.status, xhr.statusText, result, xhr.getAllResponseHeaders());
      });
...
RB.setActivityIndicator = function (enabled, options) {
  const $activityIndicator = options._$activityIndicator || $('#activity-indicator');

  if (enabled) {
    if (RB.ajaxOptions.enableIndicator && !options.noActivityIndicator) {
      $activityIndicator.children('.indicator-text').text(options.type && options.type === 'GET' ? gettext("Loading...") : gettext("Saving..."));
      $activityIndicator.removeClass('error').show();
    }
  } else if (RB.ajaxOptions.enableIndicator && !options.noActivityIndicator && !$activityIndicator.hasClass('error')) {
    if (options._activityIndicatorHideImmediately) {
      $activityIndicator.hide();
    } else {
      $activityIndicator.delay(250).fadeOut('fast');
    }
  }
};
/**
...
RB.apiCall = function (options) {
  ...
  function showErrorPage(xhr, data) {
    ...
    $('<div class="server-error-box"/>').appendTo(document.body).append('<p><b>' + gettext("Error Code:") + '</b> ' + xhr.status + '</p>').append('<p><b>' + gettext("Error Text:") + '</b> ' + xhr.statusText + '</p>').append('<p><b>' + gettext("Request URL:") + '</b> ' + url + '</p>').append('<p><b>' + gettext("Request Data:") + '</b> ' + requestData + '</p>').append('<p class="response-data"><b>' + gettext("Response Data:") + '</b></p>').append(gettext("<p>There may be useful error details below. The following error page may be useful to your system administrator or when <a href=\"https://www.reviewboard.org/bugs/new/\">reporting a bug</a>. To save the page, right-click the error below and choose \"Save Page As,\" if available, or \"View Source\" and save the result as a <tt>.html</tt> file.</p>")).append(gettext("<p><b>Warning:</b> Be sure to remove any sensitive material that may exist in the error page before reporting a bug!</p>")).append($iframe).on('resize', function () {
  ...
  function doCall() {
    ...
    const defaultOptions = {
      ...
      error: function (xhr, textStatus, errorThrown) {
        ...
        $activityIndicator.addClass('error').text(gettext("A server error occurred.")).append($('<a href="#" />').text(gettext("Show Details")).click(() => showErrorPage(xhr, responseText))).append($('<a href="#" />').text(gettext("Dismiss")).click(function () {
    ...
    if (data.data === null || data.data === undefined || data.data instanceof Object && !(window.Blob && data.data instanceof Blob)) {
      data.data = $.extend({
        api_format: 'json'
      }, data.data || {});
    }
  ...
  if (options.type !== 'GET' && options.type !== 'HEAD' && RB.UserSession.instance.get('readOnly')) {
    console.error('%s request not sent. Site is in read-only mode.', options.type);
    return;
...
RB.promiseToCallbacks = function (options, context, callable) {
  const success = _.isFunction(options.success) ? options.success : _.isFunction(options.ready) ? options.ready : undefined;
  const error = _.isFunction(options.error) ? options.error : undefined;
  const complete = _.isFunction(options.complete) ? options.complete : undefined;
  callable(_.omit(options, ['success', 'ready', 'error', 'complete'])).then(result => {
    if (success) {
      success.call(context, result);
    }
  }).catch(err => {
    if (error) {
      error.call(context, err.modelOrCollection, err.xhr, err.options);
    }
  }).finally(() => {
    if (complete) {
