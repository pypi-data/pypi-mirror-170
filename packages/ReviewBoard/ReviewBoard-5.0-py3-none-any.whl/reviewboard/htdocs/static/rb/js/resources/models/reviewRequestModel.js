...
RB.ReviewRequest = RB.BaseResource.extend({
  ...
  deserializedAttrs: ['approved', 'approvalFailure', 'branch', 'bugsClosed', 'closeDescription', 'dependsOn', 'description', 'lastUpdated', 'public', 'reviewURL', 'summary', 'targetGroups', 'targetPeople', 'testingDone'],
   ...
   */
  createFromCommit() {
    let optionsOrCommitID = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    let context = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

    if (_.isObject(optionsOrCommitID)) {
      console.assert(optionsOrCommitID.commitID);
      console.warn('RB.ReviewRequest.createFromCommit was called ' + 'using callbacks. Callers should be updated to ' + 'use promises instead.');
      return RB.promiseToCallbacks(optionsOrCommitID, context, () => this.createFromCommit(optionsOrCommitID.commitID));
    }
  ...
  createReview(reviewID) {
    let extraAttrs = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    let review;
  ...
  setStarred(starred) {
    ...
    if (_.isFunction(options.success) || _.isFunction(options.error) || _.isFunction(options.complete)) {
      console.warn('RB.ReviewRequest.setStarred was called using ' + 'callbacks. Callers should be updated to use ' + 'promises instead.');
  ...
  async close(options) {
    ...
    if (_.isFunction(options.success) || _.isFunction(options.error) || _.isFunction(options.complete)) {
      console.warn('RB.ReviewRequest.close was called using ' + 'callbacks. Callers should be updated to use ' + 'promises instead.');
      return RB.promiseToCallbacks(options, context, newOptions => this.close(newOptions));
    }
  ...
  async reopen() {
    ...
    if (_.isFunction(options.success) || _.isFunction(options.error) || _.isFunction(options.complete)) {
      console.warn('RB.ReviewRequest.reopen was called using ' + 'callbacks. Callers should be updated to use ' + 'promises instead.');
      return RB.promiseToCallbacks(options, context, newOptions => this.reopen());
    }
  ...
  async beginCheckForUpdates(type, lastUpdateTimestamp) {
    ...
    await this.ready();
    setTimeout(this._checkForUpdates.bind(this), RB.ReviewRequest.CHECK_UPDATES_MSECS);
  },
  ...
  parseResourceData(rsp) {
    ...
    const data = RB.BaseResource.prototype.parseResourceData.call(this, rsp);
    data.state = state;
...
//# sourceMappingURL=reviewRequestModel.js.map