...
RB.NotificationManager = Backbone.View.extend({
  ...
  setup() {
    ...
    this._canNotify = Notification !== undefined && RB.UserSession.instance.get('enableDesktopNotifications');
  ...
  _haveNotificationPermissions() {
    return this._canNotify && RB.NotificationManager.Notification.permission === 'granted';
  },
  ...
  notify(options) {
    ...
    console.assert(options.hasOwnProperty('title'), 'RB.NotificationManager.notify requires "title" property');
    const notification = new RB.NotificationManager.Notification(options.title, {
      body: options.body,
    ...
    _.delay(() => notification.close(), RB.NotificationManager.NOTIFICATION_LIFETIME_MSECS);
  }
