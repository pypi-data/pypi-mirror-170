 ...
 <div class="diff-box">
  ...
  render() {
    ...
    if (!this.model.commits.isEmpty()) {
      ...
      this.listenTo(this.model.revision, 'change:baseCommitID change:tipCommitID', model => commitListModel.set({
        baseCommitID: model.get('baseCommitID'),
        tipCommitID: model.get('tipCommitID')
      }));
      this.listenTo(commitListModel, 'change:baseCommitID change:tipCommitID', this._onCommitIntervalChanged);
    ...
    if (this.model.diffReviewables.length > 0) {
      this.model.diffReviewables.each(diffReviewable => this._onDiffReviewableAdded(diffReviewable));
  ...
  _renderFileDiff(diffReviewable) {
    ...
    if (this._startAtAnchorName) {
      ...
      let $anchor = $(document.getElementsByName(this._startAtAnchorName));
      /*
