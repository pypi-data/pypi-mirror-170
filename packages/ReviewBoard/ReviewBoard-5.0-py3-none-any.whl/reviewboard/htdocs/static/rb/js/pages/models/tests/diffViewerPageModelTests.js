...
suite('rb/pages/models/DiffViewerPage', function () {
  describe('parse', function () {
    ...
    it('reviewRequestData', function () {
      ...
      expect(reviewRequest.url()).toBe('/s/foo/api/review-requests/123/');
      expect(reviewRequest.get('bugTrackerURL')).toBe('http://bugs.example.com/--bug_id--/');
      ...
      expect(reviewRequest.get('closeDescription')).toBe('This is closed');
      expect(reviewRequest.get('closeDescriptionRichText')).toBe(true);
      expect(reviewRequest.get('description')).toBe('This is a description');
      expect(reviewRequest.get('descriptionRichText')).toBe(true);
      expect(reviewRequest.get('hasDraft')).toBe(true);
      expect(reviewRequest.get('lastUpdatedTimestamp')).toBe('2017-08-23T15:10:20Z');
      expect(reviewRequest.get('public')).toBe(true);
      expect(reviewRequest.get('reviewURL')).toBe('/s/foo/r/123/');
      expect(reviewRequest.get('state')).toBe(RB.ReviewRequest.CLOSE_SUBMITTED);
      expect(reviewRequest.get('summary')).toBe('This is a summary');
      ...
      expect(reviewRequest.get('testingDone')).toBe('This is testing done');
      expect(reviewRequest.get('testingDoneRichText')).toBe(true);
      expect(reviewRequest.get('visibility')).toBe(RB.ReviewRequest.VISIBILITY_ARCHIVED);
    ...
    it('extraReviewRequestDraftData', function () {
      ...
      expect(draft.get('changeDescription')).toBe('Draft change description');
    ...
    it('editorData', function () {
      ...
      expect(editor.get('changeDescriptionRenderedText')).toBe('Change description');
      expect(editor.get('closeDescriptionRenderedText')).toBe('This is closed');
    ...
    it('lastActivityTimestamp', function () {
      ...
      expect(page.get('lastActivityTimestamp')).toBe('2017-08-22T18:20:30Z');
    });
  ...
  describe('loadDiffRevision', function () {
    ...
    describe('Generates URL for', function () {
      it('First page, normal diffs', function () {
        ...
        expect($.ajax).toHaveBeenCalledWith('/api/review-requests/123/diff-context/?revision=1');
      });
      it('Other page, normal diffs', function () {
        ...
        expect($.ajax).toHaveBeenCalledWith('/api/review-requests/123/diff-context/' + '?revision=1&page=2');
      });
      it('First page, interdiffs', function () {
        ...
        expect($.ajax).toHaveBeenCalledWith('/api/review-requests/123/diff-context/' + '?revision=1&interdiff-revision=2');
      });
      it('Other page, interdiffs', function () {
        ...
        expect($.ajax).toHaveBeenCalledWith('/api/review-requests/123/diff-context/' + '?revision=1&interdiff-revision=2&page=2');
      });
      it('Filename patterns', function () {
        ...
        expect($.ajax).toHaveBeenCalledWith('/api/review-requests/123/diff-context/' + '?revision=1&page=2&filenames=*.txt%2Csrc%2F*');
      });
    });
...
//# sourceMappingURL=diffViewerPageModelTests.js.map