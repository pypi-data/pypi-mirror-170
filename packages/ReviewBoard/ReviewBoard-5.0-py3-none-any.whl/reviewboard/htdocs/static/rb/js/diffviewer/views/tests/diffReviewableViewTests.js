      ...
      describe('Finishing selection', function () {
        ...
        describe('With single line', function () {
          ...
          it('And existing comment', function () {
            ...
            expect(view.createAndEditCommentBlock).not.toHaveBeenCalled();
          ...
          it('And no existing comment', function () {
            ...
            expect(view.createAndEditCommentBlock).toHaveBeenCalledWith({
              $beginRow: $row,
              $endRow: $row,
              beginLineNum: 5,
              endLineNum: 5
    ...
    describe('Hovering', function () {
      describe('Over line', function () {
        ...
        it('Contents cell', function () {
          ...
          expect($row.hasClass('selected')).toBe(false);
          expect(selector._$ghostCommentFlag.css('display')).toBe('none');
        });
        describe('Line number cell', function () {
          ...
          it('With no column flag', function () {
            ...
            expect($row.hasClass('selected')).toBe(true);
            expect(selector._$ghostCommentFlag.css('display')).not.toBe('none');
          });
        });
      });
  ...
  describe('Incremental expansion', function () {
    ...
    describe('Expanding', function () {
      ...
      describe('Fetching fragment', function () {
        beforeEach(function () {
          spyOn(model, 'getRenderedDiffFragment').and.resolveTo('abc');
        });
      ...
      describe('Injecting HTML', function () {
        it('Whole chunk', function (done) {
          spyOn(model, 'getRenderedDiffFragment').and.resolveTo(['<tbody class="equal tests-new-chunk">', ' <tr line="6">', '  <th></th>', '  <td>', '   <div class="collapse-floater">', '    <img class="diff-collapse-btn"', '         data-chunk-index="1"', '         data-lines-of-context="0" />', '   </div>', '  </td>', '  <th></th>', '  <td></td>', ' </tr>', '</tbody>'].join(''));
          view.on('chunkExpansionChanged', () => {
            ...
            expect($($tbodies[1]).hasClass('tests-new-chunk')).toBe(true);
        ...
        it('Merging adjacent expanded chunks', function (done) {
          ...
          $('<tbody class="equal loaded"/>').append($('<img class="diff-collapse-btn" />')).insertAfter(view.$('tbody')[1]).clone().insertBefore(view.$('tbody')[1]);
    ...
    describe('Collapsing', function () {
      ...
      describe('Injecting HTML', function () {
        it('Single expanded chunk', function (done) {
          spyOn(model, 'getRenderedDiffFragment').and.resolveTo(['<tbody class="equal tests-new-chunk">', ' <tr line="6">', '  <th></th>', '  <td></td>', '  <th></th>', '  <td></td>', ' </tr>', '</tbody>'].join(''));
          view.on('chunkExpansionChanged', () => {
            expect(model.getRenderedDiffFragment).toHaveBeenCalled();
            const $tbodies = view.$('tbody');
            expect($tbodies.length).toBe(3);
 ...
 <tr line="11">
  ...
  <td></td>
      ...
      beforeEach(function () {
        ...
        spyOn(view.model, 'getRenderedDiffFragment').and.resolveTo(diffFragmentHTML);
        $commentFlags = view.$('.commentflag');
        $rows = view.$el.find('tbody tr');
      });
      it('On initial render', function () {
        expect($commentFlags.length).toBe(2);
        expect($($commentFlags[0]).find('.commentflag-count').text()).toBe('1');
        expect($($commentFlags[1]).find('.commentflag-count').text()).toBe('2');
      ...
      it('On chunk expand', function (done) {
        ...
        view.on('chunkExpansionChanged', () => {
          ...
          expect($($commentFlags[2]).find('.commentflag-count').text()).toBe('1');
    ...
    describe('toggleWhitespaceOnlyChunks', function () {
      ...
      describe('Toggle on', function () {
        it('Chunk classes', function () {
          ...
          expect($($children[$children.length - 1]).hasClass('last')).toBe(true);
        });
        it('chunkDimmed event triggered', function () {
          spyOn(view, 'trigger');
          view.toggleWhitespaceOnlyChunks();
          expect(view.trigger).toHaveBeenCalledWith('chunkDimmed', '0.0');
        });
      ...
      describe('Toggle off', function () {
        it('Chunk classes', function () {
          ...
          expect($($children[$children.length - 1]).hasClass('last')).toBe(false);
        });
        it('chunkDimmed event triggered', function () {
          view.toggleWhitespaceOnlyChunks();
          spyOn(view, 'trigger');
          view.toggleWhitespaceOnlyChunks();
          expect(view.trigger).toHaveBeenCalledWith('chunkUndimmed', '0.0');
        });
        it('Whitespace-only file classes', function () {
          const $tbodies = view.$el.children('tbody');
...
//# sourceMappingURL=diffReviewableViewTests.js.map