...
(function () {
  ...
  RB.ReviewRequestPage.ReviewRequestPageView = RB.ReviewablePageView.extend({
    ...
    initialize() {
      ...
      this.listenTo(this.model, 'updatesProcessed', () => this.diffFragmentQueue.loadFragments());
      /*
      ...
      this.listenTo(this.model, 'applyingUpdate:entry', (metadata, html) => {
        ...
        this.listenToOnce(this.model, `appliedModelUpdate:entry:${entryID}`, (metadata, html) => this._reloadView(entryView, html));
        this.listenToOnce(this.model, `appliedUpdate:entry:${entryID}`, metadata => {
    ...
    render() {
      ...
      this._issueSummaryTableView = new RB.ReviewRequestPage.IssueSummaryTableView({
        el: $('#issue-summary'),
      ...
      this.listenTo(this._issueSummaryTableView, 'issueClicked', this._onIssueClicked);
      this.listenTo(this.model, 'appliedUpdate:issue-summary-table', (metadata, html) => {
    ...
    openCommentEditor(contextType, contextID) {
      for (let i = 0; i < this._entryViews.length; i++) {
        ...
        const reviewReplyEditorView = _.isFunction(entryView.getReviewReplyEditorView) ? entryView.getReviewReplyEditorView(contextType, contextID) : null;
...
//# sourceMappingURL=reviewRequestPageView.js.map