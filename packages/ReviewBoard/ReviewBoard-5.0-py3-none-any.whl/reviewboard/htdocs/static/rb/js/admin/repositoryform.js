...
(function () {
  const prevTypes = {};
  const origRepoTypes = [];
  const powerPackTemplate = `<h3>${gettext("Power Pack Required")}</h3>
<p>
${gettext("<span class=\"power-pack-advert-hosting-type\"></span> support is available with <a href=\"https://www.reviewboard.org/powerpack/\">Power Pack</a>, an extension which also offers powerful reports, document review, and more.")}
  ...
  const gerritPluginRequiredTemplate = `<h3>
...
${interpolate(gettext("The <code>gerrit-reviewboard</code> plugin is required for Gerrit integration. See the <a href=\"%s\" target=\"_blank\">instructions</a> for installing the plugin on your server."), [MANUAL_URL + 'admin/configuration/repositories/gerrit/'])}
  ...
  function updatePlanEl($row, $plan, serviceType, isFake) {
    ...
    if (planTypes.length === 1 || isFake) {
      ...
      for (let i = 0; i < planTypes.length; i++) {
        ...
        const opt = $('<option/>').val(planType.type).text(planType.label).appendTo($plan);
  ...
  function updateRepositoryType() {
    ...
    const newRepoTypes = hostingType === 'custom' ? [] : HOSTING_SERVICES[hostingType].scmtools;
  ...
  function updateAccountList() {
    ...
    accounts.forEach(account => {
      ...
      const $opt = $('<option/>').val(account.pk).text(text).data('account', account).appendTo($hostingAccount);
  ...
  $(document).ready(function () {
    ...
    const $hostingAccountRelink = $('<p/>').text(gettext("The authentication requirements for this account have changed. You will need to re-authenticate.")).addClass('errornote').hide().appendTo($hostingAccountRow);
    const $scmtoolAuthForms = $('.scmtool-auth-form');
    const $scmtoolRepoForms = $('.scmtool-repo-form');
    const $associateSshKeyFieldset = $('#row-associate_ssh_key').parents('fieldset');
    ...
    const $bugTrackerUsernameRow = $('#row-bug_tracker_hosting_account_username');
    ...
    const $editHostingCredentialsLabel = $('#repo-edit-hosting-credentials-label');
    const $forceAuth = $('#id_force_authorize');
    const $hostingPowerPackAdvert = $('<div class="powerpack-advert" />').html(powerPackTemplate).hide().appendTo($hostingType.closest('fieldset'));
    const $toolPowerPackAdvert = $('<div class="powerpack-advert" />').html(powerPackTemplate).hide().appendTo($tool.closest('fieldset'));
    const $gerritPluginInfo = $('<div class="gerrit-plugin-advert" />').html(gerritPluginRequiredTemplate).hide().appendTo($('#row-hosting_type'));
    ...
    $repoPlan.change(() => updateHostingForm($hostingType, 'repo-form-hosting', $repoPlan, $hostingRepoForms));
    ...
    $hostingType.change(() => {
      ...
      const isFake = !isCustom && HOSTING_SERVICES[hostingType].fake === true;
      ...
      if (isFake) {
        $hostingPowerPackAdvert.find('.power-pack-advert-hosting-type').text($hostingType.find(':selected').text());
      }
    ...
    $([$hostingType[0], $hostingAccount[0]]).change(() => {
      ...
      if (hostingType === 'custom') {
        ...
        if (hostingInfo.fake !== true) {
          ...
          if ($hostingAccount.val() === '') {
            ...
            const $selectedOption = $($hostingAccount[0].options[selectedIndex]);
    ...
    $tool.change(() => {
      if ($hostingType.val() === 'custom') {
        ...
        if (isFake) {
          $toolPowerPackAdvert.find('.power-pack-advert-hosting-type').text(toolInfo.name);
        }
