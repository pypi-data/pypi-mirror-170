...
RB.CenteredElementManager = Backbone.View.extend({
  ...
  initialize() {
    ...
    this._updatePositionThrottled = _.throttle(() => this.updatePosition(), 10);
  ...
  updatePosition() {
    ...
    this._elements.forEach((containers, el) => {
      ...
      const containerBottom = $bottomContainer.offset().top + $bottomContainer.height();
      /*
      ...
      if (containerTop < windowBottom && containerBottom > windowTop) {
        ...
        if (windowTop >= containerTop && windowBottom <= containerBottom) {
          ...
          const elTop = top - containerTop + Math.round((bottom - top - $el.outerHeight()) / 2);
          $el.css({
            position: 'absolute',
            left: '',
            top: elTop
