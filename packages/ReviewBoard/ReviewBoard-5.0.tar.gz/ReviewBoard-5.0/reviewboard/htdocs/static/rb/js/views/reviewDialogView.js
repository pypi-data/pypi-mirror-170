 ...
 <div class="edit-field">
  ...
  <% } %>
    ...
    render() {
      ...
      this._$issueVerify = this.$('.issue-verify').prop('checked', this.model.requiresVerification()).change(() => {
        const extraData = _.clone(this.model.get('extraData'));

        extraData.require_verification = this._$issueVerify.prop('checked');
...
<div class="edit-field body-bottom"></div>`),
    ...
    _renderDialog() {
      ...
      $rightButtons.append($('<input type="button"/>').val(gettext("Discard Review")).click(() => this._onDiscardClicked()));
      $rightButtons.append($('<input type="button"/>').val(gettext("Close")).click(() => {
      ...
      this._$dlg = $('<div/>').attr('id', 'review-form').append(this.$el).modalBox({
        ...
        stretchY: true,
        buttons: buttons
      }).keypress(e => e.stopPropagation()).attr('scrollTop', 0).trigger('ready');
     ...
     */
    _onDiscardClicked() {
      const $cancelButton = $('<input type="button">').val(gettext("Cancel"));
      const $discardButton = $('<input type="button">').val(gettext("Discard")).click(async () => {
        this.close();
    ...
    _saveReview(publish) {
      ...
      function maybeSave(view) {
        if (view.needsSave()) {
          $.funcQueue('reviewForm').add(() => {
            madeChanges = true;
            view.save().then(() => $.funcQueue('reviewForm').next());
          });
        }
      }
      ...
      $.funcQueue('reviewForm').add(() => {
        ...
        if (this.model.get('public') === publish && this.model.get('shipIt') === shipIt) {
          ...
          }).then(() => $.funcQueue('reviewForm').next()).catch(err => {
