  ...
  RB.ReviewRequestPage.ChangeEntryView = ParentView.extend({
    ...
    render() {
      ...
      _.each(this.$('.file-container'), thumbnailEl => {
        ...
        this.reviewRequestEditorView.buildFileAttachmentThumbnail(model, null, {
          $el: $thumbnail,
          canEdit: false
        });
      });
    ...
    _updateLabels() {
      if (this._reviewViews.some(view => view.hasOpenIssues())) {
        const openIssueCount = this._reviewViews.reduce((sum, view) => sum + view.getOpenIssueCount(), 0);
