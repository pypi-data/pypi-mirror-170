...
RB.DraftReviewBannerView = Backbone.View.extend({
  ...
  render() {
    ...
    this.listenTo(model, 'saving destroying', () => this._$buttons.prop('disabled', true));
    this.listenTo(model, 'saved destroyed', () => this._$buttons.prop('disabled', false));
  ...
  show() {
    ...
    this.$el.prop('hidden', false).removeClass('hidden').css({
      maxHeight: height,
      height: height
    });
  ...
  hide() {
    ...
    this.$el.prop('hidden', true).addClass('hidden').css('max-height', '');
    /*
  ...
  _onDiscardClicked() {
    $('<p/>').text(gettext("If you discard this review, all related comments will be permanently deleted.")).modalBox({
      ...
      buttons: [$('<input type="button">').val(gettext("Cancel")), $('<input type="button">').val(gettext("Discard")).click(() => this.model.destroy())]
    });
    return false;
  }
