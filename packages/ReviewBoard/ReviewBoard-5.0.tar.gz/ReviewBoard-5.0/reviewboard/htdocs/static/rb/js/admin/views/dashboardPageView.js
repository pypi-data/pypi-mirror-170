...
RB.Admin.DashboardPageView = RB.Admin.PageView.extend({
  ...
  renderPage() {
    ...
    const $sizerGutter = this._$widgetsContainer.children('.rb-c-admin-widgets__sizer-gutter');

    const $sizerColumn = this._$widgetsContainer.children('.rb-c-admin-widgets__sizer-column');
  ...
  _loadWidgets() {
    ...
    this.model.loadWidgets(widgetOptions => {
      ...
      widgetView.render();
      this.listenTo(widgetView, 'sizeChanged', () => this._onWidgetSizeChanged(widgetModel.id, widgetView.$el));
