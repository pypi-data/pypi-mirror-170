...
RB.DiffReviewable = RB.AbstractReviewable.extend({
  ...
  defaultCommentBlockFields: ['baseFileDiffID', 'fileDiffID', 'interFileDiffID'],
  ...
  loadSerializedCommentBlock(serializedCommentBlock) {
    this.createCommentBlock({
      ...
      endLineNum: serializedCommentBlock.linenum + serializedCommentBlock.num_lines - 1,
  ...
  getRenderedDiff() {
    ...
    let oldOptions = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};

    if (_.isFunction(options.success) || _.isFunction(options.error) || _.isFunction(options.complete)) {
      console.warn('RB.DiffReviewable.getRenderedDiff was called ' + 'using callbacks. Callers should be updated to ' + 'use promises instead.');
      return RB.promiseToCallbacks(_.defaults({}, options, oldOptions), context, newOptions => this.getRenderedDiff(newOptions));
    }
  ...
  getRenderedDiffFragment(options) {
    ...
    if (_.isFunction(options.success) || _.isFunction(options.error) || _.isFunction(options.complete)) {
      console.warn('RB.DiffReviewable.getRenderedDiffFragment was ' + 'called using callbacks. Callers should be updated ' + 'to use promises instead.');
      return RB.promiseToCallbacks(_.defaults({}, options, oldOptions), context, newOptions => this.getRenderedDiffFragment(newOptions));
    }
...
//# sourceMappingURL=diffReviewableModel.js.map