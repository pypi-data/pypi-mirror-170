...
RB.StarManagerView = Backbone.View.extend({
  ...
  initialize() {
    ...
    this.$('div.star').on('click', this._toggleStar.bind(this)).each((idx, el) => {
      const $el = $(el);
  ...
  _updateStarColumn(objID) {
    ...
    $el.toggleClass('rb-icon-star-on', starred).toggleClass('rb-icon-star-off', !starred).attr('title', starred ? gettext("Starred") : gettext("Click to star"));
  },
  ...
  _toggleStar(e) {
    ...
    $target.toggleClass('rb-icon-star-on', objStarred).toggleClass('rb-icon-star-off', !objStarred).attr('title', objStarred ? gettext("Starred") : gettext("Click to star"));
  }

});

//# sourceMappingURL=starManagerView.js.map