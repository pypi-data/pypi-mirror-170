...
RB.MenuButtonView = Backbone.View.extend({
  ...
  template: _.template(`<% if (hasPrimaryButton) { %>
...
<% } %>`),
  ...
  initialize() {
    ...
    this._hasPrimaryButton = !!this._buttonOnClick || options.hasPrimaryButton;
    this.menu = null;
  ...
  render() {
    ...
    this.$el.addClass(this.className).attr('role', 'group').html(this.template({
    ...
    if (hasPrimaryButton) {
      this.$primaryButton = this.$('.rb-c-menu-button__primary').on('click', this._buttonOnClick.bind(this));
...
//# sourceMappingURL=menuButtonView.js.map