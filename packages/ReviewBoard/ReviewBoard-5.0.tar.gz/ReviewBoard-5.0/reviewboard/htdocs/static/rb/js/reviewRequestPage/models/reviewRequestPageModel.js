...
RB.ReviewRequestPage.ReviewRequestPage = RB.ReviewablePage.extend({
  ...
  _loadUpdates() {
    ...
    urlQuery.sort();
    const urlQueryStr = urlQuery.length > 0 ? `?${urlQuery.join('&')}` : '';
    Backbone.sync('read', this, {
      url: `${updatesURL}${urlQueryStr}`,
      dataType: 'arraybuffer',
      noActivityIndicator: true,
      success: arrayBuffer => this._processUpdatesFromPayload(arrayBuffer, options.onDone)
    });
  },
  ...
  _processUpdatesFromPayload(arrayBuffer, onDone) {
    ...
    while (!done) {
      const parsed = this._processUpdateFromPayload(arrayBuffer, dataView, pos);
  ...
  _processUpdateFromPayload(arrayBuffer, dataView, pos) {
    ...
    return {
      ...
      load(cb) {
        const metadataBlob = new Blob([arrayBuffer.slice(metadataStart, metadataStart + metadataLen)]);
        const htmlBlob = new Blob([arrayBuffer.slice(htmlStart, htmlStart + htmlLen)]);
        RB.DataUtils.readManyBlobsAsStrings([metadataBlob, htmlBlob], (metadata, html) => cb(JSON.parse(metadata), html));
      }
