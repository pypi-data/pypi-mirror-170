...
RB.InlineEditorView = Backbone.View.extend({
  ...
  defaultOptions: {
    ...
    isFieldDirty: (editor, initialValue) => {
      ...
      const normValue = (editor.options.hasRawValue ? value : value.htmlEncode()) || '';
  ...
  setupEvents() {
    ...
    if (!this.options.useEditIconOnly) {
       ...
       */
      let isDragging = true;
      this.$el.on('click', 'a', e => e.stopPropagation()).click(e => {
        e.stopPropagation();
        e.preventDefault();
  ...
  showEditor() {
    ...
    this.$buttons.show(); // Execute this after the animation, if we performed one.
...
RB.RichTextInlineEditorView = RB.InlineEditorView.extend({
  ...
  createField() {
    ...
    this.once('beginEdit', () => {
      ...
      const $markdownRef = $('<a class="markdown-info" target="_blank">').attr('href', `${MANUAL_URL}users/markdown/`).text(gettext("Markdown Reference")).setVisible(this.textEditor.richText).appendTo(this.$buttons);
...
RB.DateInlineEditorView = RB.InlineEditorView.extend({
 ...
 <input type="date"/>
</span>`).prepend(this.options.descriptorText);
    this._$datePickerInput = this._$datePicker.find('input').attr({
