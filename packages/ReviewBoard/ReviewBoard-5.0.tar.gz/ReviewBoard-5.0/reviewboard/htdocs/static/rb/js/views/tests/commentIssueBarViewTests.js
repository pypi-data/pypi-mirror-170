...
suite('rb/views/CommentIssueBarView', function () {
  ...
  describe('Actions', function () {
    ...
    it('Resolving as fixed', function (done) {
      spyOn(commentIssueManager, 'setCommentState').and.callFake((reviewID, commentID, commentType, state) => {
  ...
  describe('Event handling', function () {
    describe('CommentIssueManager.issueStatusUpdated', function () {
      ...
      function _buildTests(commentType, CommentCls, rspNamespace, OtherCommentCls, otherRspNamespace) {
        beforeEach(function () {
          view = new RB.CommentIssueBarView({
            commentIssueManager: commentIssueManager,
        ...
        it('When comment updated', function () {
          ...
          commentIssueManager._notifyIssueStatusChanged(comment, rsp, COMMENT_STATUS);
      ...
      describe('For diff comments', function () {
        _buildTests(CommentTypes.DIFF, RB.DiffComment, 'diff_comment', RB.GeneralComment, 'general_comment');
      });
      describe('For general comments', function () {
        _buildTests(CommentTypes.GENERAL, RB.GeneralComment, 'general_comment', RB.DiffComment, 'diff_comment');
      });
      describe('For file attachment comments', function () {
        _buildTests(CommentTypes.FILE_ATTACHMENT, RB.FileAttachmentComment, 'file_attachment_comment', RB.DiffComment, 'diff_comment');
      });
      describe('For screenshot comments', function () {
        _buildTests(CommentTypes.SCREENSHOT, RB.ScreenshotComment, 'screenshot_comment', RB.GeneralComment, 'general_comment');
      });
    });
  });
  describe('Issue states', function () {
    ...
    describe('Fixed', function () {
      ...
      it('Text', function () {
        expect(view._$message.text()).toBe('The issue has been resolved.');
      });
      describe('Button visibility', function () {
    ...
    describe('Dropped', function () {
      ...
      it('Text', function () {
        expect(view._$message.text()).toBe('The issue has been dropped.');
      });
      describe('Button visibility', function () {
        it('"Drop" hidden', function () {
          expect($dropButton.is(':visible')).toBe(false);
    ...
    describe('Verifying Fixed', function () {
      ...
      it('Text', function () {
        expect(view._$message.text()).toBe('Waiting for verification before resolving...');
      });
      describe('Button visibility', function () {
        it('"Drop" hidden', function () {
          expect($dropButton.is(':visible')).toBe(false);
    ...
    describe('Verifying Dropped', function () {
      ...
      it('Text', function () {
        expect(view._$message.text()).toBe('Waiting for verification before dropping...');
      });
      describe('Button visibility', function () {
        it('"Drop" hidden', function () {
          expect($dropButton.is(':visible')).toBe(false);
...
//# sourceMappingURL=commentIssueBarViewTests.js.map