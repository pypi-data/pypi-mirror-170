...
RB.DiffFragmentView = Backbone.View.extend({
  ...
  showControls() {
    ...
    this._$table.removeClass('collapsed').addClass('expanded');
    /*
  ...
  hideControls(animate) {
    ...
    this._$table.removeClass('expanded').addClass('collapsed');
    ...
    if ($firstDiffHeader.hasClass('diff-header-above')) {
       ...
       */
      const translateY = $firstDiffHeader.height() - this.COLLAPSED_HEADERS_HEIGHT;
  ...
  _onExpandOrCollapseFinished() {
    ...
    if ($collapseButtons.length > 0) {
      this._centeredMgr = new RB.CenteredElementManager({
        elements: new Map(Array.prototype.map.call($collapseButtons, el => {
          const $chunks = $(el).closest('.sidebyside').children('tbody').not('.diff-header');
          return [el, {
            $top: $chunks.eq(0),
            $bottom: $chunks.eq(-1)
          }];
...
//# sourceMappingURL=diffFragmentView.js.map