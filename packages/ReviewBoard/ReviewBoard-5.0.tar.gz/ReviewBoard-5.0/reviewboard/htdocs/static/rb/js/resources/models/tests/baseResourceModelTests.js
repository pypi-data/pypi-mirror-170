  ...
  describe('fetch', function () {
    describe('Basic functionality', function () {
      ...
      it('With isNew=true', async function () {
        ...
        await expectAsync(model.fetch()).toBeRejectedWith(Error('fetch cannot be used on a resource without an ID'));
      ...
      it('With isNew=false and parentObject with error', async function () {
        ...
        spyOn(parentObject, 'ready').and.rejectWith(new BackboneError(parentObject, {
          errorText: 'Oh nosers.'
        }, {}));
        await expectAsync(model.fetch()).toBeRejectedWith(Error('Oh nosers.'));
        expect(parentObject.ready).toHaveBeenCalled();
        expect(Backbone.Model.prototype.fetch).not.toHaveBeenCalled();
      });
      it('With callbacks', function (done) {
  ...
  describe('save', function () {
    ...
    it('With isNew=true and no parentObject', async function () {
      ...
      try {
        ...
        expect(err.message).toBe('The object must either be loaded from the server ' + 'or have a parent object before it can be saved');
    ...
    it('With isNew=false and no parentObject', async function () {
      ...
      spyOn(Backbone.Model.prototype, 'save').and.callFake((attrs, options) => {
        if (options && _.isFunction(options.success)) {
          options.success();
        }
      });
    ...
    it('With isNew=false and parentObject with error', async function () {
      ...
      await expectAsync(model.save()).toBeRejectedWith(Error('Oh nosers.'));
      expect(parentObject.ready).toHaveBeenCalled();
      expect(Backbone.Model.prototype.save).not.toHaveBeenCalled();
      expect(model.trigger).not.toHaveBeenCalledWith('saved');
    });
    ...
    describe('With file upload support', function () {
      ...
      it('With file', async function () {
        ...
        spyOn($, 'ajax').and.callFake(request => {
          ...
          expect(request.contentType.indexOf('multipart/form-data; boundary=')).toBe(0);
          ...
          fileReader.onload = function () {
            ...
            expect(data.join('')).toBe('--' + boundary + '\r\n' + 'Content-Disposition: form-data; name="file"' + '; filename="myfile"\r\n' + 'Content-Type: text/plain\r\n\r\n' + 'Hello world!' + '\r\n' + '--' + boundary + '\r\n' + 'Content-Disposition: form-data; ' + 'name="myfield"\r\n\r\n' + 'myvalue\r\n' + '--' + boundary + '--\r\n\r\n');
            request.success({
              stat: 'ok',
              foo: {
                id: 42
    ...
    describe('With form upload support', function () {
      ...
      it('Overriding toJSON attributes', async function () {
        ...
        spyOn(RB, 'apiCall').and.callThrough();
        spyOn($, 'ajax');
        spyOn(form, 'ajaxSubmit').and.callFake(request => request.success({}));
        await model.save({
          form: form
        });
        expect(RB.apiCall).toHaveBeenCalled();
...
//# sourceMappingURL=baseResourceModelTests.js.map