...
<% } %>`),
  ...
  const DraftBannerView = BannerView.extend({
    ...
    _newDraftTemplate: _.template(`<div class="interdiff-link">
 <%- newDiffText %>
 <a href="<%- interdiffLink %>"><%- showChangesText %></a>
  ...
  RB.ReviewRequestEditorView = Backbone.View.extend({
    ...
    _archiveActionsTemplate: _.template(`<% if (visibility === RB.ReviewRequest.VISIBILITY_VISIBLE) { %>
...
<% } %>`),
     ...
     */
    initialize() {
      _.bindAll(this, '_checkResizeLayout', '_scheduleResizeLayout', '_onCloseDiscardedClicked', '_onCloseSubmittedClicked', '_onDeleteReviewRequestClicked', '_onUpdateDiffClicked', '_onArchiveClicked', '_onUnarchiveClicked', '_onMuteClicked', '_onUnmuteClicked', '_onUploadFileClicked');
    ...
    addFieldView(view) {
      ...
      this.listenTo(view, 'fieldError', err => {
        this._$warning.delay(6000).fadeOut(400, () => this._$warning.hide()).html(err.errorText).show();
      });
    ...
    render() {
      ...
      this.listenTo(reviewRequest, 'change:visibility', this._updateArchiveVisibility);
      ...
      if (this.model.get('editable')) {
        RB.DnDUploader.instance.registerDropTarget(this._$attachmentsContainer, gettext("Drop to add a file attachment"), this._uploadFile.bind(this));
      }
      ...
      this.listenTo(fileAttachments, 'remove', model => {
        const index = this._fileAttachmentThumbnailViews.findIndex(view => view.model === model);
      ...
      _.each(this._$screenshots.find('.screenshot-container'), this._importScreenshotThumbnail, this);

      _.each($('.binary'), this._importFileAttachmentThumbnail, this); // Render all the field views.
    ...
    showBanner() {
      ...
      } else if (state === RB.ReviewRequest.PENDING && this.model.get('hasDraft')) {
    ...
    publishDraft(options) {
      ...
      const fields = Object.values(this._fieldViews).filter(view => view.needsSave());
      this.model.set({
        publishing: true,
     ...
     */
    buildFileAttachmentThumbnail(fileAttachment, collection) {
      ...
      this.listenTo(view, 'hoverIn', $thumbnail => {
        this._$attachments.find('.file').not($thumbnail.find('.file')[0]).addClass('faded');
      });
      this.listenTo(view, 'hoverOut', () => this._$attachments.find('.file').removeClass('faded'));
    ...
    _resizeLayout() {
      ...
      $detailsValues.css('max-width', detailsWidth - $detailsLabels.outerWidth() - detailsPadding * 3 + 'px');
      /*
      ...
      this._$main.height(Math.ceil(this._$extra.offset().top - this._$main.offset().top));
      ...
      if ($lastContent.outerHeight() + lastContentTop < height) {
        ...
        const contentHeight = $lastContent.height() - Math.ceil($lastFieldContainer.position().top);
        /*
        ...
        if (lastFieldView.inlineEditorView.editing() && editor) {
          editor.setSize(null, contentHeight - lastFieldView.inlineEditorView.$buttons.height());
        } else {
          /*
           ...
           * height.
           */
          const newEditableHeight = contentHeight + $lastField.getExtents('m', 'tb');
    ...
    _onDeleteReviewRequestClicked() {
      const $dlg = $('<p>').text(gettext("This deletion cannot be undone. All diffs and reviews will be deleted as well.")).modalBox({
        ...
        buttons: [$(`<input type="button" value="${gettext("Cancel")}"/>`), $(`<input type="button" value="${gettext("Delete")}"/>`).click(() => {
     ...
     */
    _onArchiveClicked() {
      return this._updateArchiveState(RB.UserSession.instance.archivedReviewRequests, true, RB.ReviewRequest.VISIBILITY_ARCHIVED);
    },
     ...
     */
    _onUnarchiveClicked() {
      return this._updateArchiveState(RB.UserSession.instance.archivedReviewRequests, false, RB.ReviewRequest.VISIBILITY_VISIBLE);
    },
     ...
     */
    _onMuteClicked() {
      return this._updateArchiveState(RB.UserSession.instance.mutedReviewRequests, true, RB.ReviewRequest.VISIBILITY_MUTED);
    },
    ...
    _updateArchiveVisibility() {
      ...
      const iconClass = visible ? 'rb-icon-archive-off' : 'rb-icon-archive-on';
      const iconTitle = visible ? gettext("Archive review request") : gettext("Unarchive review request");
      const iconId = visible ? 'toggle-archived' : 'toggle-unarchived';
      this.$('#hide-review-request-link').html(`<span class="rb-icon ${iconClass}" id="${iconId}" title="${iconTitle}"></span>`);
...
//# sourceMappingURL=reviewRequestEditorView.js.map