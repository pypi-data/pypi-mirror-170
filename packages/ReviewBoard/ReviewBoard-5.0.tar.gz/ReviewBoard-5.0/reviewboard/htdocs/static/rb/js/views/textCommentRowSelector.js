...
RB.TextCommentRowSelector = Backbone.View.extend({
  ghostCommentFlagTemplate: `<span class="commentflag ghost-commentflag">
 ...
 <span class="commentflag-inner"></span>
  ...
  initialize(options) {
    ...
    this._supportsSetClipboard = window.getSelection !== undefined && window.Range !== undefined && window.Range.prototype.cloneContents !== undefined;
   ...
   */
  render() {
    this._$ghostCommentFlag = $(this.ghostCommentFlagTemplate).on({
  ...
  findLineNumRow(lineNum, startRow, endRow) {
    ...
    if (table.rows.length - rowOffset > lineNum) {
      row = table.rows[rowOffset + lineNum]; // Account for the "x lines hidden" row.
  ...
  _onCopy(e) {
    ...
    if (clipboardData && this._supportsSetClipboard && this._copySelectionToClipboard(clipboardData)) {
      /*
  ...
  _onMouseDown(e) {
    ...
    const node = this._$ghostCommentFlagCell ? this._$ghostCommentFlagCell[0] : e.target;
  ...
  _onMouseOver(e) {
    ...
    } else if (this._$ghostCommentFlagCell && $node[0] !== this._$ghostCommentFlagCell[0]) {
...
//# sourceMappingURL=textCommentRowSelector.js.map