...
suite('rb/utils/apiUtils', function () {
    ...
    spyOn($, 'ajax').and.callFake(request => {
      ...
      if (statusCode >= 200 && statusCode < 300 || statusCode === 304) {
        ...
        if (request.success !== undefined) {
          request.success(rsp !== undefined ? rsp : options.responseText, statusText, xhr);
        }
      } else {
  ...
  describe('apiCall', function () {
    ...
    function runAPICallTest(done, options) {
      ...
      const $activityIndicator = $('<div><span class="indicator-text"></span></div>').css('display', 'none').appendTo($testsScratch);
      /*
      ...
      function runCommonSuccessErrorChecks() {
        ...
        expect($activityIndicator.find('.indicator-text').text()).toBe(options.method === 'GET' ? 'Loading...' : 'Saving...');
      }
      ...
      data = {
        ...
        complete: function (xhr, statusText) {
          ...
          const activityIndicatorHasError = $activityIndicator.hasClass('error');
          const activityIndicatorDisplay = $activityIndicator.css('display');
        ...
        data.success = function (rsp, statusText, xhr) {
          if (options.expectSuccess) {
            ...
            if (options.expectAPIErrorSuccess) {
              ...
              expect(arguments[1]).toBe(options.expectedStatusText);
    ...
    ['DELETE', 'GET', 'POST', 'PATCH', 'PUT'].forEach(method => addAPICallTests(method, 'success', 'error'));
    addAPICallTests('HEAD', 'nocontent', 'nocontent');
  });
});

//# sourceMappingURL=apiUtilsTests.js.map