...
RB.ReviewRequestPage.ReviewReplyDraftBannerView = RB.FloatingBannerView.extend({
  className: 'banner',
  template: _.template(`<h1><%- draftText %></h1>
<p><%- reminderText %></p>
...
<% } %>`),
  ...
  render() {
    ...
    this.listenTo(this.model, 'saving destroying', () => this.$('input').prop('disabled', true));
    this.listenTo(this.model, 'saved', () => this.$('input').prop('disabled', false));
    this.listenTo(this.model, 'publishError', errorText => alert(errorText));
    return this;
  },
...
//# sourceMappingURL=reviewReplyDraftBannerView.js.map