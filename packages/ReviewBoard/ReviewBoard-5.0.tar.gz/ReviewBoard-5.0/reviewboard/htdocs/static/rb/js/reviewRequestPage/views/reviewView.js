...
RB.ReviewRequestPage.ReviewView = Backbone.View.extend({
  ...
  render() {
    ...
    const $comment = this._$reviewComments.find('.review-comment-details .review-comment');
    ...
    _.each(this._$reviewComments.find('.issue-indicator'), el => {
      ...
      if ($issueState.length > 0) {
        ...
        issueBar.render();
        this.listenTo(issueBar, 'statusChanged', (oldStatus, newStatus) => {
    ...
    for (let i = 0; i < diffCommentsData.length; i++) {
      ...
      page.queueLoadDiff(diffCommentData[0], diffCommentData[1], view => this._diffFragmentViews.push(view));
    }
    /*
  ...
  _setupNewReply(reviewReply) {
    ...
    if (this._reviewReply !== null) {
      this.stopListening(this._reviewReply); // Update all the existing editors to point to the new object.

      this._replyEditors.forEach(editor => editor.set('reviewReply', reviewReply));

      this.trigger('hasDraftChanged', false);
    }

    this.listenTo(reviewReply, 'destroyed published', () => this._setupNewReply());
