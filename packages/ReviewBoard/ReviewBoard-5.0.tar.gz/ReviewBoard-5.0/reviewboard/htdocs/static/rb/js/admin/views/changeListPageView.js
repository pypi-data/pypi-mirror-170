 ...
 <ul class="rb-c-drawer__action-group">
  ...
  RB.Admin.ChangeListPageView = RB.Admin.PageView.extend({
    ...
    renderPage() {
      ...
      const modelNameLowerPlural = model.get('modelNamePlural').toLowerCase();
      this.listenTo(model, 'change:selectionCount', (model, count) => {
        this.drawer.$summary.text(interpolate(ngettext("%(count)s %(modelNameLower)s selected", "%(count)s %(modelNameLowerPlural)s selected", count), {
          "count": count,
