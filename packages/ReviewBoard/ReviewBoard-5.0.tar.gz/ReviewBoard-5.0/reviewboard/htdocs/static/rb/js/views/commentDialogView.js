...
(function () {
  ...
  const CommentsListView = Backbone.View.extend({
    itemTemplate: _.template(`<li class="<%= itemClass %>">
 <h2>
  <%- comment.user.name %>
  <span class="actions">
 ...
 <pre><%- comment.text %></pre>
  ...
  RB.CommentDialogView = Backbone.View.extend({
    ...
    template: _.template(`<div class="other-comments">
   ...
   <input type="button" class="close" value="<%- closeButton %>">
    ...
    render() {
      ...
      this.$el.hide().html(this.template({
        ...
        loginText: interpolate(RB.CommentDialogView._loginTextTemplate, [userSession.get('loginURL')]),
        draftWarning: interpolate(RB.CommentDialogView._draftWarningTextTemplate, [reviewRequest.get('reviewURL')]),
      ...
      this._$issueOptions = this._$commentOptions.children('.comment-issue-options').bindVisibility(this.model, 'canEdit');
      this._$markdownOptions = this._$commentOptions.children('.comment-markdown-options').bindVisibility(this.model, 'canEdit');
      this._$issueField = this._$issueOptions.find('#comment_issue').bindProperty('checked', this.model, 'openIssue').bindProperty('disabled', this.model, 'editing', {
        elementToModel: false,
        inverse: true
      });
      this._$issueVerificationField = this._$issueOptions.find('#comment_issue_verify').bindProperty('checked', this.model, 'requireVerification').bindProperty('disabled', this.model, 'editing', {
        elementToModel: false,
        inverse: true
      });
      this._$enableMarkdownField = this._$markdownOptions.find('#enable_markdown').bindProperty('checked', this.model, 'richText').bindProperty('disabled', this.model, 'editing', {
        elementToModel: false,
        inverse: true
      });
      this.$buttons = this._$footer.find('.buttons');
      this.$saveButton = this.$buttons.find('input.save').bindVisibility(this.model, 'canEdit').bindProperty('disabled', this.model, 'canSave', {
        elementToModel: false,
        inverse: true
      });
      this.$cancelButton = this.$buttons.find('input.cancel').bindVisibility(this.model, 'canEdit');
      this.$deleteButton = this.$buttons.find('input.delete').bindVisibility(this.model, 'canDelete').bindProperty('disabled', this.model, 'canDelete', {
        elementToModel: false,
        inverse: true
      });
      this.$closeButton = this.$buttons.find('input.close').bindVisibility(this.model, 'canEdit', {
        inverse: true
      });
      ...
      this.listenTo(this._textEditor, 'change', () => this.model.set('text', this._textEditor.getText()));
      ...
      this.listenTo(this.model, 'change:text', () => this._textEditor.setText(this.model.get('text')));
     ...
     */
    close() {
      let onClosed = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : undefined;
     ...
     */
    _updateTitle() {
      this._$title.text(this.model.get('dirty') ? RB.CommentDialogView._yourCommentDirtyText : RB.CommentDialogView._yourCommentText);
    },
    ...
    _handleResize() {
      ...
      if (this._$commentsPane.is(':visible')) {
        this._$commentsPane.outerWidth(this.COMMENTS_BOX_WIDTH).outerHeight(height).move(0, 0, 'absolute');
      ...
      this._$draftForm.outerWidth(width).outerHeight(height).move(commentsWidth, 0, 'absolute');

      const $textField = this._textEditor.$el;

      this._textEditor.setSize(this._$body.width() - $textField.getExtents('b', 'lr'), this._$draftForm.height() - this._$header.outerHeight() - this._$commentOptions.outerHeight() - this._$footer.outerHeight() - $textField.getExtents('b', 'tb'));
    },
    ...
    create: function () {
      ...
      dlg.render().$el.css('z-index', 999) // XXX Use classes for z-indexes.
...
//# sourceMappingURL=commentDialogView.js.map