...
RB.UploadDiffView = Backbone.View.extend({
  ...
  render() {
    ...
    this._$fileInput = $('<input type="file">').hide().appendTo(this.$el).change(() => this.model.handleFiles(this._$fileInput.get(0).files));
    this._$diffRevisionError = this.$('#parent-diff-error-contents');
    this._$error = this.$('#error-indicator').hide();
    this._$errorContents = this.$('#error-contents');
    this._$processingDiff = this.$('#processing-diff').hide();
    this._$promptForBasedir = this.$('#prompt-for-basedir').hide();
    this._$promptForChangeNumber = this.$('#prompt-for-change-number').hide();
    this._$promptForDiff = this.$('#prompt-for-diff').hide();
    this._$promptForParentDiff = this.$('#prompt-for-parent-diff').hide();
    this._$uploading = this.$('#uploading-diffs').hide();
   ...
   */
  _canDragDrop() {
    return ('draggable' in this.el || 'ondragstart' in this.el && 'ondrop' in this.el) && !navigator.userAgent.match('iPhone OS') && !navigator.userAgent.match('iPad');
  },
  ...
  _onStateChanged(model, state) {
    this._$promptForDiff.setVisible(state === this.model.State.PROMPT_FOR_DIFF);

    this._$promptForParentDiff.setVisible(state === this.model.State.PROMPT_FOR_PARENT_DIFF);

    this._$promptForBasedir.setVisible(state === this.model.State.PROMPT_FOR_BASEDIR);

    this._$processingDiff.setVisible(state === this.model.State.PROCESSING_DIFF);

    this._$promptForChangeNumber.setVisible(state === this.model.State.PROMPT_FOR_CHANGE_NUMBER);
...
//# sourceMappingURL=uploadDiffView.js.map