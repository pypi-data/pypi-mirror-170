      ...
      describe('With existing review', function () {
        ...
        describe('General comments', function () {
          it('Disabled', async function () {
            ...
            expect($.ajax.calls.argsFor(0)[0].url).not.toBe('/general-comments/');
          ...
          describe('Enabled', function () {
            async function testLoadGeneralComments() {
              ...
              expect($.ajax).toHaveBeenCalled();
              expect($.ajax.calls.argsFor(0)[0].url).toBe('/general-comments/');
              ajaxData = $.ajax.calls.argsFor(0)[0].data;
              expect(dlg._commentViews.length).toBe(1);
              commentView = dlg._commentViews[0];
              expect(commentView.$editor.text()).toBe(generalCommentPayload.text);
              expect(commentView.$issueOpened.prop('checked')).toBe(generalCommentPayload.issue_opened);
              expect(dlg._bodyBottomView.$el.is(':visible')).toBe(true);
              expect(dlg._$spinner).toBe(null);
            }
            ...
            it('Deleting comment', async function () {
              ...
              spyOn(dlg._commentViews[0].$el, 'fadeOut').and.callFake(opts => opts.complete());
        ...
        describe('Diff comments', function () {
          async function testLoadDiffComments() {
            ...
            dlg = await createReviewDialog();
            expect($.ajax).toHaveBeenCalled();
            expect($.ajax.calls.argsFor(3)[0].url).toBe('/diff-comments/');
            ...
            expect(dlg._commentViews.length).toBe(1);
            commentView = dlg._commentViews[0];
            expect(commentView.$editor.text()).toBe(diffCommentPayload.text);
            expect(commentView.$issueOpened.prop('checked')).toBe(diffCommentPayload.issue_opened);
            expect(dlg._bodyBottomView.$el.is(':visible')).toBe(true);
            expect(dlg._$spinner).toBe(null);
          }
        ...
        describe('File attachment comments', function () {
          async function testLoadFileAttachmentComments() {
            ...
            dlg = await createReviewDialog();
            expect($.ajax).toHaveBeenCalled();
            expect($.ajax.calls.argsFor(2)[0].url).toBe('/file-attachment-comments/');
            ajaxData = $.ajax.calls.argsFor(2)[0].data;
            expect(dlg._commentViews.length).toBe(1);
            commentView = dlg._commentViews[0];
            expect(commentView.$editor.text()).toBe(fileAttachmentCommentPayload.text);
            expect(commentView.$issueOpened.prop('checked')).toBe(fileAttachmentCommentPayload.issue_opened);
            expect(commentView.$('.filename a').attr('href')).toBe(fileAttachmentCommentPayload.review_url);
            expect(commentView.$('.filename a').text()).toBe(fileAttachmentCommentPayload.link_text);
            expect(commentView.$('.thumbnail').html()).toBe(fileAttachmentCommentPayload.thumbnail_html);
            expect(dlg._bodyBottomView.$el.is(':visible')).toBe(true);
            expect(dlg._$spinner).toBe(null);
          }
          ...
          it('Deleting comment', async function () {
            spyOn(window, 'confirm').and.returnValue(true);
            await testLoadFileAttachmentComments();
            expect(dlg._fileAttachmentCommentsCollection.length).toBe(1);
            spyOn(dlg._commentViews[0].$el, 'fadeOut').and.callFake(opts => opts.complete());
            let caughtEvent = new Promise((resolve, reject) => {
            ...
            dlg.$('.delete-comment').click();
            await caughtEvent;
            expect(dlg._fileAttachmentCommentsCollection.length).toBe(0);
          });
          it('Deleting comment and cancelling', async function () {
            spyOn(window, 'confirm').and.returnValue(false);
            await testLoadFileAttachmentComments();
        ...
        describe('Screenshot comments', function () {
          async function testLoadScreenshotComments() {
            ...
            dlg = await createReviewDialog();
            expect($.ajax).toHaveBeenCalled();
            expect($.ajax.calls.argsFor(1)[0].url).toBe('/screenshot-comments/');
            ajaxData = $.ajax.calls.argsFor(1)[0].data;
            expect(dlg._commentViews.length).toBe(1);
            commentView = dlg._commentViews[0];
            expect(commentView.$editor.text()).toBe(screenshotCommentPayload.text);
            expect(commentView.$issueOpened.prop('checked')).toBe(screenshotCommentPayload.issue_opened);
            const $img = commentView.$('img');
            expect($img.attr('src')).toBe(screenshotCommentPayload.thumbnail_url);
            expect($img.attr('width')).toBe(screenshotCommentPayload.w.toString());
            expect($img.attr('height')).toBe(screenshotCommentPayload.h.toString());
            expect($img.attr('alt')).toBe(screenshotCommentPayload.screenshot.caption);
            const $filenameA = commentView.$('.filename a');
            expect($filenameA.attr('href')).toBe(screenshotCommentPayload.screenshot.review_url);
            expect($filenameA.text()).toBe(screenshotCommentPayload.screenshot.caption);
            expect(dlg._bodyBottomView.$el.is(':visible')).toBe(true);
            expect(dlg._$spinner).toBe(null);
          }
          ...
          it('Deleting comment', async function () {
            spyOn(window, 'confirm').and.returnValue(true);
            await testLoadScreenshotComments();
            expect(dlg._screenshotCommentsCollection.length).toBe(1);
            spyOn(dlg._commentViews[0].$el, 'fadeOut').and.callFake(opts => opts.complete());
            let caughtEvent = new Promise((resolve, reject) => {
            ...
            dlg.$('.delete-comment').click();
            await caughtEvent;
            expect(dlg._screenshotCommentsCollection.length).toBe(0);
          });
          it('Deleting comment and cancelling', async function () {
            spyOn(window, 'confirm').and.returnValue(false);
            await testLoadScreenshotComments();
...
//# sourceMappingURL=reviewDialogViewTests.js.map