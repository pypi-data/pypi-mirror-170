...
RB.CommentEditor = Backbone.Model.extend(_.defaults({
  ...
  initialize() {
    ...
    this.on('change:openIssue change:requireVerification ' + 'change:richText change:text', () => {
      if (this.get('editing')) {
        this.set('dirty', true);
  ...
  _updateCanEdit() {
    const reviewRequest = this.get('reviewRequest');
    const userSession = RB.UserSession.instance;
    this.set('canEdit', userSession.get('authenticated') && !userSession.get('readOnly') && !reviewRequest.get('hasDraft'));
  },
