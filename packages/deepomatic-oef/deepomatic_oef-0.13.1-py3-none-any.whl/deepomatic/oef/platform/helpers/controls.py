...
def set_protobuf_value(protobuf, value, key):
    protobuf, key = protobuf_iterate(protobuf, key)
    field_descriptor = protobuf.DESCRIPTOR.fields_by_name[key]

    if field_descriptor.message_type is not None:
        # The field we are trying to set is a full message protobuf (i.e. not a scalar)
        message_class = load_proto_class_from_protobuf_descriptor(field_descriptor.message_type)

        # If field is repeated (a list):
        if field_descriptor.label == FieldDescriptor.LABEL_REPEATED:
            # this weird syntax sets the list (e.g. equivalent of protobuf.key[:] = [1, 2, 3])
            protobuf.ClearField(key)
            for v in value:
                message = build_message_from_payload(message_class, v)
                getattr(protobuf, key).append(message)
        else:  # it's a single message
            message = build_message_from_payload(message_class, value)
            getattr(protobuf, key).MergeFrom(message)

    else:
        # The field we are trying to set is a scalar or a list of scalars
        # If field is repeated (a list):
        if field_descriptor.label == FieldDescriptor.LABEL_REPEATED:
            # this weird syntax sets the list (e.g. equivalent of protobuf.key[:] = [1, 2, 3])
            getattr(protobuf, key)[:] = value
        else:
            # otherwise we assume it's a scalar
            setattr(protobuf, key, value)


def set_protobuf_oneof(protobuf, value, key):
    """The oneof name is assumed to be the last key"""
    protobuf, key = protobuf_iterate(protobuf, key, check_last_one=False)
    if protobuf.WhichOneof(key) != value:
        field = getattr(protobuf, value)
        field_descriptor = protobuf.DESCRIPTOR.fields_by_name[value]
        message_class = load_proto_class_from_protobuf_descriptor(field_descriptor.message_type)
        field.MergeFrom(message_class())
...
class Control:
    ...
    def __init__(self,
                 property_name: str,
                 protobuf_setter_fn: Callable[[Message, Any], None],
                 message: str,
...
class SelectControl(Control):
    ...
    def __init__(self, property_name, message, values, tags=None, display_ifs=None, default_value=''):
        """
        ...
        if isinstance(self, ModelControl):
            ...
            protobuf_setter_fn = functools.partial(set_protobuf_oneof, key=property_name)
...
class InputControl(Control):
    ...
    def __init__(self, property_name, message, min_value=None, max_value=None, increment_value=None, percent=False, display_ifs=None, default_value=''):
        """
...
class ListControl(Control):
    ...
    def __init__(self, property_name, message, form, display_ifs=None, default_value=''):
        """
        A helper class to define a INPUT. See Control for a description of other arguments.
...
class ToggleControl(Control):
    ...
    def __init__(self, property_name, message, tags=None, display_ifs=None, default_value='', protobuf_setter_fn=None):
        ...
        Args:
            ...
            display_ifs (list): see Control
            default_value: see Control
            protobuf_setter_fn: see Control
        """
        def check_control_default_fn():
