...
class NamespaceClient:
    ...
    def create(self, path: str, *, recursive: bool = False) -> None:
        if not recursive:
        ...
        for partial_path in path.chain():
            try:
                self._fetch_raw(partial_path.resolve())
            except service.Error:
                self._create(partial_path.resolve())

    def _create(self, path: str) -> None:
        path = keyspace.Path.from_path(path)
        parent, _, parent_credentials = self._fetch_raw(path.parent.resolve())
    ...
    def _fetch_raw(
        ...
        response: protocol.FetchNamespaceChainEgress = self._origin.react(
            action="fetch_namespace",
    ...
    def list_children(self, path: str) -> typing.List[typing.Union[models.Namespace, models.Entry]]:
        path = keyspace.Path.from_path(path)
        response: protocol.ListNamespaceChildrenEgress = self._origin.react(
            action="list_namespace_children",
    ...
    def grant(
        ...
        _, _, namespace_credentials = self._workspace_client.namespace._fetch_raw(path.resolve())
    ...
    def list_grants(self, path: str) -> typing.List[models.NamespaceGrant]:
        path = keyspace.Path.from_path(path)

        response: protocol.ListNamespaceGrantsEgress = self._origin.react(
            action="list_namespace_grants",
