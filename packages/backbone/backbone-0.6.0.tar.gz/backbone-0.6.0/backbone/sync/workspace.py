...
class WorkspaceClient:
    ...
    def create(self) -> None:
        ...
        self._origin.react(action="create_workspace", data=data)

        try:
            self._user_client.trust(workspace)
        except:
            self.delete()
            raise

    @endpoint(action="fetch_workspace", ingress=protocol.WorkspaceBound, egress=protocol.FetchWorkspaceEgress)
    def _fetch_raw(self) -> (models.SignedWorkspace, typing.List[models.SignedSharedWorkspaceGrant]):
        response: protocol.FetchWorkspaceEgress = self._origin.react(
    ...
    def fetch(self) -> models.Workspace:
        signed_workspace, _ = self._fetch_raw()
        return self._trust.authenticate_workspace(signed_workspace)

    def share_identity(self) -> str:
        workspace, _ = self._fetch_raw()
    ...
    def list_grants(self) -> typing.List[models.SharedWorkspaceGrant]:
        signed_workspace, signed_workspace_grants = self._fetch_raw()
    ...
    def delete(self) -> None:
        self._origin.react(action="delete_workspace", data=protocol.WorkspaceBound(workspace_name=self.name))
    ...
    def fetch_grant(self, user_name: str) -> models.SharedWorkspaceGrant:
        response: protocol.FetchWorkspaceGrantEgress = self._origin.react(
            action="fetch_workspace_grant",
    ...
    def trust(self, user: models.User, access: typing.Set[enums.WorkspacePermission] = None):
    ...
    def untrust(self, user_name: str):
        self._origin.react(
