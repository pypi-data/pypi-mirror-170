...
class EntryClient:
    ...
    async def set_bytes(self, path: str, value: bytes, duration_ms: typing.Optional[int] = None) -> None:
        ...
        try:
            _, _, _, existing_entry_credentials = await self._fetch_raw(path.resolve())
        except service.Error:
            existing_entry_credentials: typing.Optional[models.EntryGrantCredentials] = None

        parent, _, parent_credentials = await self._workspace_client.namespace._fetch_raw(path.parent.resolve())
    ...
    async def set(self, path: str, value: str, duration_ms: typing.Optional[int] = None) -> None:
        assert isinstance(value, str), "Value must be a string"
        return await self.set_bytes(path, value.encode("utf-8"), duration_ms=duration_ms)
    ...
    async def _fetch_raw(
        ...
        response: protocol.FetchEntryChainEgress = await self._origin.react(
            action="fetch_entry",
    ...
    async def get_bytes(self, path: str) -> bytes:
        ...
        entry, encrypted_content, _, entry_credentials = await self._fetch_raw(path)
    ...
    async def get(self, path: str) -> str:
        entry_bytes = await self.get_bytes(path)
    ...
    async def grant(
        ...
        _, _, _, entry_credentials = await self._fetch_raw(path.resolve())
    ...
    async def list_grants(self, path: str) -> typing.List[models.EntryGrant]:
        path = keyspace.Path.from_path(path)

        response: protocol.ListEntryGrantsEgress = await self._origin.react(
            action="list_entry_grants",
