...
class UserClient:
    ...
    async def create(self) -> None:
        ...
        await self._origin.react(action="create_user", data=protocol.CreateUserIngress(user=signed_secret_user))

    @endpoint(action="fetch_self", egress=protocol.FetchSelfEgress)
    async def fetch_self(self) -> models.User:
        """Fetch and verify the current user's details"""
        result: protocol.FetchSelfEgress = await self._origin.react(action="fetch_self")
    ...
    async def update(self, trusted_workspaces: typing.List[models.WorkspaceIdentity]):
    ...
    async def trust(self, workspace: models.Workspace):
        ...
        await self.update(new_trusted_workspaces)

    async def untrust(self, workspace: models.Workspace):
    ...
    async def delete(self) -> None:
        ...
        await self._origin.react(action="delete_self")
