...
class StreamClient:
    ...
    async def create(self, stream: str) -> None:
        await self._workspace_client.namespace.create(keyspace.SERVICE_STREAM_PATH.resolve(), recursive=True)

        stream_path: keyspace.Path = keyspace.SERVICE_STREAM_PATH / stream
        await self._workspace_client.entry.set(stream_path.resolve(), "")

    @endpoint(action="publish_stream", ingress=protocol.PublishWorkspaceStreamIngress)
    async def publish(self, stream: str, *payloads: bytes) -> None:
        stream_path: keyspace.Path = keyspace.SERVICE_STREAM_PATH / stream
        _, _, _, existing_entry_credentials = await self._workspace_client.entry._fetch_raw(stream_path.resolve())
    ...
    async def consume(self, stream: str, batch: int, *, shared: bool = False) -> typing.List[bytes]:
        stream_path: keyspace.Path = keyspace.SERVICE_STREAM_PATH / stream

        entry, _, _, existing_entry_credentials = await self._workspace_client.entry._fetch_raw(stream_path.resolve())

        response: protocol.ConsumeStreamEgress = await self._origin.react(
    ...
    async def delete(self, stream: str) -> None:
        stream_path: keyspace.Path = keyspace.SERVICE_STREAM_PATH / stream
        await self._workspace_client.entry.delete(stream_path.resolve())
    ...
    async def grant(
        ...
        stream_path: keyspace.Path = keyspace.SERVICE_STREAM_PATH / stream
        await self._workspace_client.entry.grant(stream_path.resolve(), recipient, access)

    async def list_grants(self, stream: str) -> typing.List[models.EntryGrant]:
        stream_path: keyspace.Path = keyspace.SERVICE_STREAM_PATH / stream
