...
class PostProcessor:
    ...
    def get_boundary_box_and_seg(bbox: list, mask: list) -> tuple:
        ...
        # Bbox corners
        corner_1 = [bbox[0], bbox[1]]
        corner_2 = [bbox[2], bbox[3]]
        corner_3 = [bbox[2], bbox[1]]
        corner_4 = [bbox[0], bbox[3]]
        ...
        if len(polygon_points) > Config.threshold_line:
            ...
            for i in range(0, len(polygon_points) - 1, 1):
                distance_list.append(
                    (Pixel.distance_pixels(polygon_points[i], polygon_points[i + 1]), polygon_points[i], polygon_points[i + 1]))
