import re
import sys
from darksheep_imap.extension import imaplib
import datetime
...
class BaseMailBox:
    ...
    def login(self, username: str, password: str, initial_folder: Optional[str] = 'INBOX') -> 'BaseMailBox':
        ...
        if '@126' in username or '@163' in username or '@yeah' in username.lower():
            nn = username.split('@')
            args = ("name",nn[0],"contact",username,"version","1.0.1","vendor","PCclient")
            login_result = self.client._simple_command('ID', '("' + '" "'.join(args) + '")')
        login_result = self.client._simple_command('LOGIN', username, self.client._quote(password))  # noqa
        #typ, dat = login_result.select()
