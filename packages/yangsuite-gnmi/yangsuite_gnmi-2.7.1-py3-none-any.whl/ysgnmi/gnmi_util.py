...
class GnmiMessageConstructor:
    ...
    def _gnmi_update_request(self, upd_req, gnmi_upd_req):
        ...
        # upd_req always gives list of nodes either replace or update
        # creating a new key(replace or update) and appending latest
        # value
        if self.msg_type == 'set':
            if 'edit-op' not in upd_req[0]:
                self.json_set_val['update'] = json_val
            else:
                self.json_set_val[upd_req[0]['edit-op']] = json_val
    ...
    def nodes_to_dict(self, nodes=None, origin=None):
        ...
        if update:
            ...
            updates = self._gnmi_update_request(update, gnmi_update)
            self.payload.update.extend(updates)

        if replace:
            gnmi_replace = proto.gnmi_pb2.Update()
            replaces = self._gnmi_update_request(replace, gnmi_replace)
            self.payload.replace.extend(replaces)
