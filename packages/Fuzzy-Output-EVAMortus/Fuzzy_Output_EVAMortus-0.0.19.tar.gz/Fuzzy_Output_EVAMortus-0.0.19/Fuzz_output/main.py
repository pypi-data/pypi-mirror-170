...
class Linguistic_variable: # Кортеж вида (бета,T,X,G,M), где 
    def __init__(self, Name:str, Input_parameter_value:float = None, 
        ...
        self.Input_parameter_value = Input_parameter_value  # Input_parameter_value - точное скалярное Input_parameter_value лингвистической переменной (например для роста может быть от 150 до 200)
...
class Fuzzy_output:
    ...
    def Withdrawal (self, Model = Model(),Reading_files = True):
        ...
        def Дефаззификация_выходных_переменных(Метод):
            for ЛП in self.Linguistic_variables:
                if ЛП.Input_parameter_value is None:
                    ЛП.Дефаззифицированное_Input_parameter_value = Метод(ЛП)
        ...
        for ЛП in self.Linguistic_variables:
            try:
                if hasattr(ЛП,"Accumulated_function"): 
                    print(ЛП.Дефаззифицированное_Input_parameter_value)
            except:
                print("Ошибка")  
