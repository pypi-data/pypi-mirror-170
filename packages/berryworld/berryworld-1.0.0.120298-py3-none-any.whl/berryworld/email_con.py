...
class EmailConnection:
    ...
    def get_folder_id(self, folder_name, folder_id=None):
        ...
        if folder_id is None:
            ...
            folder_url = f'{self.base_url}/me/mailFolders/{folder_id}/childFolders'
        ...
        folder_id = folders_list.loc[folders_list['displayName'].str.lower() == folder_name.lower(), 'id'].values[0]
    ...
    def query_messages(self, folder_name, unread=True, mark_as_read=True, download_attachments=True,
        ...
        for folder in folders:
            if folders.index(folder) == 0:
                folder_id = self.get_folder_id(folder, folder_id=None)
            else:
                folder_id = self.get_folder_id('Prepworld', folder_id=folder_id)
        ...
        if download_attachments:
            ...
            for _, msg in emails_df.iterrows():
                response = requests.get(
                    ...
                    headers=header)
                if not str(response.status_code).startswith('2'):
                    print(f"There are no attahments for email: {msg['subject']}. Error Code {response.json()}")
                ...
                all_attachments = all_attachments.append(attachments_df)
        ...
        try:
            ...
            if attachment is not None:
                ...
                for file in attachment:
                    ...
                    attachment_dictionary: Dict = {"attachments": [
                            {
