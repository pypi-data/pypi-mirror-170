...
def sensitivity_plots(jacobian):
    ...
    for out, raw_sens in enumerate(jacobian.raw_sens):
        ...
        sns.barplot(x=sens.index.values.tolist(), y=sens['mean_squared'].values,
                    order=sens.sort_values(by=['mean_squared'], ascending=True).index,
                    palette='Blues_d', ax=ax[axes[1]])
