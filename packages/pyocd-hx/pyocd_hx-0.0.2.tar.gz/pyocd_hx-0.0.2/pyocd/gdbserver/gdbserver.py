...
class GDBServer(threading.Thread):
    ...
    def breakpoint(self, data):
        ...
        if data[1:2] == b'0':
            if data[0:1] == b'Z':
                #YX 20220927
                z0_breakpoint_type_auto = self.session.options.get('enable_z0_breakpoint_type_auto')
                result = False

                if (z0_breakpoint_type_auto):
                    result = self.target.set_breakpoint(addr, Target.BreakpointType.AUTO)

                if (result == False):
                    result = self.target.set_breakpoint(addr, Target.BreakpointType.SW)
