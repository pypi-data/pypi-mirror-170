...
class Common_DB_DatasourceFields:
    ...
    schemas_config : Dict = attr.ib(default={})
...
class Postgres(Common_DB_DatasourceFields):
    ...
    datasource_type: str = attr.ib(default=DatasourcesNames.POSTGRES, init=False)
...
class Snowflake(Common_DB_DatasourceFields):
    ...
    datasource_type: str = attr.ib(default=DatasourcesNames.SNOWFLAKE, init=False)
...
class PowerBIToken:
    ...
    refresh_auth_token: str = attr.ib()


@attr.s(auto_attribs=True, kw_only=True)
class PowerBI(Common_Dashboard_DatasourceFields, PowerBIToken):
    name: str = attr.ib(default='')
    datasource_type: str = attr.ib(default=DatasourcesNames.POWERBI, init=False)
...
class Redshift(Common_DB_DatasourceFields):
    host: str = attr.ib()
    port: str = attr.ib()
    datasource_type: str = attr.ib(default=DatasourcesNames.REDSHIFT, init=False)
