...
class ModelInfo:
    ...
    def validate_missing_value_encodings(missing_value_encodings: dict, all_columns: List[Column]) -> None:
        ...
        for k, v in missing_value_encodings.items():
            ...
            if matched_column is None:
                ...
                for value in v:
                    # check if val in missing_value_encodings is a valid datatype
                    if matched_column.violation_of_type(value):
                        raise ValueError(
                            f'Missing_value_encodings(fall_back) specified for Column({k}) has to be a list of '
                            f'values of type {matched_column.data_type}, instead got type {type(value)}'
                        )
    ...
    def ranking_cls_inference(
        ...
        return output_names, target_class_order, ranking_top_k, group_by
