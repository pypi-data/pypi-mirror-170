...
from src.mcsrv.javaversion import JavaVersion
...
@click.option("--console", "-c", "open_console", help="attach to the servers console after it started", is_flag=True,
              default=False)
@click.option("--java", "-j", "java_version", default=None, type=click.STRING)
@click.pass_context
def start(ctx: click.Context, ram_: str, open_console: bool, java_version: str):
...
@main.group(help="manage java versions", invoke_without_command=False)
@click.pass_context
...
@click.argument("path", type=click.Path(exists=True, dir_okay=False, executable=True))
@click.pass_context
...
@click.argument("java_version_name", type=click.STRING, required=False, nargs=1)
@click.pass_context
def set_java_version(ctx: click.Context, java_version_path: str):
    ...
    if java_version_path is None:
        ...
        options = [f"{j.version} ({j.path})" for j in installations]
