...
class CablePath:
    def __init__(self, termination_a, termination_b, cable_inp):
    ...
    def get_renderable_path(self):
        ...
        for i, needed_cable in enumerate(needed_cables):
            ...
            if i >= len(self.cable_list):
                continue

            next_existing_cable = self.cable_list[i]

            is_swapped = next_existing_cable.termination_b.id == next_attachment_point.id
            ...
            next_attachment_point = next_existing_cable.termination_b if is_swapped else next_existing_cable.termination_a
            open_element = {
                "attachment_a": next_attachment_point,
                "device": next_attachment_point.device,
            }
    ...
    def get_needed_cables(self):
        ...
        for rack in self.rack_list:
            if rack.id == self.termination_a.device.rack.id:
                # Start Rack
...
class CableRouter:
    ...
    def __init__(self, termination_a, termination_b, media_type):
        self.termination_a = termination_a
        self.termination_b = termination_b
