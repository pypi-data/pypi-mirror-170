...
class YSYangModels(object):
    ...
    def __init__(self, ctx, modulenames,
        ...
        if include_augmentees or include_supermodules:
            ...
            while True:
                ...
                for name in all_modules:
                    # new_modules.update(
                    #     ctx.repository.modules_providing_grouping(name)
                    # )
                    try:
                        node = dg.node[name]
                    except KeyError:
        ...
        self.ctx.load_status['info'] = 'Generating tree(s)...'
        ...
        self.ctx.load_status['info'] = 'Loading tree(s)...'
...
class ParseYang(object):
    ...
    def __init__(self, name, revision, ctx,
        ...
        self.tree = self.generate_tree(node_callback)
    ...
    def get_module_node_data(self, node_callback=None):
        ...
        if stmt.keyword == "module":
            ...
            mod = stmt.i_prefix
            try:
                rev = stmt.i_ctx.repository.digraph.node[mod]['revision']
            except:
                rev = None
            mod_stmt = self.ctx.get_module(mod, rev)
            self.namespace = self.set_data_subst_arg(data, mod_stmt,
                                                     'namespace',
                                                     data_key=stmt.i_prefix,
                                                     default=self.namespace)
        ...
        for (pfx, (mod, rev)) in sorted(all_i_prefixes.items()):
            ...
            if not mod_stmt:
                log.warning("No module %s, can't get namespace for prefix %s",
                            mod, pfx)
                continue
            if mod_stmt.keyword == "submodule":
                mod = mod_stmt.i_prefix
                try:
                    rev = stmt.i_ctx.repository.digraph.node[mod]['revision']
                except:
                    rev = None
    ...
    def get_node_data(self, stmt,
        ...
        if not data['prefix']:
            self.set_data_subst_arg(data, stmt.i_module, 'namespace',
                                    default='')
        else:
            mod = stmt.i_module.i_prefix
            try:
                rev = stmt.i_module.i_ctx.repository.digraph.    \
                      node[mod]['revision']
            except:
                rev = None
            mod_stmt = self.ctx.get_module(mod, rev)
            self.set_data_subst_arg(data, mod_stmt,
                                    'namespace',
                                    default=self.namespace)
    ...
    def get_type_specific_data(self, stmt, data):
        ...
        if isinstance(stmt.i_type_spec, pyang.types.BitTypeSpec):
            ...
            elif hasattr(stmt.i_type_spec, 'path_'):
                data['leafref_path'] = stmt.i_type_spec.path_.arg
...
class TreeWalker(object):
    ...
    def get_node_by_xpath(self, xpath, **kwargs):
        ...
        # remove prefixes in xpath when no_prefix=True
        if kwargs.get('no_prefix', False):
            xpath_tokens = xpath.split("/")
            for i in range(len(xpath_tokens)):
                if xpath_tokens[i].count(':') == 1:
                    xpath_tokens[i] = xpath_tokens[i].split(':')[1]
    ...
    def get_node_by_schema_xpath(self, xpath):
        """Walk the jstree and return node that matches the schema xpath.

        Args:
          xpath (str): Schema Xpath of the node in the schema tree to match.
        ...
        """
        for invalid_char in "[]=":
            if invalid_char in xpath:
                raise ValueError("input xpath should not contain key: {}"
                                 .format(xpath))
        return self._walk_schema_tree(self.tree, xpath)

    def _walk_schema_tree(self, node, xpath):
        if node.get('data') is None:
            return None
        schema_node_id = node['data'].get('schema_node_id', '')
        if schema_node_id == xpath:
            return node
        if schema_node_id != xpath[:len(schema_node_id)]:
            return None
        else:
            for child in node.get('children', []):
                found = self._walk_schema_tree(child, xpath)
                if found:
                    return found
            return None
