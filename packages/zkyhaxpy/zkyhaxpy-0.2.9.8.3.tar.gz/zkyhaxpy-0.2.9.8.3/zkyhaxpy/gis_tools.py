...
def shape_to_raster(in_shp, out_tif, ref_tif, no_data=0, all_touched=False, attribute=None, burn_val=1, datatype=None):
    ...
    if attribute:
        rasterize_option = gdal.RasterizeOptions(allTouched=all_touched, attribute=attribute)
        gdal.RasterizeLayer(Output, [1], Shapefile_layer, options=rasterize_option)    
    else:
        rasterize_option = gdal.RasterizeOptions(allTouched=all_touched)
        gdal.RasterizeLayer(Output, [1], Shapefile_layer, burn_values=[burn_val], options=rasterize_option)    
