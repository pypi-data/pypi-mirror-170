...
class CategoryReport:
    ...
    """
    # name, excluded, skipped, added, deleted, total
    name: str
    urls_excluded: int = 0
    urls_skipped: int = 0
    urls_added: int = 0
...
class ZscalerSession:
    ...
    def update_category_urls(self, category_id: str, urls: List[str]) -> Union[None, int]:
        ...
        if limit < len(urls_to_add):
            self.add_error('General error', -1, 'Max URLs provisioned')
    ...
    def lookup_urls(self, urls: List[str], show_progress_bar=False) -> List[dict]:
        ...
        if len(urls) <= MAX_URLS_PER_LOOKUP:
            with L_LOOKUP_RATE_LIMITER and H_LOOKUP_RATE_LIMITER:
                time.sleep(1)
    ...
    def add_error(self, error_type: str, code: int, message: str) -> None:
        ...
        :return:
        """
        for categoryError in self.sessionErrors:
            if categoryError.error_type == error_type:
                categoryError.append_error(code, message)
                return
