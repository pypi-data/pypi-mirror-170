...
class Connection(RequestSessionBase):
    ...
    def undoable_transactions(self, ueid=None, **actionfilters):
        ...
        * `public`: when additional filtering is provided, they are by default
          ...
          and set to false.
        """
        return self.repo.system_source.undoable_transactions(
            self, ueid, **actionfilters
        )
    ...
    def delete_relation(self, fromeid, rtype, toeid):
        ...
        with self.security_enabled(False, False):
            if self.vreg.schema[rtype].inlined:
                ...
                self.repo.glob_update_entity(self, entity, {rtype})
            else:
    ...
    def _update_entity_rel_cache_add(self, eid, rtype, role, targeteid):
        ...
        if rcache is not None:
            ...
            entity._cw_related_cache[f"{rtype}_{role}"] = (rset, tuple(entities))
    ...
    def _update_entity_rel_cache_del(self, eid, rtype, role, targeteid):
        ...
        if rcache is not None:
            ...
            for idx, row in enumerate(rset.rows):
                ...
                self.debug(
                    "cache inconsistency for %s %s %s %s", eid, rtype, role, targeteid
                )
                return
            rset = rset.copy()
            entities = list(entities)
    ...
    def commit(self):
        ...
        if cstate == "postcommit":
            self.critical(
                "postcommit phase is not allowed to write to the db; ignoring commit"
            )
            return
                    ...
                    with self.running_hooks_ops():
                        for operation in reversed(processed):
                            ...
                            try:
                                ...
                                self.critical(
                                    "error while reverting precommit", exc_info=True
                                )
                    ...
                    while self.pending_operations:
                        ...
                        try:
                            ...
                            if self.repo.config.mode == "test":
                                raise
                            self.critical(
                                "error while postcommit", exc_info=sys.exc_info()
                            )
                if debug:
    ...
    @_open_only
    def system_sql(
        self, sql, args=None, rollback_on_failure=True, rql_query_tracing_token=None
    ):
        """return a sql cursor on the system database"""
        source = self.repo.system_source
        try:
            return source.doexec(
                self,
                sql,
                args,
    ...
    def rtype_eids_rdef(self, rtype, eidfrom, eidto):
        ...
        return self.vreg.schema.relation_schema_for(rtype).relation_definitions[
            (subjtype, objtype)
        ]
...
class InternalManager:
    ...
    def __init__(self, lang="en"):
        self.eid = -1
        self.login = "__internal_manager__"
        self.properties = {}
