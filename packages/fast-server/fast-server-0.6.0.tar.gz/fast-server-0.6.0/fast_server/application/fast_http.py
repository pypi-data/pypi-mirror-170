...
class HttpRequest(HttpBase):
    ...
    """
    def __init__(self, environ: dict, sessions):
        """
        初始化HttpRequest
        ...
        # 初始化cookies
        self.__init__cookies(Cookie(environ.get('HTTP_COOKIE', None)), sessions)

    def redirect(self, path: str, parameter: dict = None) -> HttpBase:
        """
        0.3.0 新增
        实现了页面的重定向
        ------------------------------------------------
        ...
        :return: HttpBase
        """
        response = HttpResponse(data=self.body, status=302)
    ...
    def __init__cookies(self, c: Cookie, sessions):
        if c.id in sessions:
            session = sessions[c.id]
            session.value = c
        else:
            session = Session(value=c, expires=30*60)
...
class HttpResponse(HttpBase):
    ...
    def response_headers(self):
        ...
        if self.cookie is not None:
            response_headers = self.cookie.to_wsgi_header()
