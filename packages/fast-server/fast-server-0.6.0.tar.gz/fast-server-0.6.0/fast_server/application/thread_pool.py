...
class ThreadPoolExecutor:
    ...
    def __init__(self, max_workers=1, max_waiters=None):
        """
        初始化线程池
        :param max_workers: 线程池最大线程数量
        :param max_waiters: 任务队列长度
        """
        # 如果提供了最大任务数的参数，则将队列的最大元素个数设置为这个值。
        if max_waiters:
            self.q = queue.Queue(max_waiters)
        # 默认队列可接受无限多个的任务
        else:
            self.q = queue.Queue()
    ...
    def call(self):
        ...
        self.generate_list.add(current_thread)
        # 从任务队列中获取一个任务
        event = self.q.get()
        # 让获取的任务不是终止线程的标识对象时
    ...
    def modify_size(self, max_works, max_waiters=None):
        """
        该方法将修改线程池大小
        :param max_works: 最大线程数
        :param max_waiters: 等待队列长度
        :return:
        """
        # 创世关闭线程
        self.close()
    ...
    def get_free_list_num(self):
        ...
        if len1 < self.max_workers:
            return self.max_workers-(len1-len2)
        return len2
