...
def _delete(args: argparse.Namespace) -> None:
    ...
    res = requests.delete(url, verify=CLIENT_CA_FILE, timeout=REQUEST_TIMEOUT)
...
def _create(args: argparse.Namespace) -> None:
    ...
    res = requests.post(url, json=data, verify=CLIENT_CA_FILE, timeout=REQUEST_TIMEOUT)
    if res.status_code == 200:
        if not silent_mode:
            tweet_id = res.json()['data']['id']
            print('successfully tweeted:', tweet_id)
    else:
