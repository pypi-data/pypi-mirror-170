"""
The `saysynth.cli.options` module inlcudes shared `click.option` instances
for use throughout all commands.

**NOTE**: This documentation for this moduleis not rendered properly
by `pdoc`, so its best to  click "Expand Source Code" to scan its contents.
"""
import click
import yaml
...
from saysynth.constants import (DEFAULT_BPM_TIME_BPM, DEFAULT_BPM_TIME_COUNT,
                                DEFAULT_BPM_TIME_SIG, SAY_EMPHASIS,
                                SAY_PHONEME_CLASSES, SAY_SEGMENT_MAX_DURATION,
                                SAY_SHOW_VOLUME_PER_NOTE,
                                SAY_SHOW_VOLUME_PER_SEGMENT, SAY_TUNED_VOICES,
                                SAY_VOLUME_RANGE)
...
quality_opt = click.option(
    ...
    type=int,
    default=127,
    help="Quality of the signal (1:127)",
    show_default=True,
)
...
velocities_opt = click.option(
    ...
    show_default=True,
    type=lambda x: [int(i.strip()) for i in x.split(",")] if x else [],
    help="A list of velocities to apply to the notes, if this list is shorter than the list of notes, a modulo lookup is performed.",
)
...
seq_config_overrides_opt = click.option(
    "-c",
    "--config-overrides",
    type=lambda x: yaml.safe_load(x),
    default="{}",
    help="""
...
text_opt = click.option(
    ...
    type=str,
    default=None,
    help="Text to to use when selecting phonemes; the text to 'sing'",
)
