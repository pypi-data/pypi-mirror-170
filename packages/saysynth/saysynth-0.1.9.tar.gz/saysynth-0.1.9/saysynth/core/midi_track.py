...
class MidiTrack(SayObject):
    ...
    def _start_text(self) -> str:
        ...
        if not self.start:
            return ""
        _start_segments = []
        time_breaks = list(frange(0.0, self.start, SAY_SEGMENT_SILENCE_DURATION, 10))[
            1:
        ]
        ...
        if total_time < self.start and len(_start_segments) < self.start_segment_count:
            # add final silent step
            dur = self.start - total_time
            _start_segments.append(Segment(type="silence", velocity=0, duration=dur))
