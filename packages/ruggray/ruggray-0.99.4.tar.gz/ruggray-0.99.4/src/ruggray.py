...
class ruggray(plain):
	...
	def __init__(self, form):
		...
		self.box_checked_image = self.form.experiment.resource(os.path.join(
			u'widgets', u'gray', u'box-checked-large.png'))
		self.box_unchecked_image = self.form.experiment.resource(os.path.join(
			u'widgets', u'gray', u'box-unchecked-large.png'))
	...
	def frame(self, x, y, w, h, style=u'normal'):
		...
		elements = [
			Line(x+1, y+1, x+w-2, y+1, color=u'#bbbbbb'),
			Line(x+1, y+1, x+1, y+h-2, color=u'#bbbbbb'),
			Line(x+1, y+h-2, x+w-2, y+h-2, color=u'#565656'),
			Line(x+w-2, y+1, x+w-2, y+h-2, color=u'#565656')
			]
		if style in (u'active', u'normal'):
			elements.insert(0, Rect(x, y, w, h, color=u'#2e3436'))
		if style == u'normal':
			elements.append(
				Rect(x+2, y+2, w-4, h-4, color=u'#888888', fill=True))
