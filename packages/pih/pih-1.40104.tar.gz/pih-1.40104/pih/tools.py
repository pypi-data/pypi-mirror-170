...
from pih.collection import T, FieldItem, FieldItemList, FullName, LoginPasswordPair, Result, User, InventoryReportItem, ServiceRoleValue
    ...
    def to_result(result_string: str, class_type = None, first_data_item: bool = False) -> Result:
        ...
        def obtain_data():
            return (list(map(lambda x: DataTools.fill_data_from_source(class_type(), x), data)) if isinstance(data, list)
                    and class_type else DataTools.fill_data_from_source(class_type(), data) if class_type else data) if data else None
...
class PIHEncoder(json.JSONEncoder):
    def default(self, obj):
        ...
        if isinstance(obj, FieldItemList):
            return obj.list
        if isinstance(obj, (FullName, LoginPasswordPair, Result, InventoryReportItem, ServiceRoleValue)):
...
class FullNameTool:
    ...
    @staticmethod
    def to_given_name(full_name: Any, join_symbol: str = " ") -> str:
