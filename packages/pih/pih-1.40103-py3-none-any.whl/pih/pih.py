...
class PIH:
    ...
    class PSTOOLS:
        ...
        @staticmethod
        def create_remote_executor(host: str, interactive: bool = False) -> List[str]:
            host = "\\\\" + host
            ...
            return [ps_executor_path, host, "-i" if interactive else "-d", "-u", user, "-p", password, CONST.PYTHON.EXECUTOR]

        @staticmethod
        def create_remote_executor_for_service_role_value(service_role_value: ServiceRoleValue, test: bool = False, interactive: bool = False) -> List[str]:
            return PIH.PSTOOLS.create_remote_executor((PIH.OS.get_host() if test else service_role_value.host), interactive)
    ...
    class SERVICE:
        ...
        class ADMIN:
            ...
            @staticmethod
            def start(role: ServiceRoles, check_if_started: bool = True, test: bool = False, show_output: bool = False) -> bool:
                if check_if_started:
                    if PIH.SERVICE.check_availability(role):
                        return None
                service_role_value: ServiceRoleValue = role.value
                remote_executor_command_list: List[str] = PIH.PSTOOLS.create_remote_executor_for_service_role_value(
                    service_role_value, test)
                ...
                if service_role_value.service_path is None:
                    ...
                    service_file_path =  os.path.join(service_role_value.service_path, f"{CONST.SERVICE.NAME}.{CONST.FILE.EXTENSION.PYTHON}")
            ...
            @staticmethod
            def stop(role: ServiceRoles, check_if_started: bool = True, test: bool = False, show_output: bool = False) -> bool:
                if check_if_started:
        ...
        def ping(role: ServiceRoles) -> ServiceRoleBaseValue:
            service_role_value: ServiceRoleBaseValue = RPC.ping(role)
            if service_role_value is not None:
                DataTools.fill_data_from_source(
                    role.value, DataTools.to_data(service_role_value))
    ...
    class SESSION:
        ...
        def say_hello() -> None:
            ...
            if user is not None:
                ...
                PR.new_line()
                return
            PR.alert(f"Ты кто такой? Давай, до свидания...")
            sleep(5)
            exit()
        ...
        @staticmethod
        def authenticate(exit_on_fail: bool = False) -> bool:
            try:
                PR.green("Инициализация...")
                if PIH.SERVICE.check_availability(ServiceRoles.AD):
                    ...
                    PR.alert("Сервис аутентификации недоступен. До свидания...")
    ...
    class RESULT:
        ...
        class PRINTER:
            ...
            def list() -> Result[List[PrinterADInformation]]:
                ...
                result: Result[List[PrinterADInformation]] = DataTools.to_result(
                    RPC.call(ServiceCommands.printer_list, PrinterReport))
            ...
            def status(redirect_to_log: bool = True) -> Result[List[PrinterReport]]:
                return DataTools.to_result(
                    RPC.call(ServiceCommands.printers_status, (redirect_to_log)), PrinterReport)

        class MARK:

            @staticmethod
            def by_tab_number(value: str) -> Result[Mark]:
                return DataTools.to_result(RPC.call(ServiceCommands.get_mark_by_tab_number, value), Mark, True)
            ...
            @staticmethod
            def free_marks_group_statistics() -> Result[List[MarkGroupStatistics]]:
                return DataTools.to_result(RPC.call(ServiceCommands.get_free_marks_group_statistics), MarkGroupStatistics)
        ...
        class POLIBASE:
            ...
            def patient_by_pin(value: int) -> Result[Patient]:
                return DataTools.to_result(RPC.call(ServiceCommands.get_patient_by_pin, value), Patient)
        ...
        class USER:
            ...
            def template_list() -> Result[List[User]]:
                return DataTools.to_result(RPC.call(ServiceCommands.get_template_list), User)
            ...
            while True:
                ...
                if check:
                    if PIH.CHECK.tab_number(tab_number):
                        ...
                        PR.alert("Неправильный формат номера карты доступа")
                        #return None
                else:
                    return tab_number
        ...
        class ACCESS:
            ...
            def by_group(group: CONST.AD.Groups, exit_on_access_denied: bool = False) -> bool:
                ...
                if exit_on_access_denied:
                    if result:
                        return result
                    PR.alert("Функционал недоступен...")
                    sleep(5)
                    exit()
                else:
                    return result
        ...
        class INVENTORY:
            ...
            def available() -> bool:
                try:
                    ...
                    return False
                return PIH.CHECK.ACCESS.inventory()
    ...
    def log(message: str, channel: LogChannels = LogChannels.DEFAULT, level: Any = LogLevels.DEFAULT) -> None:
        ...
        def internal_log(message: str, channel_name: str, level_value: int) -> None:
            try:
                ...
                PR.alert(f"Log send error: {error}")
    ...
    class MESSAGE:
        ...
        class WHATS_APP:
            ...
            @staticmethod
            def send_message(telephoneNumber: str, message: str) -> bool:
                ...
                try:
                    ...
                    pwk.sendwhatmsg_instantly(telephoneNumber, message)
                except:
                    PR.alert("Ошибеп при отправке сообщения!")

            @staticmethod
            def send_message_by_login(login: str, message: str) -> bool:
                user: User = PIH.RESULT.USER.by_login(login).data
                if user is not None:
                    return PIH.MESSAGE.WHATS_APP.send_message(user.telephoneNumber, message)
                else:
                    return False
        ...
        class COMMAND:
            ...
            def it_notify_about_new_user(login: str, password: str) -> None:
                ...
                PIH.log_command(LogCommands.IT_NOTIFY_ABOUT_NEW_USER, (
                    user.name, user.description, user.samAccountName, password, user.telephoneNumber, user.mail, mark.TabNumber))
    ...
    class VISUAL:
        ...
        def free_marks(use_index: bool = False) -> None:
            PIH.VISUAL.table_with_caption_first_title_is_centered(
                PIH.RESULT.MARK.free_list(), "Свободные карты доступа:", use_index)
    ...
    class ACTION:
        ...
        class USER:
            ...
            def set_status(user: User, status: str, container: UserContainer) -> bool:
                return DataTools.rpc_unrepresent(RPC.call(ServiceCommands.user_set_status, (user.distinguishedName, status, DataTools.if_check(container, lambda: container.distinguishedName))))

            @staticmethod
            def remove(user: User) -> bool:
                return DataTools.rpc_unrepresent(RPC.call(ServiceCommands.user_remove, user.distinguishedName))
        ...
        class INVENTORY:
            ...
            def create_barcodes(report_file_path: str, result_directory: str) -> bool:
                return DataTools.rpc_unrepresent(RPC.call(ServiceCommands.create_inventory_barcodes, (report_file_path, result_directory)))
        ...
        class MARK:
            ...
            def create(full_name: FullName, person_division_id: int,  tab_number: str, telephone: str = None) -> bool:
                return DataTools.rpc_unrepresent(RPC.call(ServiceCommands.create, (full_name, person_division_id, tab_number, telephone)))
            ...
            def remove(mark: Mark) -> bool:
                return DataTools.rpc_unrepresent(RPC.call(ServiceCommands.remove_by_tab_number, mark.TabNumber))
        ...
        def create_user_document(path: str, full_name: FullName, tab_number: str, pc: LoginPasswordPair, polibase: LoginPasswordPair, email: LoginPasswordPair) -> bool:
            ...
            return DataTools.rpc_unrepresent(RPC.call(ServiceCommands.create_user_document, (path, date_now_string, CONST.SITE, CONST.SITE_PROTOCOL + CONST.SITE, CONST.EMAIL_ADDRESS, full_name, tab_number, pc, polibase, email)))
        ...
        def generate_login(full_name: FullName, ask_for_remove_inactive_user_if_login_is_exists: bool = True) -> str:
            ...
            if not PIH.INPUT.yes_no(f"Использовать логин '{login_string}' для аккаунта пользователя?", True):
...
class PR:
    ...
    def green_str(string: str, text_before: str = None, text_after: str = None) -> str:
        return PR.color_str(Back.LIGHTGREEN_EX, string, text_before, text_after)
    ...
    def magenta_str(string: str, text_before: str = None, text_after: str = None) -> str:
        return PR.color_str(Back.MAGENTA, string, text_before, text_after)
    ...
    def red_str(string: str, text_before: str = None, text_after: str = None) -> str:
        return PR.color_str(Back.RED, string, text_before, text_after)
    ...
    def call_actions_by_index(self, index: int = 0, change: bool = False):
        ...
        while True:
            try:
                ...
                PR.new_line()
                PR.alert("Повтор предыдущих действия")
                PR.new_line()
                if index > 0:
