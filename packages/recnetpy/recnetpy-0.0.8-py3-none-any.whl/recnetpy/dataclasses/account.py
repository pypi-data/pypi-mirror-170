...
class Account(BaseDataClass['AccountResponse']):
    ...
    async def get_featured_rooms(self, force: bool = False) -> List['Room']:
        """
        Fetches a list of rooms featured by the player. Returns a
        cached result, if this function has been already called.
        ...
        if self.featured_rooms is None or force:
            data = await self.rec_net.rooms.showcase(self.id).make_request('get')
