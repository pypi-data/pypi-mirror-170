...
#define MPREAL_HAVE_DYNAMIC_STD_NUMERIC_LIMITS // Enable extended std::numeric_limits<mpfr::mpreal>
											   // specialization. Meaning that "digits",
											   // "round_style" and similar members are defined as
											   // functions, not constants. See
											   // std::numeric_limits<mpfr::mpreal> at the end of
...
#else
#	define IsInf(x)                                                                               \
		std::isinf(x) // GNU C/C++ (and/or other compilers), just hope for C99 conformance
#endif
...
namespace mpfr {
	...
	public:
		...
		// shared = true allows to avoid deep copy, so that mpreal and 'u' share the same data &
		// pointers.
...
// MPFR < 2.4.2 Specifics
	...
	inline mpreal::mpreal(mpreal &&other) {
		mpfr_set_uninitialized(
		  mpfr_ptr()); // make sure "other" holds null-pointer (in uninitialized state)
	...
	inline mpreal &mpreal::operator=(mpreal &&other) {
		if (this != &other) {
			mpfr_swap(mpfr_ptr(),
					  other.mpfr_ptr()); // destructor for "other" will be called just afterwards
			MPREAL_MSVC_DEBUGVIEW_CODE;
		}
		return *this;
	}
	...
	namespace internal {
		...
		// This is needed for smooth integration with libraries based on expression templates, like
		// Eigen.
	...
	inline const typename internal::result_type<Rhs>::type operator+(const mpreal &lhs,
																	 const Rhs &rhs) {
		return mpreal(lhs) += rhs;
	}

	template<typename Lhs>
	inline const typename internal::result_type<Lhs>::type operator+(const Lhs &lhs,
																	 const mpreal &rhs) {
	...
	inline mpreal &mpreal::operator<<=(const int u) {
		mpfr_mul_2si(
		  mpfr_ptr(), mpfr_srcptr(), static_cast<long int>(u), mpreal::get_default_rnd());
		MPREAL_MSVC_DEBUGVIEW_CODE;
		return *this;
	}
	...
	// WARNING:
	//
	// Please note that following checks for double-NaN are guaranteed to work only in IEEE math
	// mode:
	//
	// isnan(b) =  (b != b)
	// isnan(b) = !(b == b)  (we use in code below)
	//
	// Be cautions if you use compiler options which break strict IEEE compliance (e.g. -ffast-math
	// in GCC). Use std::isnan instead (C++11).
	...
	inline std::string mpreal::toString(int n, int b, mp_rnd_t mode) const {
		// TODO: Add extended format specification (f, e, rounding mode) as it done in output
		// operator
		(void)b;
		(void)mode;
	...
	inline std::ostream &mpreal::output(std::ostream &os) const {
		...
		char *s = NULL;
		if (!(mpfr_asprintf(
				&s, format.str().c_str(), mpfr::mpreal::get_default_rnd(), mpfr_srcptr()) < 0)) {
			os << std::string(s);
			mpfr_free_str(s);
		}
		return os;
...
#define MPREAL_BINARY_MATH_FUNCTION_UI_BODY(f, u)                                                  \
...
#if (MPFR_VERSION >= MPFR_VERSION_NUM(4, 0, 0))
	inline const mpreal gammainc(const mpreal &a, const mpreal &x,
								 mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
		/*
	...
	inline const mpreal beta(const mpreal &z, const mpreal &w,
							 mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
		/*
	...
	inline const mpreal log_ui(unsigned long int n, mp_prec_t prec = mpreal::get_default_prec(),
							   mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
	...
	/* f(x,u) = f(2*pi*x/u) */
	inline const mpreal cosu(const mpreal &x, unsigned long u,
							 mp_rnd_t r = mpreal::get_default_rnd()) {
		MPREAL_BINARY_MATH_FUNCTION_UI_BODY(cosu, u);
	}
	inline const mpreal sinu(const mpreal &x, unsigned long u,
							 mp_rnd_t r = mpreal::get_default_rnd()) {
		MPREAL_BINARY_MATH_FUNCTION_UI_BODY(sinu, u);
	}
	inline const mpreal tanu(const mpreal &x, unsigned long u,
							 mp_rnd_t r = mpreal::get_default_rnd()) {
		MPREAL_BINARY_MATH_FUNCTION_UI_BODY(tanu, u);
	}
	inline const mpreal acosu(const mpreal &x, unsigned long u,
							  mp_rnd_t r = mpreal::get_default_rnd()) {
		MPREAL_BINARY_MATH_FUNCTION_UI_BODY(acosu, u);
	}
	inline const mpreal asinu(const mpreal &x, unsigned long u,
							  mp_rnd_t r = mpreal::get_default_rnd()) {
		MPREAL_BINARY_MATH_FUNCTION_UI_BODY(asinu, u);
	}
	inline const mpreal atanu(const mpreal &x, unsigned long u,
							  mp_rnd_t r = mpreal::get_default_rnd()) {
		MPREAL_BINARY_MATH_FUNCTION_UI_BODY(atanu, u);
	}

	/* f(x) = f(pi*x) */
	inline const mpreal cospi(const mpreal &x, mp_rnd_t r = mpreal::get_default_rnd()) {
		MPREAL_UNARY_MATH_FUNCTION_BODY(cospi);
	}
	inline const mpreal sinpi(const mpreal &x, mp_rnd_t r = mpreal::get_default_rnd()) {
		MPREAL_UNARY_MATH_FUNCTION_BODY(sinpi);
	}
	inline const mpreal tanpi(const mpreal &x, mp_rnd_t r = mpreal::get_default_rnd()) {
		MPREAL_UNARY_MATH_FUNCTION_BODY(tanpi);
	}
	inline const mpreal acospi(const mpreal &x, mp_rnd_t r = mpreal::get_default_rnd()) {
		MPREAL_UNARY_MATH_FUNCTION_BODY(acospi);
	}
	inline const mpreal asinpi(const mpreal &x, mp_rnd_t r = mpreal::get_default_rnd()) {
		MPREAL_UNARY_MATH_FUNCTION_BODY(asinpi);
	}
	inline const mpreal atanpi(const mpreal &x, mp_rnd_t r = mpreal::get_default_rnd()) {
		MPREAL_UNARY_MATH_FUNCTION_BODY(atanpi);
	}

	inline const mpreal log2p1(const mpreal &x, mp_rnd_t r = mpreal::get_default_rnd()) {
		MPREAL_UNARY_MATH_FUNCTION_BODY(log2p1);
	} /* log2 (1+x) */
	inline const mpreal log10p1(const mpreal &x, mp_rnd_t r = mpreal::get_default_rnd()) {
		MPREAL_UNARY_MATH_FUNCTION_BODY(log10p1);
	} /* log10(1+x) */
	inline const mpreal exp2m1(const mpreal &x, mp_rnd_t r = mpreal::get_default_rnd()) {
		MPREAL_UNARY_MATH_FUNCTION_BODY(exp2m1);
	} /* 2^x-1      */
	inline const mpreal exp10m1(const mpreal &x, mp_rnd_t r = mpreal::get_default_rnd()) {
		MPREAL_UNARY_MATH_FUNCTION_BODY(exp10m1);
	} /* 10^x-1     */

	inline const mpreal atan2u(const mpreal &y, const mpreal &x, unsigned long u,
							   mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
		/*
	...
	inline const mpreal atan2pi(const mpreal &y, const mpreal &x,
								mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
	...
	inline const mpreal powr(const mpreal &x, const mpreal &y,
							 mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
	...
	inline const mpreal compound(const mpreal &x, long n,
								 mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
		/* compound(x,n) = (1+x)^n */
	...
	inline const mpreal fmod(const mpreal &x, unsigned long u,
							 mp_rnd_t r = mpreal::get_default_rnd()) {
...
namespace std {
	...
	public:
		...
		// Following members should be constant according to standard, but they can be variable in
		// MPFR So we define them as functions here.
		//
		...
		// libraries, e.g. boost. See below for compatible implementation.
		...
		//
		// These (default) values are preserved for compatibility with existing libraries, e.g.
		// boost. Change them accordingly to your application.
		//
