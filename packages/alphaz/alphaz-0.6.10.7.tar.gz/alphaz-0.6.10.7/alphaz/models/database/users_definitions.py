...
class User(db.Model, AlphaTable):
    ...
    username = AlphaColumn(String(30), unique=True, primary_key=True)
    mail = AlphaColumn(String(40), primary_key=True, default="-")
    ...
    roles = relationship("Role", secondary="user_role", cascade="all,delete",)
...
class Role(db.Model, AlphaTableUpdateDate):
    ...
    id_app = AlphaColumn(Integer, ForeignKey("application.id"),)
    ...
    permissions = relationship(
        "Permission", secondary="role_permission", cascade="all,delete",
    )
