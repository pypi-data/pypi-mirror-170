...
from ...libs import date_lib, json_lib, py_lib, flask_lib
...
def dict_to_model(model, parameters):
    ...
    for relationship, relationship_property in inspected.relationships.items():
        if relationship in parameters:
            ...
            relationship_model = core.get_table_model(relationship_table_name)
...
class Parameter:
    ...
    def set_value(self, method: str, dataDict, api_json, form, args):
        ...
        if self.ptype == list or py_lib.is_subtype(self.ptype, typing.List):
            ...
            is_empty_value = all(x in str(self._value) for x in [";", ",", "["])
