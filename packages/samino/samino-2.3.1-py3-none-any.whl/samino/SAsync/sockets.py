...
class Wss(Callbacks, WssClient):
    def __init__(self, client, ses, Trace):
        self.client = client
        self.ses = ses
    ...
    async def uploadMedia(self, file: BinaryIO, fileType: str):
        ...
        async with self.ses.post(f"{self.narvi}/g/s/media/upload", data=data, headers=headers) as response:
            if await response.json()["api:statuscode"] != 0:
                return exception(await response.json())
            else:
                return await response.json()["mediaValue"]

    async def sendWebActive(self, comId: str):
        data = {"ndcId": comId}
        async with self.ses.post("https://aminoapps.com/api/community/stats/web-user-active-time", json=data, headers=self.web_headers) as response:
