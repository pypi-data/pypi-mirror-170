...
class Installer:
    ...
    def __check_installation_dir(self, installation_dir='.'):
        '''
    ...
    def download_ressource(self, url, installation_dir='.', verbose=1):
        '''
        ...
        if not self.__check_installation_dir(installation_dir): return False
        ...
        with open(ressource_path, "wb") as fp:
            ...
            if total_length is None: # no content length header
                ...
                for data in response.iter_content(chunk_size=4096):
                    fp.write(data)
                    if verbose>0: utils.update_progress('Download : ',i,imax, redraw=False, verbosity=2, total_max=total_hsize)
                    i+=1
        return ressource_path


    def extract_ressource(self, ressource_path, installation_dir='.', verbose=1):
        '''
        ...
        with tarfile.open(name=ressource_path) as tar:
            ...
            for member in tar.getmembers():
                ...
                if verbose>0: utils.update_progress('Extract  :',i,imax, redraw=False, verbosity=2, total_max=total_files)
        
        return installation_dir



    def move_to_old(self, ressource_name):
        print('hello')
    ...
    def install_ressource(self, catalog_name=None, ressource_name=None, installation_dir='.', verbose=1):
        '''
        Install a ressource
        args:
            catalog_name     : Catalog name, 'notebooks'|'datasets'
        ...
        # ---- Check intallation_dir
        #
        if verbose>1: print('Check installation directory...')
        installation_dir = os.path.expanduser(installation_dir)
        if not self.__check_installation_dir(installation_dir): return False

        if os.path.isdir( f'{installation_dir}/{ressource_name}' ):
            print(f'\n*** Oups, the installation directory already contains a directory "{ressource_name}"...\n')
            return
        ...
        ressource_path   = self.download_ressource(ressource_url, installation_dir, verbose=verbose)
        installation_dir = self.extract_ressource(ressource_path, installation_dir, verbose=verbose)
    ...
    def check(self):
        ...
        try:
            # Get location
            datasets_dir = utils.get_datasets_dir()
            # Get about file
            ...
            print(f'    Fidle datasets       : **NOT FOUND**')
        ...
        try:
            notebooks_dir=None
            for d in ['.', '..', '../..', '../../..']:
                if os.path.isdir(f'{d}/fidle-master'): 
                    # Get version from about file
                    with open(f'{notebooks_dir}/fidle/about.yml') as file:
                        about         = yaml.full_load(file)
                        version       = about['version']
                        notebooks_dir = d
                    break
        ...
        if notebooks_dir is None:
            print(f'    Fidle notebooks      : Not found')
        else:
            print(f'    Fidle notebooks      : Ok         ({version})')
