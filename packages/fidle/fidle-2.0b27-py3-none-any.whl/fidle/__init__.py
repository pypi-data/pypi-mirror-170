...
def init(run_id=None, run_dir=None, datasets_env_var='FIDLE_DATASETS_DIR'):
    ...
    # ---- datasets location : method 1 (right next door)
    #
    __datasets_dir=None
    for d in ['./fidle-master', '../fidle-master', '../../fidle-master']:
        if os.path.isdir(d): __datasets_dir=d

    # ---- datasets location : method 2 (env var)
    #
    __datasets_dir = os.getenv(datasets_env_var, __datasets_dir)

    if __datasets_dir is None:
        __error_datasets_not_found(datasets_env_var)
    # Resolve tilde...
    __datasets_dir=os.path.expanduser(__datasets_dir)    
    
    # ---- Update Keras cache
    #
    updated = __update_keras_cache()
...
# ------------------------------------------------------------------
#
def __error_datasets_not_found(datasets_env_var):        
    print('WARNING !!\n----')
    print('The directory containing the datasets cannot be found.\n')
    print(f'This directory must be specified by the environment variable : {datasets_env_var}')
    print('----')
    assert False, f'datasets folder not found, please set {datasets_env_var} env var.'
