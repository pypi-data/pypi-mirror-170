...
def get_datasets_dir(datasets_env_var='FIDLE_DATASETS_DIR'):
    ...
    First, try to find a 'datasets-fidle' directory
    Secondly, try to get defined environment var
    ...
    # ---- Method 1 : right next door
    #
    datasets_dir=None
    for d in ['.', '..', '../..', '../../..']:
        if os.path.isdir(f'{d}/datasets-fidle'): 
            datasets_dir=f'{d}/datasets-fidle'
            break

    # ---- Method 2 : env var
    #
    datasets_dir = os.getenv(datasets_env_var, datasets_dir)

    # ---- Resolve tilde and return
    if datasets_dir is not None: datasets_dir=os.path.expanduser(datasets_dir)
    return  datasets_dir