...
class Installer:
    ...
    def check(self):
        ...
        #
        print('Note :')
        print('    Notebooks and datasets can only be found if they are in the current folder.')
        print(f'    Current folder is : {os.getcwd()}')

        # datasets_dir by env var
        datasets_dir = os.getenv('FIDLE_DATASETS_DIR', 'undefined')
        # Searching
        notebooks, datasets = utils.looking_about()

        # ---- Check datasets
        #
        print('\nCheck datasets dir : \n')

        print(f'    var FIDLE_DATASETS_DIR : {datasets_dir}')

        for d in datasets:
            print(f"    {d['path']:32s}   ({d['name']} / {d['version']})")
