...
class APIView(HTTPMethodView):
    async def get(self, request: sanic_request):
        try:
            ...
            if not ark_arg:
                return redirect("{}/".format(CONFIG["ARK_URL_PREFIX"]))
...
@server.route("{}/".format(CONFIG["ARK_URL_PREFIX"]))
...
<html lang="en">
    ...
    <body style="font-family: monospace;">
        ...
        <dl>
            ...
            <dd>{}</dd>
            <dt><strong>Identifiers alphabet</strong></dt>
            <dd>{}</dd>
        </dl>
    </body>
</html>
    ...
    return html(
        page.format(
            CONFIG["ARK_URL_PREFIX"],
            CONFIG["ARK_SERVER_ENDPOINT"],
            CONFIG["ARK_SERVER_NAAN"],
            "yes" if CONFIG["ARK_SERVER_DEBUG"] else "no",
            CONFIG["ARK_SERVER_ALPHABET"],
        )
    )


@server.route(
    "{}{}batch/".format(CONFIG["ARK_URL_PREFIX"], CONFIG["ARK_SERVER_ENDPOINT"]),
    methods=["POST"],
)
...
@server.get(
    r"{}/<ignore:ark:\/{}>/<ark_name:.+>".format(
        CONFIG["ARK_URL_PREFIX"], CONFIG["ARK_SERVER_NAAN"]
    )
)
async def resolve(request: sanic_request, ignore, ark_name):
    async with aiopg_cursor() as cur:
        try:
...
server.add_route(
    APIView().as_view(), CONFIG["ARK_URL_PREFIX"] + CONFIG["ARK_SERVER_ENDPOINT"]
)
...
def run_cli():
    ...
    parser.add_commands([serve, make_api_access, install, csv_import])
    parser.dispatch()
