...
class FeatureViews:
    ...
    def get(self, name: str, version: Optional[int] = None):
        request = pb.GetFeatureViewRequest(
            project_id=self._project.id, name=name, version=(version or 0)
        )
    ...
    def list(self):
        ...
        return [
            FeatureView(self._stub, feature_view)
            for feature_view in response.feature_view
        ]
