...
class AuthWrapper:
    ...
    def _is_access_token_expired(self):
        ...
        if self._access_token_expiration_date is not None:
            expires_in = (
                self._access_token_expiration_date - datetime.datetime.now()
            ).total_seconds()
    ...
    def _obtain_token(self):
        if self._get_access_token_external is not None:
            ...
            try:
                ...
                return (
                    False,
                    f"The authentication token is no longer valid. Please login again.",
                )

            self._access_token = resp.access_token
            ConfigUtils.store_token(self._props, resp.refresh_token)
            self._access_token_expiration_date = (
                datetime.datetime.now() + datetime.timedelta(seconds=resp.expires_in)
            )
