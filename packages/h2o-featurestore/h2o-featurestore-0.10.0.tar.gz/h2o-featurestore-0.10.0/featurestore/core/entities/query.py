...
class Query:
    ...
    @classmethod
    def select(cls, features: list[Union[Feature, fref.FeatureRef]]):
        return Select(
            [
                feature
                if isinstance(feature, fref.FeatureRef)
                else feature._reference()
    ...
    def join(
        self, feature_set: Union[FeatureSet, fsref.FeatureSetRef], alias
    ) -> OpenedJoin:
        return OpenedJoin(
            self,
            feature_set
            if isinstance(feature_set, fsref.FeatureSetRef)
            else feature_set._reference(),
            alias,
        )
    ...
    def _from_proto(cls, feature_query: pb.FeatureQuery) -> Query:
        ...
        def to_feature_ref(
            ...
            return fref.FeatureRef(
                selected_feature.feature, aliases.get(selected_feature.prefix)
            )

        from_feature_set = to_feature_set_ref(
            feature_query.from_feature_set.feature_set
        )
        ...
        alias_to_fs = dict(
            ...
            + [
                (join.alias, to_feature_set_ref(join.feature_set))
                for join in feature_query.join
                if len(join.alias) > 0
            ]
        )
        ...
        query_after_join = reduce(
            join_proto,
            feature_query.join,
            Query.select(selected_features).from_feature_set(
                from_feature_set, feature_query.from_feature_set.alias
            ),
        )

        if feature_query.where:
            return query_after_join.filter(
                Condition._from_proto(feature_query.where, alias_to_fs)
            )
        else:
...
class Select:
    def __init__(self, features: list[fref.FeatureRef]):
...
class SelectedFrom:
    def __init__(
        self,
        features: list[fref.FeatureRef],
        feature_set: fsref.FeatureSetRef,
        alias: Optional[str],
    ):
        self.features = features
        self.feature_set = feature_set
        self.alias = str(alias or None)
...
class LogicOperator(Enum):
    ...
    @classmethod
    def _from_proto(
        cls, proto_value: pb.FeatureFilterLogicOperator.ValueType
    ) -> LogicOperator:
        found_values = [
            member
            for name, member in LogicOperator.__members__.items()
...
class FilterOperator(Enum):
    ...
    @classmethod
    def _from_proto(
        cls, proto_value: pb.FeatureFilterOperator.ValueType
    ) -> FilterOperator:
        found_values = [
            member
            for name, member in FilterOperator.__members__.items()
            if member.value == proto_value
        ]
        if len(found_values) < 1:
            raise Exception(f"Not supported value {proto_value}")
...
class Condition(ABC):
    ...
    def _to_proto(self, aliases: dict[fsref.FeatureSetRef, str]):
        raise NotImplementedError(
            "Method `_to_proto` needs to be implemented by the child class"
        )

    @classmethod
    def _from_proto(
        cls, where: pb.FilterConditionUnion, aliases: dict[str, fsref.FeatureSetRef]
    ) -> Condition:
...
class FilterCondition(Condition):
    def __init__(
        self, selected_feature: fref.FeatureRef, operator: FilterOperator, value: str
    ):
    ...
    def _to_proto(
        self, aliases: dict[fsref.FeatureSetRef, str]
    ) -> pb.FilterConditionUnion:
...
class Join:
    ...
    def end(self) -> Query:
        return Query(
            self._query._select, self._query._joins + [self], self._query._filter
        )

    def _to_proto(
        self, aliases: dict[fsref.FeatureSetRef, str]
    ) -> pb.FeatureQuery.Join:
        return pb.FeatureQuery.Join(
            feature_set=pb.VersionedId(
                id=self._feature_set.id, major_version=self._feature_set.major_version
            ),
            alias=self._alias,
            on=[
