...
class SingleTokenLookupAnnotator(BaseAnnotator):
    def __init__(
        self, lookup_values: Optional[Iterable] = None, *args, matching_pipeline: Optional[list[BaseStringModifier]] = None, tokenizer_name: Optional[str] = "default", **kwargs
    ):
    ...
    def annotate_lookup_values(self, doc: Document, lookup_values: Union[LookupSet, set]):
        ...
        if isinstance(lookup_values, LookupSet):
            ...
            if lookup_values.has_matching_pipeline():
                ...
                return self._tokens_to_annotations(
                    [token
                     for token in tokens
                     if token.text in lookup_values]
                )
...
class TokenPatternAnnotator(BaseAnnotator):
    ...
    def annotate(self, doc: Document) -> list[Annotation]:
        ...
        for token in doc.get_tokens():
            ...
            match = self.pattern.match(token, doc._metadata)
