...
IR_FRAMEWORK_MODE_HEADLESS = os.getenv("IR_FRAMEWORK_MODE_HEADLESS", None) is not None
if not IR_FRAMEWORK_MODE_HEADLESS:
...
class LabelPrintedLenCalc:
    def __init__(self, family="Arial", font_size=20, char_num=63488, location=None):
        self.known_chars = {}
        self.family = family
        self.font_size = font_size
        self.char_num = char_num
        if location is None:
            self.location = os.path.join(os.getcwd(), 'cache', 'utf8_char_map.json')
        else:
            self.location = location
        self.__get_char_lens_tkinter()

    def __get_char_lens_tkinter(self):
        if not os.path.exists(self.location):
            if IR_FRAMEWORK_MODE_HEADLESS:
                print("Framework is running in headless mode. Using fallback for char map.")
                from .__utf_char_map__ import get_char_map_fallback
                self.known_chars = get_char_map_fallback()
                return
            print("Generating utf char map. This may take a while.")
            self.generate_utf8_char_map()
        else:
            print(f"Using cached char map @ {self.location}")
        with open(self.location, 'r') as file:
            self.known_chars = json.load(file)
    
    def generate_utf8_char_map(self):
        root = tk.Tk()
        font = tkFont.Font(family=self.family, size=self.font_size)
        print(f'Calculating UTF8 char len map for n = {self.char_num} characters.')
        for i in tqdm(range(self.char_num)):
            try:
                char = chr(i)
                (w, h) = (font.measure(char), font.metrics("linespace"))
                self.known_chars[char] = w
            except UnicodeEncodeError or ValueError:
                continue
        os.makedirs(os.path.dirname(self.location), exist_ok=True)
        with open(self.location, 'w') as file:
            json.dump(self.known_chars, file, indent=4)

    def __calc_char_len(self, char):
        if char not in self.known_chars:
            return int(sum(self.known_chars.values()) / len(self.known_chars))
        return self.known_chars[char]

    def calc_label_len(self, label):
        return sum([self.__calc_char_len(char) for char in label])


def get_printed_text_size(text, predictor=LabelPrintedLenCalc()):
    return predictor.calc_label_len(text)