...
class BackgroundLightCalibration(
    ...
    def resample_spatial_dimension(data: np.ndarray, num_spatial_bins: int) -> np.ndarray:
        ...
        this function simply loops over the unused dimensions and calls `block_reduce` on a much smaller array.
        """
        num_spec, num_wave, num_slit_pos = data.shape

        output = np.zeros((num_spec, num_wave, num_spatial_bins), dtype=np.float64)
        for i in range(num_spec):
            for j in range(num_wave):
                output[i, j, :] = skime.block_reduce(
                    data[i, j, :], block_size=(num_slit_pos // num_spatial_bins), func=np.median
                )
