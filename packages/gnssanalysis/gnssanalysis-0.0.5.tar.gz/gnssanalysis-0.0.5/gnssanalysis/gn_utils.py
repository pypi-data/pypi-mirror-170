...
def trace(ctx, plot):
    from gnssanalysis.gn_diffaux import difftrace
...
@_click.pass_context
def sinex(ctx):
    from gnssanalysis.gn_diffaux import diffsnx
    diffutil_verify_input(ctx.parent.params["input"])
    status = diffsnx(
...
@_click.pass_context
def ionex(ctx):
    from gnssanalysis.gn_diffaux import diffionex
    diffutil_verify_input(ctx.parent.params["input"])
    status = diffionex(
...
@_click.pass_context
def stec(ctx):
    from gnssanalysis.gn_diffaux import diffstec
    diffutil_verify_input(ctx.parent.params["input"])
    status = diffstec(
...
def clk(ctx):
    from gnssanalysis.gn_diffaux import diffclk
    diffutil_verify_input(ctx.parent.params["input"])
    status = diffclk(
...
def sp3(ctx, aux1, aux2, hlm_mode, plot):  # no coef
    from gnssanalysis.gn_diffaux import diffsp3
    diffutil_verify_input(ctx.parent.params["input"])
    status = diffsp3(
...
@_click.pass_context
def pod(ctx):  # no coef
    from gnssanalysis.gn_diffaux import diffpodout
    diffutil_verify_input(ctx.parent.params["input"])
    status = diffpodout(
...
def sp3merge(sp3paths, clkpaths, output):
    ...
    """
    from gnssanalysis.gn_io import sp3
