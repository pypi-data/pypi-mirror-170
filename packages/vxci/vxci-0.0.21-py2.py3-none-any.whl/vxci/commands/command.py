...
def build_image(**kwargs):
    ...
    if config.get('push_image'):
        ...
        # TODO: Old compatibility, remove this line after all repo are migrated
        common.save_imagename(config["ci_commit_ref_name"], config)
...
def check_log(logpath):
    ...
    logpaths = glob(path.join(logpath, "odoo.log_*"))
