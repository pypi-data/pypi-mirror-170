        ...
        PyObject *qobj_wrapper)
{
    ...
    for (Py_ssize_t li = 0; li < PyList_Size(dir); ++li)
    {
        ...
        {
            QByteArray name = qpycore_convert_ASCII(name_obj);
            if (name.isNull())
                continue;

            PyErr_Clear();

            connect(qobj, slot_obj, name, QByteArray());
        }
    }
        ...
        const QByteArray &slot_nm, const QByteArray &args)
{
    ...
    for (int m = 0; m < mo->methodCount(); ++m)
    {
        ...
        if (pyqt6_get_connection_parts(slot_obj, eobj, sig.constData(), false, &receiver, slot_sig) != sipErrorNone)
            continue;
