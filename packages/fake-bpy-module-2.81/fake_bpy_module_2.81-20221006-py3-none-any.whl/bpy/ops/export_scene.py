...
def gltf(override_context: typing.Union[typing.
    ...
    :param export_image_format: Images, Output format for images. PNG is lossless and generally preferred, but JPEG might be preferable for web applications due to the smaller file size * NAME Automatic, Determine the image format from the blender image name. * JPEG JPEG Format (.jpg), Encode and save textures as .jpg files. Be aware of a possible loss in quality. * PNG PNG Format (.png), Encode and save textures as .png files.
