...
class HostManager:
    ...
    def getHost(self):
        ...
        Raises:
            None
        """
        v = 0 if self.__host_len__ == 1 else random.randint(0, self.__host_len__-1)
    ...
    def reload(self):
        ...
        for k,v in self.__host_cnf_backup__.items():
            self.register(k,v["host"],v["port"],v["sniffer"],v["proto"],v["status"])
    ...
    def health(self):
        ...
        for k in self.__host__:
            ...
            if m_uri:
                m_uri+= "/"+self.__host_cnf__[k]["sniffer"].strip('/').replace('\\',"/")
