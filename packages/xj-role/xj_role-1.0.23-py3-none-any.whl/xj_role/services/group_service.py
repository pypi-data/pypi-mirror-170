...
class GroupService(object):
    ...
    @staticmethod
    def bind_user_role(params):
        params = format_params_handle(param_dict=params, filter_filed_list=[
            "user_id",
            "role_list"
        ])
        if not params:
            return None, "参数不能为空"
        id = params['user_id']
        role = params['role_list']
        try:
            for i in role.split(','):
                res = UserToRole.objects.filter(user_id=id, role_id=i).exists()
                if not res:
                    data = {
                        "user_id": id,
                        "role_id": i
                    }
                    instance = UserToRole.objects.create(**data)
            return None, None
        except Exception as e:
            return None, str(e)

    @staticmethod
    def bind_user_group(params):
        params = format_params_handle(param_dict=params, filter_filed_list=[
            "user_id",
            "group_list"
        ])
        if not params:
            return None, "参数不能为空"
        id = params['user_id']
        role = params['group_list']
        try:
            for i in role.split(','):
                res = UserToGroup.objects.filter(user_id=id, user_group_id=i).exists()
                if not res:
                    data = {
                        "user_id": id,
                        "user_group_id": i
                    }
                    instance = UserToGroup.objects.create(**data)
            return None, None
        except Exception as e:
            return None, str(e)
