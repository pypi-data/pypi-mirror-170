...
class Stage:
    ...
    def from_dict(cls, name: str, data: dict):
        ...
        if stage_def_path:
            ...
            stage_def_repo_store = GitRepoStore(options.stage_def_dir.value).load()
            stage_def_resource = stage_def_repo_store.get(stage_def_remote)
    ...
    @classmethod
    def from_config_data(cls, task_data: dict):
        stage_configs = {}
        for name, task_data in task_data.items():
            stage_configs[name] = cls.from_dict(name, task_data)
        return stage_configs

    @classmethod
    def from_config_path(cls):
        with options.atopile_file.value.open('r') as f:
