...
def add_lifecycle_policy_to_all_ecr_repositories(
    ...
    for repo in ecr_client.describe_repositories(registryId=ecr_registry_id, maxResults=1000)['repositories']:
        try:
            ...
            typer.echo(f"[INFO] Lifecycle policy added to {repo['repositoryName']}")
        except:
            typer.echo(f"[ERROR] Not able to add a lifecycle policy to {repo['repositoryName']}")
