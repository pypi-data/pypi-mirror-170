...
TFY_MLFLOW_CLIENT = "tfy-mlflow-client"
MLFLOW = "mlflow"
...
def _mlflow_conda_env(
    ...
    pip_deps = ([_get_pinned_requirement(TFY_MLFLOW_CLIENT)] if install_mlflow else []) + (
...
def _is_package_requirement(requirement_string, package: str):
    """
    Returns True if `requirement_string` represents a requirement for package
    Example,
    requests==0.1, requests will return True
    """
    try:
        ...
        return Requirement(requirement_string).name.lower() == package
        ...
        try:
            ...
            return Requirement(requirement_specifier).name.lower() == package
        except InvalidRequirement:
            return False
...
def _remove_mlflow_requirement(requirements):
    return [
        requirement
        for requirement in requirements
        if not _is_package_requirement(requirement, package=MLFLOW)
    ]


def _remove_tfy_mlflow_requirement(requirements):
    return [
        requirement
        for requirement in requirements
        if not _is_package_requirement(requirement, package=TFY_MLFLOW_CLIENT)
        and not _is_package_requirement(requirement, package=TFY_MLFLOW_CLIENT.replace("-", "_"))
    ]
...
def _process_pip_requirements(
    ...
    pip_reqs = _remove_mlflow_requirement(pip_reqs)
    pip_reqs = _remove_tfy_mlflow_requirement(pip_reqs)
    pip_reqs.insert(0, _get_pinned_requirement(TFY_MLFLOW_CLIENT))
