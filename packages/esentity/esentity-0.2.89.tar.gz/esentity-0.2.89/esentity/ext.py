...
class UIExtension(Extension):
    ...
    def _render(self, args, caller):
        ...
        # html.unescape(rv)
        s = t.render(items=items, rv=rv, args=args)
...
class ComplaintsStatsExtension(Extension):
    tags = set(['complaints_stats'])

    def __init__(self, environment):
        super(ComplaintsStatsExtension, self).__init__(environment)

    def _render(self, caller):
        stats = {}
        try:
            stats = loads(current_app.redis.get('gs:complaints_stats').decode('utf-8'))
        except Exception:
            pass

        t = current_app.jinja_env.get_template('_ext-complaint_stats.html')
        s = t.render(stats=stats)
        return s
