    ...
    def get_viewed_relation_count(
        ...
        try:
            ...
            node_from = Node.parse_obj({LABEL: USER_LABEL,
                                        PROPERTIES: {CUSTOMER_ID: str(customer_id)}})
            node_to = Node.parse_obj({LABEL: content_key,
            ...
            try:
                ...
                logging.warning(f"No VIEWED relation found on graph between {node_from} to {node_to}")
            except Exception as e:
                logging.error(f"Unable to drop VIEWED relation between {node_from} to {node_to}, Error: {e}")
            finally:
                return 0, []
