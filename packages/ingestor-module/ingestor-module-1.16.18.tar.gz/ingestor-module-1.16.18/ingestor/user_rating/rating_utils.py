...
from ingestor.common.constants import CUSTOMER_ID, VIEW_COUNT, VIEW_HISTORY, CREATED_ON, DURATION, RELATIONSHIP, \
    PROPERTIES, CONTENT_ID, CONTENT_LABEL, LABEL, USER_LABEL
...
from ingestor.user_rating.config import MILLISECONDS_IN_ONE_MINUTE, NUMBER_OF_DUPLICATE_VIEWS_THRESHOLD, VISIONPLUS_DEV, \
    VIEWED_DATA_PATH, S3_RESOURCE, HAS_RATING
from ingestor.user_rating.constants import RECENT_DURATION, IMPLICIT_RATING_SHIFTED, \
    MAX_RATING, IMPLICIT_RATING_VALUE, RECENT_DATE, CONTENT_STATUS, ACTIVE, IMPLICIT_RATING
...
class RatingUtils:
    ...
    def cap_ubd_data(data) -> DataFrame:
        quantile1, quantile3 = data[VIEW_COUNT].quantile([.25, .75])
        IQR = quantile3 - quantile1
        upper_view_threshold = ceil(quantile3 + 1.5 * IQR)
        data.loc[data[VIEW_COUNT] > upper_view_threshold, VIEW_COUNT] = upper_view_threshold
        return data
