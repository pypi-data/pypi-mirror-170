...
class DatasetVersion(Dao, Taggable):
    ...
    def __str__(self):
        return "{}Version '{}' of dataset {} {} (id: {})".format(
            bcolors.YELLOW, self.version, self.name, bcolors.ENDC, self.id
        )
    ...
    def get_resource_url_on_platform(self) -> str:
        ...
        return "{}/dataset/version/{}".format(self.connexion.host, self.id)

    @exception_handler
    @beartype
    def refresh(self, data: dict):
        object = DatasetVersionSchema(**data)
        self._name = object.name
        self._version = object.version
        self._type = object.type
        self._origin_id = object.origin_id
        return object

    @exception_handler
    @beartype
    def sync(self) -> dict:
        r = self.connexion.get("/sdk/dataset/version/{}".format(self.id)).json()
        self.refresh(r)
        return r
    ...
    def add_data(self, data: Union[Data, List[Data], MultiData]) -> Job:
        ...
        A versionned dataset (DatasetVersion) takes (Data) coming from organization's (Datalake) and transform it as annotable (Asset).
        ...
        r = self.connexion.post(
            "/sdk/dataset/version/{}/assets".format(self.id), data=orjson.dumps(payload)
        ).json()
        self.refresh(r["dataset_version"])
        logger.info(
            "Data are being added as assets to {}.\nThis operation can take some time (depending on how much asset you're adding).".format(
                self
            )
        )
    ...
    def fork(
        ...
        You can precise a description and a default type.
        ...
        If with_tags is True, tags of each asset will be transfered to the new dataset version.
        ...
        Arguments:
            ...
            description (str): description, if not defined will be "Forked from version '<version_name>'"
            assets ((MultiAsset) or (Asset)): assets to add to the new dataset version, if not defined will be all the assets
            type (InferenceType): inference type of the new dataset version, if not defined will be NOT_CONFIGURED
            with_tags (bool): if true tags of assets will be added to the new dataset version, if not defined will be false
        ...
        if description is None:
            description = "Fork from {}".format(self.version)
        ...
        r = self.connexion.post(
            "/sdk/dataset/{}/fork".format(self.origin_id), data=orjson.dumps(payload)
        ).json()
        logger.info(
            "{} forked with {} assets".format(
                self, len(assets) if assets is not None else "all"
            )
        )
    ...
    def find_asset(
        ...
        Raises:
            If no asset match the query, it will raises a NotFoundError.
            In some case, it can raise an InvalidQueryError, it might be because platform stores 2 assets matching this query (for example if filename is duplicated)
        ...
        r = self.connexion.get(
            "/sdk/dataset/version/{}/assets/find".format(self.id), params=params
        ).json()
    ...
    def find_all_assets(
        ...
        Examples:
            ...
            ```
        Arguments:
            filename (List[str]): filenames of the assets you're looking for. Defaults to None.
    ...
    def list_assets(
        ...
        Arguments:
            limit (int): limit to a certain number of assets, if -1, will retrieve all assets
    ...
    def _do_list_assets(
        ...
        r = self.connexion.get(
            "/sdk/dataset/version/{}/assets".format(self.id), params=params
        ).json()
        return (
    ...
    def delete(self) -> None:
        ...
        Examples:
            ...
            ```
        """
        self.connexion.delete("/sdk/dataset/version/{}".format(self.id))
    ...
    def set_type(self, type: InferenceType) -> None:
        ...
        self.connexion.patch(
            "/sdk/dataset/version/{}".format(self.id), data=orjson.dumps(payload)
        ).json()
        logger.info("{} is now of type {}".format(self, type.name))
    ...
    def update(
        ...
        r = self.connexion.patch(
            "/sdk/dataset/version/{}".format(self.id),
    ...
    def download(
        ...
        r = self.connexion.get(
            "/sdk/dataset/version/{}/assets/extended".format(self.id)
        ).json()
        assets = r["items"]
        ...
        logger.info(
            "{} assets (over {}) of dataset {} downloaded into {}".format(
                downloaded, self, len(results), path
            )
        )
    ...
    def list_labels(self) -> List[Label]:
        ...
        Returns:
            List of (Label)
        """
        r = self.connexion.get("/sdk/dataset/version/{}/labels".format(self.id)).json()
    ...
    def create_label(self, name: str) -> Label:
        ...
        payload = {"name": name}
        r = self.connexion.post(
            "/sdk/dataset/version/{}/labels".format(self.id), data=orjson.dumps(payload)
        ).json()
        label = Label(self.connexion, r)
        logger.info("{} has been added to {}".format(label, self))
        return label
    ...
    def get_label(self, name: str) -> Label:
        ...
        params = {"name": name}
        r = self.connexion.get(
            "/sdk/dataset/version/{}/labels/find".format(self.id), params=params
        ).json()
    ...
    def get_or_create_label(self, name: str) -> Label:
        ...
        If label does not exists, create it and return it.
    ...
    def list_annotations(
        ...
        Examples:
            ...
            ```
        Arguments:
            limit (Optional[int], optional): Limit number of annotations to retrieved. Defaults to None, all annotations will be retrieved.
            offset (Optional[int], optional): Offset to begin with when listing annotations. Defaults to None, starting at 0.
            page_size (Optional[int], optional): Size of each page when retrieving . Defaults to None, page will be equals to default pagination.
            order_by (Optional[List[str]], optional): Order annotation by some criteria. Defaults to None.
    ...
    def _do_list_annotations(
        ...
        r = self.connexion.get(
            "/sdk/dataset/version/{}/annotations".format(self.id), params=params
        ).json()
        return (
            list(
                map(
    ...
    def load_annotations(
        ...
        if chunk_size < 1 or chunk_size > 10000:
            raise exceptions.PicselliaError(
                "Impossible to load less than 1 and more than 10000 annotations by paquets. Please give another chunk_size"
            )
        ...
        ids = self.connexion.get(
            "/sdk/dataset/version/{}/annotations/ids".format(self.id), params=params
        ).json()
    ...
    def export_annotation_file(
        ...
        Precising 'worker' argument, you will retrieve only annotations of this worker if they exist.
        ...
        Examples:
            ...
            ```
        Arguments:
            type (AnnotationFileType): choose to export in Pascal VOC format or COCO format.
            target_path (str, optional): directory path where file is downloaded. Defaults to current directory.
            assets (Union[(MultiAsset), List[(Asset)], None], optional): a list of assets of this dataset version. Only these assets will be concerned by this export. Defaults to None.
        ...
        r = self.connexion.post(
            "/sdk/dataset/version/{}/annotations/export".format(self.id),
        ...
        if self.connexion._do_download_file(
            ...
            logger.warn(
                "Could not download {}. Url: {}".format(
                    r["object_name"], r["presigned_url"]
                )
            )
            return None
    ...
    def import_annotation_voc_file(
        ...
        try:
            ...
            raise FileNotFoundException("File {} not found".format(file_path))
        except ParsingError as e:
            raise UnparsableAnnotationFileException(
                "Could not parse VOC file {} because : {}".format(file_path, e)
            )
        ...
        for obj in objects:
            ...
            try:
                ...
                if force_create_label:
                    logger.error("Label {} not found, creating it".format(obj.name))
        ...
        if asset_annotation is None:
            ...
            # Else, there is already an annotation (Replace mode already handled, so Skip mode is the only edge case here)
            if mode == ImportAnnotationMode.SKIP:
                logger.info(
                    "This file {} is skipped because an anotation already exists.".format(
                        file_path
                    )
                )
        ...
        try:
            ...
            logger.error(
                "Could not create rectangle annotations for file {}  because {}".format(
                    asset.filename, e
                )
            )
        ...
        try:
            ...
            logger.error(
                "Could not create polygon annotations for file {} because {}".format(
                    asset.filename, e
                )
            )
        ...
        try:
            ...
            logger.error(
                "Could not create classification annotations for file {} because {}".format(
                    asset.filename, e
                )
            )
        ...
        if nb_created_rectangles + nb_created_polygons + nb_created_classifications > 0:
            logger.info(
                "Done! Pascal VOC file {} imported on asset {}".format(file_path, asset)
            )
            if nb_created_rectangles > 0:
                logger.info("{} rectangles created".format(nb_created_rectangles))

            if nb_created_polygons > 0:
                logger.info("{} polygons created".format(nb_created_polygons))

            if nb_created_classifications > 0:
                logger.info(
                    "{} classifications created".format(nb_created_classifications)
                )
        else:  # pragma: no cover
            logger.warn(
                "VOC file {} was imported, but no shape was created".format(file_path)
            )
    ...
    def import_annotations_coco_file(
        ...
        Returns:
            (List[Tuple[Asset, Union[Annotation, None]]]): A list with tuples of Asset X Annotation if annotation was not skipped
        """
        try:
            ...
            raise UnparsableAnnotationFileException(
                "Could not parse COCO file {} because : {}".format(file_path, e)
            )
        ...
        for category in cocofile.categories:
            if category.id in labels:
                raise UnparsableAnnotationFileException(
                    "Category id {} already defined".format(category.id)
                )
            ...
            try:
                ...
                if force_create_label:
                    logger.error(
                        "Label {} not found, creating it".format(category.name)
                    )
        ...
        for image in cocofile.images:
            ...
            try:
                ...
                logger.error("Asset does not exist {}".format(image.file_name))
        ...
        for annotation in cocofile.annotations:
            ...
            try:
                ...
                logger.error(
                    "category_id {} not found into retrieved labels".format(
            ...
            try:
                ...
                logger.error(
                    "image_id {} not found into retrieved assets".format(
                        annotation.image_id
                    )
                )
                continue
            ...
            if asset_annotation is None:
                logger.info(
                    "Skipped annotation {} because already exists".format(
                        annotation.image_id
                    )
                )
                continue
        ...
        for image_id, objects in rectangles.items():
            ...
            try:
                ...
                logger.error(
                    "Could not create rectangle annotations of image {} ({}) because {}".format(
                        asset.filename, image_id, e
                    )
                )
                continue
        ...
        for image_id, objects in polygons.items():
            ...
            try:
                ...
                logger.error(
                    "Could not create polygon annotations of image {} ({}) because {}".format(
                        asset.filename, image_id, e
                    )
                )
                continue
        ...
        for image_id, objects in classifications.items():
            ...
            try:
                ...
                logger.error(
                    "Could not create classification annotations of image {} ({}) because {}".format(
                        asset.filename, image_id, e
                    )
                )
                continue
        ...
        if nb_created_rectangles + nb_created_polygons + nb_created_classifications > 0:
            logger.info(
                "Done! COCO file {} imported, on {} assets".format(
                    file_path, len(assets.values())
                )
            )
            if nb_created_rectangles > 0:
                logger.info("{} rectangles created".format(nb_created_rectangles))
            ...
            logger.info(
                "COCO file {} was imported, but no shape was created".format(file_path)
            )
    ...
    def delete_all_annotations(self, workers: List[Worker] = None) -> None:
        ...
        self.connexion.delete(
            "/sdk/dataset/version/{}/annotations".format(self.id),
            data=orjson.dumps(payload),
        )
        logger.info("All annotations in {} were removed.".format(self))
    ...
    def synchronize(
        ...
        if len(not_uploaded) > 0:
            logger.info(
                "📚 {} assets not uploaded. You need to add data to the datalake first with :".format(
                    len(not_uploaded)
                )
            )
            logger.info(
            ...
            logger.info(
                'client.get_dataset_by_id({}).get_version("{}").add_data(list_data)'.format(
                    self.origin_id, self.version
                )
            )
        ...
        if len(not_downloaded) > 0:
            ...
            multi_assets = MultiAsset(self.connexion, self.id, assets)
            logger.info("📚 {} assets not downloaded".format(len(not_downloaded)))
            if do_download:
                logger.info("📚 Downloading {} assets".format(len(not_downloaded)))
    ...
    def create_asset_tag(self, name: str) -> Tag:
        ...
        payload = {"name": name}
        r = self.connexion.post(
            "/sdk/dataset/version/{}/tags".format(self.id), data=orjson.dumps(payload)
        ).json()
    ...
    def get_asset_tag(self, name: str) -> Tag:
        """Retrieve a asset tag used in this dataset version.
        ...
        params = {"name": name}
        r = self.connexion.get(
            "/sdk/dataset/version/{}/tags/find".format(self.id), params=params
        ).json()
        return Tag(self.connexion, r)
    ...
    def list_asset_tags(self) -> List[Tag]:
        ...
        Returns:
            A list of (Tag)
        """
        r = self.connexion.get("/sdk/dataset/version/{}/tags".format(self.id)).json()
    ...
    def train_test_split(
        ...
        """
        params = {"q": "nb_annotations > 0"}
        r = self.connexion.get(
            "/sdk/dataset/version/{}/assets/extended".format(self.id), params=params
        ).json()
        if r["count"] == 0:
        ...
        nb_assets_train = int(r["count"] * prop)
        train_eval_rep = [1] * nb_assets_train + [0] * (r["count"] - nb_assets_train)
        ...
        for item in r["items"]:
            ...
            if not annotations:  # pragma: no cover
                logger.info(
                    "No annotation for asset {}".format(item["data"]["filename"])
                )
                continue
            ...
            for label_id in label_ids:
                try:
                    ...
                    logger.warn("A shape has an unknown label ({}).".format(label_id))
