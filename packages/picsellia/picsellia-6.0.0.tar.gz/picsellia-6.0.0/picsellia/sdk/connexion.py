...
class Connexion(TokenServiceConnexion):
    ...
    def _generate_object_name(
        ...
        r = self.post(
            path="/sdk/organization/{}/connector/{}/generate_object_name".format(
                self.organization_id, connector_id
            ),
    ...
    def generate_experiment_object_name(
        ...
        if type not in (
            ...
            raise ImpossibleAction(
                "Cannot generate experiment object name with type {type}".format(
                    type=type
                )
            )
    ...
    def generate_model_version_object_name(
        ...
        if type not in (
            ...
            raise ImpossibleAction(
                "Cannot generate model version object name with type {type}".format(
                    type=type
                )
            )
        return self._generate_object_name(
    ...
    def generate_project_object_name(
        ...
        if type != ObjectDataType.SCAN_FILE:
            raise ImpossibleAction(
                "Cannot generate project object name with type {type}".format(type=type)
            )
        return self._generate_object_name(
            filename,
            type,
    ...
    def _init_upload(self, path: str) -> Tuple[bool, str]:
        if not os.path.isfile(path):
            raise exceptions.FileNotFoundException("{} not found".format(path))
    ...
    def _generate_presigned_url(
        ...
        r = self.post(
            path="/sdk/connector/{}/generate_presigned_url".format(connector_id),
            data=orjson.dumps(payload),
        )
    ...
    def _do_upload_file(
        ...
        """Uploads a small file into an S3 storage

        Arguments:
            path (str): Path to the file to upload
            object_name (str): Key name for S3 storage
            content_type (str): Content type of the file to upload
        """
        if not os.path.isfile(path):
            raise exceptions.FileNotFoundException("{} not found".format(path))
        ...
        if response.status_code != 204:
            logging.error(
                "Could not upload {} into S3. Status code: {}".format(
                    path, response.status_code
                )
            )
            raise exceptions.PicselliaError("Could not upload {} into S3".format(path))
    ...
    def upload_file(
        ...
        if is_large:
            try:
                ...
                logger.warn(
                    "This file is large but it is impossible to use multipart for this upload. Trying to upload in only one upload."
                )
    ...
    def _init_multipart(self, object_name: str, connector_id: UUID) -> str:
        ...
        r = self.post(
            path="/sdk/connector/{}/init_multipart_upload".format(connector_id),
            data=orjson.dumps(payload),
        )
    ...
    def _get_url_for_part(
        ...
        r = self.post(
            path="/sdk/connector/{}/generate_part_presigned_url".format(connector_id),
            data=orjson.dumps(payload),
        )
    ...
    def _complete_part_upload(
        ...
        try:
            r = self.post(
                path="/sdk/connector/{}/complete_part_upload".format(connector_id),
                data=orjson.dumps(payload),
            )
        except Exception as e:
            raise exceptions.NetworkError(
                "Impossible to complete part url : {}".format(e)
            )
    ...
    def _upload_large_file(
        ...
        """Upload a single large file to the server. It will sent by multipart.
    ...
    def _init_download(self, object_name: str, connector_id: UUID) -> str:
        ...
        payload = {"object_name": object_name}
        r = self.post(
            path="/sdk/connector/{}/retrieve_presigned_url".format(connector_id),
            data=orjson.dumps(payload),
        )
