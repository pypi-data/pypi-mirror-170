...
from app.utils import epoch_to_datetime, get_current_datetime, validate_mac_address
...
def query_vuln(query_id: int, regscale_ssp_id: int, create_issue: bool = False):
    """Query Tenable vulnerablities and sync assets to RegScale."""
...
def q_vuln(query_id: int, ssp_id: int, create_issue: bool) -> dict:
    """Query Tenable vulnerablities"""
...
def lookup_asset(
    asset_list: list, mac_address: str, dns_name: str = None
) -> list[Asset]:
    """Return single asset"""
    results = []
...
def create_regscale_issue_from_vuln(regscale_ssp_id: int, df: pd.DataFrame) -> None:
    ...
    """
    default_status = (
        app.config["issue"]["status"]
        if "issue" in app.config.keys() and app.config["issue"]["status"] is not None
        else "Draft"
    )
    ...
    for index, row in df.iterrows():
        if df["severity"][index] != "Info":
            if df["severity"][index] == "Critical":
                default_due_delta = (
                    app.config["issue"]["critical"]
                    if "issue" in app.config.keys()
                    and app.config["issue"]["critical"] is not None
                    else 3
                )
            elif df["severity"][index] == "High":
                default_due_delta = (
                    app.config["issue"]["high"]
                    if "issue" in app.config.keys()
                    and app.config["issue"]["high"] is not None
                    else 5
                )
            else:
                default_due_delta = (
                    app.config["issue"]["moderate"]
                    if "issue" in app.config.keys()
                    and app.config["issue"]["moderate"] is not None
                    else 30
                )
...
def log_vulnerabilities(
    data: list[dict], query_id: int, regscale_ssp_id: int, create_issue: bool
) -> pd.DataFrame:
...
def fetch_vulns(query_id: int, regscale_ssp_id: int, create_issue: bool) -> tuple:
    ...
    Returns:
        data (list) list of vulnerablities.
    """
    tsc = gen_tsc()
    data = []
    if query_id:
