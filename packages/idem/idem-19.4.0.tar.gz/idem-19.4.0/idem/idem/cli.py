...
async def sls(hub) -> int:
    ...
    if hub.OPT.idem.config:
        config_dir = f"file://{pathlib.Path(hub.OPT.idem.config).parent}"

        # If no sources were given, then try the parent directory of the config file
        if not src["sls_sources"]:
            hub.log.debug(f"Using config dir as sls source: {config_dir}")
            src["sls_sources"].append(config_dir)
        if not param["param_sources"]:
