...
class Excel:
    ...
    def read_indefinitely(self, start_cell, num_of_columns, num_of_rows=None, steps=1):
        """
        ...
        while True:
            try:
                data.append([self.cell(f'{char}{i}') for _ in range(num_of_columns)])
                if num_of_rows:
    ...
    @staticmethod
    def convert_to_dict(key, data, header_idx=0):
        """
        Convert data to dict using first row as keys
        :param key:
        :param data:
        :param header_idx:
        :return:
        ...
        dict_data = {}
        for row in data:
            dict_data[key] = dict(zip(data[header_idx], row))
