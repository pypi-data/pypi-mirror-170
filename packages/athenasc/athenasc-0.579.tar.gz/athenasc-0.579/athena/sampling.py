...
class Sampling:
    ...
    def sample(self, sim_meta=None, res_dir=None, ncells=10000, lambda_ls=0, cache=True, cache_dir=None, pop_fp=None, targeted_transcriptome=None, map_ls=True):    
        if res_dir is None:
        ...
        print (f"Simulation: {self.network_name} Sampling Cells and Molecules...", flush=True)
        meta_fp = os.path.join(res_dir, 'cell_metadata.csv.gz')
        
        sim_cells_meta = pd.read_csv(meta_fp)\
                           .reset_index()\
                           .rename(columns={"index": "cell_i"})
        
        if ncells > sim_cells_meta.shape[0]:
        ...
        sampled_cells = []
        ncells_per_grna = math.ceil(ncells / len(sim_meta.grna.unique()))
        
        cells_meta, gene_expr = self.sampling_cells(sim_meta, sim_cells_meta, ncells_per_grna, res_dir, targeted_transcriptome, sampled_cells)
        ...
        if cache:
            ...
            cells_meta.to_csv(os.path.join(cache_dir, 'cells_metadata.csv.gz'), compression='gzip', index=False)
            gene_expr.to_csv(os.path.join(cache_dir, 'gene_expression.csv.gz'), compression='gzip', index=False)
        
        return cells_meta, gene_expr
        
    def sampling_cells(self, sim_grna_meta, cell_grna_meta, ncells_per_grna, sim_dir, targeted_transcriptome, sampled_cells):
        sim_fp = os.path.join(sim_dir, 'simulated_counts.csv.gz')
        
        cell_grna_meta = self.sample_cells_per_simulation(sim_grna_meta, cell_grna_meta, ncells_per_grna, sampled_cells)
        gene_expr = self.load_cells(cell_grna_meta, sim_fp, targeted_transcriptome)
        
        return cell_grna_meta, gene_expr
    ...
    def sample_cells_per_simulation(self, sim_meta, cells_meta, ncells_per_grna, sampled_cells):
        
        for row_i in range(sim_meta.shape[0]):
            row = sim_meta.iloc[row_i]
            nsamples = int(math.ceil(ncells_per_grna * row.sample_percent))
            
            if nsamples == 0:
                continue
            
            cells_meta.loc[cells_meta.sim_name == row.sim_name, 'grna'] = row.grna
            sim_cells = cells_meta.loc[(cells_meta.sim_name == row.sim_name) &
                                       (~cells_meta.cell_i.isin(sampled_cells)), 'cell_i'].values
            sim_cells = list(sim_cells)
            
            if len(sim_cells) < nsamples:
                nsamples = len(sim_cells)            

            sampled = random.sample(sim_cells, k=nsamples)
            sampled_cells = sampled_cells + sampled
        
        cells_meta = cells_meta.loc[cells_meta.cell_i.isin(sampled_cells)]
        return cells_meta

    def split_list(self, a_list):
        half = len(a_list)//2
        batched = [a_list[:half], a_list[half:]]
        return batched