...
class GuideRNA:
    ...
    def create_multiplier_matrix(self):
        ...
        if self.crispr_type == 'knockout':
            ...
            multiplier = multiplier.loc[multiplier.index.isin(self.sim_meta.sim_name)]
        else:
        ...
        self.sim_meta['nsims'] = np.ceil(self.nsims_per_condition * self.sim_meta['sample_percent']).astype(int)
        ...
        for i in tqdm(range(self.sim_meta.shape[0])):
            ...
            for sim_i in range(row.nsims):
                ...
                sim_meta["grna"] = [row.grna] * len(feature)
