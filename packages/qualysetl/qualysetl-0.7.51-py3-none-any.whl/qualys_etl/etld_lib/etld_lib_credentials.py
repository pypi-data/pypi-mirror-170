...
def get_bearer_stored_in_env(update_bearer=True, cred={}):
    ...
    try:
        ...
        if cred['bearer'] == "":
            ...
            if age_of_file > max_age_of_bearer_token:
                cred = qualys_gateway_login_store_in_env(cred_dict=cred)
                etld_lib_functions.logger.info(f"Updated Bearer Token Successfully. age_of_file={age_of_file}, max_age={max_age_of_bearer_token}")
        else:
            cred = qualys_gateway_login_store_in_env(cred_dict=cred)
...
def qualys_gateway_login_store_in_env(cred_dict={}):
    ...
    for retry_number in range(max_retries):
        try:
            ...
            qualys_gateway_login_failed_message(
                message=f"Requests Module Failed ",
                response="",
                cred_dict=cred_dict,
                log_level=etld_lib_functions.logger.error)
            etld_lib_functions.logger.error(f"Exception: {e}")
        ...
        qualys_gateway_login_failed_message(response="",
                                            cred_dict=cred_dict,
                                            log_level=etld_lib_functions.logger.error)
        exit(3)
