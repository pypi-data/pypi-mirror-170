...
def validate_recipients(recipients: list, number: str, version: int = 2):
    ...
    try:
        ...
        raise Exception(f"Unable to list groups for {number}") from exc
    for recipient in recipients:
        if version == 1:
            recipient = recipient.replace("_", "/")
        if search(f"[?id==`{recipient}`]", group_ids):
    ...
    try:
        ...
        raise Exception("Unable to check registered recipients") from exc
    for recipient in check_registered:
        if search(f"[?number==`{recipient}`]", registered):
