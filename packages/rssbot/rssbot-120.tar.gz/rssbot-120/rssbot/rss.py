# pylint: disable=E1101,C0116,R0912,R0915,C0115,R0903,W0212
...
from .hdl import Bus, Cfg
from .obj import Class, Db, find, fntime, last, launch, save
...
def rss(event):
    if not event.args:
        _nr = 0
        for _fn, obj in find("rss"):
            event.reply("%s %s %s" % (_nr, obj.rss, elapsed(time.time() - fntime(_fn))))
            _nr += 1
        return
    url = event.args[0]
    if "http" not in url:
