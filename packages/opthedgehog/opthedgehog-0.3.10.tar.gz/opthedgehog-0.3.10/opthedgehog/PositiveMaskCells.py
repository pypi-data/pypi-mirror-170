...
class PosOptStraightWaveguideTaper(OptCell):
    """
        straight wave guide defined by grooves -- for positive resisit
    """
    etchlayer = 15
    ...
    def __init__(self, name, inports, length, *, endwidth=-1.0, layer=10, etchlayer=15, keepFirst = False):
        OptCell.__init__(self, name, layer)
        if isinstance(inports, list):
            self.inports = inports
        else:
    ...
    def createEtchingMask(self):
        ...
        wg0 = gdspy.Path(config.dfoptwgwidth, (x0,y0), )
        wg0.segment(self.length, self.arc, final_width=self.endwidth, layer=self.layer)

        wgEtch = gdspy.Path(config.dfoptwgwidth + 2*config.dfPosOptCellOpening, (x0,y0), )
        wgEtch.segment(self.length, self.arc, final_width=self.endwidth++ 2*config.dfPosOptCellOpening, layer=self.etchlayer)

        wgopen = gdspy.boolean(wgEtch, wg0, 'not',  layer=self.etchlayer)
        self.add(wgopen)
