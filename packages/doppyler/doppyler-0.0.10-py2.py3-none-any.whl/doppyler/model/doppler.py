...
class Doppler:
    ...
    async def _call_local_api(
        ...
        async with self._local_api_semaphore:
            ...
            if not self._nonce or datetime.now() > self._nonce_expires:
            ...
            try:
                ...
                await self._get_nonce_and_final_key()
                return await self._call_local_api(endpoint, method, data)
    ...
    def _call_listeners(self, alarm: Alarm, listeners: list) -> None:
        """Call all listeners."""
    ...
    async def get_all_alarms(self) -> list[Alarm]:
        ...
        new_alarms = {alarm.id: alarm for alarm in alarms}
        new_alarm_ids = set(new_alarms)
        old_alarm_ids = set(self.alarms)

        # Update existing alarms
        for alarm_id in new_alarm_ids & old_alarm_ids:
        ...
        for alarm_id in new_alarm_ids - old_alarm_ids:
            ...
            self._call_listeners(self.alarms[alarm_id], self._added_alarm_listeners)

        # Remove deleted alarms and call on removed listeners
        for alarm_id in old_alarm_ids - new_alarm_ids:
            self._call_listeners(
                self.alarms.pop(alarm_id, None), self._removed_alarm_listeners
            )
    ...
    async def get_alarm_sounds(self) -> list[str]:
        ...
        sounds = await self._call_api("alarms/sounds")
    ...
    async def get_all_data(self) -> dict[str, Any]:
        ...
        self.alarm_sounds.extend(data[ATTR_ALARM_SOUNDS])
        return data
