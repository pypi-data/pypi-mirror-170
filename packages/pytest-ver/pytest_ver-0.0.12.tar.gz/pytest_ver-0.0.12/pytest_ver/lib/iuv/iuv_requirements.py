...
class IUVRequirements:
    ...
    # @param passed    whether the call passed or faild
    # @param location  the iuv_verify() location
    # @return None
    def save_reqids_to_trace(self, reqids, passed, location):
        for reqid in reqids:
            ...
            self._trace[reqid]['passed'] = passed and self._trace[reqid]['passed']
    ...
    def report_trace(self, log_it):
        ...
        for reqid in sorted(results_keys):
            ...
            total_tested_locations += len(info['locations'])

        log_it('Total tested requirements', total_tested_reqmts, total_tested_reqmts)
        log_it('Total tested passed', total_tested_passed, total_tested_reqmts)
        log_it('Total tested failed', total_tested_failed, total_tested_reqmts)
        log_it('Total tested locations', total_tested_locations)
        log_it('')
