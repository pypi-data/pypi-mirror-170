...
def check_and_install_dependencies(product: AbstractSoftwareProduct,
                                   ...
                                   include_dependencies: bool = True,  # --no-deeps TODO DOC
                                   ):
    """
    ...
    # TODO HOST Variabel to generalize the host.has_lib_installed() ??
    import site
    ...
    if offline_zip_dir and not os.path.exists(offline_zip_dir):
        py_dir = f'{offline_zip_dir}/py_installs'
    ...
    while hard_nodes or licensed_nodes_left_to_install() or optional_nodes_left_to_install():
        ...
        if not v.licensed and not install_licensed:
            continue
        if v.check_installed(python_exec_path=python_exec_path, download_folder=py_dir) \
                and v.check_installed_correct_version():
            # It's already installed and has correct version
            continue
        elif is_spark_nlp(v) and not py_dir:
            # TODO same for pyspark??
        ...
        install_results[v] = v.install(
            secrets=secrets, py_path=python_exec_path, download_folder=py_dir,
            include_dependencies=include_dependencies, re_install=v.check_installed_correct_version()
        )
    if offline_zip_dir:
        finalize_zip_folder(offline_zip_dir, java_dir, license_dir)
        ...
        if not get_pip_lib_version('spark-nlp', py_exec=python_exec_path).equals(
            ...
            install_results[Software.spark_nlp] = \
                Software.spark_nlp.install(re_install=True,
                                           ...
                                           py_path=python_exec_path, download_folder=py_dir,
    ...
    if len(install_results) > 0:
        ...
        for installed_software, result in install_results.items():
            if installed_software.check_installed(python_exec_path=python_exec_path, download_folder=py_dir):
                print(
                    f'{installed_software.logo} {installed_software.name}=={installed_software.get_installed_version(prefer_pip=True)} installed! ✅ {installed_software.slogan} ')
            else:
...
def finalize_zip_folder(offline_zip_dir, new_java_dir, new_license_dir):
