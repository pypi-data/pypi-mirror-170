...
class SparkHcSoftware(AbstractSoftwareProduct):
    ...
    latest_version = LibVersion(
        '0.1.13')  # TODO REMOVE THIS HACK!! MAPS TO INTERNAL_FINLEG! LatestCompatibleProductVersion.healthcare.value
    jsl_url_resolver = HcLibResolver
    py_module_name = 'sparknlp_jsl'
    pypi_name = 'internal_with_finleg'  # TODO REMOVE THIS HACK!! 'spark-nlp-jsl'
    licensed = True
    is_py4j = True
...
class JohnSnowLabsSoftware(AbstractSoftwareProduct):
    ...
    pypi_name = 'jsl_tmp'  # TODO UPDATE THIS!! AFer release
...
class JslFullSoftware(AbstractSoftwareProduct):
    ...
    def check_installed(cls, python_exec_path=None) -> bool:
        ...
        # If python_exec_path=None, then check is for current Python executable
...
class Software:
    ...
    def for_name(name: Union[str, ProductName]) -> AbstractSoftwareProduct:
        ...
        return False # raise ValueError(f'Unknown Product {name}')
