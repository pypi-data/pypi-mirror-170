...
def test_create_or_update_session_data(
    ...
    if update_session:
        ...
        session_controller.anyscale_api_client.create_session.assert_called_once_with(
            create_session_data
        )
...
def test_ssh_session(
    ...
    with patch.multiple(
        ...
        "ray", init=mock_ray_init, nodes=mock_ray_nodes
    ), patch.multiple(
...
def test_anyscale_push_session(
    ...
    with patch.multiple("subprocess", run=mock_subprocess_run), patch.multiple(
        ...
        mock_validate_cluster_configuration.assert_called_once_with(
            config, api_instance=session_controller.api_client
        )
