...
class CreateScheduleConfig(JobConfig):
    ...
    def populate_default_project_id(cls, project_id):
        if not project_id:
            return get_default_project().id
        return project_id
...
class ScheduleController(BaseController):
    ...
    def apply(
        ...
        schedule_config = self.resolve_config(
            schedule_config_file, name=name, description=description
        )
        id = self.schedule_api.apply(schedule_config)
        self.url(id)
        self.log.info(f"Scheduled Job {id} has been updated successfully.")
        return id

    def resolve_config(self, schedule_config_file: str, **overrides):
    ...
    def resolve_file_name_or_id(
        ...
        *,
        ...
        if id:
            ...
            config = self.resolve_config(schedule_config_file)
            name = config.name
    ...
    def list(self, project_id=None, name=None, creator_id=None, id=None):
        if id:
    ...
    def pause(self, id, is_paused=True):
