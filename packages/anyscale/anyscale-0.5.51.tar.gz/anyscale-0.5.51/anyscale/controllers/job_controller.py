...
class JobController(BaseController):
    ...
    def generate_config_from_entrypoint(
        ...
        return self._populate_job_config(config_dict)
    ...
    def generate_config_from_file(
        ...
        job_config = self._populate_job_config(config_dict)
        if name:
    ...
    def _validate_config_dict(self, config_dict: Dict[str, Any]):
        """
        Throws an exception if there are invalid values in the config dict.
        """
        compute_config: Optional[
            Union[ComputeTemplate, DecoratedComputeTemplate]
        ] = None
        if "compute_config" in config_dict and isinstance(
            ...
            compute_config = get_cluster_compute_from_name(
                config_dict["compute_config"], self.api_client
            )
    ...
    def schedule(
        ...
        if is_anyscale_workspace() and is_entrypoint_cmd:
            entrypoint = [job_config_file, *entrypoint]
            config = self.generate_config_from_entrypoint(entrypoint, name, description)
            id = self.schedule_from_config(config, cron_expression=cron_expression)
        elif len(entrypoint) == 0:
            config = self.generate_config_from_file(
                job_config_file, name=name, description=description
            )
            ...
            msg = (
                "Within an Anyscale Workspace, `anyscale job schedule` takes either a file, or a command. To submit a command, use `anyscale job schedule -- my command`."
                if is_anyscale_workspace()
                else "`anyscale job schedule` takes one argument, a YAML file configuration. Please use `anyscale job schedule my_file`."
            )
            raise click.ClickException(msg)
        return id

    def schedule_from_config(
        self, job_config: JobConfig, cron_expression: Optional[str] = None
    ):
        # If project id is not specified, try to infer it
        project_id = infer_project_id(
            job_config.project_id, self.anyscale_api_client, self.log
        ...
        self.log.info(f"Scheduled Job {job.id} has been created successfully.")
    ...
    def _populate_job_config(self, config_dict: Dict[str, Any]) -> JobConfig:
