...
CLOUDFORMATION_TIMEOUT_SECONDS = 300
...
class CloudController(BaseController):
    ...
    def run_cloudformation(
        ...
        cfn_client.create_stack(
            ...
            Parameters=[
                ...
                {
                    "ParameterKey": "RayAutoscalerIAMRoleArn",
                    "ParameterValue": ray_iam_role_arn,
                },
            ],
    ...
    def update_cloud_with_resources(
        self, cfn_stack: Dict[str, Any], cloud_id: str, ray_iam_role_arn: str
    ):
        ...
        create_cloud_resource = CreateCloudResource(
            ...
            aws_iam_role_arns=[anyscale_iam_role_arn, ray_iam_role_arn],
    ...
    def prepare_for_managed_cloud_setup(
        ...
        # TODO:
        # 1. Move this to be created by cloudformation
        # 2. Have this to be a per cloud role
        ray_iam_role = self.setup_aws_ray_role(region, DEFAULT_RAY_IAM_ROLE)
        ...
        created_cloud = self.api_client.create_cloud_api_v2_clouds_post(
            write_cloud=WriteCloud(
                provider="AWS",
                region=region,
                credentials=f"arn:aws:iam::{user_aws_account_id}:role/{anyscale_iam_role_name}",
                name=cloud_name,
                is_bring_your_own_resource=False,
            )
        )
        return anyscale_iam_role_name, ray_iam_role.arn, created_cloud.result.id
        ...
        if provider == "aws":
            ...
            try:
                cfn_stack = self.run_cloudformation(
                    region, cloud_id, anyscale_iam_role_name, ray_iam_role_arn
                )
                self.update_cloud_with_resources(cfn_stack, cloud_id, ray_iam_role_arn)
    ...
    def verifiy_aws_cloud_resources(
        ...
        verify_aws_subnets_result, reordered_cloud_resource = verify_aws_subnets(
            cloud_resource=cloud_resource, region=region, logger=self.log
        )
        ...
        self.log.info(
            f"Verification result:\nvpc: {self._passed_or_failed_str_from_bool(verify_aws_vpc_result)}\nsubnets: {self._passed_or_failed_str_from_bool(verify_aws_subnets_result)}\niam roles: {self._passed_or_failed_str_from_bool(verify_aws_iam_roles_result)}\nsecurity groups: {self._passed_or_failed_str_from_bool(verify_aws_security_groups_result)}\ns3: {self._passed_or_failed_str_from_bool(verify_aws_s3_result)}\nefs: {self._passed_or_failed_str_from_bool(verify_aws_efs_result)}\ncloudformation stack: {self._passed_or_failed_str_from_bool(verify_aws_cloudformation_stack_result) if not is_bring_your_own_resource else 'N/A'}\n"
        )
    ...
    def register_aws_cloud(
        ...
        try:
            ...
            role = _get_role(get_role_name_from_role_arn(anyscale_iam_role_id), region)
            assert (
                role is not None
