...
def deal_fail(f, fail_error, handler, sync_trans_info=None, j_response=True):
    """处理异常情况的返回，兼容异步 微事务同步
    """
    if isinstance(fail_error, Exception):
        if SESS_DEBUG or not j_response:
            # 对j_response 是兼容本地情况，走到这里一定是异常；直接包处理啊就没有喜爱吧的些事情了
            raise fail_error
    logger.error('db->deal_fail f:%s, e:%s, self:%s', f, fail_error, handler)
    fail_error = fail_result(RESPONSE_SERVICE_ERROR, 'func is:%s, error is:%s' % (f, str(fail_error)))
