    ...
    def __init__(self, session=None):
        ...
        if session:
            ...
            def log_response(response):
                ...
                response.read()  # Must read response body in order to access elapsed time
                print(f"HTTP {request.method} {request.url} response time: {response.elapsed}")

            self.session = httpx.Client(timeout=timeout, headers=self.headers, event_hooks={'response': [log_response]})
