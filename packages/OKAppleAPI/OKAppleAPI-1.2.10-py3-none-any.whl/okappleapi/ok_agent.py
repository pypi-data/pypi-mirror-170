...
class OKProfileManager:
    ...
    def confirm_bundle_id_exist(self, bundle_id_str: str, name: str = None):
        """
        确保bundleID存在
        @param bundle_id_str: bundle_id字符串
        @param name: 创建bundleID时需要的name
        @return:
        """
        id_obj = self.get_bundle_id(bundle_id_str)
        if id_obj:
            return

        # 创建新的bundle_id
        if not name:
            raise Exception('name is None when create bundleID')
        self.agent.register_bundle_id(bundle_id_str, name)
        # 更新 device_list
        self._device_list = self.agent.list_devices()
