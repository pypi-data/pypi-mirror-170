...
class JslSecrets(WritableBaseModel):
    ...
    def from_access_token(access_token, license_number=0):
        ...
        ocr_candidates = list(filter(lambda x: x.version_secret == settings.raw_version_secret_ocr and x.product == ProductName.ocr, secrets))
        hc_handidates = list(filter(lambda x: x.version_secret == settings.raw_version_secret_medical and x.product == ProductName.hc, secrets))
        if hc_handidates:
