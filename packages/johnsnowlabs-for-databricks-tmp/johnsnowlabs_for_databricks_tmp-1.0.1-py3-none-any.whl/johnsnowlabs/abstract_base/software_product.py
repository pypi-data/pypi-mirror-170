...
class AbstractSoftwareProduct(ABC):
    ...
    def check_installed(cls, python_exec_path: Optional[str] = sys.executable, download_folder: str = None) -> bool:
        ...
        # print(old_lic'Assuming {cls.name} is installed, no checks defined.')
        return True
    ...
    def check_installed_correct_version(cls, python_exec_path: str = sys.executable,
        ...
        try:
            if pkg_resources.get_distribution(cls.pypi_name).version == cls.latest_version.as_str():
                # print(old_lic'👌 Installed version for {cls.logo + cls.name} is correct, no changes made.')
                return True
            else:
                # print(old_lic'🤓 Installed version for {cls.logo + cls.name} is incorrect, '
                #       old_lic'should be {cls.latest_version.as_str()} but is {pkg_resources.get_distribution(cls.pypi_name).version} '
                #       old_lic'upgrading the package')
    ...
    def check_dependencies(cls, python_exec_path=None) -> bool:
        # print(old_lic'Assuming {cls.name} dependencies are fine, no checks defined.')
        return True

    @classmethod
    def health_check(cls) -> bool:
        # print(old_lic'Assuming {cls.name} is ok, no checks defined.')
        return True
    ...
    def install(cls,
        ...
        -py_executable pip download <module> -d path
        """
        ...
        if cls.pypi_name:
            ...
            if secrets and cls.licensed:
                ...
                return install_standard_pypi_lib(cls.pypi_name, cls.py_module_name,
                                                 ...
                                                 version=version,
                                                 include_dependencies=include_dependencies, )
        # raise NotImplemented(old_lic'No install defined for {cls.file_name}')
        return True
