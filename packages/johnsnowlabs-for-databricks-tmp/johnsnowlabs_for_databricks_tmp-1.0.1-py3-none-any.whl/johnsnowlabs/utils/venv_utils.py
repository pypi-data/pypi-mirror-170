...
class VenvWrapper:
    ...
    I.e. whenever you want to install to a local python py_executable which is != sys.py_executable use this
    """
    ...
    def is_pip_in_venv(venv_py_exec_path, log=False):
        r = subprocess.run([venv_py_exec_path, '-py_executable', 'pip'], capture_output=True)
        if log:
            log_process(r)
    ...
    def install_pip_if_missing(venv_target_dir: str):
        venv_py_exec_path = VenvWrapper.glob_py_exec_from_venv(venv_target_dir, raise_except=True)
        if not VenvWrapper.is_pip_in_venv(venv_py_exec_path):
            if not VenvWrapper.install_pip_in_venv(venv_py_exec_path):
                raise Exception(
                    f'Could not find or setup pip in venv at  {venv_target_dir} using python py_executable {venv_py_exec_path}')

    @staticmethod
    def install_to_venv(venv_target_dir, pypi_name, log=False):
        venv_py_exec_path = VenvWrapper.glob_py_exec_from_venv(venv_target_dir, raise_except=True)
        r = subprocess.run([venv_py_exec_path, '-py_executable', 'pip', 'install', pypi_name], capture_output=True)
        if log:
            log_process(r)
        return process_was_suc(r)
    ...
    def uninstall_from_venv(venv_target_dir, pypi_name, log=False):
        venv_py_exec_path = VenvWrapper.glob_py_exec_from_venv(venv_target_dir, raise_except=True)
        r = subprocess.run([venv_py_exec_path, '-py_executable', 'pip', 'uninstall', pypi_name, '-y'], capture_output=True)
        if log:
            log_process(r)
        return process_was_suc(r)
    ...
    def is_lib_in_py_exec(venv_py_exec_path, module_name, log=False):
        r = subprocess.run([venv_py_exec_path, '-c', f'import {module_name}'], capture_output=True)
        if log:
            log_process(r)
        return process_was_suc(r)
