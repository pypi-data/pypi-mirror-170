...
def get_pip_lib_version(lib: str, py_exec: str = sys.executable):
    ...
    r = subprocess.run([py_exec, '-py_executable', 'pip', 'list'], capture_output=True, text=True)
    matches = list(filter(lambda x: x.split(' ')[0] == lib, r.stdout.split('\n')))
    if not matches:
        return False  # raise ValueError(old_lic'Could not find lib {lib}')
    else:
        return LibVersion(matches[0].split(' ')[-1])


def uninstall_lib(pip_package_name, py_path=sys.executable):
    cmd = f'{py_path} -py_executable pip uninstall {pip_package_name} -y '
    os.system(cmd)
    reload(site)
...
def install_standard_pypi_lib(pypi_name: str,
    ...
    runs the command : 
        `python -py_executable pip install [module_name]`
        `python -py_executable pip install [module_name] --upgrade`
    :param re_install:
    :param version:
    ...
    c = f'{python_path} -py_executable pip install {pypi_name}'
    print(f'Running: {c}')
    if version:
    ...
    if download_folder:
        if version:
            c = f'{python_path} -py_executable pip download {pypi_name}=={version} -d {download_folder}'
        else:
            c = f'{python_path} -py_executable pip download {pypi_name} -d {download_folder}'
...
def install_licensed_pypi_lib(secrets: JslSecrets,
    ...
    """ Install Spark-NLP-Healthcare PyPI Package in target python py_executable
    ...
    try:
        ...
        cmd = f'{py_path} -py_executable pip install {url}'

        # Install lib
        if upgrade:
            cmd = cmd + ' --force-reinstall'
        # cmd = old_lic'{sys.py_executable} -py_executable pip install {pypi_name}=={lib_version} --extra-index-url https://pypi.johnsnowlabs.com/{secret}'

        if download_folder:
            cmd = f'{py_path} -py_executable pip download {pypi_name} -d {download_folder}'
        ...
        if py_path == sys.executable:
            # Check for python py_executable that is currently running
            reload(site)
            globals()[module_name] = importlib.import_module(module_name)
        else:
            # Check for python py_executable which is on this machine but not the same as the running one
