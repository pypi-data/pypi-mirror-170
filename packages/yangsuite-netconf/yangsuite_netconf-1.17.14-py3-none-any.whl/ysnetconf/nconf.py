...
- name : {{rpcs['p_name']}}
  ...
  tasks:
    - name: {{rpcs['t_name']}}
      {{rpcs['op']}}:
        {{rpcs['content_tag']}}: |
            {{ rpcs['content']|indent(width=12, first=False, blank=False) }}
...
def gen_ansible_api(request):
    ...
    Returns:
      dict: with format::
        ...
        {'rpc': ansible template <XML string or error message>,}
    """

    result = {'rpc': 'Failed to generate RPC', 'status': "Failure"}
    rpctext = ''
    rpcs = {}
    data = ''
    proto_op = request.get("proto_op")
    p_name = request.get('p_name')
    t_name = request.get('t_name')
    rpcs['p_name'] = p_name
    rpcs['t_name'] = t_name
    if ('string_rpc' in request) and (request.get('string_rpc')):
        str_rpc = request.get('cfgd')
        try:
            rpcobj = et.fromstring(str_rpc)
        except et.XMLSyntaxError as xm:
            result['rpc'] = " Invalid XML "
            result['status'] = 500
            log.error("Invalid XML : \n " + str(xm) + "\n\n" + str(str_rpc))
            return result
        except Exception as exc:
            result['rpc'] = "Failed : Invalid XML"
            result['status'] = 500
            log.error("Failed  \n " + str(exc) + "\n\n" + str(str_rpc))
            return result
        for obj in rpcobj.iter():
            if (obj.tag.endswith('config')) and (
                    not(obj.tag.endswith('-config'))):
                data = obj
                rpcs['op'] = 'netconf_config'
                rpcs['content_tag'] = 'xml'
                break
            elif obj.tag.endswith('filter'):
                data = obj
                rpcs['op'] = 'netconf_get'
                rpcs['content_tag'] = 'filter'
                break
        else:
            print('no tag')
            result['status'] = 500
            return result
        rpc_str = et.tostring(data, encoding='unicode', pretty_print=True)
        rpcs['content'] = rpc_str
    else:
        if 'cfg' in request:
        ...
        for rpc in cfgd:
            ...
            try:
                rpctext = et.tostring(elem[0], encoding='unicode',
                                      pretty_print=True)
                for item in elem[0]:
                    elm1 = et.tostring(item, encoding='unicode',
                                       pretty_print=True)
                    if elm1.startswith('<config '):
                        rpctext = elm1
                        rpcs['content'] = rpctext
                        rpcs['op'] = 'netconf_config'
                        rpcs['content_tag'] = 'xml'
                        break
                    elif elm1.startswith('<filter '):
                        rpctext = elm1
                        rpcs['content'] = rpctext
                        rpcs['op'] = 'netconf_get'
                        rpcs['content_tag'] = 'filter'
                ...
                result['rpc'] = 'Some Error Occured'
                result['status'] = 500
                log.error(
                    "Some Error occured".format(
                        traceback.format_exc()))
                return result

    tplt = Template(ansible_template)
    result['rpc'] = tplt.render({'rpcs': rpcs, 'proto_op': proto_op})
    result['status'] = 200
    return result
...
def gen_rpc_api(request):
    ...
    if gentype == 'raw':
        ...
        if ('string_rpc' in request) and (request.get('string_rpc')):
            ...
            try:
                rpcobj = et.fromstring(str_rpc)
                rpctext = et.tostring(rpcobj[0], encoding='unicode',
                                      pretty_print=True)
                rpcs.append(rpctext)
            except et.XMLSyntaxError as xm:
                result['rpc'] = "Invalid XML \n "+str(xm)+"\n\n"+str(str_rpc)
            ...
            for rpc in cfgd:
                ...
                try:
                    ...
                    result['rpc'] = str(exc)
                    return result
                # Unlike the 'raw' case above,
                # here we don't want the outer <rpc> element, we just
                # want its contents, its first/only child element
                rpctext = et.tostring(elem[0], encoding='unicode',
                                      pretty_print=True)
                rpcs.append(rpctext)
                if (rpc.get('commit', False) and
                        rpc_cfgd['dsstore'] == 'candidate'):
                    rpcs.append(
                        _commit_rpc(gentype,
                                    prefix_namespaces=prefix_namespaces))
    ...
    for seg in task['segments']:
        try:
            ...
            for mod, data in cfgd.get('modules', {}).items():
                if 'revision' in list(data.keys()):
                    if [mod, data['revision']] not in mods:
                        mods.append([mod, data['revision']])

            if cfgd['proto-op'] == 'rpc' or cfgd['proto-op'] == 'custom':
                segments += cfgd['rpc']
                if gentype == 'run':
