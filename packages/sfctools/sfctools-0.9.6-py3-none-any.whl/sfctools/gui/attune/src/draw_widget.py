    ...
    def paintEvent(self, event):
        ...
        for conn in self.connectors:
            ...
            if conn in active_connectors:
                ...
                offset_box = 25
                tx,ty = self.curved_connector(qp,self.gox + int(conn.a.x+offset_box), self.goy + int(conn.a.y+offset_box),self.gox + int(conn.b.x+offset_box), self.goy + int(conn.b.y+offset_box), self.gox + int(conn.c.x+5), self.goy + int(conn.c.y+5))
                ...
                if self.parent().checkBox_5.isChecked(): # draw at handles
                    if self.parent().checkBox_2.isChecked():
                        if self.highlighted == conn.c:
                            ...
                            brush = QtGui.QBrush(self.parent().theme_manager.get_color(2),1)

                        a = self.zoom_factor*(self.gox +conn.c.x)
                        b = self.zoom_factor*(self.goy + conn.c.y-7)
                        if self.parent().checkBox_4.isChecked():
                            qp.fillRect(QtCore.QRect(QtCore.QPoint(int(a-.1),b-10),QtCore.QPoint(int(a+5.1*len(conn.c.name)),b+10)),brush)

                        qp.drawText(a,b,conn.c.name)
                else:
                    if self.parent().checkBox_2.isChecked():
                        if self.highlighted == conn.c:
                            brush = QtGui.QBrush(self.parent().theme_manager.get_color(4),1)
        ...
        for box in self.boxes:
            ...
            if box.ishelper:
                ...
                if self.parent().checkBox_3.isChecked():
