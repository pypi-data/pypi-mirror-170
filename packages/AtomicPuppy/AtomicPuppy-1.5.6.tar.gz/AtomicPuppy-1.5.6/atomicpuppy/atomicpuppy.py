...
class Page:
    ...
    @property
    def _default_link_metadata(self):
        return None
    ...
    def _make_event(self, e):
        ...
        try:
            metadata = json.loads(metadata) if metadata else self._default_metadata
        except ValueError:
            self._logger.warning(
                "Failed to parse json metaData for %s message %s",
                e.get("eventType"), e.get("eventId"))
            metadata = self._default_metadata

        link_metadata = e.get("linkMetaData")
        try:
            link_metadata = json.loads(link_metadata) if link_metadata else self._default_link_metadata
        except ValueError:
            self._logger.warning(
                "Failed to parse json linkMetaData for %s message %s",
                e.get("eventType"), e.get("eventId"))
            link_metadata = self._default_link_metadata
    ...
    async def start_consuming(self, once=False):
        ...
        while self._running:
            try:
                ...
                # TODO: The asyncio docs say you're generally not supposed to
                # prevent CancelledError propagating like this
                # We're handling it so that this task keeps reading events even
                # when a task gets cancelled (which is sometimes caused by an
                # unhandled exception in another task)
                self.logger.exception("Continuing after task cancellation")
                pass
