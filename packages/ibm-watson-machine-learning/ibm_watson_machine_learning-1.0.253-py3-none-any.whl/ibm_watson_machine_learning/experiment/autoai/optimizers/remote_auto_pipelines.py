...
from ibm_watson_machine_learning.utils.autoai.enums import (
    ...
    ForecastingPipelineTypes, PredictionType)
...
class RemoteAutoPipelines(BaseAutoPipelines):
    ...
    cognito_transform_names: List['Transformers'], optional
        List of transformers to include in the feature enginnering computation process.
    ...
    def fit(self,
        ...
        if self.params.get('data_join_graph'):
            ...
            #else:
            #    print(Messages.get_message(message_id="obm_deprecation_message_cloud"))
    ...
    def get_pipeline_notebook(self,
        ...
        filename: str, optional
            Filename under which the pipeline notebook will be saved.

        insert_to_cell: bool, optional
            If run in jupyter notebook, if set inserts the notebook into cell below.

        Returns
        -------
        Path to saved pipeline notebook.
    ...
    def predict(self, X: Union['DataFrame', 'ndarray']=None, observations: Union['DataFrame', 'ndarray']=None,
        ...
        observations: numpy.ndarray or pandas.DataFrame,
            New observations of forecasting data that were used to train AutoAI model. Supported only for forecasting pipelines.
        supporting_features: numpy.ndarray or pandas.DataFrame,
            Future values of exogenous features. Supported only for forecasting pipelines.
