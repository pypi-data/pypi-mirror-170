...
class MCBlockStateManipulator:
    ...
    @staticmethod
    def parseBlockStateProperties(blockStateProperties: str) -> dict[str, str]:
        """
        Converts the inputted blockState properties into a map mapping
        a string which is the property name, to its value.

        Example:
            - blockState = "minecraft:stairs[facing=east,shape=inner_left]"
            - the blockState properties are "[facing=east,shape=inner_left]"
            - and they will get parsed into {"facing": "east", "shape": "inner_left"}

        :param blockStateProperties: The blockState properties to parse

        :return: A parsed map of the inputted blockState properties.
        """

        # Check if the block has no properties at all
        if blockStateProperties == "":
            return {}

        # Remove the "[" and "]" of the blockProperties
        blockStateProperties = blockStateProperties[1:-1]
        # Remove the spaces in case someone uses spaces in their properties
        blockStateProperties = blockStateProperties.replace(" ", "")
        # Split the properties to only get strings like "north=none"
        splittedRawProperties = blockStateProperties.split(",")

        # Creation of the map
        outPropertyMap = {}
        for rawProperty in splittedRawProperties:
            # Split the property at "=" to get the key and value
            propKV = rawProperty.split("=")
            propertyName, propertyValue = propKV[0], propKV[1]
            # Put in map
            outPropertyMap[propertyName] = propertyValue

        # Retrun
        return outPropertyMap

    @classmethod
    def getRotatedHorizontalFacingProperty(cls, horizontalFacingProperty: str, ninetyDegreeTurnCount: int) -> str:
        """
        Returns the inputted horizontal facing property turned by the amount of inputted ninety degree turns.
        A horizontal facing property can be any of the 4 cardinal directions.

        :param horizontalFacingProperty: The horizontal facing property to rotate
        :param ninetyDegreeTurnCount: The number of 90 degree turns to apply

        :return: The rotated inputted horizontal facing property.
        """

        rotations = cls._horizontalFacingPropertyRotationList
        newRotationIndex = \
            RandomUtils.mathModulo((rotations.index(horizontalFacingProperty) + ninetyDegreeTurnCount), 4)
    ...
    '''
    @staticmethod
    def fillAbsentPropertiesInPropertyMap(propertyMap: dict[str, str], defaultPropertyMap: dict[str, str]):
        """
        When parsing property maps, there might be properties that aren't there at all.
        If so, add them to the map according to a passed defaultPropertyMap.

        :param propertyMap: The property map to fill up properties with
        :param defaultPropertyMap: The property map with all the default values we want to fill up
                                   the main map with.
        """

        # Loop over every property of the default property map, and if the
        # property isn't in the propertyMap then we add it
        for propertyName in defaultPropertyMap.keys():  # using .keys() because more explicit
            if propertyName not in propertyMap:
                propertyMap[propertyName] = defaultPropertyMap[propertyName]

    @staticmethod
    def propertyMapToBlockStatePropertiesString(propertyMap: dict[str, str]) -> str:
        """
        Returns the inputted property map as a properties string used in blockStates.

        Example:
            - PropertyMap = {"facing": "east", "shape": "inner_left"}
            - PropertyMap to string -> [facing=east,shape=inner_left]

        :param propertyMap: The property map to convert to blockState property string

        :return: The blockState property map to blockState property string.
        """

        # If the property map is empty, then the blockState doesn't have anything
        # after its block Id, but without this check it would return "[]"
        if len(propertyMap) == 0:
            return ""

        entriesAsStrings = list(map(lambda kv: f"{kv[0]}={kv[1]}", propertyMap.items()))
        return "[" + ",".join(entriesAsStrings) + "]"

    @staticmethod
    def rotateHorizontalFacingBlockStatesPropertyMap(propertyMap: dict[str, str],
                                                     defaultPropertyMap: dict[str, str],
                                                     ninetyDegreeTurnCount: int):
        """
        Rotates the inputted property map assuming it is linked to a horizontal facing blockState.
        (some block states have facings like "up" and "down" as well which isn't supported here.)

        :param propertyMap: The property map to rotate the horizontal facing property of
        :param defaultPropertyMap: The default property map of the inputted property map's blockState
        :param ninetyDegreeTurnCount: The amount of 90 degree turns to apply
        """

        # Fill up the property map's absent values
        MCBlockStateManipulator.fillAbsentPropertiesInPropertyMap(propertyMap, defaultPropertyMap)
        # To rotate horizontally we just need to rotate the facing
        newFacing = MCBlockStateManipulator.getRotatedHorizontalFacingProperty(propertyMap["facing"],
                                                                               ninetyDegreeTurnCount)
        # Put the new facing in the hashmap
        propertyMap["facing"] = newFacing

    @classmethod
    def getHorizontallyFlippedFacingProperty(cls, horizontalFacingProperty: str, flippingPlane: str) -> str:
        """
        Returns the inputted horizontal facing property but flipped along the inputted plane.

        :param horizontalFacingProperty: The horizontal facing property to flip
        :param flippingPlane: The plane to flip the facing property by

        :return: The flipped horizontal facing property
        """

        if flippingPlane == "xy":
            if horizontalFacingProperty in ("north", "south"):
                # Flip
                return "south" if horizontalFacingProperty == "north" else "north"

        if flippingPlane == "yz":
            if horizontalFacingProperty in ("east", "west"):
                # Flip
                return "west" if horizontalFacingProperty == "east" else "east"

        return horizontalFacingProperty

    @staticmethod
    def getHorizontallyFlippedSideValueList(sideValueList: list, flippingPlane: str) -> list[str]:
        """
        Returns a new list which is the horizontal flip of the inputted side value list depending on the
        inputted flipping plane.
        Primarily used for redstone and wall flipping!

        A side value list is the list of the 4 values of the 4 cardinal directions of blocks
        that supports such side values, like redstone. Where the north, east, south, west
        properties are either: none, side or up

        :param sideValueList: The side value list to be horizontally flipped
        :param flippingPlane: The plane to flip the side value list by

        :return: A new list which is the flipped inputted side value list
        """

        flippedList = sideValueList.copy()

        if flippingPlane == "xy":
            # If plane is XY, switch the north and south values
            flippedList[0], flippedList[2] = flippedList[2], flippedList[0]

        if flippingPlane == "yz":
            # If plane is YZ, switch the east and west values
            flippedList[1], flippedList[3] = flippedList[3], flippedList[1]
...
class MCStructure:
    """
    Class representing a Minecraft structure, nothing to do with actual minecraft NBT
    structure files though. It's just a class that holds blocks, with additional methods
    for manipulating them such as translation, rotation etc...

    Used in the MCSchematic class as its block container, but an MCStructure can be a
    standalone.
    """
    ...
    def setBlock(self, position: tuple[int, int, int], blockData: str):
        """
        Sets a blockData in the structure at the inputed position.

        A blockData is a string format representing any block in
        Minecraft, used in commands such as /setblock or /fill.
    ...
    def getBlockStateAt(self, position: tuple[int, int, int]) -> str:
        """
        Returns the blockState at the inputted position in the structure.

        :param position: The position in the structure to get the blockState from.

        :return The blockState at the inputted position.
        """
    ...
    def getBlockDataAt(self, position: tuple[int, int, int]) -> str:
        """
        Return the blockData at the inputted position.

        :param position: The position in the structure to get the blockData from.
    ...
    def getInternalBlockPalette(self) -> DictView:
        ...
        The unmodifiable view is her ein an attempt to heavily discourage trying to modify the
        block palette yourself. Do not try to modify the block palette unless you know *exactly*
        what you're doing.

        :return: An unmodifiable view of this structure's internal block palette.
        """
    ...
    def getBlockPalette(self) -> DictView:
        ...
        so in the format of {blockState: id}.
        ...
        :return: The clean block palette of this structure.
        """
    ...
    def getBlockStates(self) -> DictView:
        ...
        :return: An unmodifiable view of the internal blockStates hashmap.
        """
    ...
    def getBlockEntities(self) -> DictView:
        ...
        :return: An unmodifiable view of the internal blockEntities hashmap
        """
    ...
    def getBounds(self) -> tuple[tuple[int, int, int], tuple[int, int, int]]:
        ...
        randomBlockPos = (k for k in self._blockStates.keys()).__next__()
    ...
    @staticmethod
    def getStructureDimensions(structureBounds: tuple[tuple[int, int, int], tuple[int, int, int]]) \
            -> tuple[int, int, int]:
        """
        Returns the X, Y and Z lengths of this structure, from the bounds.
        We need to input the bounds, because as they're really intensive
        to calculate, I think it's better if it's the programmer's job to
        cache them as needed.

        :param structureBounds: The bounds of this structure

        :return: The X, Y and Z lengths of this structure.
        """
    ...
    def placeStructure(self, incomingStructure: 'MCStructure', placePosition: tuple[int, int, int]):
        ...
        :param incomingStructure: The structure to place
        :param placePosition: The position at which we want to place the structure at
        """
    ...
    def makeCopy(self) -> 'MCStructure':
        ...
        :return: A copy of this MCStructure.
        """
    ...
    def getSubStructure(self, corner1: tuple[int, int, int], corner2: tuple[int, int, int], reCenter:bool = False) \
            -> 'MCStructure':
        """
        Returns a new structure which is all the blocks in the calling structure
        that are inside the inputted cuboid (both coordinates inclusive).

        Note that the position of the blocks in the new structure aren't changed
        from the calling structure, if the cuboid is off-center, the resulting
        subStructure will also be.
        (i.e a block at (0, 3, 2) will stay at (0, 3, 2) in the resulting subStructure)
        Unless the "reCenter" variable is set to True. In which case the structure is going to
        be centered around 0, 0, 0

        :param corner1: The first corner of the inputted cuboid
        :param corner2: The second corner of the inputted cuboid
        :param reCenter: If True, will center the subStructure around 0, 0, 0

        :return: Returns a new structure which is all the blocks in the calling structure
                 that are inside the inputted cuboid (both cuboids corners are inclusive).
        """

        # Instantiate the new subStructure that we're gonna return later
        subStructure = MCStructure()
        # Sort the corners for easier computing later
        corner1, corner2 = self._Utils.sortCuboidCorners(corner1, corner2)

        # Go through each block present in this structure and see if they're
        # inside the cuboid, if so we get the blockData there and .setBlock() it
        # in the newer structure.
        # We still using setBlock() and getBlockDataAt() but at least it's not over
        # every possible block of the cuboid xd
        for blockPosition, blockState in self._blockStates.items():

            # Get if in cuboid, if not just go on to the next block
            isInCuboid = (corner1[0] <= blockPosition[0] <= corner2[0]) and \
                         (corner1[1] <= blockPosition[1] <= corner2[1]) and \
                         (corner1[2] <= blockPosition[2] <= corner2[2])
            if not isInCuboid:
                continue

            # Get the block data then put it inside of the new structure
            blockDataHere = self.getBlockDataAt(blockPosition)
            subStructure.setBlock(blockPosition, blockDataHere)

        # Recenter the sturcture, by definition the bounds of the new structure is the sorted
        # cuboid coordinates inputted, so we don't have to recalculate bounds
        if reCenter:
            subStructure.center((corner1, corner2))
    ...
    def translate(self, translationVector: tuple[Union[int, float], Union[int, float], Union[int, float]]):
        ...
        Can be computationally intensive, like most transformation methods.

        :param translationVector: The amount of blocks in XYZ for each block to be translated by.
        """
    ...
    def scaleXYZ(self,
        ...
        :param anchorPoint: The point to scale the structure from
        :param scaleX: The X scalar
        :param scaleY: The Y scalar
        :param scaleZ: The Z scalar
        """
    ...
    def rotateRadians(self,
        ...
        We also have an option to rotate the block states. Right now only horizontal (yaw)
        rotation is available with block state rotation. There is currently no plan to add
        vertical rotation.

        :param anchorPoint: The point to rotate the schem around of
        :param yaw: The yaw component of the rotation
        :param pitch: The pitch component of the rotation
        :param roll: The roll component of the rotation
        :param rotateBlockStates: Whether or not we want to not only rotate the block's positions but also
                                  rotate the block's directions. (Like stairs and stuff)
        """
        ...
        # Rotate the block palette!
        if rotateBlockStates:
            # -- Calculate how many 90 degree turns we need
            # Calc the angle of rotation between 0 and 360
            positiveRotationAngle = RandomUtils.mathModulo(yaw, math.tau)
            # Calc the 90 degree turn count
            ninetyDegreeTurnCount = math.floor( (positiveRotationAngle + math.pi/4) / (math.pi / 2) ) % 4

            # Rotaté if needed
            if ninetyDegreeTurnCount != 0:
                # Epic rotaté of the epic block palette
                newBlockPalette = MCStructure._Utils.getHorizontallyRotatedBlockPalette(self._blockPalette,
                                                                                        ninetyDegreeTurnCount)
                # Set the new block palette
                self._blockPalette = newBlockPalette
    ...
    def flip(self,
        ...
        "flipBlockStates" if True, will flip the blocks themselves and not only their
        position in the world. Note that it's only horizontally with no plans on going
        vertical anytime soon.

        :param anchorPoint: The point the inputted flipping plane has to go through
        :param flippingPlane: The plane to flip the structure by
        :param flipBlockStates: Whether or not we want to not only flip the block's positions but
                                also flip their direction. (Like stairs and such)
        """
        ...
        # axis
        xInPlane = 'x' in flippingPlane
        yInPlane = 'y' in flippingPlane
        zInPlane = 'z' in flippingPlane
        ...
        # Flip the palette if we wanna flip the block states!
        if flipBlockStates:
            # If we are flipping around a plane that will make a flip be horizontal,
            # flip the block palette, otherwise don't
            if flippingPlane != "xz":
                # Epic flippance of the epic block palette
                newBlockPalette = MCStructure._Utils.getHorizontallyFlippedBlockPalette(self._blockPalette,
                                                                                        flippingPlane)
                # Set the new block palette
                self._blockPalette = newBlockPalette
    ...
    def centerAround(self,
                     anchorPoint: tuple[Union[int, float], Union[int, float], Union[int, float]],
                     structureBounds: tuple[tuple[int, int, int], tuple[int, int, int]]):
        """
        [Transformation method]

        Centers this structure around the anchor point as best as it can with integer coordinates.
        The centering algorithm is biased to bulge out in the XYZ+ directions if the midpoint
        isn't exact.

        Structure bounds have to be inputted manually as it's better if it's the dev's job to cache
        bounds as it's computationally intensive to compute them.

        :param anchorPoint: The point to center the structure around of
        :param structureBounds: The bounds of this structure
        """

        # Calculate the midpoint of the structure to get a vector from 0, 0, 0 to
        # the middle of it. Then negate it to map the struct middle to 0, 0, 0 and
        # all the remaining blocks are gonna follow it
        # Now add the anchorPoint vector, if we want to center around 1, 1, 1 instead
        # of 0, 0, 0, we're gonna have to add 1, 1, 1 to every coordinate!
        # You can think of the anchorPoint as the vector from 0, 0, 0 to where you
        # want to center your build around to
        # The translation vector is floored so that the structure is biased to
        # buldge out in the XYZ+ directions if the midpoint isn't exact instead
        # of the XYZ- dir.
        translationVector = (-((structureBounds[1][0] + structureBounds[0][0]) // 2),
                             -((structureBounds[1][1] + structureBounds[0][1]) // 2),
                             -((structureBounds[1][2] + structureBounds[0][2]) // 2))
        # Add the anchor point
        translationVector = ( translationVector[0] + anchorPoint[0] ,
                              translationVector[1] + anchorPoint[1] ,
                              translationVector[2] + anchorPoint[2] )

        # Translate and return for chaining
        return self.translate(translationVector)

    def center(self, structureBounds: tuple[tuple[int, int, int], tuple[int, int, int]]):
        """
        [Transformation method]

        Centers this structure around (0, 0, 0) as best as it can with integer coordinates.
        ...
        return self.centerAround((0, 0, 0), structureBounds)
    ...
    def cuboidFilled(self, blockData: str, corner1: tuple[int, int, int], corner2: tuple[int, int, int]):
        ...
        Generates a filled cuboid with only one blockData from corner1 to corner2. Will be faster than a bunch
        of .setBlocks() as some checks are bypassed because we got access to internals.

        :param blockData: The blockData that makes up this filledCuboid
    ...
    def cuboidHollow(self, blockData: str, corner1: tuple[int, int, int], corner2: tuple[int, int, int]):
        """
        Creates a hollow cuboid with the inputted blockData from corner1 to corner2. Will be faster
        than a bunch of .setBlocks() as some checks are bypassed because we got access to internals.

        :param blockData: The blockData that makes up this hollow cuboid.
        :param corner1: The first corner of this cuboid
        :param corner2: The second corner of this cuboid
        """
    ...
    def cuboidOutlines(self, blockData: str, corner1: tuple[int, int, int], corner2: tuple[int, int, int]):
        """
        Creates the outlines of the inputted cuboid. Will be faster than a bunch of .setBlocks() as some
        checks are bypassed because we got access to internals.

        :param blockData: The blockData that makes up this cuboid outlines.
        :param corner1: The first corner of this cuboid
        :param corner2: The second corner of this cuboid
        """
    ...
    class _Utils:
        ...
        '''
        @classmethod
        def generateAll8CuboidCorners(cls, corner1: tuple[int, int, int], corner2: tuple[int, int, int])\
                -> tuple[tuple[int, int, int]]:
            # Firstly sort corner1 and 2
            corner1, corner2 = cls.sortCuboidCorners(corner1, corner2)
        ...
        '''
        Returns the inputted blockPalette horizontally rotated by the amount of 90 degree
        turns we ask to turn it for. 90 degree horizontal rotation defined by the .rotate() 
        yaw transform.
        '''
        @staticmethod
        def getHorizontallyRotatedBlockPalette(blockPalette: {Union[str, int]: Union[str, int]},
                                               ninetyDegreeTurnCount: int) -> {Union[str, int]: Union[str, int]}:
            # Don't rotate the block palette if the ninetyDegreeTurnCount is a multiple
            # of 4. The check if already done in .rotate() but if I need this method
            # again I'll be happy that this check is here lmfao
            if ninetyDegreeTurnCount % 4 == 0:
                return blockPalette

            # Init the new block palette which is gonna be a rotation of
            # the current one
            rotatedBlockPalette = {}

            # Go through each block palette entry that starts with a number,
            # rotate the linked blockState string to it then put {num: newState} and
            # {newState: num} inside the new palette
            for k, v in blockPalette.items():
                # Filtering out all entries where the key isn't an integer,
                # don't need to rotate a blockState twice
                if type(k) != int: continue

                # Renaming because I love :sparkles: clarity :sparkles:
                blockStatePaletteId = k
                blockState = v

                # Rotate epic block state
                rotatedBlockState = MCBlockStateManipulator.getHorizontallyRotatedBlockState(blockState,
                                                                                             ninetyDegreeTurnCount)

                # Time to put in hashmap bois
                rotatedBlockPalette[blockStatePaletteId] = rotatedBlockState
                rotatedBlockPalette[rotatedBlockState] = blockStatePaletteId

            # Retrun
            return rotatedBlockPalette

        '''
        Returns the inputted block palette but flipped according to the inputted flipping plane.
        Since it's only horizontal flipping, the flipping plane can be anything except "xz".
        '''
        @staticmethod
        def getHorizontallyFlippedBlockPalette(blockPalette: {Union[str, int]: Union[str, int]},
                                               flippingPlane: str) -> {Union[str, int]: Union[str, int]}:
            # If the flipping plane is "xz" then don't flip anything.
            # Check already done in .flip() but it's here just in case!
            if flippingPlane == "xz":
                return blockPalette

            # Init the new block palette which is gonna be a flip of
            # the current one
            flippedBlockPalette = {}

            # Go through each block palette entry that starts with a number,
            # flip the linked blockState string to it then put {num: newState} and
            # {newState: num} inside the new palette
            for k, v in blockPalette.items():
                # Filtering out all entries where the key isn't an integer,
                # don't need to flip a blockState twice
                if type(k) != int: continue
                ...
                # Flip epic block state
                flippedBlockState = MCBlockStateManipulator.getHorizontallyFlippedBlockState(blockState,
                                                                                             flippingPlane)

                # Time to put in hashmap bois
                flippedBlockPalette[blockStatePaletteId] = flippedBlockState
                flippedBlockPalette[flippedBlockState] = blockStatePaletteId

            # Retrun
            return flippedBlockPalette
...
class MCSchematic:
    ...
    Which is really just a wrapper class around an MCStructure that just
    implements loading and saving from files haha
    """
    ...
    def save(self, outputFolderPath: str, schemName: str, version: 'Version', fastSaving:bool = False):
        ...
        The "fastSaving" argument makes saving use another algorithm for schematics with a lot of blockStates
        in its block palette, which is faster but increases the file-size, not usually more than twice the size though.

        :param outputFolderPath: The output folder path
        :param schemName: The name of the saved schematic file (without the ".schem" part)
        :param version: The Version enum entry which is the version of Minecraft we want to save the schem for.
        :param fastSaving: Whether or not to use the fast but large file size saving algorithm
        """
    ...
    def getStructure(self):
        """
        Returns the MCStructure object of this schematic. Any modifications
        on that structure object will affect this MCSchematic.

        :return: The MCStructure object of this schematic.
        """
    ...
    def placeSchematic(self, incomingSchematic: 'MCSchematic', placePosition: tuple[int, int, int]):
        """
        Places the inputted MCSchematic's structure inside of this MCSchematic's structure,
        at the inputted position.

        :param incomingSchematic: The schematic to place
        :param placePosition: The position at which we want to place the structure at
        """
    ...
    def placeStructure(self, incomingStructure: MCStructure, placePosition: tuple[int, int, int]):
        """
        Places the inputted structure inside of this MCSchematic's structure, at the
        inputted position.
    ...
    def makeCopy(self):
        """
        Makes and returns a deep copy of this schematic. So the returned schematic
        won't have its internals shared with the new one, it will have totally new
        internals. And thus modifying its structure won't affect another one.

        :return: A copy of this MCSchematic.
        """

        return MCSchematic(self._structure.makeCopy())

    def getSubSchematic(self, corner1: tuple[int, int, int], corner2: tuple[int, int, int], reCenter:bool = False):
        """
        Returns a new schematic which is all the blocks in the calling schematic
        that are inside the inputted cuboid (both coordinates inclusive).

        Note that the position of the blocks in the new schematic aren't changed
        from the calling schem, if the cuboid is off-center, the resulting
        subSchematic will also be.
        (i.e a block at (0, 3, 2) will stay at (0, 3, 2) in the resulting subSchematic)
        Unless the "reCenter" variable is set to True. In which case the structure is going to
        be centered around 0, 0, 0
        ...
        :return: Returns a new schematic which is all the blocks in the calling schematic
                 that are inside the inputted cuboid (both cuboids corners are inclusive).
        """

        return MCSchematic(self._structure.getSubStructure(corner1, corner2, reCenter))
...
class BlockDataDB:
    """
    A database for useful blockData strings such as containers containing
    a certain amount of items for precise signal strengths.
    """
