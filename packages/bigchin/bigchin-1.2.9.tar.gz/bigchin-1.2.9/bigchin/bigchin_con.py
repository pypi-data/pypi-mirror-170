...
supported_action = ["discord.Client"]
...
class kalive:
    def __init__(self, action=None):
        self.action = action
        if action == None:
            print(bcolors.FAIL + "Big Chin: Action skipped" + cr)
            return
        if action not in supported_action:
            print(bcolors.FAIL + f"Big Chin: The {action} action is not supportive" + cr)
            return
    def checkalive(self, parametr=None, token=None):
        action = self.action
        if action == None:
            print(bcolors.FAIL + "Big Chin: Action skipped" + cr)
            return
        if parametr == None:
            print(bcolors.FAIL + "Big Chin: Parametr skipped" + cr)
            return
        for key in supported_action:
            if action == "discord.Client":
                try:
                    now = time.time()
                    import discord
                    parametr.run(token)
                    ...
                    async def on_ready():
                        now2 = time.time()
                        print(bcolors.OKGREEN + f"Bot launch time: {now-now2}" + cr)
                except discord.errors.HTTPException:
                    print(bcolors.WARNING + "Big Chin: An error occurred when launching the bot. Check the correct spelling of the token and parameters on the site" + cr)
                    return
                except discord.errors.PrivilegedIntentsRequired:
                    print(bcolors.WARNING + "Big Chin: An error occurred when launching the bot. Go to the Discord Developer website and configure the confidential parameters" + cr)
                    return
                except:
                    os.system("kill 1")
