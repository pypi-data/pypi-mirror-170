...
def test_upload_chunk_for_fully_upload_binary_after_use(binary_upload_stub, binarytransfertest_stub):
    """
    - Upload a binary
    - Use the binary as command parameter
    - Try to upload a chunk of that binary again using BinaryUpload.UploadChunk
      - This should lead to a Binary Upload Failed error
    """
    upload_id = upload_binary(
        binary_upload_stub,
        b"abc" * 1_000_000,
...
def test_empty_chunks_are_accepted(binary_upload_stub, binary_download_stub, binarytransfertest_stub):
    ...
    - Call BinaryUpload.CreateBinary to upload a 3 MB binary for the parameter 'BinaryValue' of
        BinaryTransferTest.EchoBinaryValue in 3 chunks
    - Call BinaryUpload.UploadChunk and send two 1MB chunks and an empty chunk
    - Expect that the command works
    """
    # announce upload of 3 MB in 3 chunks
    info = binary_upload_stub.CreateBinary(
    ...
    assert download_binary(binary_download_stub, response.ReceivedValue.binaryTransferUUID) == b"a" * 1024 * 1024 * 2
