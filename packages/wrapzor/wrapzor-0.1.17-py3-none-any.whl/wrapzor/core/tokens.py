...
def verify_zoho_api(api: ZohoApi | None):
    if not isinstance(api, ZohoApi):
        raise ArgsError(
            "api must be an instance of ZohoApi"
        )
...
def inject_tokens(api: ZohoApi | None = ZOHO_API):
    if not isinstance(api, ZohoApi):
        raise ValueError("Please initialise wrapzor like following to enjoy the Zoho Api:\nimport wrapor\nwrapozor.init(client_id=..., client_secret=..., code | refresh_token=...)")
