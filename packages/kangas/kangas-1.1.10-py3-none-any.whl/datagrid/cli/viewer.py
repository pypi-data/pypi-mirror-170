...
def query(parsed_args):
    ...
    if "ncols" in table:
        ...
        for r, reference in enumerate(references):
            ...
            link = "datagrid viewer {dgid} --query-type {query_type} --group-by {link_group_by} --column-name {link_column_name} --column-value '{link_column_value}' --where-expr \"{where_expr}\" --computed-columns {computed_columns}"
            if reference["type"] == "asset":
                ...
                link = "datagrid viewer {dgid} --query-type asset-group --group-by {link_group_by} --column-name {link_column_name} --column-value '{link_column_value}' --column-offset {link_column_offset} --column-limit {link_column_limit} --where-expr \"{where_expr}\" --computed-columns {computed_columns}"
                env["link_group_by"] = reference["groupBy"]
                env["link_column_name"] = reference["columnName"]
                env["link_column_value"] = get_column_value(reference["columnValue"])
