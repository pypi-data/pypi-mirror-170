...
def num_params(model, d=1):
    ...
    grid.add_column()
    grid.add_column()
    grid.add_row(f"[bold]Trainable params[/]: {get_human_readable_count(total_num_params)}")
    grid.add_row(f"[bold]Non-trainable params[/]: {get_human_readable_count(total_num_params-total_num_trainable_params)}")
    grid.add_row(f"[bold]Total params[/]: {get_human_readable_count(total_num_trainable_params)}")
    console.print(grid)
...
def stats(*arrays):
    ...
    for x in arrays:
        if isinstance(x, torch.Tensor):
            ...
            x_max = x.max()
        table.add_row(f"{x_min:.3}", f"{x_25:.3}", f"{x_mean:.3}", f"{x_75:.3}", f"{x_max:.3}")
