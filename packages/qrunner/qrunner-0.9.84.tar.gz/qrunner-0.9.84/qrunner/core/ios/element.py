...
from qrunner.utils.exceptions import NoSuchElementException, ElementNameEmptyException
...
class IosElement(object):
    ...
    def __init__(self,
        ...
        # 驱动初始化
        _serial_no = conf.get_item('ios', 'serial_no')
        self._driver = IosDriver.get_instance(_serial_no)
        self._d = self._driver.d

    # 多线程处理异常弹窗
    def handle_alert(self, alert_list):
        """
        根据不同定位方式进行点击
        @return:
        """
    ...
    def _find_element(self, retry=3, timeout=3, alert_list=None):
        ...
        @return:
        """
        element = self._d.xpath(self._xpath) if \
            self._xpath else self._d(**self._kwargs)[self._index]
        self.handle_alert(alert_list)
        cur_retry = retry
        while not element.wait(timeout=timeout):
            if cur_retry > 0:
                logger.warning(f'第{retry-cur_retry+1}次重试，查找元素： {self._kwargs},{self._index}')
                cur_retry -= 1
                self.handle_alert(alert_list)
            else:
                frame = inspect.currentframe().f_back
                ...
                logger.warning(f'【{caller.function}:{caller.lineno}】未找到元素 {self._kwargs}')
                return None
        elements = self._d.xpath(self._xpath).find_elements() if \
            self._xpath else self._d(**self._kwargs)
        return elements
    ...
    def get_elements(self, retry=3, timeout=3, alert_list=None):
        ...
        @return:
        """
        elements: Union[Selector] = self._find_element(retry=retry, timeout=timeout, alert_list=alert_list)
        if elements is None:
            self._driver.screenshot(f'[控件 {self.desc}] 定位失败')
            raise NoSuchElementException(f'[控件 {self.desc}] 定位失败')
        else:
            self._driver.screenshot(self.desc)
        return elements
    ...
    def get_element(self, retry=3, timeout=3, alert_list=None):
        ...
        @return:
        """
        elements = self.get_elements(retry=retry, timeout=timeout, alert_list=alert_list)
    ...
    def child(self, *args, **kwargs):
        """不能用于PageObject，会导致在应用启动前进行元素识别"""
