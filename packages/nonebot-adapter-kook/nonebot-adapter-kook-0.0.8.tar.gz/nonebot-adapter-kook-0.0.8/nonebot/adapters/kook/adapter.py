...
from .bot import Bot
from .config import Config as KookConfig
from .event import *
...
class Adapter(BaseAdapter):
    ...
    def __init__(self, driver: Driver, **kwargs: Any):
        ...
        self.kook_config: KookConfig = KookConfig(**self.config.dict())
        self.api_root = f'https://www.kookapp.cn/api/v3/'
    ...
    async def _get_gateway(self, token: str) -> str:
        ...
        params = {'compress': 1 if self.kook_config.compress else 0}
    ...
    async def start_forward(self) -> None:
        for bot in self.kook_config.kook_bots:
            try:
                bot_token = bot.token
                ...
                log(
                    ...
                    "please get the new token from https://developer.kookapp.cn/app/index </bg #f8bbd0></r>",
                    e,
                )
            except Exception as e:
                log(
        ...
        while True:
            try:
                async with self.websocket(request) as ws:
                    ...
                    try:
                        data_decompress_func = zlib.decompress if self.kook_config.compress else lambda x: x
                        while True:
