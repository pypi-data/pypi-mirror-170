...
from dominate.tags import (
    div, small, code, br, span, strong, b, font, table,
    tr, td,
)
...
from iolanta.facets.by_type import FindFacetByInstanceTypes
from iolanta.facets.for_literal import FindFacetForLiteral
...
def render(
    ...
    if not issubclass(facet_class, Facet):
        raise ValueError(f'{facet_class} is not a Facet.')
    ...
    try:
        return _render_facet(
            ...
            facet=facet,
            environments=environments,
            debug_mode=octiron.is_debug_mode(node),
        )
