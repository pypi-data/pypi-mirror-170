...
class ImageRecognition(object):
    ...
    def batch_generator(self, batch_size, 
        ...
        """
        file_list = get_filenames(os.path.join(dataset_path + self.classes))
    ...
    def validate(self, val_generator, 
        ...
        iteration_needed = math.ceil(self.file_num / self.batch_generator)

        losses = [] 
        accs = [] 
        recalls = []
        precisions = [] 
        for i in iteration_needed: 
            x_val, y_val = next(val_generator)
            feed_dict = {}
    ...
    def optimize(self, 
        ...
        for i in range(iteration):
            ...
            print (">> epoch: {}, train loss: {}, val loss: {}, val acc: {}, val recall: {}, val prec: {} {}".format(i, round(t_loss, 3), 
                                                                                    round(val_loss, 3), round(val_acc, 3), round(val_acc, 3), 
                                                                                    round(val_recall, 3), round(val_precision, 3), sign))
