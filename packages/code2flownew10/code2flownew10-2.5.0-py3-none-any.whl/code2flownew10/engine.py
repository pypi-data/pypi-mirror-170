...
def make_file_group(tree, filename, extension):
    ...
    for node_tree in node_trees:
        for new_node in language.make_nodes(node_tree, parent=file_group):
            print("engine.py make_file_group 357 filename: ", filename, " name: ",new_node.name())
            file_group.add_node(new_node, caller_file=filename)

    file_group.add_node(language.make_root_node(body_trees, parent=file_group), is_root=True, caller_file=filename)

    for subgroup_tree in subgroup_trees:
        print("engine.py make_file_group 363 filename: ", filename, " subgroup_tree: ", subgroup_tree )
        file_group.add_subgroup(language.make_class_group(subgroup_tree, parent=file_group, caller_file=filename))
    return file_group
...
def map_it(sources, extension, no_trimming, exclude_namespaces, exclude_functions,
    ...
    for source, file_ast_tree in file_ast_trees:
        print("engine.py map_it 478 source: ", source)
