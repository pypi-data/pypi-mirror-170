...
def add_probe(probe_type: str, probe_variables: list, analysis_type: str):
    ''' Format probe row '''
    ...
    if probe_type == 'standard' or probe_type == 'CSDF':
        formatted_text = f'.PRINT {type} FORMAT=PROBE'
    else:
...
def translate_library_PSPICE_2_XYCE(in_file_library: str, out_file_library: str, verbose: bool = False):
    ...
    with open(in_file_library) as f:
        ...
        for i in range(len(idx_start)):
            ...
            old_model = ''.join(lines[idx_start[i]:idx_end[i]+1])
