...
# 전처리 함수 선언
    ...
    def anomaly_replace(self): 
        ...
        def impute_data(x):
            ...
            train_data = imputer_mice.fit_transform(x)
            train_data = pd.DataFrame(train_data)
            train_data.columns = columns
            train_data = train_data.reset_index(drop=True)
            return train_data
            # scaled_kon_df_total_28_0 = scaled_kon_df_total_28[scaled_kon_df_total_28['Group']==0]
            # scaled_kon_df_total_28_1 = scaled_kon_df_total_28[scaled_kon_df_total_28['Group']!=0]    
        ...
        #    return imputed_data
        for i in anomaly_data.columns[:-1]:
            anomaly_data[i] = pd.DataFrame(winsorize_with_pandas(anomaly_data[i], limits=(0.1 ,0.1))) 

        imputed_data = impute_data(self.data)
        
        self.db = imputed_data
