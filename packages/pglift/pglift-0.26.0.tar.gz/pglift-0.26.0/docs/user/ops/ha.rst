...
.. warning::
   When the ``patroni`` setting is activated, all instances will be managed by
   Patroni and plain PostgreSQL instances are not supported.
...
The "patroni cluster" option is required. Other options have default values
corresponding to Patroni's defaults.
...
We can see, in particular, that many fields in ``configuration`` have been set
...
.. code-block:: console
    ...
    > Instance needs to be restarted; restart now? [y/n] (n): y
    INFO     restarting instance 14/pg1
    INFO     restarting Patroni 14-pg1
    $ pglift instance exec pg1 -- postgres -C logging_collector
    off

.. warning::
   :ref:`pglift pgconf <pgconf>` commands other that ``edit`` should be
