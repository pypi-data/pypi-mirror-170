...
class PostgreSQLSettings(BaseSettings):
    ...
    dump_commands: List[List[str]] = Field(
        default=[
            [
                "{bindir}/pg_dump",
                "-Fc",
    ...
    restore_commands: List[List[str]] = Field(
        default=[
            [
                "{bindir}/pg_restore",
                "-d",
...
class Settings(BaseSettings):
    ...
    def __set_service_manager_scheduler(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        >>> Settings(systemd={}).service_manager
        'systemd'
        >>> Settings(systemd={}, service_manager="systemd").service_manager
        'systemd'
        >>> Settings(systemd={}, service_manager=None).service_manager is None
        True
        """
