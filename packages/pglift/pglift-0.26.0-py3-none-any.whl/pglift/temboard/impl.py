...
def system_lookup(
    ctx: "BaseContext", name: str, settings: "TemboardSettings"
) -> Optional[Service]:
    try:
...
def revert_setup(
    ...
    _userspath(instance.qualname, settings).unlink(missing_ok=True)
...
def start(
    ctx: "BaseContext", name: str, service: Service, *, foreground: bool = False
) -> None:
...
def stop(ctx: "BaseContext", name: str, service: Service) -> None:
