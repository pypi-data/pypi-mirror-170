...
def _get_liveAnimal_lookup_values(cycle: dict, product: dict, country_id: str, year: int = None):
    ...
    if groupingFAO:
        ...
        if price and average_carcass_weight:
            # price is per 1000kg, divide by 1000 to go back to $/kg
            # average_carcass_weight is per ton, divide by 10 to go back to kg
...
def _run_by_liveAnimal(cycle: dict, product: dict, country_id: str, year: int = None):
    ...
    should_run = all([is_liveAnimal, price_per_carcass_weight, carcass_weight])
    value = price * carcass_weight if should_run else None
...
def run(cycle: dict):
    ...
    return non_empty_list([
        (
            (
                _run_by_liveAnimal(cycle, p, country_id, year) or _run_by_country(cycle, p, country_id, year)
