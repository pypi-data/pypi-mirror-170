...
from hestia_earth.models.utils.inorganicFertilizer import get_term_lookup
from . import MODEL
...
LOOKUPS = {
    "inorganicFertilizer": ["mustIncludeId", "nitrogenContent", "phosphateContentAsP2O5", "potassiumContentAsK2O"]
}
MODEL_KEY = 'inorganicFertilizer'
...
def _should_run_input(cycle: dict, input: dict):
    ...
    has_missing_term = find_term_match(cycle.get('inputs', []), mustIncludeId, None) is None if mustIncludeId else False
...
def run(cycle: dict):
    ...
    inputs = [i for i in cycle.get('inputs', []) if _should_run_input(cycle, i)]
