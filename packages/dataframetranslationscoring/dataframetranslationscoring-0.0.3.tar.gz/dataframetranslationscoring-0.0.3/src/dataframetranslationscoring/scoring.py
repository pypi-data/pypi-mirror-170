...
def translation_quantitative_scoring(original_dataframe, translated_dataframe):
    ...
    print(
        ...
        "%" ,'(', sum(list_score_completeness), '/', len(list_score_completeness), ')',
    )
    print(
    ...
    results_dict = {'Percentage of characters translated vs available': sum(list_score_words) / len(list_score_words) * 100, 
            'Percentage of words translated vs available': sum(list_score_characters) / len(list_score_characters) * 100,
            'Percentage of non-null translations': sum(list_score_completeness) / len(list_score_completeness) * 100,
            'Total number of characters to be translated': int(total_characters_to_translate)}
    return results_dict
