...
class S3SinkConnector:
    ...
    def move_files(self, src_buckets: list[str], src_filenames: list[str], dst_buckets: list[str],
                   dst_filenames: list[str]) -> int:

        max_workers = max(30, len(src_filenames))
        moved_files = 0

        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [executor.submit(self.move, src_buckets[i], src_filenames[i], dst_buckets[i], dst_filenames[i])
