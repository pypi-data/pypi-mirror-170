    ...
    def parse_challenge_message(self, msg2):
        ...
        try:
            flags = NegotiateFlags(
                int(ntlm_auth.messages.ChallengeMessage(challenge_msg).negotiate_flags)
            )
            logger.info("challenge flags: %s", flags)
        except Exception:
            logger.warning("unable to check challenge flags")
