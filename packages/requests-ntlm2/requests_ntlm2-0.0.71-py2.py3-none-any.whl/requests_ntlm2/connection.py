        ...
        if code == PROXY_AUTHENTICATION_REQUIRED:
            ...
            while True:
                if last_line_is_blank:
                    logger.info('* reading a new line; last line is blank')
                    line = self._read_response_line_if_ready(response)
                else:
                    logger.info('* reading a new line; last line NOT blank')
                    line = response.fp.readline()
                ...
                if last_line_is_blank and this_line_is_blank:
                    ...
                    logger.info('* breaking infinite loop, this line and last one are both blank')
                    break

                if this_line_is_blank:
                    last_line_is_blank = True
                    logger.info('* skipping back to the top because this line is blank and last line is blank')  # noqa
                    continue
                else:
                    last_line_is_blank = False
