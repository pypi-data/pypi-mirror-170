...
class Client:
    ...
    def get_deps(self, live: bool = False) -> str:
        ...
        for _, row in self.get_tables().sort_index().iterrows():
            ...
            entries.append(f"  {row.name} {version}")
        if not entries:
            return ""
        joined = "\n".join(entries)
        return f"require table (\n{joined}\n)"
...
class TableVersion:
    ...
    def wait(self) -> str:
        ...
        expected_status = "Online"  # in the future, Offline may be a final state
