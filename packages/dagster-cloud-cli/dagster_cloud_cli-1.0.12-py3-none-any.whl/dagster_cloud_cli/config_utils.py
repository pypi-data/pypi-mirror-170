...
DEFAULT_LOCATION_LOAD_TIMEOUT = 300
...
def get_location_load_timeout() -> int:
    ...
    default_timeout = config_timeout if config_timeout != None else DEFAULT_LOCATION_LOAD_TIMEOUT
...
def available_deployment_names(ctx, incomplete: str = "") -> List[str]:
    ...
    try:
        with gql.graphql_client_from_url(gql.url_from_config(organization), user_token) as client:
            deployments = gql.fetch_deployments(client)
...
LOCATION_LOAD_TIMEOUT_OPTION = Option(
    ...
    "--agent-timeout",
    help="After making changes to the workspace. how long in seconds to wait for the location to load before timing out with an error",
)
...
AGENT_HEARTBEAT_TIMEOUT_OPTION = Option(
    ...
    "--agent-timeout",
    help="After making changes to the workspace. how long in seconds to wait for the agent to heartbeat before timing out with an error",
)
