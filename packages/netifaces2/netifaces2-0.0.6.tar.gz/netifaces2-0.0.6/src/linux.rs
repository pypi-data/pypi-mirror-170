use crate::types::{
    AddrPairs, IfAddrs, ADDR_ADDR, AF_INET, AF_INET6, AF_NETLINK, AF_PACKET, BROADCAST_ADDR,
    MASK_ADDR, PEER_ADDR, AF_VSOCK, AF_ALG,
};
        ...
        for (name, ss) in vec![
            ...
            if let Some(address) = ss {
                ...
                if let Some(inet_addr) = address.as_sockaddr_in() {
                    add_to_types_mat(AF_INET, inet_addr, name, &mut types_mat, &mut any);
                }
                ...
                if let Some(inet_addr) = address.as_sockaddr_in6() {
                    add_to_types_mat(AF_INET6, inet_addr, name, &mut types_mat, &mut any);
                }
            }
        }
    }
