...
class tree:
    ...
    def export_1d_simulation(self,steady=True,outdir=None,folder="1d_tmp",number_cariac_cycles=1,num_points=100):
        ...
        for ind,spline in enumerate(interp_xyzr):
            ...
            poly_line.point_data.set_vector(normal,'CenterlineSectionNormal')
            polys.append(poly_line)

        for ind in range(len(polys)):
            cent_ids = np.zeros((polys[ind],len(polys)),dtype=int)
            polys[ind].point_data.set_array(cent_ids,'CenterlineId')
            polys[ind].point_data['CenterlineId'][:,ind] = 1

        bifucation_point_ids = [] # polys ind index, polys jnd index, polys jnd point index
        for ind in range(1,len(polys)):
            for jnd in range(len(polys)):
                if jnd == ind:
                    continue
                closest_point_id = polys[jnd].find_closest_point(polys[ind].points[0])
                closest_point    = polys[jnd].points[closest_point_id]
                dist             = np.linalg.norm(polys[ind].points[0] - closest_point)
                if dist < polys[ind].point_data['Radius'][0]:
                    polys[jnd].point_data['CenterlineId'][0:closest_point_id+1,ind] = 1
                    bifurcation_point_ids.append([ind,jnd,closest_point_id])
        ...
        for ind in range(len(polys)):
            ...
            branch_tmp_ids = np.zeros(len(polys.points.shape[0]),dtype=int)
        ...
        for ind in range(len(polys)):
            ...
            tmp_bif = np.array(tmp_bif)
            tmp_bif_pt = np.array(tmp_bif_pt)
            bifurcation_id_tmp = np.ones(len(polys.points.shape[0]),dtype=int)*-1
            bifurcation_id_tmp[tmp_bif] = 2
            bifurcation_id_tmp[tmp_bif_pt] = 1
            polys[ind].point_data['BifurcationIdTmp'] = bifurcation_id_tmp

        # Assign branch ids
        # np.argwhere(polys[ind].point_data['BifurcationIdTmp']==)
        return polys
