...
"""
    ...
    def title(self, value = None):
        ...
        if (not value) and 'title' in self.json:
            del self.json['title']
        else:
    ...
    def description(self, value = None):
        ...
        if (not value) and 'description' in self.json:
    ...
    def tags(self, value = None):
        ...
        if (not value) and 'tags' in self.json:
            del self.json['tags']
        else:
    ...
    def authors(self, value=None):
        ...
        if isinstance(value, Person):
            ...
            if (not value) and 'authors' in self.json:
    ...
    def filename(self, value = None):
        ...
        if (not value) and 'filename' in self.json:
    ...
    def validate(self):
        ...
        if self.validator is None:
            with open(Flow.schemafn,'r') as fp:
                Flow.validator = Draft201909Validator(JSON.loads(fp.read()))

        return Flow.validator.is_valid(self.json)
...
def NewFromMenu(menulist, title, filename="", description="", par=None):
    """
    Convenience function to create a flow from menus
    ...
    menulist: List of str
        ...
        Each menu can be provided as a relative or absolute path. In case of
        relative path, each path in setup's menudirs is inspected.
