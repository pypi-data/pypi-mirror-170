...
def add_enums(ctx):
    ...
    session = ctx.session
    # This is identical to ENUMS, with one difference: the final value is an
    # ORM object rather than a simple ID. We use this structure to populate
    # GenderGroup below.
    enum_objects = {}
    type_to_class = {
        'case': Case,
    ...
    for row in util.read_csv(ctx.config['ENUMS']):
        ...
        enum_objects.setdefault(enum_abbr, {})
        ...
        enum_objects[enum_abbr][abbreviation] = e
    session.commit()
    ...
    for row in util.read_csv(ctx.config['ENUMS']):
        ...
        if set(abbreviation).issubset('mfn'):
            e.members = [enum_objects['gender'][x] for x in abbreviation]
