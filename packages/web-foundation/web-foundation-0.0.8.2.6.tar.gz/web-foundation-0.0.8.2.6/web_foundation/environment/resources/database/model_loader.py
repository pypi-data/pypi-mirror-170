...
from web_foundation.environment.resources.database.models import ConcurrentEntity
...
class ModelLoader(Generic[GenericDbType]):
    ...
    @classmethod
    async def read_all(cls, model: GenericDbType, protection: AccessProtectIdentity, limit: int = None, offset: int = None,
    ...
    async def update(cls, model: GenericDbType, entity_id: EntityId, protection: AccessProtectIdentity, dto: PdModel,
                     **kwargs) -> GenericDbType:
        entity = await cls.get_entity(model, entity_id)
        m2m_fields = {}
