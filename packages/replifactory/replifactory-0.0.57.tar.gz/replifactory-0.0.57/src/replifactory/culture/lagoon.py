...
"""
    ...
    def feed_phage(self, feed_volume=None):
        ...
        try:
            ...
            self.device.pump2.pump(v1, rot_per_sec=1)
            ...
            # Fill pump2 tube with air
            self.device.stirrers.set_speed(fv, 0)
            self.device.valves.open(self.feed_vial_number)
            self.device.pump4.pump(7, rot_per_sec=2)
            while self.device.pump4.is_pumping():
                time.sleep(0.5)
            self.device.pump4.stop()
            self.device.valves.close(self.feed_vial_number)

            self.device.valves.open(self.vial_number)
            self.device.pump2.pump(5, rot_per_sec=1)
            while self.device.pump2.is_pumping():
                time.sleep(0.5)
            self.device.pump2.stop()
            self.device.valves.close(self.vial_number)
    ...
    def lower_od_phage(self, factor=None, end_volume=None):
        ...
        self._last_dilution_start_time = time.time()
        min_dilution_factor_single_dil = self.dead_volume/(self.max_dilution_volume + self.dead_volume)
        # log_dilution_factor = np.log(self.dilution_factor_before_feed) - np.log(self.dead_volume/end_volume)
        # min_n_dilutions = int(np.ceil(log_dilution_factor / np.log(min_dilution_factor_single_dil)))
        min_n_dilutions = np.ceil(np.log(self.dilution_factor_before_feed) / np.log(min_dilution_factor_single_dil))
        min_n_dilutions = int(min_n_dilutions)

        # print(min_n_dilutions,"min n dilutions")
        # print(min_dilution_factor_single_dil, "")
        if min_n_dilutions < 1:
            pumped_volume = 0
        else:
            dilution_factor = factor**(1/min_n_dilutions)
            pumped_volume = self.dead_volume/dilution_factor-self.dead_volume
        # print("pumped_volume",pumped_volume)
        ...
        try:
            ...
            while self.device.pump4.is_pumping():
                time.sleep(0.5)
                if self.device.pump4.get_pumped_volume() > excess_volume - 10 and not stirrer_stopped:
        ...
        if self.is_active():
            ...
            if self._last_dilution_start_time is None:
                ...
                if time_passed >= delay_needed:
                    self.lower_od_phage(end_volume=self.working_volume-self.feed_volume)
                    self.feed_phage()
                else:
                    if verbose:
