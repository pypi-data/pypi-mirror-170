...
class ConsumptionGenerator:
  ...
  def get_community_flexibility(self, community):
    ...
    print("Getting the flexibility power of each house");
  ...
  def create_consumption_profiles(self, community, days, config_files_path):
    ...
    for x in range(num_houses):
      ...
      if not os.path.exists(self.path_steps_seconds + str(x)):
        os.mkdir(self.path_steps_seconds + str(x))
      ...
      subprocess.call(["python3", antgen_path + "/main.py", "-n", baseload, "-d", days, "-o", self.path_steps_seconds + str(x), "-w", "-m", antgen_path + "/mapping.conf", "-p", config_files_path + "/house" + str(x) + ".conf"])
  ...
  def show_community_baseload_graph(self, community):
    ...
    baseload = pd.read_csv('output/minute/community_baseload.csv', sep=';')["Power"]
    not_baseload = pd.read_csv('output/minute/community_not_baseload.csv', sep=';')["Power"]
    baseline = pd.read_csv('output/minute/community_not_baseload.csv', sep=';')["Power"]
    ...
    baseline.iloc[0:1440] = baseline_val
    width = 0.35  # the width of the bars: can also be len(x) sequence
  ...
  def calculate_community_baseload_consumption(self, community):
    ...
    df_date = pd.read_csv('output/minute/community.csv', sep=';')
  ...
  def calculate_community_not_baseload_consumption(self):
    ...
    community = pd.read_csv('output/minute/community.csv', sep=';').set_index('Date')
    baseload = pd.read_csv('output/minute/community_baseload.csv', sep=';').set_index('Date')
  ...
  def convert_data_to_steps_of_one_minute(self, community, just_a_day, database_files = False):
    """
    ...
    for z in total:
      ...
      # All the files of the output (to convert from steps of a second to steps of a minute)
      files_to_convert = [f for f in listdir(self.path_steps_seconds + str(z)) if isfile(join(self.path_steps_seconds + str(z), f))]
      ...
      for file in files_to_convert:
        ...
        if (file != "events.csv"):
          ...
          df = pd.read_csv(self.path_steps_seconds + str(z) + '/' + file, header=None,
  ...
  def calculate_energy(self, contracted_power):
    ...
    df = pd.read_csv('output/minute/community.csv', sep=';')
  ...
  def execute(self, days, config_files_path, just_a_day = True, database_files = False):
    """
    ...
    self.show_community_baseload_graph(community)
    # show_house_graph('0')
    # show_house_graph('4')
