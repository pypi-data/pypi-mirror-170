...
class CommunityManagerStrategy(CommunityManager):
  ...
  def remove_flexible_consumption(self):
    ...
    df_netload.columns = ['Date', 'Demand', 'Production', 'Netload']
    ...
    df_netload.to_csv(outname, columns=['Date', 'Demand', 'Production', 'Netload'], sep=";", index=False)
  ...
  def update_consumption_profiles_based_on_optimization(self, placed_timeslots, all_timeslots_objects, initial_path, final_path, short_initial_path, short_final_path, remove_flex_cons, n_bins_per_hour, fact):
    ...
    netload_after.columns = ['Date', 'Demand', 'Production', 'Netload']
    ...
    # After all timeslots updated - update the community profile
    outname = os.path.join(output_directory, 'netload.csv')
    netload_after.to_csv(outname, columns=['Date', 'Demand', 'Production', 'Netload'], sep=";", index=False)
  ...
  def execute(self):
    ...
    netload.columns = ['Date', 'Demand', 'Production', 'Netload']
    ...
    netload_second_optim.columns = ['Date', 'Demand', 'Production', 'Netload']
