...
class Search():
    ...
    def run_search_latent(self, HMS=10, min_classes=1, max_classes=10,
            ...
            if self.multi_objective:
                ...
                for sol_pareto in Pareto:
                    if sol_pareto['class_num'] == q:
                        stop_run = False
                if stop_run:
                    logger.info(f"No {q} class solutions in Pareto front")
        ...
        if self.multi_objective:
            ...
            current_time = datetime.datetime.now().strftime("%d%m%Y-%H%M%S")
            plot_filename = self.code_name + current_time + "_MOOF.png"
            plt.savefig(plot_filename,
                        bbox_extra_artists=(lgd,), bbox_inches='tight')
        else:  # Plot for SOOF
            fig, ax1 = plt.subplots()
            ...
            plot_filename = self.code_name + current_time + "_SOOF.png"
            plt.savefig(plot_filename,
                        bbox_extra_artists=(lgd,), bbox_inches='tight')
