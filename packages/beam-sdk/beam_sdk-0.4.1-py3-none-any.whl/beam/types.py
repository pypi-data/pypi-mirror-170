...
class Types:
    ...
    @staticmethod
    def get_serializer(serializer_type: str):
        return Types.type_registry[serializer_type]()
    ...
    def serialize(objects, schema):
        ...
        for key in schema.keys():
            if key not in objects.keys() and getattr(schema[key], "required", True):
                ...
                _type = Types.get_serializer(schema[key])
                _input = objects[key]
    ...
    def load_schema(description):
        ...
        for key, val in description.items():
            ...
            serializer = Types.get_serializer(val["type"])

            schema[key] = val
            kwargs = copy.copy(val)
            del kwargs["type"]
            serializer.define(**kwargs)
