...
def rtask_avg_proc(channel, threshold, trend_task, window_size, task=None):
    if (yield channel.subscribe(task)) != 0:
        return(-1)
    ...
    while True:
        ...
        if num is None:
            break
        cumsum += (num - data[i])
        # TODO: avg is valid only if at least window_size entries are filled
...
def client_proc(task=None):
    ...
    channel = pycos.Channel('trend')

    trend_task = pycos.Task(trend_proc)

    # run average and save tasks at two different servers
    rtask_avg = yield client.rtask(rtask_avg_proc, channel, 0.4, trend_task, 10)
    assert isinstance(rtask_avg, pycos.Task)
    rtask_save = yield client.rtask(rtask_save_proc, channel)
    ...
    assert (yield channel.deliver('start', n=2)) == 2
