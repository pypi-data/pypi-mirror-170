...
class TestStringTools(unittest.TestCase):
    ...
    def fracture_string(self):
        self.assertEqual(string_tools.fracture_string(u"Emma"),
                         (u'Em', u'ma'))
        self.assertEqual(string_tools.fracture_string(u"Frank"),
                         (u'Fr', u'ank'))

    def reduce_from_middle(self):
        for title, expected, limit in [(u'The Long Good Friday', u'The L…iday', 10),
                                       (u'The Long Good Friday', u'The …iday', 9),
                                       (u'Little Women', u'Little Women', 12),
                                       (u'Little Women', u'Little Women', 20),
                                       (u'Little Women', u'Littl…Women', 11),
                                       ]:
            actual = string_tools.reduce_from_middle(title=title, limit=limit)
            test = u"{actual} != {expected}".format(actual=actual, expected=expected)
            self.assertEqual(string_tools.reduce_from_middle(title=title, limit=limit),
                             expected,
                             test.encode('utf-8'))
