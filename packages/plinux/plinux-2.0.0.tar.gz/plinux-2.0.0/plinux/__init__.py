...
class Plinux:
    ...
    def run_cmd(self, cmd: str, sudo: bool = False, timeout: int = 30) -> SSHResponse:
        ...
        try:
            ...
            if parsed.exited:  # Exit code != 0
                msg_err = f'{self.host:<14} | Something went wrong!'
                ...
                self.logger.warning(msg_to_log)
                return parsed  # Exit code != 0 and no stderr
            return parsed
        finally:
            client.close()
    ...
    def cat_file(self, path: str, sudo: bool = False) -> str | dict:
        """Get file content"""

        self.logger.warning(f'Method will be deprecated in the next release. Use .cat()')
    ...
    def get_json(self, path: str, sudo: bool = False, pprint: bool = False, remove_bom: bool = False) -> dict:
        ...
        Args:
            path: Full file path
            sudo: Use sudo
            pprint: pretty print into console
    ...
    def get_dir_size(self, path: str, sudo: bool = False) -> int:
        ...
        Command:
            - du -sh -b /mnt/ssd-cache | awk {'print $1'}
        ...
        cmd = f"du -sh -b {path} | awk {{'print $1'}}"
        result = self.run_cmd(cmd, sudo=sudo).stdout  # 876592118	/mnt/ssd-cache/
        print(result)
    ...
    def get_ntp_servers(self) -> list:
        ...
        response = self.run_cmd(cmd).stdout
        try:
            result_lines = response.splitlines()
        except AttributeError:
            return []

        result = [ntp.split('=')[-1] for ntp in result_lines]
