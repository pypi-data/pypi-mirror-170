...
def _set_met_parameters(params_set, met_set):
    ...
    settling_speed = met_set.calculate_settling_speed(params_set, np.atleast_1d(0.0))[0]
...
def _get_gfs_archive(latitude, longitude, cycle_datetime, forecast_hr):
    gfs_data = met.gfs_grib(cycle_datetime, forecast_hr, lat=latitude, lon=longitude)
    met_set = met.MetData()
    met_set = met.gfsgrib_to_Met(met_set, gfs_data)
    return met_set
