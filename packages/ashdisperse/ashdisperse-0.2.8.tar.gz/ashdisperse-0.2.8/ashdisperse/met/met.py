...
from ..utilities.utilities import SA_Density, SA_Pressure, SA_Temperature, interp_ex
...
class gfs_grib:
    def __init__(
        self, cycle_datetime, forecast_hr, lat, lon, Ra=287.058, g=9.80665, verbose=True
    ):
        self.lat = lat
        self.lon = lon
        self.Ra = Ra
        ...
        self.filename = gfs_arc.get_remoteFileName
        ...
        idx = gfs_arc.read_idx()

        levels = idx.loc[(idx["level"].str.match("(\d+(?:\.\d+)?) mb"))].level.unique()
        ...
        for j, l in enumerate(levels):
            ...
            data = gfs_arc.xarray(f":HGT|TMP|(U|V)GRD:{l}", verbose=verbose)
            data_interp = data.interp(latitude=lat, longitude=lon, method="cubic")
            data_Z[j] = np.float64(data_interp["gh"].values) / self.g
            data_T[j] = np.float64(data_interp["t"].values)
            data_U[j] = np.float64(data_interp["u"].values)
            data_V[j] = np.float64(data_interp["v"].values)
            # T = gfs_arc.xarray(f":TMP:{l}", verbose=verbose)
            # data_T[j] = np.float64(
            #     T["t"].interp(latitude=lat, longitude=lon, method="cubic").values
            # )
            # UV = gfs_arc.xarray(f":(U|V)GRD:{l}", verbose=verbose)
            # uv_interp = UV.interp(latitude=lat, longitude=lon, method="cubic")
            # data_U[j] = np.float64(uv_interp["u"].values)
            # data_V[j] = np.float64(uv_interp["v"].values)

        self.altitude = np.flipud(data_Z)
        self.pressure = np.flipud(data_P)
        self.temperature = np.flipud(data_T)
        self.wind_U = np.flipud(data_U)
        self.wind_V = np.flipud(data_V)

        return


def gfsgrib_to_Met(met, gfs_data, Ra=287.058):
    z = gfs_data.altitude
    temp = gfs_data.temperature
    u = gfs_data.wind_U
    v = gfs_data.wind_V
    pres = gfs_data.pressure
