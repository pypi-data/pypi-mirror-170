...
class gfs_grib:
    def __init__(
        ...
        for j, l in enumerate(levels):
            ...
            ZT = gfs_arc.xarray(f":HGT|TMP:{l}", verbose=verbose)
            zt_interp = ZT.interp(latitude=lat, longitude=lon, method="cubic")
            data_Z[j] = np.float64(zt_interp["gh"].values) / self.g
            data_T[j] = np.float64(zt_interp["t"].values)
