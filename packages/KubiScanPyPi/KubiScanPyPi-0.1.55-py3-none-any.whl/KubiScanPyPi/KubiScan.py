...
def get_color_by_priority(priority):
    color = misc.WHITE
    if priority == engine.Priority.CRITICAL:
        color = misc.RED
    elif priority == engine.Priority.HIGH:
        color = misc.LIGHTYELLOW
...
def generic_print(header, objects, show_rules=False):
    ...
    if show_rules:
        ...
        for o in objects:
            if o.time is None:
                t.add_row([get_color_by_priority(o.priority) + o.priority.name + misc.WHITE, o.kind, o.namespace, o.name, 'No creation time', get_pretty_rules(o.rules)])
            else:
                t.add_row([get_color_by_priority(o.priority) + o.priority.name + misc.WHITE, o.kind, o.namespace, o.name, o.time.ctime() + " (" + str(get_delta_days_from_now(o.time)) + " days)", get_pretty_rules(o.rules)])
    else:
        t = PrettyTable(['Priority', 'Kind', 'Namespace', 'Name', 'Creation Time'])
        for o in objects:
            if o.time is None:
                t.add_row([get_color_by_priority(o.priority) + o.priority.name + misc.WHITE, o.kind, o.namespace, o.name, 'No creation time'])
            else:
                t.add_row([get_color_by_priority(o.priority) + o.priority.name + misc.WHITE, o.kind, o.namespace, o.name, o.time.ctime() + " (" + str(get_delta_days_from_now(o.time)) + " days)"])
...
def print_all_risky_containers(priority=None, namespace=None, read_token_from_container=False):
    ...
    for pod in pods:
        ...
        for container in pod.containers:
            ...
            t.add_row([get_color_by_priority(container.priority)+container.priority.name+misc.WHITE, pod.name, pod.namespace, container.name, container.service_account_namespace, all_service_account])
    print_table_aligned_left(t)
...
def print_all_risky_subjects(priority=None, namespace=None):
    ...
    for subject in subjects:
        if subject.user_info.namespace == namespace or namespace is None:
           t.add_row([get_color_by_priority(subject.priority)+subject.priority.name+misc.WHITE, subject.user_info.kind, subject.user_info.namespace, subject.user_info.name])
...
Requirements:
    ...
    api.api_init(kube_config_file=args.kube_config, host=args.host, token_filename=args.token_filename, cert_filename=args.cert_filename, context=args.context)
    ...
    if args.all:
        ...
        if args.kind:
            if args.kind == misc.constants.SERVICEACCOUNT_KIND:
                if args.namespace:
        ...
        print_subjects_by_kind(misc.constants.USER_KIND)
    elif args.subject_groups:
        print_subjects_by_kind(misc.constants.GROUP_KIND)
    elif args.subject_serviceaccounts:
        print_subjects_by_kind(misc.constants.SERVICEACCOUNT_KIND)
