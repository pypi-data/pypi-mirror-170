...
def is_risky_resource_name_exist(source_rolename, source_resourcenames):
    ...
    for resource_name in source_resourcenames:
        ...
        if resource_name != source_rolename:
            ...
            role = get_role_by_name_and_kind(resource_name, misc.CLUSTER_ROLE_KIND)
            if role is not None:
...
def is_risky_role(role):
    is_risky = False
    priority = engine.Priority.LOW
    for risky_role in engine.STATIC_RISKY_ROLES:
...
def find_risky_roles(roles, kind):
    ...
    for role in roles:
        ...
        if is_risky:
            risky_roles.append(
                engine.Role(role.metadata.name, priority, rules=role.rules, namespace=role.metadata.namespace, kind=kind,
...
def get_roles_by_kind(kind):
    all_roles = []
    if kind == misc.ROLE_KIND:
        all_roles = api.api_client.RbacAuthorizationV1Api.list_role_for_all_namespaces()
    else:
        # all_roles = api_client.RbacAuthorizationV1Api.list_cluster_role()
        all_roles = api.api_client.api_temp.list_cluster_role()
    return all_roles
...
def find_risky_rolebindings_or_clusterrolebindings(risky_roles, rolebindings, kind):
    ...
    for rolebinding in rolebindings:
        ...
        if is_risky:
            risky_rolebindings.append(engine.RoleBinding(rolebinding.metadata.name,
                                                  priority,
...
def get_rolebinding_by_kind_all_namespaces(kind):
    all_roles = []
    if kind == misc.ROLE_BINDING_KIND:
        all_roles = api.api_client.RbacAuthorizationV1Api.list_role_binding_for_all_namespaces()
    # else:
...
def get_risky_rolebindings(all_risky_roles=None):
    if all_risky_roles is None:
        all_risky_roles = get_risky_roles_and_clusterroles()
    all_rolebindings = get_rolebinding_by_kind_all_namespaces(misc.ROLE_BINDING_KIND)
...
def get_risky_clusterrolebindings(all_risky_roles=None):
    ...
    all_clusterrolebindings = api.api_client.api_temp.list_cluster_role_binding()
    ...
    for risky_rolebinding in all_risky_rolebindings:
        ...
        for user in risky_rolebinding.subjects or []:
            ...
            if ''.join((user.kind, user.name, str(user.namespace))) not in passed_users:
                ...
                all_risky_users.append(engine.Subject(user, risky_rolebinding.priority))
...
def pod_exec_read_token(pod, container_name, path):
    ...
    resp = ''
    try:
        resp = stream(api.api_client.CoreV1Api.connect_post_namespaced_pod_exec, pod.metadata.name, pod.metadata.namespace,
                    ...
                    if jwt_body:
                        ...
                        if risky_user:
                            risky_containers.append(
                                engine.Container(container.name, risky_user.user_info.name, risky_user.user_info.namespace,
                                          risky_user.priority))
...
def get_jwt_and_decode(pod, risky_users, volume):
    ...
    try:
        secret = api.api_client.CoreV1Api.read_namespaced_secret(name=volume.secret.secret_name,
...
def get_risky_pods(namespace=None, deep_analysis=False):
    ...
    for pod in pods.items:
        ...
        if len(risky_containers) > 0:
            risky_pods.append(engine.Pod(pod.metadata.name, pod.metadata.namespace, risky_containers))
...
def get_rolebindings_all_namespaces_and_clusterrolebindings():
    namespaced_rolebindings = api.api_client.RbacAuthorizationV1Api.list_role_binding_for_all_namespaces()

    # TODO: check when this bug will be fixed
    # cluster_rolebindings = api_client.RbacAuthorizationV1Api.list_cluster_role_binding()
    cluster_rolebindings = api.api_client.api_temp.list_cluster_role_binding()
    ...
    for rolebinding in rolebindings_all_namespaces.items:
        ...
        for subject in rolebinding.subjects or []:
            if subject.name.lower() == subject_name.lower() and subject.kind.lower() == kind.lower():
                if kind == misc.SERVICEACCOUNT_KIND:
...
def get_rolebindings_associated_to_role(role_name, namespace):
    rolebindings_all_namespaces = api.api_client.RbacAuthorizationV1Api.list_role_binding_for_all_namespaces()
    associated_rolebindings = []

    for rolebinding in rolebindings_all_namespaces.items:
        if rolebinding.role_ref.name.lower() == role_name.lower() and rolebinding.role_ref.kind == misc.ROLE_KIND and rolebinding.metadata.namespace.lower() == namespace.lower():
            associated_rolebindings.append(rolebinding)
...
def get_rolebindings_and_clusterrolebindings_associated_to_clusterrole(role_name):
    ...
    for rolebinding in rolebindings_all_namespaces.items:
        if rolebinding.role_ref.name.lower() == role_name.lower() and rolebinding.role_ref.kind == misc.CLUSTER_ROLE_KIND:
            associated_rolebindings.append(rolebinding)
    ...
    for clusterrolebinding in cluster_rolebindings:
        if clusterrolebinding.role_ref.name.lower() == role_name.lower() and clusterrolebinding.role_ref.kind == misc.CLUSTER_ROLE_KIND:
...
def dump_containers_tokens_by_pod(pod_name, namespace, read_token_from_container=False):
    ...
    try:
        pod = api.api_client.CoreV1Api.read_namespaced_pod(name=pod_name, namespace=namespace)
    except ApiException:
                ...
                if container.ready:
                    ...
                    if jwt_body:
                        containers_with_tokens.append(
                            engine.Container(container.name, token=jwt_body, raw_jwt_token=raw_jwt_token))
        ...
        for volume_mount in container.volume_mounts or []:
            for volume in pod.spec.volumes or []:
                if volume.name == volume_mount.name and volume.secret:
                    try:
                        secret = api.api_client.CoreV1Api.read_namespaced_secret(volume.secret.secret_name,
                                                                             pod.metadata.namespace)
                        if secret and secret.data and secret.data['token']:
                            decoded_data = decode_base64_jwt_token(secret.data['token'])
                            token_body = json.loads(decoded_data)
                            containers_with_tokens.append(engine.Container(container.name, token=token_body,
                                                                    raw_jwt_token=None))
                    except ApiException:
...
def dump_all_pods_tokens_or_by_namespace(namespace=None, read_token_from_container=False):
    ...
    for pod in pods.items:
        ...
        if containers is not None:
            pods_with_tokens.append(engine.Pod(pod.metadata.name, pod.metadata.namespace, containers))
...
def dump_pod_tokens(name, namespace, read_token_from_container=False):
    ...
    pod_with_tokens.append(engine.Pod(name, namespace, containers))
...
def get_subjects_by_kind(kind):
    subjects_found = []
    rolebindings = api.api_client.RbacAuthorizationV1Api.list_role_binding_for_all_namespaces()
    clusterrolebindings = api.api_client.api_temp.list_cluster_role_binding()
...
def get_rolebinding_role(rolebinding_name, namespace):
    ...
    role = None
    try:
        rolebinding = api.api_client.RbacAuthorizationV1Api.read_namespaced_role_binding(rolebinding_name, namespace)
        if rolebinding.role_ref.kind == misc.ROLE_KIND:
            role = api.api_client.RbacAuthorizationV1Api.read_namespaced_role(rolebinding.role_ref.name,
                                                                          rolebinding.metadata.namespace)
        else:
            role = api.api_client.RbacAuthorizationV1Api.read_cluster_role(rolebinding.role_ref.name)
...
def get_clusterrolebinding_role(cluster_rolebinding_name):
    cluster_role = ''
    try:
        cluster_rolebinding = api.api_client.RbacAuthorizationV1Api.read_cluster_role_binding(cluster_rolebinding_name)
        cluster_role = api.api_client.RbacAuthorizationV1Api.read_cluster_role(cluster_rolebinding.role_ref.name)
    except ApiException as e:
        print(e)
        exit()
...
def list_pods_for_all_namespaces_or_one_namspace(namespace=None):
    try:
        if namespace is None:
            pods = api.api_client.CoreV1Api.list_pod_for_all_namespaces(watch=False)
        else:
            pods = api.api_client.CoreV1Api.list_namespaced_pod(namespace)
        return pods
    except ApiException:
        return None
...
def list_boostrap_tokens_decoded():
    tokens = []
    secrets = api.api_client.CoreV1Api.list_namespaced_secret(namespace='kube-system',
