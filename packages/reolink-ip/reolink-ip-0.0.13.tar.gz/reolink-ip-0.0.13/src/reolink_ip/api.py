...
class Host:
        ...
        if json_data is not None:
            try:
                if json_data[0]["code"] == 0:
                    ...
                    # At least I've seen RLC-510A failed with 0.7s, but OK with 0.12s. Need to gather some more logging data from users...
                    asyncio.sleep(0.1)
                    return True
            except Exception:
    ...
    async def logout(self):
        body  = [{"cmd": "Logout", "action": 0, "param": {"User": {"userName": self._username}}}]
        param = {"cmd": "Logout"}
