...
class Host:
    ...
    def session_active(self) -> bool:
        if self._token is not None and self._lease_time > (datetime.now() + timedelta(seconds = 30)):
            return True
        return False
    ...
    def api_version_getrec(self, channel: int) -> Optional[int]:
        if self._api_version_getrec is not None and channel in self._api_version_getrec:
            self._api_version_getrec[channel]
        else:
            return None
    #endof api_version_getrec()
        ...
        if json_data is not None:
            try:
                if json_data[0]["code"] == 0:
                    ...
                    lease_time: float = float(json_data[0]["value"]["Token"]["leaseTime"])
    ...
    async def set_ftp(self, channel: Optional[int], enable: bool) -> bool:
        ...
        body = None
        if channel is None:
            if self._api_version_getftp[0] == 0:
                OK = True
                for c in self._channels:
            ...
            if self._api_version_getftp[channel] == 0:
    ...
    async def set_audio_alarm(self, channel: int, enable: bool, *args) -> bool:
        ...
        if self._api_version_getalarm[channel] == 0:
            body = [{'cmd': 'SetAudioAlarm', 'param': {"Audio": {"schedule": {"enable": 1 if enable else 0}}}}]
        else:
    ...
    async def set_siren(self, channel: int, enable: bool) -> bool:
        ...
        body = [
            {'cmd': 'AudioAlarmPlay',
                'action': 0,
                'param': {
                    "alarm_mode": 'manul', #FIXME: should be "manual"???
    ...
    async def set_recording(self, channel: Optional[int], enable: bool) -> bool:
        ...
        body = None
        if channel is None:
            if self._api_version_getrec[channel] == 0:
                OK = True
                for c in self._channels:
    ...
    async def send(self, body, param = None, expected_content_type: Optional[str] = None) -> Optional[list]:
        ...
        if (body is None or (body[0]["cmd"] != "Login" and body[0]["cmd"] != "Logout")) and not self.session_active:
            if not await self.login():
                return None
    ...
    def renewtimer(self) -> int:
        ...
        remote_time += timedelta(seconds = self._subscription_time_difference)

        diff = self._subscription_termination_time - remote_time
    ...
    async def subscription_send(self, headers, data) -> Optional[str]:
        ...
        try:
            async with aiohttp.ClientSession(timeout = self._timeout, connector = aiohttp.TCPConnector(verify_ssl = False)) as session:
                _LOGGER.debug("Host %s:%s: subscription request data: %s", self._host, self._port, data)
    ...
    async def subscribe(self, webhook_url: str) -> bool:
        ...
        if termination_time_element is None:
            ...
            _LOGGER.error("Host %s:%s: failed to subscribe.", self._host, self._port)
            return False
        self._subscription_termination_time = await self.convert_time(termination_time_element.text)

        if (self._subscription_manager_url is None or remote_time is None or self._subscription_termination_time is None):
            await self.unsubscribe_all()
    ...
    async def renew(self) -> bool:
        ...
        The Reolink renew functionality has a bug: it always returns the initial Termination Time.
        ...
        current_times = root.findall('.//{http://docs.oasis-open.org/wsn/b-2}CurrentTime')
        if current_times is None or len(current_times) < 1:
            await self.unsubscribe()
            _LOGGER.error("Host %s:%s: failed to renew subscription.", self._host, self._port)
            return False
        remote_time = await self.convert_time(current_times[0].text)
        ...
        self._subscription_time_difference = await self.calc_time_difference(local_time, remote_time)
        self._subscription_termination_time += timedelta(minutes = SUBSCRIPTION_TERMINATION_TIME)
    ...
    async def unsubscribe(self):
        ...
        self._subscription_time_difference = 0
