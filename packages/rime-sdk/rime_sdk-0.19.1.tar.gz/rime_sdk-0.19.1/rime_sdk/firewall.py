...
class Firewall:
    ...
    def get_config(self) -> dict:
        ...
        typed_config_proto = firewall.typed_cli_config
    ...
    def _get_firewall(self) -> FirewallProto:
    ...
    def _get_location_update_info(
        ...
        data_location_info = location_args_to_data_location(
    ...
    def update_managed_image(self, rime_managed_image: str,) -> UpdateFirewallResponse:
    ...
    def get_model_status(self, test_run_id: Optional[str] = None) -> pd.DataFrame:
        ...
        """
        if test_run_id:
            req = GetModelStatusRequest(
                firewall_id=self._firewall_id, test_run_id=test_run_id
            )
        else:
            req = GetModelStatusRequest(firewall_id=self._firewall_id,)

        with self._backend.GRPCErrorHandler():
            with self._backend.get_firewall_stub() as firewall_tester:
                res: GetModelStatusResponse = firewall_tester.GetModelStatus(req)
        res_dict: dict = {
            "name": [],
            "value": [],
            "severity": [],
...
def get_reference_set_source(
    ...
    """
    if rolling_window_duration is not None and reference_set_time_bin is not None:
        raise ValueError(
            "Only one of `rolling_window_seconds` or `reference_set_time_bin` "
            "may be set."
        )
    elif rolling_window_duration is not None:
        return ReferenceSetSource(
            rolling_window_seconds=int(rolling_window_duration.total_seconds())
        )
    elif reference_set_time_bin is not None:
        start_time, end_time = reference_set_time_bin
        reference_set_time_bin_pb = TimeRange(
            start_time_epoch_seconds=Timestamp(seconds=int(start_time.timestamp())),
            end_time_epoch_seconds=Timestamp(seconds=int(end_time.timestamp())),
        )
        return ReferenceSetSource(time_range=reference_set_time_bin_pb)
    else:
        return None
