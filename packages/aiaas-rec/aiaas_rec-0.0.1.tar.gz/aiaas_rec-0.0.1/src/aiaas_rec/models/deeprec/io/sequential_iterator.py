...
class SequentialIterator(BaseIterator):
    ...
    def parse_file(self, input_file, inlines=None):
    ...
    def load_data_from_file(self, infile, batch_num_ngs=0, min_seq_length=1, inlines=None):
        ...
        cnt = 0
        if inlines:
            lines = self.parse_file(None, inlines=inlines)
        elif infile not in self.iter_data:
    ...
    def load_data_from_list(self, list_ins, batch_num_ngs=0, min_seq_length=1):
        """Read and parse data from an instance object.

        Args:
            list_ins ([Instance]): List of instances
            batch_num_ngs (int): The number of negative sampling here in batch.
                0 represents that there is no need to do negative sampling here.
            min_seq_length (int): The minimum number of a sequence length.
                Sequences with length lower than min_seq_length will be ignored.
