...
def Recovery (A, xs, t = 'DCT', PSI = None, solver = 'LASSO', fast_lasso = False, display = True):
    ...
    if solver == 'LASSO':
        ...
        if  t == common.PSI_NAMES[5] or t == common.PSI_LONGNAMES[5] or t == 'rom' or \
            t == 'DFT' or t == 'dft' or \
            t == 'DWT' or t == 'dwt' or \
            t == 'LDA' or t == 'lda':
    ...
    if t == common.PSI_NAMES[0] or t == common.PSI_LONGNAMES[0] or t == 'idm':
        z = np.linalg.pinv(A) @ xs
        xr = z
    elif  t == common.PSI_NAMES[1] or t == common.PSI_LONGNAMES[1] or t == 'dct':
        xr = cv2.idct(z)
    elif t == common.PSI_NAMES[2] or t == common.PSI_LONGNAMES[2] or t == 'dft':
        xr = cv2.idft(z)
    elif t == common.PSI_NAMES[3] or t == common.PSI_LONGNAMES[3] or t == 'dwt' or \
        t == common.PSI_NAMES[4] or t == common.PSI_LONGNAMES[4] or t == 'hwt' or \
        t ==common. PSI_NAMES[5] or t == common.PSI_LONGNAMES[5] or t == 'rom' or \
...
def Sensing_n_Recovery(x, k = 0.2, t = 'DCT', solver = 'LASSO', \
    ...
    if VECTORIZATION:
        ...
        xs = PHI @ x
        PSI = common.Generate_PSI(len(x), t)
        # print(xs)
        # print(PSI)
        A = PHI @ PSI
