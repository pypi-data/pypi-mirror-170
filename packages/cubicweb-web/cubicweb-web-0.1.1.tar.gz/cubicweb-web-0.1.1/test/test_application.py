...
class UtilsTC(TestCase):
    ...
    def test_execute_linkto(self):
        ...
        self.ctrl.set_form({"__linkto": "works_for:12_13_14:object", "eid": 8})
        self.ctrl.execute_linkto()
        self.assertEqual(
            self.ctrl._cursor.executed,
            ["SET Y works_for X WHERE X eid 8, Y eid %s" % i for i in (12, 13, 14)],
        )

        self.ctrl.new_cursor()
        self.ctrl.set_form({"__linkto": "works_for:12_13_14:subject", "eid": 8})
        self.ctrl.execute_linkto()
        self.assertEqual(
            self.ctrl._cursor.executed,
...
class ApplicationTC(CubicWebTC):
    ...
    def test_cnx_user_groups_sync(self):
        with self.admin_access.client_cnx() as cnx:
            ...
            # cleanup
            cnx.execute(
                'DELETE X in_group G WHERE X eid %s, G name "guests"' % user.eid
            )
            cnx.commit()
    ...
    def test_validation_error_dont_loose_subentity_data_ctrl(self):
        ...
        with self.admin_access.web_request() as req:
            ...
            req.form = {
                ...
                "__maineid": "X",
                "__type:X": "CWUser",
                "_cw_entity_fields:X": "login-subject",
                # missing required field
                "login-subject:X": "",
                # but email address is set
                "__type:Y": "EmailAddress",
                "_cw_entity_fields:Y": "address-subject",
                "address-subject:Y": "bougloup@logilab.fr",
            ...
            self.assertEqual(
                forminfo["error"].errors, {"login-subject": "required field"}
            )
    ...
    def test_handle_request_with_lang_negotiated_prefix_in_url(self):
        ...
        with self.admin_access.web_request(url="fr/toto") as req:
            result = self.app.handle_request(req)
        self.assertIn(
            "this resource does not exist",  # NotFound.
    ...
    def test_handle_request_no_lang_negotiation_fixed_language(self):
        ...
        try:
            with self.admin_access.cnx() as cnx:
                props.append(
                    cnx.create_entity("CWProperty", value="de", pkey="ui.language").eid
                )
                cnx.commit()
    ...
    def test_validation_error_dont_loose_subentity_data_repo(self):
        ...
        with self.admin_access.web_request() as req:
            ...
            req.form = {
                ...
                "upassword-subject:X": "admin",
                "upassword-subject-confirm:X": "admin",
                "__type:Y": "EmailAddress",
                "_cw_entity_fields:Y": "address-subject",
                "address-subject:Y": "bougloup@logilab.fr",
                "use_email-object:Y": "X",
    ...
    def _edit_parent(
        self, dir_eid, parent_eid, role="subject", etype="Directory", **kwargs
    ):
        ...
        with self.admin_access.web_request() as req:
            req.form = {
                ...
                f"parent-{role}:{dir_eid}": parent_eid,
            }
    ...
    def test_create_and_link_directories(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            self.assertTrue(cnx.find("Directory", name="topd"))
            self.assertTrue(cnx.find("Directory", name="subd"))
            self.assertEqual(
                1,
                cnx.execute(
                    "Directory SUBD WHERE SUBD parent TOPD,"
                    ' SUBD name "subd", TOPD name "topd"'
                ).rowcount,
            )

    def test_create_subentity(self):
        with self.admin_access.repo_cnx() as cnx:
            topd = cnx.create_entity("Directory", name="topd")
            cnx.commit()
    ...
    def test_subject_subentity_removal(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            topd = cnx.create_entity("Directory", name="topd")
            sub1 = cnx.create_entity("Directory", name="sub1", parent=topd)
            sub2 = cnx.create_entity("Directory", name="sub2", parent=topd)
            cnx.commit()
    ...
    def test_reparent_subentity(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            top1 = cnx.create_entity("Directory", name="top1")
            top2 = cnx.create_entity("Directory", name="top2")
            subd = cnx.create_entity("Directory", name="subd", parent=top1)
            cnx.commit()
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertEqual(cnx.find("Directory", eid=subd.eid).one().parent[0], top2)

    def test_reparent_subentity_inlined(self):
        """Editcontroller: re-parenting a subentity does not remove it
        (inlined case)"""
        with self.admin_access.repo_cnx() as cnx:
            version1 = cnx.create_entity("Version", name="version1")
            version2 = cnx.create_entity("Version", name="version2")
            ticket = cnx.create_entity("Ticket", title="ticket", in_version=version1)
            cnx.commit()
    ...
    def test_subject_mixed_composite_subentity_removal_1(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            topd = cnx.create_entity("Directory", name="topd")
            fs = cnx.create_entity("Filesystem", name="/tmp")
            subd = cnx.create_entity("Directory", name="subd", parent=(topd, fs))
            cnx.commit()
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertEqual(cnx.find("Directory", eid=subd.eid).one().parent, (topd,))
    ...
    def test_object_mixed_composite_subentity_removal_1(self):
        ...
        self._edit_parent(fs.eid, parent_eid=None, role="object", etype="Filesystem")
    ...
    def test_delete_mandatory_composite(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            mydir = cnx.create_entity("Directory", name="dir", has_permission=perm)
            cnx.commit()
        ...
        with self.temporary_appobjects(ErrorAjaxView):
            with real_error_handling(self.app) as app:
                with self.admin_access.web_request(
                    vid="test.ajax.error", url=""
                ) as req:
                    req.ajax_request = True
                    app.handle_request(req)
        self.assertEqual(http.client.INTERNAL_SERVER_ERROR, req.status_out)
    ...
    def test_http_auth_no_anon(self):
        ...
        authstr = base64.encodebytes(
            (f"{self.admlogin}:{self.admpassword}").encode("ascii")
        )
    ...
    def test_login_by_email(self):
        with self.admin_access.client_cnx() as cnx:
            ...
            address = login + "@localhost"
            cnx.execute(
                "INSERT EmailAddress X: X address %(address)s, U primary_email X "
    ...
    def _reset_cookie(self, req):
        ...
        req.set_request_header("Cookie", cookie[sessioncookie].OutputString(), raw=True)
        clear_cache(req, "get_authorization")
    ...
    def test_notfound_raised_on_empty_underlying_rset(self):
        ...
        SimpleReqRewriter.rules = [
            (
                rgx(r"/mycwetypeurl/([^/]+)"),
                dict(vid="primary", rql=r'Any T WHERE T is CWEType, T name "\1"'),
            )
        ]
