...
class PageNavigation(NavigationComponent):
    ...
    def iter_page_links(self, basepath, params):
        ...
        while start < rset.rowcount:
            ...
            yield self.page_link(
                basepath, params, start, stop, self.index_display(start, stop)
            )
            start = stop + 1
...
class PageNavigationSelect(PageNavigation):
    ...
    page_link_templ = '<option value="%s" title="%s">%s</option>'
...
class SortedNavigation(NavigationComponent):
    ...
    def display_func(self, rset, col, attrname):
        ...
        elif self._cw.vreg.schema.entity_schema_for(rset.description[0][col]).final:
    ...
    def sort_on(self):
        ...
        """
        relation_schema_for = self._cw.vreg.schema.relation_schema_for
        for sorterm in self.cw_rset.syntax_tree().children[0].orderby:
            ...
            for ref in var.references():
                ...
                if attrname in ("is", "has_text"):
                    continue
                if not relation_schema_for(attrname).final:
...
class NextPrevNavigationComponent(EntityCtxComponent):
    ...
    def prev_icon(self):
        return '<img src="{}" alt="{}" />'.format(
            xml_escape(self._cw.data_url("go_prev.png")),
            self._cw._("previous page"),
        )

    @property
    def next_icon(self):
        return '<img src="{}" alt="{}" />'.format(
            xml_escape(self._cw.data_url("go_next.png")),
            self._cw._("next page"),
        )
    ...
    def prevnext_entity(self, w, entity, type):
        ...
        if type == "prev":
            ...
            icon = self.prev_icon
            cssclass = "previousEntity left"
            content = icon + "&#160;&#160;" + content
        else:
            title = self._cw._("i18nprevnext_next")
            icon = self.next_icon
            cssclass = "nextEntity right"
            content = content + "&#160;&#160;" + icon
        self.prevnext_div(w, type, cssclass, entity.absolute_url(), title, content)

    def prevnext_div(self, w, type, cssclass, url, title, content):
        w('<div class="%s">' % cssclass)
        w(
