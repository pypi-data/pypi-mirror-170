...
from cubicweb.predicates import one_line_rset, non_final_entity, match_search_state
from cubicweb.view import EntityView, StartupView
from cubicweb_web import httpcache
from cubicweb_web.views import baseviews, linksearch_select_url
...
class SearchForAssociationView(EntityView):
    ...
    __select__ = one_line_rset() & match_search_state("linksearch") & non_final_entity()
    ...
    def filter_box_context_info(self):
        ...
        rql, args = entity.cw_unrelated_rql(
            rtype, etype, role, ordermethod="fetch_order", vocabconstraints=False
        )
...
class OutOfContextSearch(EntityView):
    ...
    def cell_call(self, row, col):
        ...
        if self._cw.match_search_state(erset):
            self.w(
                '<a href="%s" title="%s">%s</a>&#160;<a href="%s" title="%s">[...]</a>',
                linksearch_select_url(self._cw, erset),
                self._cw._("select this entity"),
                entity.view("textoutofcontext"),
                entity.absolute_url(vid="primary"),
...
class EditableFinalView(baseviews.FinalView):
    ...
    def cell_call(self, row, col, props=None):
        ...
        if entity is not None:
            ...
            super().cell_call(row, col, props)


try:
    from cubicweb_web import captcha
except ImportError:
    # PIL not installed
    pass
else:
    ...
    class CaptchaView(StartupView):
        ...
        def call(self):
            text, data = captcha.captcha(
                self._cw.vreg.config["captcha-font-file"],
