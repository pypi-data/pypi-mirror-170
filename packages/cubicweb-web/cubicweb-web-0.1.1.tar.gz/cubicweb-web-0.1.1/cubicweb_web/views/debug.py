...
from cubicweb_web.views import actions, tabs
...
def dict_to_html(w, dict):
    ...
    if dict:
        ...
        for key in sorted(dict):
            w("<li><span>%s</span>: <span>%s</span></li>", str(key), repr(dict[key]))
        w("</ul>")
    ...
    def call(self, **kwargs):
        ...
        for k in stats:
            ...
            if k.endswith("_cache_size"):
                stats[k] = "{} / {}".format(stats[k]["size"], stats[k]["maxsize"])
        ...
        pyvalue = [
            (sname, format_stat(sname, sval)) for sname, sval in sorted(stats.items())
        ]
        self.wview("pyvaltable", pyvalue=pyvalue, header_column_idx=0)
...
class GCView(StartupView):
    ...
    def call(self, **kwargs):
        ...
        self.w("<h2>%s</h2>", _("Garbage collection information"))
        self.w("<h3>%s</h3>", self._cw._("Looked up classes"))
        self.wview("pyvaltable", pyvalue=stats["lookupclasses"])
        self.w("<h3>%s</h3>", self._cw._("Most referenced classes"))
        self.wview("pyvaltable", pyvalue=stats["referenced"])
        if stats["unreachable"]:
            self.w("<h3>%s</h3>", self._cw._("Unreachable objects"))
