...
LoginPasswordRetreiver = class_renamed(
    "LoginPasswordRetreiver",
    LoginPasswordRetriever,
...
class Session:
    ...
    def __str__(self):
        return "<session {} (0x{:x})>".format(self.user.login, id(self))
...
class RepositoryAuthenticationManager:
    ...
    def __init__(self, repo):
        ...
        self.authinforetrievers = sorted(
            vreg["webauth"].possible_objects(vreg), key=lambda x: x.order
        )
