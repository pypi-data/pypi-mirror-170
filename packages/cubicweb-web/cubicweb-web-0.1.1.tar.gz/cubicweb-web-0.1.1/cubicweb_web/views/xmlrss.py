...
class XMLRelatedItemView(EntityView):
    ...
    def entity_call(self, entity):
        ...
        self.w(
            '    <%s eid="%s" cwuri="%s"/>\n', entity.e_schema, entity.eid, entity.cwuri
        )
        ...
        for rowindex, row in enumerate(self.cw_rset):
            ...
            for colindex, val in enumerate(row):
                ...
                if "(" in tag:
                    attrs["expr"] = tag
                    tag = "funccall"
                if val is not None and not entity_schema_for(etype).final:
                    attrs["eid"] = val
                    # csvrow.append(val) # val is eid in that case
                    val = self._cw.view(
                        "textincontext", rset, row=rowindex, col=colindex
                    )
                else:
                    val = self._cw.view(
                        "final", rset, row=rowindex, col=colindex, format="text/plain"
                    )
                self.w(simple_sgml_tag(tag, val, **attrs))
            self.w("\n </row>\n")
        self.w("</%s>\n", self.xml_root)
...
class RSSIconBox(component.CtxComponent):
    ...
    __regid__ = "rss"
    __select__ = component.CtxComponent.__select__ & appobject_selectable(
        "components", "rss_feed_url"
    )
    ...
    def render(self, w, **kwargs):
        try:
            ...
            return
        urlgetter = self._cw.vreg["components"].select(
            "rss_feed_url", self._cw, rset=self.cw_rset
        )
        url = urlgetter.feed_url()
        w('<a href="%s"><img src="%s" alt="rss"/></a>\n', url, rss)
...
class RSSView(XMLView):
    ...
    def _open(self):
        req = self._cw
        self.w('<?xml version="1.0" encoding="%s"?>\n', req.encoding)
        self.w('<rss version="2.0" xmlns:dc="http://purl.org/dc/elements/1.1/">\n')
        self.w("  <channel>\n")
        self.w("    <title>%s RSS Feed</title>\n", self.page_title())
        self.w("    <description>%s</description>\n", req.form.get("vtitle", ""))
        params = req.form.copy()
        params.pop("vid", None)
        self.w("    <link>%s</link>\n", self._cw.build_url(**params))
...
class RSSItemView(EntityView):
    ...
    def cell_call(self, row, col):
        ...
        self.w("<item>\n")
        self.w('<guid isPermaLink="true">%s</guid>\n', entity.absolute_url())
