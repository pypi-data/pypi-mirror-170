...
class DeleteConfFormView(FormViewMixIn, EntityView):
    ...
    show_composite_skip_rtypes = set(
        "wf_info_for",
    )

    def _iter_composite_entities(self, entity, limit=None):
        eids = set()
        for rdef, role in sorted(
            entity.e_schema.composite_rdef_roles, key=lambda x: x[0].rtype
        ):
            if rdef.relation_type in self.show_composite_skip_rtypes:
                continue
            for centity in entity.related(
                rdef.relation_type, role, limit=limit
            ).entities():
    ...
    def call(self, onsubmit=None):
        ...
        form = self._cw.vreg["forms"].select(
            self.__regid__, req, rset=self.cw_rset, onsubmit=onsubmit
        )
        w("<ul>\n")
            ...
            if self.show_composite:
                ...
                for count, centity in enumerate(
                    ...
                    if content is not None:
                        w('<li><a href="%s">%s</a></li>', *content)
                    if count == page_size - 1:
                        w(
                            '<li class="last">%s</li></ul>',
                            _("And more composite entities"),
                        )
                        break
                    content = (centity.absolute_url(), centity.view("textoutofcontext"))
                else:
                    if content is not None:
                        w('<li class="last"><a href="%s">%s</a></li></ul>', *content)
            w("</li>\n")
        w("</ul>\n")
...
class EditionFormView(FormViewMixIn, EntityView):
    ...
    def render_form(self, entity):
        ...
        form = self._cw.vreg["forms"].select(
            self.form_id, self._cw, entity=entity, submitmsg=self.submited_message()
        )
    ...
    def form_title(self, entity):
        ...
        self.w(
            '<div class="formTitle"><span>%s %s</span></div>',
            entity.dc_type(),
            ptitle and "(%s)" % ptitle,
        )
...
class CreationFormView(EditionFormView):
    ...
    def url(self):
        ...
        req = self._cw
        return req.vreg["etypes"].etype_class(req.form["etype"]).cw_create_url(req)
...
class CopyFormView(EditionFormView):
    ...
    def render_form(self, entity):
        ...
        super().render_form(self.newentity)
        del self.newentity

    def init_form(self, form, entity):
        """customize your form before rendering here"""
        super().init_form(form, entity)
        if entity.eid == self.newentity.eid:
            form.add_hidden(eid_param("__cloned_eid", entity.eid), self.copying.eid)
...
class TableEditForm(forms.CompositeForm):
    ...
    form_buttons = [
        fw.SubmitButton(_("validate modifications on selected items")),
