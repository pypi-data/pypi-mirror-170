...
````````````````````````````````````
.. autoclass:: cubicweb_web.views.primary.PrimaryView
...
class PrimaryView(EntityView):
    ...
    .. automethod:: cubicweb_web.views.primary.PrimaryView.render_entity_title
    .. automethod:: cubicweb_web.views.primary.PrimaryView.render_entity_attributes
    .. automethod:: cubicweb_web.views.primary.PrimaryView.render_entity_relations
    .. automethod:: cubicweb_web.views.primary.PrimaryView.render_side_boxes
    ...
    .. automethod:: cubicweb_web.views.primary.PrimaryView.content_navigation_components
    ...
    def entity_call(self, entity, **kwargs):
        ...
        if self.rsection is None:
            self.rsection = uicfg_reg.select(
                "primaryview_section", self._cw, entity=entity
            )
        if self.display_ctrl is None:
            self.display_ctrl = uicfg_reg.select(
                "primaryview_display_ctrl", self._cw, entity=entity
            )
    ...
    def render_entity_title(self, entity):
        ...
        if title:
            if self.is_primary():
                ...
                atitle = (
                    self._cw._("follow this link for more information on this %s")
                    % entity.dc_type()
                )
                self.w(
    ...
    def render_entity_attributes(self, entity):
        ...
        display_attributes = []
        for rschema, __, role, dispctrl in self._section_def(entity, "attributes"):
        ...
        for rschema, tschemas, role, dispctrl in self._section_def(entity, "relations"):
            if rschema.final or dispctrl.get("rtypevid"):
                ...
                try:
                    ...
                    continue
                value = rview.render(
                    row=entity.cw_row, col=entity.cw_col, rtype=rschema.type, role=role
                )
            else:
                vid = dispctrl.get("vid", "autolimited")
                limit = (
                    dispctrl.get("limit", defaultlimit)
                    if vid == "autolimited"
                    else None
                )
    ...
    def _prepare_side_boxes(self, entity):
        ...
        sideboxes += boxesreg.poss_visible_objects(
            self._cw, rset=self.cw_rset, view=self, context="incontext"
        )
    ...
    def _section_def(self, entity, where):
        ...
        for rschema, tschemas, role in eschema.relation_definitions(True):
            ...
            for tschema in tschemas:
                section = self.rsection.etype_get(eschema, rschema, role, tschema)
                if section == where:
    ...
    def _rel_label(self, entity, rschema, role, dispctrl):
        ...
        if showlabel:
            if dispctrl.get("label"):
                ...
                label = display_name(
                    self._cw, rschema.type, role, context=entity.cw_etype
                )
            return label
        return ""
    ...
    def call(self, **kwargs):
        ...
        # else show links to display related entities
            ...
            if list_limit is None:
                ...
                self.w(
                    '[<a href="%s">%s</a>]',
                    self._cw.build_url(rql=rql, vid=subvid),
