...
from cubicweb_web import component, stdmsgs, formwidgets as fw
...
class AddRelationView(component.EditRelationMixIn, View):
    ...
    __select__ = match_form_params("rtype", "target") | match_kwargs("rtype", "target")
    ...
    def cell_call(self, row, col, rtype=None, target=None, etype=None):
        ...
        if not self.etype:
            ...
            if len(etypes) == 1:
                self.etype = etypes[0]
        self.w('<div id="%s">', self.domid)
        self.w(
            "<h1>%s</h1>",
            self._cw._("relation %(relname)s of %(ent)s")
            % {
                "relname": rschema.display_name(self._cw, role(self)),
                "ent": entity.view("incontext"),
            },
        )
        self.w('<ul class="list-unstyled">')
    ...
    def unrelated_entities(self, entity):
        ...
        if getattr(self, "etype", None):
            rset = entity.unrelated(
                self.rtype, self.etype, role(self), ordermethod="fetch_order"
            )
            self.paginate(self._cw, rset=rset, w=self.w)
            return rset.entities()
        super().unrelated_entities(self)


def ajax_composite_form(container, entity, rtype, okjs, canceljs, entityfkwargs=None):
    """
    ...
    * if container has not eid, will be created (see vcreview InsertionPoint)
    ...
    mkwargs = dict(
        action="#",
        domid=f"{rtype}Form{container.eid}",
    ...
    if parentexists:  # creation / edition
        ...
        form = freg.select("composite", req, form_renderer_id="default", **mkwargs)
        form.add_subform(
            freg.select(
                "edition", req, entity=container, mainform=False, mainentity=True
            )
        )
