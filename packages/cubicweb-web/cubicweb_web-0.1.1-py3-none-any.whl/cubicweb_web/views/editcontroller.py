...
from cubicweb_web import RequestError, NothingToEdit, ProcessFormError
from cubicweb_web.views import basecontrollers, autoform
...
class IEditControlAdapter(EntityAdapter):
    ...
    def __init__(self, _cw, **kwargs):
        if self.__class__ is not IEditControlAdapter:
            warn(
                "[3.14] IEditControlAdapter is deprecated, override EditController"
...
class RqlQuery:
    ...
    def __repr__(self):
        return "Query <edited={!r} restrictions={!r} kwargs={!r}>".format(
            self.edited,
            self.restrictions,
            self.kwargs,
        )
    ...
    def insert_query(self, etype):
        ...
        if self.edited:
            rql = "INSERT {} X: {}".format(etype, ",".join(self.edited))
        else:
    ...
    def update_query(self, eid):
        ...
        while var in self.kwargs:
            var = next(varmaker)
        rql = "SET {} WHERE X eid %({})s".format(",".join(self.edited), var)
        if self.restrictions:
    ...
    def set_attribute(self, attr, value):
        ...
        self.edited.append(f"X {attr} %({attr})s")

    def set_inlined(self, relation, value):
        self.kwargs[relation] = value
        self.edited.append(f"X {relation} {relation.upper()}")
        self.restrictions.append(f"{relation.upper()} eid %({relation})s")
...
class EditController(basecontrollers.ViewController):
    ...
    def _default_publish(self):
        ...
        assert not pending_values, (
            "unexpected remaining pending values %s" % pending_values
        )
        ...
        if self.errors:
            errors = {f.name: str(ex) for f, ex in self.errors}
    ...
    def edit_entity(self, formparams, multiple=False):
        ...
        if not rqlquery.canceled:
            if self.errors:
                errors = {f.role_name(): str(ex) for f, ex in self.errors}
                raise ValidationError(valerror_eid(entity.eid), errors)
            if eid is None:  # creation or copy
                entity.eid = eid = self._insert_entity(
                    etype, formparams["eid"], rqlquery
                )
    ...
    def handle_formfield(self, form, field, rqlquery=None):
        ...
        try:
            for field, value in field.process_posted(form):
                ...
                rschema = self._cw.vreg.schema.relation_schema_for(field.name)
                if rschema.final:
                ...
                if entity.has_eid():
                    origvalues = {
                        row[0] for row in entity.related(field.name, field.role).rows
                    }
                else:
                    origvalues = set()
                ...
                if rschema.inlined and rqlquery is not None and field.role == "subject":
                    self.handle_inlined_relation(
                        form, field, value, origvalues, rqlquery
                    )
    ...
    def handle_composite_removal(
        self, form, field, removed_values, new_values, rqlquery
    ):
        """
        ...
        for unlinked_eid in removed_values:
            ...
            rdef = rschema.role_rdef(
                form.edited_entity.cw_etype, unlinked_entity.cw_etype, field.role
            )
            if rdef.composite is not None:
                if rdef.composite == field.role:
                    ...
                    self.info(
                        "Edition of %s is cancelled (deletion requested)", to_be_removed
                    )
                    rqlquery.canceled = True
                self.info(
                    "Scheduling removal of %s as composite relation " "%s was removed",
                    to_be_removed,
                    rdef,
                )
    ...
    def handle_relation(self, form, field, values, origvalues):
        ...
        if field.role == "object" or not rschema.inlined or not values:
            ...
            rql = "DELETE {} {} {} WHERE X eid %(x)s, Y eid %(y)s".format(
                subjvar,
                rschema,
                objvar,
            )
        ...
        if seteids:
            rql = "SET {} {} {} WHERE X eid %(x)s, Y eid %(y)s".format(
                subjvar,
                rschema,
                objvar,
            )
    ...
    def check_concurrent_edition(self, formparams, eid):
        ...
        if req.execute(
            ...
            msg = _(
                "Entity %(eid)s has changed since you started to edit it."
