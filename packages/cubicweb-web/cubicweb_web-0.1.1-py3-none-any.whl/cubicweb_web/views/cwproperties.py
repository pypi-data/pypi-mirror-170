...
from cubicweb_web.formfields import FIELDS, StringField
from cubicweb_web.formwidgets import (
    Select,
    TextInput,
    SubmitButton,
    FieldWidget,
)
from cubicweb_web.views import uicfg, primary, formrenderers, editcontroller
...
def make_togglable_link(nodeid, label):
    ...
    return f"<a href=\"javascript: togglePrefVisibility('{nodeid}')\">{label}</a>"
...
class SystemCWPropertiesForm(FormViewMixIn, StartupView):
    ...
    def _cookie_name(self, somestr):
        return str(f"{self._cw.vreg.config.appid}_property_{somestr}")

    def _group_status(self, group, default="hidden"):
    ...
    def call(self, **kwargs):
        self._cw.add_js(
            ("cubicweb.preferences.js", "cubicweb.edition.js", "cubicweb.ajax.js")
        )
        ...
        _ = self._cw._
        self.w("<h1>%s</h1>\n", _(self.title))
        for label, group, form in sorted((_(g), g, f) for g, f in mainforms.items()):
    ...
    def cwprops_rset(self):
        return self._cw.execute(
            "Any P,K,V WHERE P is CWProperty, P pkey K, P value V, NOT P for_user U"
        )
    ...
    def form_row(self, form, key, splitlabel):
        ...
        subform = self._cw.vreg["forms"].select(
            "base", self._cw, entity=entity, mainform=False
        )
        subform.append_field(
            PropertyValueField(name="value", label=label, role="subject", eidparam=True)
        )
    ...
    def wrap_main_form(self, group, label, form):
        ...
        self.w(
            '<div class="propertiesform">%s</div>\n',
            make_togglable_link("fieldset_" + group, label),
            escape=False,
        )
        self.w('<div id="fieldset_%s" %s>', group, status, escape=False)
        self.w('<fieldset class="preferences">')
        self.w(form)
        self.w("</fieldset></div>")
    ...
    def wrap_object_form(self, group, oid, label, form):
        self.w('<div class="component">')
        self.w(
            """<div class="componentLink"><a href="javascript:$.noop();"
                   onclick="javascript:toggleVisibility('field_%s_%s')"
                   class="componentTitle">%s</a>""",
            label,
            oid,
            group,
        )
        self.w(
            """ (<div class="openlink"><a href="javascript:$.noop();"
                onclick="javascript:openFieldset('fieldset_%s')">%s</a></div>)""",
            group,
            self._cw._("open all"),
        )
        self.w("</div>")
        docmsgid = f"{group}_{oid}_description"
        doc = self._cw._(docmsgid)
        if doc != docmsgid:
            self.w('<div class="helper">%s</div>', xml_escape(doc).capitalize())
        self.w("</div>")
        self.w(
            '<fieldset id="field_%s_%s" class="%s preferences hidden">',
            oid,
            group,
            group,
        )
...
class CWPropertiesForm(SystemCWPropertiesForm):
    ...
    def cwprops_rset(self):
        return self._cw.execute(
            ...
            "P for_user U, U eid %(x)s",
            {"x": self.user.eid},
        )

    def form_row(self, form, key, splitlabel):
        subform = super().form_row(form, key, splitlabel)
...
class PlaceHolderWidget(FieldWidget):
    def render(self, form, field, renderer):
        ...
        # this id)
        return '<div id="div:{}"><span id="{}">{}</span></div>'.format(
            domid,
            domid,
            form._cw._("select a key first"),
        )
...
class NotEditableWidget(FieldWidget):
    ...
    def render(self, form, field, renderer):
        domid = field.dom_id(form)
        value = f'<span class="value" id="{domid}">{self.value}</span>'
        if self.msg:
...
class PropertyValueField(StringField):
    ...
    def form_init(self, form):
        ...
        try:
            ...
            form.warning(
                "%s (you should probably delete that property " "from the database)", ex
            )
            msg = form._cw._("you should probably delete that property")
            self.widget = NotEditableWidget(
                entity.printable_value("value"), f"{msg} ({ex})"
            )
            return
        if entity.pkey.startswith("system."):
            msg = form._cw._("value associated to this key is not editable " "manually")
...
class CWPropertiesFormRenderer(formrenderers.FormRenderer):
    ...
    def open_form(self, form, values):
        ...
        return super().open_form(form, values) + err

    def _render_fields(self, fields, w, form):
        for field in fields:
            w('<div class="preffield">\n')
            if self.display_label:
                w("%s" % self.render_label(form, field))
            error = form.field_error(field)
            if error:
                w('<span class="error">%s</span>' % error)
            w("%s" % self.render_help(form, field))
            w('<div class="prefinput">')
...
def prop_widget(self, propkey, varname):
    ...
    return vfield.render(form, renderer) + renderer.render_help(form, vfield)
