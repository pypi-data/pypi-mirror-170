...
class InlineHelpView(StartupView):
    ...
    def call(self):
        ...
        vreg = self._cw.vreg
        for lang in chain(
            (self._cw.lang, vreg.property_value("ui.language")),
        ...
        try:
            ...
            self.w("<h1>%s</h1>", title_for_lang(node, self._cw.lang))
        with open(join(resourcedir, rid)) as f:
    ...
    navinfo = {
        "prev": ("", "data/previous.png", _("i18nprevnext_previous")),
    ...
    def navsection(self, node, navtype):
        ...
        self.w('<span class="%s">', htmlclass)
        self.w("%s : ", self._cw._(msgid))
        self.w(
            '<a href="%s">%s</a>',
...
class InlineHelpImageView(StartupView):
    ...
    def call(self):
        fid = self._cw.form["fid"]
        for lang in chain(
            (self._cw.lang, self._cw.vreg.property_value("ui.language")),
            self._cw.vreg.config.available_languages(),
        ):
            rid = join("images", f"{fid}_{lang}.png")
