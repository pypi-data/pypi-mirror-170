...
from cubicweb_web.action import UnregisteredAction
from cubicweb_web.component import (
    EditRelationMixIn,
    Separator,
)
from cubicweb_web.htmlwidgets import BoxLink, BoxWidget, SideBoxWidget, RawBoxItem
...
class BoxTemplate(View, metaclass=class_deprecated):
    ...
    __deprecation_warning__ = (
        "[3.10] *BoxTemplate classes are deprecated, use *CtxComponent "
        "instead (%(cls)s)"
    )
    ...
    def boxitem_link_tooltip(self, action):
        if action.__regid__:
            return "keyword: %s" % action.__regid__
        return ""
    ...
    def box_action(self, action):
        ...
        return BoxLink(
            ...
            klass,
            self.boxitem_link_tooltip(action),
        )
...
class EditRelationBoxTemplate(EditRelationMixIn, EntityBoxTemplate):
    ...
    def cell_call(self, row, col, view=None, **kwargs):
        ...
        title = display_name(
            self._cw, self.rtype, get_role(self), context=entity.cw_etype
        )
    ...
    def box_item(self, entity, etarget, rql, label):
        ...
        return RawBoxItem(label, liclass="invisible")
