...
def inject_dll(proc, dll_path):
    ...
    dll_path_address = k32.VirtualAllocEx(proc.__int__(), 0, len(dll_path),
                                          VIRTUAL_MEM, PAGE_READWRITE)

    # Write dll_path into the allocated memory
    bytesWritten = c_int(0)
    k32.WriteProcessMemory(proc.__int__(), dll_path_address, dll_path,
                           len(dll_path), byref(bytesWritten))
    ...
    remoteData = win32process.CreateRemoteThread(proc, None, 0, hLoadLibraryA,
                                                 dll_path_address, 0)

    # Wait for thread to end.
    # Timeout of 60 seconds.
    eventState = win32event.WaitForSingleObjectEx(remoteData[0], 60 * 1000,
                                                  False)
...
def main():
    ...
    procData = win32process.CreateProcess('C:/Windows/notepad.exe', None, None,
                                          None, False,
                                          win32process.CREATE_SUSPENDED, None,
                                          None, si)

    # Inject the dll into the suspended process
    inject_dll(procData[0], 'hook.dll')
