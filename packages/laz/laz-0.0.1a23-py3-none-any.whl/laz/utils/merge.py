...
def merge(left: Data, right: Data) -> Data:
    if isinstance(left, dict):
        left = prodictify(left)
    if isinstance(right, dict):
        right = prodictify(right)
    if type(left) != type(right):
        raise LazTypeError(
            f"merge inputs must be same type. Got {type(left)} and {type(right)}"
        )
...
def _merge(left: Data, right: Data) -> Data:
    ...
    elif isinstance(left, (type(None), bool, int, str)) and isinstance(
        right, (type(None), bool, int, str)
    ):
