...
class LoanappGroup(Group):
    def create_loanapp(self, data=None, params={}):
        """
        Creates a prime loanapp
        """
        api_url = f'/{self.client.get_namespace()}{LENDSMART_LOANAPP_ROOT}'
        if params:
            api_url+=f'?{urlencode(params)}'
        result = self.client.post(api_url, data=data)
        if not 'id' in result:
    ...
    def get_loanapp_by_id(self, loanapp_id):
        ...
        """
        result = self.client.get(f'/{self.client.get_namespace()}{LENDSMART_LOANAPP_ROOT}/{loanapp_id}')
        if is_empty(result):
...
class LendsmartClient:
    ...
    def __default_headers(self):
        """
            This function will set the headers
        """
        return {
            'Authorization': "Bearer {}".format(self.service_account.bearer_token()),
            'Content-Type': 'application/json',
            'User-Agent': self._user_agent,
            'X-AUTH-LENDSMART-SERVICE-ACCOUNT-NAME': self.service_account.CONST_SERVICE_ACCOUNT_NAME,
        }

    def append_header(self, key = None, value = None):
        """
            This function will append the headers
        """
    ...
    def is_valid_404(self, response):
        try:
            res_json = response.json()
            if isinstance(res_json, dict):
                print("====Valid 404===>>True")
                return True
            return False
        except Exception as error:
            print("====Valid 404===>>False")
            return False

    @retry(ApiError, tries =RETRY_COUNT, delay=RETRY_DELAY)
    def get_response(self, method, url, headers, body):
        """
            This function returns the api response
        """
        # TODO : We have to filter retry based on request type in future
        # In future we will have to retry only for error from cluster not for absolute api error
        self.retry_count +=1
        print("Retrying ",self.retry_count, RETRY_COUNT)
        response = method(url, headers=headers, data=body)
        if response.status_code in RETRY_STATUS_CODES and not self.is_valid_404(response) and RETRY_COUNT != self.retry_count:
            raise ApiError("Not found", status=response.status_code, json=None)
        return response
    ...
    def _api_call(self, endpoint, model=None, method=None, data=None, filters=None):
        ...
        print("The api method======>", method)
        print("Trying to connect to api ====>", url)
        self.retry_count = 0
        response = self.get_response(method, url, headers, body)
