...
def csrf_scanner(url, secure = True, bot = False):
    ...
    clear()
    user_input = input("1 = url | 2 = hyperlinks\n")
    clear()

    if user_input == "1":
        try:
            result = web_session.get(my_url, verify = True, headers = user_agent, timeout = (5, 30)).text
        ...
        for i in forms:
            if "hidden" not in i and "password" in i:
                my_list.append(my_url)

    if user_input == "2":
        web_list = []
        web_list.append(my_url)
        counter = 0
