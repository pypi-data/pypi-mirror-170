...
def form_scanner(url, secure = True, bot = False):
    clear()
    ...
    for i in forms:
        method = str(re.findall("method.+\".+\"", i)).split("\"")
        action = str(re.findall("action.+\".+\"", i)).split("\"")
        user_input = re.findall("<input.+?>", i)

        for ii in user_input:
            form_list.append("method: " + method[1] + "\naction: " + action[1] + "\n" + ii)
        ...
        for links in my_result:
            ...
            try:
                ...
                for forms in form_list:
                    ...
                    if mal_script in send_data.text or mal_script in get_data.text:
                        print(color + "true: " + links + " form: " + forms)
...
sql_injection_scanner("networkchuck.com", bot = True)
