...
class dynamicObject():
    """
        ...
        :var object_relation_dict_list: Gives the UUID of all sourind objects as a dict in format ```{"front_ego": <uuid>,  "behind_ego": <uuid>,  "front_left": <uuid>,  "behind_left": <uuid>,  "front_right": <uuid>,  "behind_right": <uuid>}``` If an object relation does not exist, e.g. since there is no object, than the value will be ```None``. 
        :var lane_change_flag_vec: Vector of bool values, whereby True means that a lane change occured
