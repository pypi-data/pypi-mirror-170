...
class Statistics:
    ...
    def generate_coordinates(self, matches: list[str], steps: list[float] | None) -> list[str]:
        if not steps:
            return [f"({i*1},{match})" for i, match in enumerate(matches)]
        return [f"({key},{value})" for key, value in zip(steps, matches)]

    def save_to_file(self, data: list[str], filename: str) -> None:
...
"""
    ...
    def save_all(self, log: str) -> None:
        ...
        self.base_filename = logfile.name
