...
class MolcasProg:
    ...
    def __str__(self, *args, **kwargs):
        ...
        return '\n'.join(
            ["&{}".format(self.name)] +
            [indent + str(val) for val in keywords if val is not None] +
...
class Casscf(MolcasProg):
    ...
    def __str__(self):
        active_space = {"Nactel": f"{self.nactel} 0 0", "RAS2": f"{self.ras2}"}
        return super().__str__(**active_space)
...
def generate_input(labels, coords, central_atom, charge, n_active_elec,
                   ...
                   basis_set_remaining="ANO-RCC-VDZ", rasscf_extra=((), {}),
                   high_S_only=False, initial_orb=None, max_orb=None,
                   caspt2=False, rassi=True, rassi_extra=((), {}),
                   single_aniso=True, single_aniso_extra=((), {}), quax=None,
    ...
    xyz_out = f"{abs_stem}.xyz"
    xfield_out = f"{abs_stem}.xfield"

    # xyz coordinate file
    xyzp.save_xyz(xyz_out, labels, coords, verbose=False)

    # write external xfield file
    if xfield is not None:
        save_xfield(xfield_out, xfield)

    sections = []
    sections.append(Gateway(
        "AMFI",
        *gateway_extra[0],
        Coords=os.path.basename(xyz_out),
        XField=os.path.basename(xfield_out) if xfield else None,
        Angmom=' '.join(map(str, coords[ctr_idx])) + " ANGSTROM",
        **gateway_extra[1]))

    sections.append(Seward(
        *["RICD", "acCD"] if decomp == 'RICD_acCD' else [decomp]))
    ...
    rasscf_sections = [[
        Casscf(
            n_active_elec, n_active_orb, *rasscf_extra[0], Spin=spin,
            Charge=charge, CiRoot=f"{root} {root} 1", ORBA="FULL",
            MAXORB=str(root),
            FILEORB=initial_orb if i == 1 and initial_orb else None,
            **rasscf_extra[1]),
