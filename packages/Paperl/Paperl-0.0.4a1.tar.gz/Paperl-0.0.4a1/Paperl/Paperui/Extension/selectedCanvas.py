...
class SelectedCanvas(Canvas):
    ...
    def create_widget(self, widget: Widget):
        if self.have_child:
            return
        self.have_child = True
        self.widget = widget
        self.widget.pack(fillType='both', expandType=True, marginX=9, marginY=9)
        # 即使拖动组件，也可以移动
...
class ResizeHandle(Widget):
    def __init__(self, parent: Widget, borderColor: str = "#006eb8", arrowColor: str = "#0099ff",):
        """
from Paperl import *
...
Resize = ResizeHandle(Window)

Resize.addWidget(Button(Resize, "Hello"))

Resize.place(x=0, y=0, width=100, height=100)
...
Application.run(Window)
        ...
        self.build(parent.Me, borderColor, arrowColor)

    def build(self, parent, borderColor: str = "#006eb8", arrowColor: str = "#0099ff",):
        self.Me = SelectedCanvas(parent, borderColor=borderColor, arrowColor=arrowColor)

    def setBorderColor(self, color):
        self.Me.setBorderColor(color)

    def setArrowColor(self, color):
        self.Me.setArrowColor(color)

    def addWidget(self, widget: Widget):
        self.Me.create_widget(widget)


class Resize(ResizeHandle):
    def __init__(self, parent: Widget, widget: Widget):
