...
root_drive = path.expanduser('~')  ## fix me

g_project_root = path.join(root_drive, 'ultron')  ## fix me

g_project_data_dir = path.join(g_project_root, 'data')  ## fix me

g_project_cache_dir = path.join(g_project_data_dir, 'cache')  ## fix me


def _do_gmm_cluster(sub_ncs, x, df, threshold):
    """
    在MainBase中fit并行启动的子进程执行的gmm cluster函数，进程函数，
    ...
    clf_component_dict = {}
    cluster_df_dict = {}
    # 启动多进程进度显示UltronMulPidProgress
    with Progress(len(sub_ncs), 'gmm fit') as progress:
...
class MainBase(Base):
    ...
    def ump_main_clf_dump(cls,
                          ...
                          show_info=False):
        """
        类方法，通过交易训练集orders_pd_train构造UltronUmpMainBase子类对象，透传brust_min，p_ncs等参数
        ...
        :return: UltronUmpMainBase子类对象实例
        """
        ump = cls(orders_pd_train, market_name=market_name)
        # 训练抽取分类簇，根据brust_min
        ump.fit(p_ncs=p_ncs, brust_min=brust_min)
        ump.dump_clf()
    ...
    def __init__(self,
        ...
        :param orders_pd: 回测结果生成的交易订单构成的pd.DataFrame对象, 最好是经过度量类
                          UltronMetricsBase对象进行度量fit_metrics之后的orders_pd
        ...
        if isinstance(market_name, UMPEnv.EMarketTargetType):
            market_name = market_name.value

        # predict或者训练的情况都需要对应裁判的唯一名称, 默认使用对应市场的字符串名字 eg，'us'， 'cn'
        self.market_name = UMPEnv.g_market_target.value if market_name is None else market_name
        ...
        if orders_pd is not None and 'profit_cg' not in orders_pd.columns:
            # profit_cg等度量参数是要在UltronMetricsBase结束后才会有
            self.log_func(
                'you do better UltronMetricsBase.fit_metrics in orders_pd!!!!')
            from ultron.ump.metrics import base as metrics_base
            # 这里只做fit_metrics_order，没做fit_metrics因为比如期货，比特币会有自己的度量类，使用通用的fit_metrics_order
            metrics_base(orders_pd, None, None, None).fit_metrics_order()
        # 实例化特征构造对象self.fiter
        self.fiter = self.fiter_cls(orders_pd=self.orders_pd, **kwarg)
        """
            self.fiter是UltronMLPd子类对象，在init中即通过make_xy筛选出orders_pd中需要的训练集特征，在子类
            实现的make_xy函数都被主裁装饰器函数ump_main_make_xy装饰对筛选出的训练集特征进行转换矩阵，提取x,
            y序列等统一操作，详阅UltronUmpMainDeg等示例主裁具体子类
    ...
    def fit(self, p_ncs=None, threshold=0.65, brust_min=True):
        """
        ...
        if p_ncs is None:
            if self.fiter().df.shape[0] < K_DEFAULT_NCS_MAX:
                ...
                ncs = np.arange(K_DEFAULT_NCS_RANG.start,
                                ...
                                K_DEFAULT_NCS_RANG.step)
            else:
                # 交易单数量 > K_DEFAULT_NCS_MAX时，重新计算start，stop，step
                ncs_stop = int(
                    math.ceil(self.fiter().df.shape[0] *
                              K_DEFAULT_NCS_RANG.stop / K_DEFAULT_NCS_MAX))
        ...
        if ncs[-1] >= self.fiter().df.shape[0] / 2:
            ...
            ncs = np.arange(int(self.fiter().df.shape[0] / 4),
                            int(self.fiter().df.shape[0] / 2))
        ...
        clf_component_dict = {}
        cluster_df_dict = {}
        if self.fiter().df.shape[0] < 10000:
            # 交易单总数小于1000个，单进程
            clf_component_dict, cluster_df_dict = _do_gmm_cluster(
                ncs,
                self.fiter().x, df, threshold)
        else:
            n_jobs = g_cpu_cnt
            # 根据进程数切割ncs为n_jobs个子ncs形成ncs_group
            ncs_group = self._sub_ncs_split(ncs, n_jobs)
            parallel = Parallel(n_jobs=len(ncs_group),
                                verbose=0,
                                pre_dispatch='2*n_jobs')

            out = parallel(
                delayed(_do_gmm_cluster)(sub_ncs, self.fiter().x, df,
                                         threshold) for sub_ncs in ncs_group)
    ...
    def _fit_cprs(self):
        ...
        for cluster_df_key in self.nts:
            ...
            """
            loss_rate = df_cluster['result'].value_counts(
            )[0] / df_cluster['result'].value_counts().sum()
            # 在fit开始时使用df['ind'] = np.arange(0, df.shape[0])就是为了这里与原始交易单子保留对应关系
            cluster_order_df = self.fiter.order_has_ret.iloc[
                df_cluster.ind.values]
    ...
    def _fit_brust_min(self, brust_min):
        ...
        # 通过最优参数对分类簇集合进行筛选
        self.llps = self.cprs[(self.cprs['lps'] <= brust_min[0])
                              & (self.cprs['lms'] <= brust_min[1]) &
    ...
    def brust_min(self):
        ...
        factor_step = 3  #1 #if UltronEnv._g_enable_example_env_ipython else 3
        ...
        if lrs_min == lrs_max and lrs_min > 0:
            lrs_min -= 0.05
        ###
        ...
        bnds = (slice(lps_min, 0, lps_step), slice(lms_min, 0, lms_step),
                slice(lrs_min, lrs_max, lrs_step))
        """
            eg: bnds形如
        ...
        for bnds_pos in (0, 1, 2):
            progress *= len(
                np.arange(bnds[bnds_pos].start, bnds[bnds_pos].stop,
                          bnds[bnds_pos].step))
        # 进行最优时使用的进度条
        self.brust_progress = Progress(
            progress, 0,
            '{}: brute min progress'.format(self.__class__.__name__))
        # 为提高运行效率，不用每次都使用_calc_llps_improved计算
        self.brust_cache = dict()
        brust_result = sco.brute(self.min_func_improved,
                                 bnds,
                                 full_output=False,
                                 finish=None)
        return brust_result
    ...
    def min_func(self, l_pmr):
        ...
        llps = self.cprs[(self.cprs['lps'] <= l_pmr[0])
                         & (self.cprs['lms'] <= l_pmr[1]) &
                         (self.cprs['lrs'] >= l_pmr[2])]
        """
    ...
    def _calc_llps_improved(self, llps):
        ...
        # 按照比例有可能提升的效果
        improved = (effect_num /
                    self.fiter.order_has_ret.shape[0]) * (loss_rate - win_rate)
        """
            eg：(effect_num / self.fiter.order_has_ret.shape[0]) * (loss_rate - win_rate)
                effect_num ＝ 10, self.fiter.order_has_ret.shape[0] ＝ 100
                loss_rate ＝ 0.8， win_rate ＝0.2
    ...
    def dump_file_fn(self):
        """
            主裁本地缓存的存储路径规则：
            UltronEnv.g_project_data_dir ＋ 'ump/ump_main_' ＋ market_name + self.class_unique_id()
        """

        # TODO 如果有裁判覆盖，保留备份，显示通知
        model_path = os.environ[
            'UMP_MODEL_PATH'] if 'UMP_MODEL_PATH' in os.environ else g_project_data_dir
        unique_ump_name = 'ump/ump_main_{}_{}'.format(self.market_name,
                                                      self.class_unique_id())
        return os.path.join(model_path, unique_ump_name)
    ...
    def dump_clf(self, llps=None):
        ...
        3. 通过UltronFileUtil.dump_pickle最终将元素序列化保存在本地
        ...
        for clf_cluster in llps.index:
            ...
            """
        # 通过UltronFileUtil.dump_pickle将clf_cluster_dict进行序列化
        file_util.dump_pickle(clf_cluster_dict,
                              self.dump_file_fn(),
                              how='zero')
    ...
    def predict(self, x, need_hit_cnt=1):
        ...
        clf_cluster_dict = MainBase.dump_clf_manager.get_ump(self)
        """
            eg: clf_cluster_dict
                {
                '14_7': (GaussianMixture(covariance_type='full', init_params='kmeans', max_iter=100,
    ...
    def predict_kwargs(self, w_col=None, need_hit_cnt=1, **kwargs):
        ...
        for col in w_col:
            if col not in kwargs:
                ...
                raise ValueError('{0} col not in kwargs!'.format(col))
        # 将kwargs中的特征值转换为x
    ...
    def choose_cprs_component(self, llps=None):
        ...
        if llps is None:
            llps = self.llps
        improved, effect_num, loss_rate, nts_pd = self._calc_llps_improved(
            llps)
        ...
        nts_pd['profit_cg'] = nts_pd.apply(
            lambda x: self.fiter.order_has_ret.ix[int(x.ind)].profit_cg,
            axis=1)
        """
    ...
    def hit_cnt(self, x):
        ...
        """
        # 统一从CachedUmpManager中获取缓存ump，没有缓存的情况下load_pickle
        clf_cluster_dict = Base.dump_clf_manager.get_ump(self)
        hit_cnt = 0
        for clf, cluster in clf_cluster_dict.values():
            """
                eg: (clf, cluster)
