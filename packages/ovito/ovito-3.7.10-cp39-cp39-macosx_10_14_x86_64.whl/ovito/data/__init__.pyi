...
.. seealso:: 
    ...
    def identifier(self) -> str:
        """The unique identifier string of the data object. It serves as lookup key in object dictionaries, for example the :py:attr:`DataCollection.tables` collection, or as a target name in various places where a data object needs to be referenced by name, e.g. the `TimeAveragingModifier.operate_on` field. 
...
Data objects generated by modifiers in a pipeline typically have an automatically assigned identifier, as documented in the description of the respective modifier. When writing your own modifier function, you are responsible for giving new data objects created by your modifier function a meaningful identifier, so that subsequent modifiers in the pipeline can refer to these data objects."""
    ...
    def make_mutable(self, subobj: DataObjectType) -> DataObjectType:
        """This helper method requests a deep copy of *subobj*, which must be a child :py:class:`DataObject` of this parent :py:class:`DataObject`. A copy will only be made in case the sub-object is currently referenced by at least one more parent object. If, however, the sub-object is exclusively owned by this :py:class:`DataObject`, no copy is made and the original sub-object is returned as is. The returned object is safe to modify without unexpected side effects, because any shared ownership is converted to an exclusive ownership by the method. 
...
A data collection essentially consists of a bunch of `DataObjects`, which are all stored in the :py:attr:`DataCollection.objects` list. Typically, you don't access the data objects through this list directly but rather use one of the special accessor fields provided by the :py:class:`DataCollection` class, which give more convenient access to data objects of a particular kind. For example, the :py:attr:`surfaces` dictionary provides key-based access to all the :py:class:`SurfaceMesh` instances currently in the data collection. 
...
```"""
    ...
    def next_face_edge(self, edge: int) -> int:
        """Given the halfedge *edge* bounding some face, this method returns the following halfedge when going around the face in forward direction (counter-clockwise - when looking on the face from the front). All halfedges of the face form a circular sequence - without a particular beginning or end. You can loop over this circular sequence in forward or backward direction with the :py:meth:`next_face_edge` and :py:meth:`prev_face_edge` methods. Given some mesh face, you can obtain a first halfedge through the :py:meth:`first_face_edge` method. 
...
```"""
    ...
    def prev_face_edge(self, edge: int) -> int:
        """Given the halfedge *edge* bounding some face, this method returns the previous halfedge going around that face in backward direction (clockwise - when looking on the face from the front). All halfedges of a face form a circular sequence - without a specific beginning or end. You can loop over this circular sequence in forward or backward direction with the :py:meth:`next_face_edge` and :py:meth:`prev_face_edge` methods."""
        ...
...
:guilabel:`Filled`                                  int        
:guilabel:`Selection`                               int        
:guilabel:`Surface Area`                            float      
:guilabel:`Volume`                                  float      
=================================================== ========== ===================================================
...
Note that the returned Numpy array is a copy of the internal data stored by the :py:class:`SurfaceMesh`."""
    ...
    def get_face_vertices(self) -> numpy.typing.NDArray:
        """Returns an array with the vertex indices of all mesh faces. 

If all :math:`n` faces of the surface mesh have the same, uniform number of vertices, :math:`m`, for example, if they are all triangles, then this method returns a NumPy array of shape :math:`(n, m)` containing the zero-based vertex indices. Otherwise, it returns a list of lists, in which each nested list may have different length."""
        ...
...
```"""
    ...
    def create_faces(self, vertex_lists: Union[Sequence[Sequence[int]], numpy.typing.ArrayLike]) -> int:
        """Adds several new polygonal faces to the mesh. 

:param vertex_lists: A sequence of sequences, one for each face to be created, which specify the vertex indices to be connected by the new mesh faces.
...
```"""
    ...
    def create_vertices(self, coords: numpy.typing.ArrayLike) -> int:
...
.. note::
  ...
  For this method to work, all faces of the mesh must have been created using the same winding order. That means their vertices   must all be ordered either clockwise or counter-clockwise when viewed from the same side of the surface manifold.   Only then do the halfedges of neighboring faces run in opposite directions and can be successfully paired by this method. 

.. seealso:: :py:attr:`SurfaceMeshTopology.is_closed`, :py:meth:`SurfaceMeshTopology.opposite_edge`, :py:meth:`SurfaceMeshTopology.has_opposite_edge`"""
        ...
...
Each bar is then assigned a text label by creating an :py:class:`ElementType` instance and adding it to the `Property.types` list:
