...
The OpenGL renderer makes use of GPU graphics hardware in your computer to accelerate the generation of images.
See the corresponding user manual page for more information. 

Enabling OpenGL support

This rendering backends requires an environment where OpenGL graphics support is available. Standalone Python scripts typically 
run in a headless environment, i.e., within a terminal lacking graphics support, which prevents the :py:class:`OpenGLRenderer` from
...
you have to take one of the following steps to enable OpenGL graphics support -- or completely avoid the problem by 
using one of the :ref:`software-based rendering backends <manual:rendering>` instead.

When you import the ``ovito`` Python module in a Python program, the module sets up a headless environment by default, in which OpenGL rendering
is *not* available. To request a proper graphics environment, two methods are available:

  1. Setting the environment variable ``OVITO_GUI_MODE=1`` before import of the ``ovito`` package or any of its sub-modules. 

    You can set this variable either externally, before launching your Python script program, or within the Python program 
    by including the following lines *before* the first ``import ovito`` statement at the top of your .py file::
  ...
  2. Create a Qt application object with GUI support before importing the ``ovito`` module.
    ...
    Standard behavior of the ``ovito`` module is to create a global `QCoreApplication <https://doc.qt.io/qtforpython/PySide6/QtCore/QCoreApplication.html>`__ object 
    during module import, which is only good for headless operation, e.g. file I/O and pipeline computations, but not sufficient for OpenGL-based rendering. 
    If, however, a global Qt application object already exists at the time the ``ovito`` module gets loaded, then OVITO will make use of that pre-existing
    ...
    .. note::
      ...
      also be setting up a global Qt application object, which will subsequently be shared with the ovito module. Furthermore, if you are executing 
...
On Linux/Unix systems, an X11 or Wayland display server is required to initialize a session with OpenGL graphics support. When you request a graphics session 
with one of the methods described above, the Qt framework will try to establish a connection to the X11/Wayland server.
You have to make sure this succeeds, e.g., by making sure the `DISPLAY environment variable <https://stackoverflow.com/questions/20947681/understanding-linux-display-variable>`__ 
is set correctly; otherwise Qt reports an error and the program will quit.  

Remote servers and HPC clusters, which are typically accessed via SSH terminals, usually do not provide a graphical desktop environment
that would allow the use of OpenGL functions by OVITO. But even in such a headless environment, it may be possible to
provide a `virtual X server <https://en.wikipedia.org/wiki/Xvfb>`__ to the Python application, e.g., using the `xvfb-run <https://manpages.ubuntu.com/manpages/xenial/man1/xvfb-run.1.html>`__ command."""
...
The standard particle properties ``Force``, ``Displacement``, and ``Dipole`` already have an existing :py:class:`VectorVis` element
