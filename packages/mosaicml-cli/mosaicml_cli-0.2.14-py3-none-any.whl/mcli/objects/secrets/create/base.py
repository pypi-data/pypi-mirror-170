...
class SecretValidator():
    ...
    def __init__(self):
        ...
        self.existing_secrets = self.get_existing_secrets(conf.clusters[0])
        self.existing_secret_names = {secret.name for secret in self.existing_secrets}
        super().__init__()

    @staticmethod
    def get_existing_secrets(cluster: MCLICluster) -> List[Secret]:
        secret_manager = SecretManager(cluster)
