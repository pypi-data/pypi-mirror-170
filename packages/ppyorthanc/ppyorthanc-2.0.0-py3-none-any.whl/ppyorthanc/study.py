...
from ppyorthanc.util import make_datetime_from_dicom_date
...
class Study:
    ...
    def __init__(
        self, study_id: str, client: Orthanc, study_information: Dict = None
    ) -> None:
        """Constructor
    ...
    def get_main_information(self) -> Dict:
        ...
        if self.information is None:
            self.information = self.client.get_studies_id(self.id_).json()
    ...
    def get_zip(self) -> bytes:
        ...
        Examples
        --------
        >>> from ppyorthanc import Orthanc, Study
        >>> a_study = Study(
        ...     'STUDY_IDENTIFIER',
        ...
        """
        return self.client.get_studies_id_archive(self.id_).content

    def build_series(self) -> None:
        """Build a list of the study's series."""
        series_information = self.client.get_studies_id_series(self.id_).json()
    ...
    def remove_empty_series(self) -> None:
        """Delete empty series."""
        self._series = list(filter(lambda series: series.instances != [], self._series))
