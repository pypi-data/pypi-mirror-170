...
class Patient:
    ...
    def __init__(
        self, patient_id: str, client: Orthanc, patient_information: Dict = None
    ) -> None:
        """Constructor
    ...
    def get_main_information(self) -> Dict:
        ...
        if self.information is None:
            self.information = self.client.get_patients_id(self.id_).json()
    ...
    def get_zip(self) -> bytes:
        ...
        Examples
        --------
        >>> from ppyorthanc import Orthanc, Patient
        >>> a_patient = Patient(
        ...
        """
        return self.client.get_patients_id_archive(self.id_).content
    ...
    def get_patient_module(self, simplify: bool = False, short: bool = False) -> Dict:
        ...
        return dict(self.client.get_patients_id_module(self.id_, params=params).json())
    ...
    def is_protected(self) -> bool:
        ...
        """
        return 1 == self.client.get_patients_id_protected(self.id_).json()
    ...
    def set_to_protected(self):
        ...
        self.client._put(f"{self.client.url}/patients/{self.id_}/protected", json=1)
    ...
    def set_to_unprotected(self):
        ...
        """
        # As of version 1.11.1, the Orthanc OPEN API file has missing information
        self.client._put(f"{self.client.url}/patients/{self.id_}/protected", json=0)
    ...
    def build_studies(self) -> None:
        ...
        studies_information_response = self.client.get_patients_id_studies(self.id_)
        if studies_information_response.status_code == 200:
            studies_information = studies_information_response.json()
        else:
            studies_information = []
    ...
    def anonymize(
        ...
        anonymous_patient_response = self.client.post_patients_id_anonymize(
            self.id_,
            json={"Remove": remove, "Replace": replace, "Keep": keep, "Force": force},
        )

        return anonymous_patient_response
    ...
    def remove_empty_studies(self) -> None:
        ...
        self._studies = list(filter(lambda s: s.series != [], self._studies))
