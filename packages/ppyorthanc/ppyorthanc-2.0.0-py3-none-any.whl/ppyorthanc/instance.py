...
class Instance:
    ...
    def __init__(
        self, instance_id: str, client: Orthanc, instance_information: Dict = None
    ) -> None:
        """Constructor
    ...
    def get_dicom_file_content(self) -> bytes:
        ...
        Examples
        --------
        >>> from ppyorthanc import Instance
        ...
        """
        return self.client.get_instances_id_file(self.id_).json()
    ...
    def get_main_information(self) -> Dict:
        ...
        if self.information is None:
            self.information = self.client.get_instances_id(self.id_).json()
    ...
    def creation_date(self) -> datetime:
        ...
        datetime
            Creation Date
        """
        date_string = self.get_main_information()["MainDicomTags"][
            "InstanceCreationDate"
        ]
        time_string = self.get_main_information()["MainDicomTags"][
    ...
    def tags(self) -> Dict:
        ...
        """
        return dict(self.client.get_instances_id_tags(self.id_).json())
    ...
    def simplified_tags(self) -> Dict:
        ...
        """
        return dict(
            self.client.get_instances_id_tags(
                self.id_, params={"simplify": True}
            ).json()
        )
    ...
    def get_content_by_tag(self, tag: str) -> Any:
        ...
        """
        result = self.client.get_instances_id_content_tags_path(
            id_=self.id_, tags_path=tag
        )

        try:
            return result.content.decode("utf-8").strip().replace("\x00", "")
