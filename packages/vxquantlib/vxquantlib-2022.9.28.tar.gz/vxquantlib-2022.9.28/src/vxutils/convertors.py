...
def _auto_detect_timeformater(time_str: str) -> str:
    """自动监测时间字符串格式
...
@singledispatch
def to_datetime(date_time: Any) -> datetime.datetime:
    """
...
def _(date_time: str) -> datetime.datetime:
    """处理str类型"""
    return parser.parse(date_time).astimezone(local_tzinfo)


@to_datetime.register(float)
@to_datetime.register(int)
def _(date_time: Union[float, int]) -> datetime.datetime:
    """处理int类型"""
    if date_time == float("inf"):
        return datetime.datetime.strptime(
            "2199-12-31 23:59:59", "%Y-%m-%d %H:%M:%S"
...
def _(date_time: time.struct_time) -> datetime.datetime:
    """处理struct_time类型"""
    return datetime.datetime(*date_time[:6]).astimezone(local_tzinfo)


@to_datetime.register(datetime.date)
def _(date_time: datetime.date) -> datetime.datetime:
    """处理date类型"""
    return datetime.datetime(
        date_time.year,
        date_time.month,
...
def _(date_time: datetime.datetime) -> datetime.datetime:
...
def _(date_time: str) -> float:
    """处理str类型"""
    return parser.parse(date_time).timestamp()
