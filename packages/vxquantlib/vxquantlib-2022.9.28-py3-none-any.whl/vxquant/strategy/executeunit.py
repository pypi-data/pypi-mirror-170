...
from vxquant.model.exchange import OrderStatus
...
def on_reply_sync_orders(context: vxContext, event: vxEvent) -> None:
    ...
    for trade_id in missing_trades:
        vxscheduler.handler.trigger(
            vxEvent(type="on_broker_trade_status", data=broker_trades[trade_id])
        )
    ...
    for gateway_order in gateway_orders:
        ...
        vxscheduler.handler.trigger(
            vxEvent(type="on_broker_order_status", data=broker_order)
        )
...
@vxscheduler.handler("on_submit_broker_order_batch")
def on_recv_gateway_order_batch(context, event):
    """收到gateway批量下单"""
    logger.info(f"收到{event.channel} 批量委托订单: {len(event.data)}个")
    gateway_orders = event.data
    with context.lock:
        broker_orders = context.tdapi.order_batch(*gateway_orders)
        for gateway_order, broker_order in zip(gateway_orders, broker_orders):
            gateway_order.exchange_order_id = broker_order.exchange_order_id
            gateway_order.status = OrderStatus.New
            context.gateway_orders[gateway_order.exchange_order_id] = gateway_order
            context.publisher("on_order_status", gateway_order)
            logger.info(
                f"通过{context.publisher.channel_name} 上报 on_order_status 更新 :"
                f" {gateway_order.order_id} === {gateway_order.exchange_order_id} ==="
                f" {gateway_order.status}"
            )


@vxscheduler.handler("on_submit_broker_order_batch_cancel")
def on_recv_gateway_order_batch_cancel(context, event):
    logger.info(f"收到{event.channel} 批量取消委托订单: {len(event.data)}个.")
    gateway_orders = event.data
    with context.lock:
        cancel_orders = []
        for gateway_order in gateway_orders:
            if gateway_order.exchange_order_id:
                cancel_orders.append(gateway_order)
            else:
                gateway_order.status = OrderStatus.Canceled
                context.publisher("on_order_status", gateway_order)
                logger.info(
                    f"通过{context.publisher.channel_name} 上报 order_order_status 更新 :"
                    f" {gateway_order}"
                )
        if cancel_orders:
            context.tdapi.order_cancel(*cancel_orders)
    ...
    if context.pending_broker_trades:
        ...
        for broker_trade in broker_trades.values():
            if broker_trade.exchange_order_id in context.gateway_orders:
                vxscheduler.handler.trigger(
                    vxEvent(
                        type="on_broker_execution_reports_status", data=broker_trade
                    )
                )
            else:
                with context.lock:
