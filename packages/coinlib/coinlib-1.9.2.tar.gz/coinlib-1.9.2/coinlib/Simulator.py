...
class Simulator:
    ...
    def testBroker(self, broker_id: str, broker_login_data: BrokerTestLoginData):
        ...
        info = self.stub.simulateBroker(brokerInputs, metadata=self.registrar.getAuthMetadata())
        exit = False
        if info.success:
    ...
    def simulateFeature(self, identifier, options=None):
        ...
        info = self.stub.simulateFeature(notificationInputs, metadata=self.registrar.getAuthMetadata())
        exit = False
        if info.success:
    ...
    def testNotification(self, identifier, channels, inputs):
        ...
        info = self.stub.simulateNotification(notificationInputs, metadata=self.registrar.getAuthMetadata())
        exit = False
        if info.success:
    ...
    def testSymbolBroker(self, symbolbroker_id, testSymbol1, testSymbol2, options):
        ...
        info = self.stub.simulateSymbolBroker(symbolBrokerInputs, metadata=self.registrar.getAuthMetadata())
        exit = False
        if info.success:
            self._registrar.logger.info("Your symbol Broker does work, read message after!")
        else:
    ...
    def statsMethod(self, methods, name="simulatorDefaultWorkspace"):
        ...
        info = self.stub.simulateStatisticsMethod(simulatorChartConfig, metadata=self.registrar.getAuthMetadata())
    ...
    def statsRule(self, name="simulatorDefaultWorkspace"):
        ...
        info = self.stub.simulateStatisticsRule(simulatorChartConfig, metadata=self.registrar.getAuthMetadata())
