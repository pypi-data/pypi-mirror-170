...
class SAPAbapODataConnector(HTTPConnector):
    ...
    # def menu_action_metadata_info(self):
    #     if self.metadata_util == None or self.endpoint == None:
    #         print("wrong context for metadata information")
    #     else:
    #         entries = {}
    #         entries[SAPAbapODataConnector.MENU_METADATA_XML] = f"Display metadata XML for selected endpoint {self.endpoint}"
    #         entries[SAPAbapODataConnector.MENU_METADATA_JSON] = f"Display metadata JSON for selected endpoint {self.endpoint}"

    #         #show menu
    #         valid = True
    #         while valid == True:
    #             action = MenuSelection("Select Metadata Option", entries, False, True).show_menu()
    #             if action.is_cancel_entered() == True:
    #                 valid = False
    #             elif action.is_selected() == True:
    #                 selected_action = action.get_selection()
    #                 if selected_action == SAPAbapODataConnector.MENU_METADATA_XML:
    #                     print(self.metadata_util.get_metadata_xml())
    #                 elif selected_action == SAPAbapODataConnector.MENU_METADATA_JSON:
    #                     print(self.metadata_util.get_metadata_json())
    #                 else:
    #                     print(f"unknown action {selected_action}")
    ...
    def menu_process_selection(self, selected: str, text: str = None):
        try:
            if selected == SAPAbapODataConnector.MENU_FIND_ENDPOINTS:
                ...
                self.menu_result_single = self.menu_result.menu_get_result_single() 
                if self.menu_result_single == None:
                    print("No single record selected")
                else:
                    print("Single record selected: ", self.menu_result_single.get_description())               
    ...
    def get_endpoints(self, max_results: int = 10000):
        ...
        http_code = request.get(
            "/sap/opu/odata/IWFND/CATALOGSERVICE;v=2/ServiceCollection", params)
...
class SAPAbapODataRequest(HTTPRequest):
    ...
    def select(self, max_rows: int = 1000, odataFilter: str = None, entity_key: str = None, add_params: dict = None) -> Result:
        # prepare url prefix
        url_prefix = "/sap/opu/odata/sap/"
