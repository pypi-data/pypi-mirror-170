...
class RabbitmqPipeline(object):
    ...
    def from_crawler(cls, crawler):
        if hasattr(crawler.spider, 'items_key'):
            ...
            item_key = 'items_{spider_name}'.format(spider_name=crawler.spider.name)
        return cls(item_key=item_key, connection_url=crawler.settings.get('RABBITMQ_CONNECTION_PARAMETERS'))

    def process_item(self, item, spider):
        data = self.serialize(item)

        self.channel.basic_publish(exchange='', routing_key=self.item_key, body=data)
        return item
