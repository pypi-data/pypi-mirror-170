...
class DNNForecastWrapper(torch.nn.Module, ForecastModelWrapperBase):
    ...
    def _pipeline_forecast_quantiles_internal(
        ...
        raise ConfigException._with_error(
            AzureMLError.create(
                ...
                scenario_name="forecasting TCN model",
                reference_code=ReferenceCodes._TCN_FORECAST_QUANTILES_UNSUPPORT)
        )
    ...
    def _rolling_forecast_internal(self, preprocessors: List[Any], forecaster: Any,
        ...
        for tsid, df_one in Xy_ts_features.groupby(self.grain_column_names):
            ...
            window_sz = lookback + horizon
            n_windows = np.ceil((df_combined_one.shape[0] - window_sz) / step + 1).astype(int)
            required_len = (n_windows - 1) * step + window_sz if n_windows > 0 else window_sz
            padding_len = required_len - df_combined_one.shape[0]
            if padding_len > 0:
        ...
        for idx in range(len(ds_pred)):
            ...
            df_samp_lb = df_samp.iloc[:lookback]
            df_samp_pred = df_samp.iloc[lookback:]
