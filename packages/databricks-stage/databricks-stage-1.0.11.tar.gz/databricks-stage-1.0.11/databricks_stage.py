...
def run():
    ...
    if args.access_key_id and args.secret_access_key:
        ...
        url = f"s3a://{args.access_key_id}:{args.secret_access_key}@{url}"

    # GCP Cloud Storage.
    elif args.service_account_private_key_id and \
            args.service_account_private_key and \
            args.service_account_email:
        existing_email = spark.conf.get("fs.gs.auth.service.account.email", None)
        if existing_email:
            raise ValueError(f"Databricks cluster already has service account \
                             with email ({existing_email}) configured, conflicting with the \
                             Native Stage service account ({args.service_account_email})")
