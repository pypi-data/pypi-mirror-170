...
.. code:: python
  ...
  ADMIN_IPS = ['172.31.0.0/16']
  ADMIN_OTP_KEY = "quDsdI&*+HO)*&[%RNS#&]ihi4"
  RESOURCE_DIR = os.path.join (os.path.dirname (__file__), 'resources')

  if __name__ == "__main__":
      skitai.add_option ('-i=INTERVAL_SEC', '--interval=', 'indexing or replicating interval seconds', 30)
      skitai.add_option (None, '--index', 'enable indexing')
      skitai.add_option (None, '--replica=ORIGIN_SERVER', 'enable replicating from remote, ex) http://localhost/cols')
      INTERVAL_SEC = skitai.options.get ('--interval')

      if skitai.options.get ('--replica') and '--replica' in skitai.options:
          raise RuntimeError ('complicated options, cannot use both --replica and --index')

      if "--index" in skitai.options:
          os.system ("delune index --interval {} -v {} -d".format (INTERVAL_SEC, RESOURCE_DIR))
      elif "--replicate" in skitai.options:
          os.system ("delune replicate --interval {} -v {} --origin {} -d".format (INTERVAL_SEC, RESOURCE_DIR, skitai.options.get ('--replica')))

      with skitai.preference () as pref:
          pref.config.ADMIN_IPS = ADMIN_IPS
          pref.config.ADMIN_OTP_KEY = ADMIN_OTP_KEY
          pref.config.resource_dir = RESOURCE_DIR
          skitai.mount ("/", delune, pref)

      skitai.run (workers = 2, port = 5491, name = 'deluned')
