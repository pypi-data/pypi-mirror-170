        ...
        if ds.HasField("batch_data_source"):
            ...
            if batch_data_source.HasField("snowflake"):
                ...
                if snowflake_args.HasField("table"):
                    if snowflake_args.HasField("query"):
                ...
                elif snowflake_args.HasField("query"):
                    sql_str = snowflake_args.query
                else:
...
class _ODFVPipelineBuilder:
    ...
    def _transformation_node_to_df(self, transformation_node: TransformationNode) -> "snowflake.snowpark.DataFrame":
        ...
        output_df = self._call_udf(ondemand_udf, input_columns, self._input_df)
        # rename feature columns
        if self._append_prefix:
            for column in self._output_schema.keys():
                output_df = output_df.withColumnRenamed(column, self._namespace + SEPERATOR + column)
        return output_df
