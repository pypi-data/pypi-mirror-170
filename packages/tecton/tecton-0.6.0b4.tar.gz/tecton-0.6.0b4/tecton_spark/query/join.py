    ...
    def to_dataframe(self, spark: pyspark.sql.SparkSession) -> pyspark.sql.DataFrame:
        ...
        if conf.get_bool("ENABLE_TEMPO"):
            ...
            left_full_cols = (
                ...
                + [F.lit(None).alias(f"{self.right_container.prefix}_{x}") for x in right_nonjoin_cols]
            )
            right_full_cols = (
                [F.lit(False).alias("is_left")]
                + [F.col(x) for x in common_cols]
                + [F.lit(None).alias(x) for x in left_nonjoin_cols]
                + [F.col(x).alias(f"{self.right_container.prefix}_{x}") for x in right_nonjoin_cols]
            )
            ...
            for column_name in right_nonjoin_cols:
                right_window_funcs.append(
                    F.last(F.col(f"{self.right_container.prefix}_{column_name}"), ignorenulls=True)
                    .over(window_spec)
                    .alias(f"{self.right_container.prefix}_{column_name}")
                )
            ...
            res = (
                union.select(common_cols + left_nonjoin_cols + right_window_funcs)
                .filter(f"is_left")
                .drop("is_left", ASOF_JOIN_TIMESTAMP_COL_1, ASOF_JOIN_TIMESTAMP_COL_2)
            )
            return res
