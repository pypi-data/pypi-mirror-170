    ...
    async def get_device_async(self, device_id) -> Dict:
        ...
        if self._cacheBinarySensors is None or timeNow - self._cacheStampP > 2.0:
            ...
            if self._cacheBinarySensors is None:
                _LOGGER.debug("__init__.py.get_device_async(): cacheBinarySensors is none, update from Lupusec System needed.")
            if timeNow - self._cacheStampP > 2.0:
                _LOGGER.debug("__init__.py.get_device_async(): time elapsed, update from Lupusec System needed.")    
            self._cacheStampP = timeNow
        ...
        _LOGGER.debug("__init__.py.get_device_async(): Devices do not need update from Lupusec System...using Cache")
        if (len(self._cacheBinarySensors) != 0):  
            ...
            for sensor in self._cacheBinarySensors:
                ...
                if (sensor["sid"] == device_id):
                    ...
                    _LOGGER.error("__init__.py.get_device_async(): no device found with sid=%s", device_id)
                    return {}
        else :
