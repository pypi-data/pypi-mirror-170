...
class Specification(Mapping):
    ...
    def _get_spec_version(spec):
        ...
        try:
            ...
            err = (
                "Unable to convert version string to semantic version tuple: " "{version_string}."
            )
...
class Swagger2Specification(Specification):
    ...
    openapi_schema = json.loads(
        pkgutil.get_data("especifico", "resources/schemas/v2.0/schema.json"),  # type: ignore
    )
...
class OpenAPISpecification(Specification):
    ...
    openapi_schema = json.loads(
        pkgutil.get_data("especifico", "resources/schemas/v3.0/schema.json"),  # type: ignore
    )
