...
class CloudBeta(OldCloud, Generic[IsAsynchronous]):
    ...
    async def _create_cluster(
        ...
        *,
        ...
        auto_env: Optional[
            Tuple[List[magic.PipPackageInfo], List[magic.CondaPackageInfo]]
        ] = None,
    ) -> int:
        ...
        if auto_env is not None:
            pip_env = auto_env[0]
            env = []
            for pip_dep in pip_env:
                if not isinstance(pip_dep["sdist"], bool):
                    files.append(typing.cast(BinaryIO, pip_dep["sdist"]))
                    pip_dep["sdist"] = True
                    env.append(pip_dep)
                else:
                    env.append(pip_dep)

            data["auto_env"] = {"pip": pip_env, "conda": auto_env[1]}
        ...
        if files:
            upload_resp = await self._do_request(
                "POST",
                self.server
                + f"/api/v2/clusters/account/{account}/{response_json['id']}/sdist_upload",
                data=[("files", file) for file in files],
            )
            # TODO: Cancel cluster if this fails?
            # maybe upload before cluster creation instead?
