...
def next_longest(matches: MatchSet):
    for match in sorted_by_longest_match(matches):
        yield match
...
class Concatenation:  # pylint: disable=too-few-public-methods
    ...
    def lparse_old(self, source: Source, start: int):
...
class Repetition:  # pylint: disable=too-few-public-methods
    ...
    def lparse_(self, source: Source, start: int) -> Matches:
        cache_key = (source, start)
        try:
            cached_matchset = self.lparse_cache[cache_key]
        except KeyError:
        ...
        match_lists = [match_list]
        match_count = self.repeat.min
        while True:
            if self.repeat.max is not None and match_count == self.repeat.max:
                break
            current_match_list: list[Match] = []
            for match in match_lists[-1]:
                try:
                    for m in self.element.lparse(source, match.start):
            ...
            if current_match_list:
                ...
                print([[str(n) for n in match.nodes] for match in current_match_list])
                match_lists.append(current_match_list)
                match_count = match_count + 1
                print(match_lists)
                print(match_count)
            else:
                break
        final_match_list = list(match for match in chain(*reversed(match_lists)))
        self.lparse_cache[cache_key] = final_match_list
