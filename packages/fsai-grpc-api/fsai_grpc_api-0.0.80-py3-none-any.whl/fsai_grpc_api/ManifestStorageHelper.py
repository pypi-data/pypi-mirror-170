...
class ManifestStorageHelper:
    ...
    @beartype
    def get_detected_at(self, detected_at: Union[str, None]) -> Timestamp:
    ...
    @beartype
    def get_geo_bbox(self, detection: dict) -> Union[dict, None]:
    ...
    def update_detection_instance_geo_bbox(
        ...
        try:
            ...
            logger.info(
                "Updated Detection Instance GeoBbox: {}".format(
                    MessageToDict(detection_instance)
                )
            )
    ...
    def process(self, workflow_name: str, source_name: str) -> None:
        ...
        for line in self.manifest_reader:
            ...
            for detection in line["detections"]:
                ...
                geo_bbox = self.get_geo_bbox(detection)

                detected_at_raw = get(detection, "detected_at", None)

                detected_at = self.get_detected_at(detected_at_raw)
