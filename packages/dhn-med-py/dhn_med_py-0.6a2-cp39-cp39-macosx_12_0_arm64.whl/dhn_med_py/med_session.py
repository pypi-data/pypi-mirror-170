...
class MedSession():
    ...
    def getRawPage(self, start_time, end_time, n_out_samps):
        """
        Read all active channels of a MED session, by specifying start and end times.
        
        This function returns a "raw page", which includes a matrix of samples.  The samples
        are a 2-dimensional NumPy array, with the axes being channels and samples.  Such an array
        ...
        Returns
        -------
        MedSession.raw_page: dict
            data structure (member of class) is the output of this function.
        """
    
        self.raw_page = get_raw_page(self.__pointers, start_time, end_time, n_out_samps)
        return
        
    def sortChansByAcqNum(self):
        """
        Re-orders channels by acquisition_channel_number, lowest to highest.
        
        Any future reads (readByTime, readByIndex, getRawPage) will use this new ordering for
