...
class Poisson_PseudoY(PseudoY_Seeger):
    ...
    def ratefn(self, X):
        ...
        return np.log(1+s.exp(X)) + 0.0001
    ...
    def calculateELBO(self):
        ...
        # Precompute terms
        ZW = Z.dot(W.T)
        ZZWW = np.square(ZW) - s.dot(np.square(Z),np.square(W).T) + ZZ.dot(WW.T)
...
class Bernoulli_PseudoY(PseudoY_Seeger):
    ...
    def calculateELBO(self):
        ...
        lb = self.obs*tmp - np.log(1.+s.exp(tmp))
        lb[mask] = 0.
...
class Bernoulli_PseudoY_Jaakkola(PseudoY):
    ...
    def updateParameters(self, ix=None, ro=None):
        ...
        self.params["zeta"] = np.sqrt(np.square(Z["E"].dot(W["E"].T)) - s.dot(np.square(Z["E"]), np.square(W["E"].T)) + s.dot(Z["E2"],W["E2"].T))
    ...
    def calculateELBO(self):
        ...
        tmp1 = np.square(ZW) - s.dot(np.square(Z),np.square(SW).T) # this is for terms in k != k'
