...
class SpectrumTree(treemixin.DragAndDrop,
    ...
    def MoveItem(self, itemToMove, newItemParent):
        pass#print "MOVING " + itemToMove.GetText()
        movedItem = self.add_object(newItemParent, 
                                    text = itemToMove.GetText(),
                                    data = itemToMove.GetData())
        if itemToMove.GetChildrenCount() > 0:
            pass#print "%s HAS %d CHILDREN %s" % (itemToMove.GetText(), itemToMove.GetChildrenCount(),
            pass#                                 [x.GetText() for x in itemToMove.GetChildren()])
    ...
    def makeSpectralDatabase(self, evt):
        ...
        if scanData:
            ...
            pass#print "No spectra found; file not written."
        ...
        if self.GetData():
            ...
            if droptype == wx.DF_FILENAME:
                ...
                try:
                    ...
                    pass#print "Invalid drop. " + repr(err)
                    return defResult
