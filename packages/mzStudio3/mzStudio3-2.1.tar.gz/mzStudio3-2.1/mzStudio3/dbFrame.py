...
command_history_file = os.path.join(os.path.dirname(__file__), 'command_history.txt')
try:
    command_history = open(command_history_file, 'r').read().split('\n')
except IOError:
    pass#print("No command history file found, or could not be opened.")
    command_history = []
...
class dbFrame(wx.Panel):
            ...
            if self.currentFile.settings['multiFileOption'] == 'LOAD ALL':
                ...
                for name in list(file_set):
                    if name not in [x.lower() for x in self.sub_bank.files.keys()]:
                        pass#print "Loading additional file!"
                        ...
                        #pass#print currentGridFilename
    ...
    def OnClose(self, event):
        ...
        #    pass#print "%s got event but isn't being closed." % self.aui_pane.name
        #    event.Skip()
        #    return 
    ...
    def dump_bank(self):
        ...
        for member in self.sub_bank.files.keys():
            pass#print self.sub_bank.files[member].FileAbs.lower()
    ...
    def OnClick(self, event): ####################EXECUTING QUERY
        ...
        if query in command_history:
            command_history.remove(query)
        else:
            self.query.Insert(query, 0)
        command_history = [query] + command_history[:COMMAND_HISTORY_LEN-1]
        out = open(command_history_file, 'w')
    ...
    def OnLabelClick(self,event):
        ...
        if row > -1:
            ...
            if self.Check_diff:
                #pass#print self.currentFile.SearchType
                if self.currentFile.SearchType=='Mascot':
                ...
                if self.currentFile.FileAbs.lower() != currentGridFileName:
                    ...
                    ##pass#print "Attempting delete..."
                    ##pass#print self.parent.msdb.files[self.currentFile.FileAbs]
            ...
            if self.currentFile.SearchType in ["Mascot", "Proteome Discoverer"]:
                #pass#print "NTERM MODS!"
                if fixedmod == None:
                    if 'n-term' not in varmod.lower():
                        self.bpc.b.FindWindowByName("nTerm").SetValue('None')
                    if 'c-term' not in varmod.lower():
                        self.bpc.b.FindWindowByName("cTerm").SetValue('None')
                    fixedmod = ''
                    self.bpc.b.FindWindowByName("nTerm").SetValue('None')
