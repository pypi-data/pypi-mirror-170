...
from spectral_process import peak_pick_Minimizer
import multiprocessing

import pandas as pd
averagineDF = pd.read_csv('PolyAveragine.csv',index_col=0)
averagineDF['mz'] = averagineDF['mz'].apply(eval)
averagineDF['intensity'] = averagineDF['intensity'].apply(eval)
averagineDict = averagineDF.to_dict(orient='index')
...
def detect_features(datafile, **constants):
    ...
    while thing != 'done':
        scanNum, scan = thing
        foo = time.clock()
        isotopeData.append((scanNum, peak_pick_Minimizer(scan,averagineDict)[0]))
        bar += time.clock() - foo
    ...
    allIsotopes.sort(key = lambda x: x[0][0][1]) # So so the lowest intensity is first
    ...
    while allIsotopes:
        ...
        centerIndex, (centerMZ, _) = max(enumerate(highIso), 
                                         key = lambda x: x[1][1]) #centerMZ --> mz of the isotope with highest intensity
        ...
        while continuing: # Trailing the feature backwards.
            ...
            #scanSeqs.sort(key = lambda y: y[centerIndex][1], reverse = True)
