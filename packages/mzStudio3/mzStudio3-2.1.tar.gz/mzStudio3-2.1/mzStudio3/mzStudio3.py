...
import re, sys, pickle, platform, time, csv, gc
...
class MS_Data_Manager():
    ...
    def __init__(self, parent=None):
        ...
        self.pa = re.compile('.*?([FI]TMS) [+] ([cp]) [NE]SI (?:sid=35.00 )?(t|r)? ?d Full ms2 (\d+?.\d+?)@(hcd|cid)(\d+?.\d+?) \[(\d+?.\d+?)-(\d+?.\d+?)\]')
        
        #ITMS + c NSI t d Full ms2 742.3569@cid35.00 [199.000-1495.000]
        #ITMS + c NSI r d Full ms2 742.3569@cid35.00 [199.000-1495.000]
        
        self.lockms2 = re.compile('.*?([FI]TMS) [+] ([cp]) [NE]SI (t|r)? ?d? ?Full lock ms2 (\d+?.\d+?)@(hcd|cid)(\d+?.\d+?) \[(\d+?.\d+?)-(\d+?.\d+?)\]')        
        ...
        self.etdhcd = re.compile('.*?([FI]TMS) [+] ([cp]) [NE]SI sa Full ms2 (\d+?.\d+?)@(hcd|cid|etd)(\d+?.\d+?) (\d+?.\d+?)@(hcd|cid|etd)(\d+?.\d+?) \[(\d+?.\d+?)-(\d+?.\d+?)\]')
        ...
        self.thermo_filters = [[self.ms1, fm.Onms1], [self.lockms1, fm.Onlockms1], 
                               ...
                               [self.targ_ms3, fm.Ontarg_ms3], [self.sim_ms1, fm.Onsim_ms1], [self.etdhcd, fm.Onetdhcd],
        ...
        self.mass_dict = {"Thermo_ms2":[self.pa, 6,7],
                          "Thermo_lock_ms2":[self.lockms2, 6,7],
    ...
    def GetMaxIntProf(self, fm, lm, filename):
        if not self.files[filename].Processing:
            sub_scan = []
            for member in self.files[filename].profile_scan:
    ...
    def addFile(self,filename):
        ...
        if current.vendor in ['Thermo', 'ABI', 'mgf']:
            ...
            ##print "Ignoring possible dict file."
                ...
                pickle_file = open(dict_file, "rb")
                current.scan_dict = pickle.load(pickle_file)
                current.rt_dict = pickle.load(pickle_file)
                current.filter_dict = pickle.load(pickle_file)
                current.rt2scan = pickle.load(pickle_file)
                if current.vendor in ['Thermo', 'ABI']:
                    current.m._headers = pickle.load(pickle_file)
                if current.vendor=='ABI':
                    current.rt2exp = pickle.load(pickle_file)
                    current.m.precursor_info = pickle.load(pickle_file)
                    current.m.MS1_data = pickle.load(pickle_file)
                    current.m.MS1_modes = pickle.load(pickle_file)
                    current.m.MS2_modes = pickle.load(pickle_file)
                    current.m.associated_MS2 = pickle.load(pickle_file)
                    current.rt2exp = pickle.load(pickle_file)    
                ...
                if current.vendor=='mgf':
                    ...
                    #except ValueError:
                        ...
                        #TITLE=Cmpd 1, +MS2(921.2310), 31.8eV, 0.0min, 1/K0=0.971, #2-6
                        current.scan_dict = dict([(int(x[0]), int(x[0])) for x in current.m.filters()])
                ...
                try:
                    pickle_file = open(dict_file, "wb")
                    pickle.dump(current.scan_dict, pickle_file)
                    pickle.dump(current.rt_dict, pickle_file)
                    pickle.dump(current.filter_dict, pickle_file)
                    pickle.dump(current.rt2scan, pickle_file)
                    pickle_list = []
                    if current.vendor=='ABI':
                        pickle.dump(current.rt2exp, pickle_file)
            ...
            if os.path.exists(current.ticname):
                ...
                current.xic = pickle.load(pickle_file)
                pickle_file.close()
            else:
                #print "Building xic..."
                ...
                #pickle.dump(current.xic, pickle_file)
                #pickle_file.close()
                #del busy
    ...
    def set_scan(self, scanNum, file_number):
        ...
        if currentFile.m.file_type == "Bruker":
            ...
            if currentFile.FileAbs.lower().endswith('raw') or currentFile.FileAbs.lower().endswith('wiff'):
                currentFile.scan = currentFile.m.scan(currentFile.scanNum, centroid = True)
            else:
                threshold = average(zip(*currentFile.profile_scan)[1]) * CENTROID_SCALE
                currentFile.scan = mz_centroid(currentFile.profile_scan, threshold = threshold)
    ...
    def build_label_dict(self, cg, filename):
        ...
        y_label = 'y'
        b_label = 'b'
        if 'reaction' in self.files[filename].fd and self.files[filename].fd['reaction']=='etd' or filename.find('dcv.mgf') > -1:
            y_label = 'z'
            b_label = 'c'
...
class DrawPanel(wx.Panel): 
    ...
    def OnOpen(self, event):
        if self.msdb.getDisplayWindows() < 4:
            ...
            dlg = wx.MessageDialog(self, 'Max Display Files = 4!', 'Alert', wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()            
    ...
    def OnKeyDown(self, event):
        ...
        #-----------------------Runs set_scan and build_current_ID
            ...
            if isinstance(activeFile.scanNum, (str, bytes)):
    ...
    def DrawFeatures(self, dc, key, rawID):
        ...
        #pdc = wx.BufferedDC(wx.ClientDC(self), self._Buffer)
        currentFile = self.msdb.files[self.msdb.Display_ID[rawID]]
        if currentFile.scanNum in currentFile.scansWithFeatures:
    ...
    def DrawTextLabels(self, scan_data, dc, thresh, cutoff, currentFile, key, scan_type, rd, rawID, filter):
        ...
        profile = False
        if filter.find("+ p")>-1: profile = True        

        if profile: # and scan_type == 'MS1'
            if currentFile.instrument =='Thermo':
                ...
                scan_data = currentFile.m.scan(currentFile.scanNum)
    ...
    def DrawProfileSpectrum(self, dc, key, rawID):
        ...
        if not currentFile.intensity_scaling:
            max_int = self.msdb.GetMaxIntProf(firstMass, lastMass, self.msdb.Display_ID[rawID])
        else:
            max_int = currentFile.intensity_scaling
    ...
    def locateMS2(self, mz, tolerance):
        ...
        if currentFile.vendor == 'Thermo':
            ...
            if currentFile.FileAbs.endswith('.raw'):
                ...
                regex = [self.msdb.tofms2, self.msdb.epi]            
                        ...
                        if currentFile.vendor=='Thermo':
                            ...
                            if id:
                                if currentFile.FileAbs.endswith('.raw'):
                                    ...
                                    elif id.groups()[0] == 'EPI':
                                        prec = float(id.groups()[2])
                                        if mz > prec - tolerance and mz < prec + tolerance:
                                            found.append([i, id.groups()[2], 'EPI', filt])                                        
                                elif currentFile.FileAbs.endswith('.wiff'):
    ...
    def On_Search_Spectrum(self):
        ...
        if searchMode == 'Mascot':
            ...
            raise ValueError("Invalid search mode string: %s" % searchMode)
...
class TopLevelFrame(wx.Frame):
    ...
    def __init__(self, parent, id=-1, title="mzStudio (version 2.1 2022-09-30)", pos=wx.DefaultPosition,
    ...
    def MakeMenuBar(self, full=True):
        if not full:
            ...
            item = menu.Append(-1, "Open MS Data File (Current Tab)")
    ...
    def OnImportFeatureFile(self, event):
        ...
        try:
            (scanToF, scanFToPeaks,
             featureToMS1s, featureByIndex) = featureUtilities.getScanFeatureDicts(data,
                                                                   featureData,
        ...
        current.featureByIndex = featureByIndex
    ...
    def OnMakeDb(self, event):
        ...
        for row in currentFile.rows:
            if currentFile.SearchType in ['Mascot', 'X!Tandem', 'COMET']:
                ...
                raise ValueError(currentFile.SearchType)
        ...
        if currentFile.SearchType == "Mascot":
            ...
            #---2022-08-06
            #pa = re.compile('.+?\(([A-Z]+?)\)')
            #hd = currentPage.msdb.files[currentPage.msdb.Display_ID[currentPage.msdb.active_file]].header
            #for check in range(0,9):
            #    if check in hd.keys():
            #        mod = hd[check].split(',')[0]
    ...
    def AddToolBarItems(self, tb):
        ...
        for pos, label, art, short_help, long_help, evt_id  in self.ToolBarData():
            if pos != "sep":
                if not isinstance(art, (str,bytes)):
                    art.Rescale(*tsize)
