...
#--------------------------------------------------------------------
# DEVELOPMENT VERSION OF mzPepCalc
...
class mzPepCalc_Frame(wx.Frame):
    def __init__(self, parent, id, organizer):
        wx.Frame.__init__(self, parent, -1, "mzPepCalc", size=(280,700))
        self.panel = MainBPC(self, id, organizer)

class MainBPC(wx.Panel):
#class MainBPC(ScrolledPanel):
    def __init__(self, parent, id, organizer):
        
        if organizer: organizer.addObject(self) # Allows only one BlaisPepCalc object to exist in a session.
...
class BlaisPepCalc(wx.Panel):
    def __init__(self, parent, id, organizer):
        ...
        self.cp = cp = wx.CollapsiblePane(self, label="Product Ions", style=wx.CP_DEFAULT_STYLE|wx.CP_NO_TLW_RESIZE, pos=(10, 240), size=(20,50))
...
if __name__ == '__main__':
    app = wx.PySimpleApp()
    frame = mzPepCalc_Frame(parent=None, id=-1, organizer=None)
    frame.Show()
    app.MainLoop()
