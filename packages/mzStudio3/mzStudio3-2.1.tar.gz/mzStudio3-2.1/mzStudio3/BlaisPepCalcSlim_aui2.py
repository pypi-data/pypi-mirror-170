...
##pass#print sys.path
##pass#print os.path.abspath(os.getcwd())
sys.path.append(".")
...
neu = mz_masses.mass_dict['mi']['N15'] - mz_masses.mass_dict['mi']['N']
...
for AA in AAs:
    ...
    #if mass and hkey in AminoAcidMasses:
    mod_lookup_table[lkey, hkey] = end[AA] * neu   
...
#class MainBPC(ScrolledPanel):
    ...
    def OnClose(self, event):
        ...
        #pass#print "PepCalc Close event!"
        #event.Skip()
...
class BlaisPepCalc(wx.Panel):
    def __init__(self, parent, id, organizer):
        ...
        self.Nterm_mods = list(mz_masses.Nterm_dict.keys())
        self.Nterm_mods.sort()
        self.Cterm_mods = list(mz_masses.Cterm_dict.keys())
    ...
    def PrecClick(self, event):
        #pass#print self.FindWindowByName('precursorListBox').GetStringSelection()
    ...
    def CreateMenuBar(self):
        ...
        for eachMenuData in self.menuData():
            ...
            #pass#print menuLabel
            menuItems = eachMenuData[1:]
            #pass#print menuItems
    ...
    def OnCalculate(self, event):
        ...
        if any(frags.values()):
            if _ions == 'b/y':
                b_ions = sorted(list(zip(*frags['b']))[1]) + [chgmass]
                y_ions = sorted(list(zip(*frags['y']))[1]) + [chgmass]
            elif _ions == 'c/z':
                b_ions = sorted(list(zip(*frags['c']))[1]) + [chgmass]
                y_ions = sorted(list(zip(*frags['z']))[1]) + [chgmass]            
    ...
    def OnXFer(self, event):
        if not self.y_ions and self.b_ions:
            #pass#print "Invalid command; no sequence stored."
            return
        if self.parent.parent.ctrl.GetSelection() == -1:
            #pass#print "Invalid command; no data selected."
            return
