...
def initialize_masses(calculate_AAs, calculate_CNHOPS, charge_list, mod_list):
    ...
    masslist = list(zip(*mass_of_molecules))[0]
    
    stoptime = time.process_time()
    pass#print "Yardstick preprocessing time: %s" % (stoptime - starttime)
...
def analyze_delta(mz_delta, tolerance):
    ...
    # Outputs pass#printable, potentially multi_line string of what the delta matches.  And quickly!
    #strings = ['MZ: %.3f\n' % mz_delta]
    strings = []
    ...
    pass#print "%f yardstick time: %s" % (mz_delta, time.clock() - starttime)
    return '\n'.join(strings)
