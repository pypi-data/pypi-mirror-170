# Copyright (C) 2012-2017 The python-litecoinlib developers
#
...
import litecoin.core.key
import litecoin.core.serialize
from litecoin.core.contrib.ripemd160 import ripemd160
...
from litecoin.core.script import *
...
class EvalScriptError(litecoin.core.ValidationError):
...
def _CastToBigNum(s, err_raiser):
    v = litecoin.core._bignum.vch2bn(s)
...
def _UnaryOp(opcode, stack, err_raiser):
    ...
    stack.append(litecoin.core._bignum.bn2vch(bn))
    ...
    for (sop, sop_data, sop_pc) in scriptIn.raw_iter():
        ...
        if sop <= OP_PUSHDATA4:
            ...
            if sop == OP_1NEGATE or ((sop >= OP_1) and (sop <= OP_16)):
                v = sop - (OP_1 - 1)
                stack.append(litecoin.core._bignum.bn2vch(v))
                ...
                check_args(1)
                stack.append(litecoin.core.serialize.Hash160(stack.pop()))

            elif sop == OP_HASH256:
                check_args(1)
                stack.append(litecoin.core.serialize.Hash(stack.pop()))
...
class VerifyScriptError(litecoin.core.ValidationError):
    pass
...
class VerifySignatureError(litecoin.core.ValidationError):
    pass
