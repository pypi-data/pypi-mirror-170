...
# later.
from litecoin.core import *
from litecoin.core.serialize import *
from litecoin.net import *
import litecoin
...
class MsgSerializable(Serializable):
    ...
    def to_bytes(self):
        ...
        self.msg_ser(f)
        body = f.getvalue()
        res = litecoin.params.MESSAGE_START
        res += self.command
    ...
    def stream_deserialize(cls, f, protover=PROTO_VERSION):
        ...
        # check magic
        if recvbuf[:4] != litecoin.params.MESSAGE_START:
            raise ValueError("Invalid message start '%s', expected '%s'" %
                             (b2x(recvbuf[:4]), b2x(litecoin.params.MESSAGE_START)))
...
class msg_version(MsgSerializable):
    ...
    def __init__(self, protover=PROTO_VERSION):
        ...
        self.strSubVer = (b'/python-litecoinlib:' +
                          litecoin.__version__.encode('ascii') + b'/')
