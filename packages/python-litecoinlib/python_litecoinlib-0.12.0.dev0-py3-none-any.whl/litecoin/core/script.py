# Copyright (C) 2012-2015 The python-litecoinlib developers
#
...
is in litecoin.core.scripteval
"""
...
class CScript(bytes):
    ...
    def __coerce_instance(cls, other):
        ...
        if isinstance(other, CScriptOp):
            ...
            if 0 <= other <= 16:
                ...
                other = CScriptOp.encode_op_pushdata(litecoin.core._bignum.bn2vch(other))
    ...
    def __repr__(self):
        ...
        def _repr(o):
            if isinstance(o, bytes):
                return "x('%s')" % litecoin.core.b2x(o)
            else:
                return repr(o)
    ...
    def to_p2sh_scriptPubKey(self, checksize=True):
        ...
        return CScript([OP_HASH160, litecoin.core.Hash160(self), OP_EQUAL])
...
class CScriptWitness(ImmutableSerializable):
    ...
    def __repr__(self):
        return 'CScriptWitness(' + ','.join("x('%s')" % litecoin.core.b2x(s) for s in self.stack) + ')'
...
def RawSignatureHash(script, txTo, inIdx, hashtype):
    ...
    txtmp = litecoin.core.CMutableTransaction.from_tx(txTo)
    ...
    if (hashtype & 0x1f) == SIGHASH_NONE:
        ...
        for i in range(outIdx):
            txtmp.vout.append(litecoin.core.CTxOut())
    ...
    txtmp.wit = litecoin.core.CTxWitness()
    s = txtmp.serialize()
    s += struct.pack(b"<i", hashtype)

    hash = litecoin.core.Hash(s)
...
def SignatureHash(script, txTo, inIdx, hashtype, amount=None, sigversion=SIGVERSION_BASE):
    ...
    if sigversion == SIGVERSION_WITNESS_V0:
        ...
        if not (hashtype & SIGHASH_ANYONECANPAY):
            ...
            hashPrevouts = litecoin.core.Hash(serialize_prevouts)

        if (not (hashtype & SIGHASH_ANYONECANPAY) and (hashtype & 0x1f) != SIGHASH_SINGLE and (hashtype & 0x1f) != SIGHASH_NONE):
            serialize_sequence = bytes()
            for i in txTo.vin:
                serialize_sequence += struct.pack("<I", i.nSequence)
            hashSequence = litecoin.core.Hash(serialize_sequence)

        if ((hashtype & 0x1f) != SIGHASH_SINGLE and (hashtype & 0x1f) != SIGHASH_NONE):
            serialize_outputs = bytes()
            for o in txTo.vout:
                serialize_outputs += o.serialize()
            hashOutputs = litecoin.core.Hash(serialize_outputs)
        ...
        return litecoin.core.Hash(f.getvalue())
