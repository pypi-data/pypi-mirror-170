...
# Copyright (C) 2011-2018 The python-litecoinlib developers
#
...
import litecoin
from litecoin.core import COIN, x, lx, b2lx, CBlock, CBlockHeader, CTransaction, COutPoint, CTxOut
from litecoin.core.script import CScript
...
class BaseProxy(object):
    ...
    def __init__(self,
        ...
        if service_url is None:
            # Figure out the path to the litecoin.conf file
            if ltc_conf_file is None:
                if platform.system() == 'Darwin':
                    ltc_conf_file = os.path.expanduser('~/Library/Application Support/Litecoin/')
                elif platform.system() == 'Windows':
                    ltc_conf_file = os.path.join(os.environ['APPDATA'], 'Litecoin')
                else:
                    ltc_conf_file = os.path.expanduser('~/.litecoin')
                ltc_conf_file = os.path.join(ltc_conf_file, 'litecoin.conf')

            # Litecoin Core accepts empty rpcuser, not specified in ltc_conf_file
            conf = {'rpcuser': ""}

            # Extract contents of litecoin.conf to build service_url
            try:
                with open(ltc_conf_file, 'r') as fd:
            ...
            # Treat a missing litecoin.conf as though it were empty
            except FileNotFoundError:
                pass

            if service_port is None:
                service_port = litecoin.params.RPC_PORT
            ...
            cookie_dir = conf.get('datadir', os.path.dirname(ltc_conf_file))
            if litecoin.params.NAME != "mainnet":
                cookie_dir = os.path.join(cookie_dir, litecoin.params.NAME)
            ...
            try:
                ...
                if 'rpcpassword' in conf:
                    ...
                    raise ValueError('Cookie file unusable (%s) and rpcpassword not specified in the configuration file: %r' % (err, ltc_conf_file))
...
class RawProxy(BaseProxy):
    """Low-level proxy to a litecoin JSON-RPC service
    ...
    def __getattr__(self, name):
        ...
        # Make debuggers show <function litecoin.rpc.name> rather than <function
        # litecoin.rpc.<lambda>>
        f.__name__ = name
        return f
...
class Proxy(BaseProxy):
    """Proxy to a litecoin RPC service

    Unlike ``RawProxy``, data is passed as ``litecoin.core`` objects or packed
    ...
    way. Assumes Litecoin Core version >= v0.16.0; older versions mostly work,
    ...
    def __init__(self,
        ...
        out of the file ``ltc_conf_file``. If ``ltc_conf_file`` is not
        specified, ``~/.litecoin/litecoin.conf`` or equivalent is used by
        default.  The default port is set according to the chain parameters in
        use: mainnet, testnet, signet, or regtest.

        Usually no arguments to ``Proxy()`` are needed; the local litecoind will
        be used.
    ...
    def generate(self, numblocks):
        """
        DEPRECATED (will be removed in litecoin-core v0.18)
    ...
    def getnewaddress(self, account=None):
        """Return a new Litecoin address for receiving payments.
    ...
    def getrawtransaction(self, txid, verbose=False, block_hash=None):
        ...
        (required when transaction not currently indexed by Litecoin Core)
    ...
    def listunspent(self, minconf=0, maxconf=9999999, addrs=None):
        ...
        for unspent in r:
            ...
            # address isn't always available as Litecoin Core allows scripts w/o
    ...
    def submitblock(self, block, params=None):
        ...
        params is optional and is currently ignored by litecoind. See
