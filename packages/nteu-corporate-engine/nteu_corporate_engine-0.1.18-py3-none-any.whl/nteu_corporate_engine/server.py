...
import asyncio
import pickle
from nteu_corporate_engine.engine import process_batch
...
async def handle_translate(req):
    ...
    try:
        ...
        print(req)
        print(batch)
        translations, scores = await process_batch(batch)
        ans = {
            "tus": []
        }
        for src, translation, score in zip(batch, translations, scores):
            ans["tus"].append({"src": src, "tgt": translation, "score": np.e**(score)*10})
