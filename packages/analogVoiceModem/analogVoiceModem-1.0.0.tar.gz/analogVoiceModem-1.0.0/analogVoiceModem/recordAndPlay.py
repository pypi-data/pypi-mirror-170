...
def record_audio():
    ...
    print("appending")
    wf = wave.open('record_new1.wav', 'wb')
    wf.setnchannels(1)
    wf.setsampwidth(1)
...
def play_audio():
    ...
    wf = wave.open('record_new1.wav', 'rb')
    chunk = 1024
    ...
    while 1:
        ...
        modem_data = analog_modem.readline()
        modem_data = analog_modem.readline()
        print("modem data after playing", modem_data)
        if b'OK' in modem_data:
            break
    ...
    while 1:
        ...
        if not disable_modem_event_listener:
            ...
            if modem_data != "":
                ...
                if (b'RING' in modem_data) or (b'DATE' in modem_data) or (b'TIME' in modem_data) or (
                        b'NMBR' in modem_data):
                    global audio_file_name
                    if (b'NMBR' in modem_data):
                        from_phone = (modem_data[5:]).strip()
                        print("from_phone:  ", from_phone)
                        print("\n")
                    if (b'DATE' in modem_data):
                        call_date = (modem_data[5:]).strip()
                        print("call_date:  ", call_date)
                        print("\n")
                    if (b'TIME' in modem_data):
                    ...
                    if b'RING' in modem_data:
                        ...
                        if ring_count == 1:
                            ...
                            ring_data = b''
                            wav_file_ext = wav_file.encode()
                            print("wav_file:  ", wav_file_ext)
                            call_second_ext = str(datetime.strftime(datetime.now(), "%S")).encode()
                            # audio_file_name = from_phone + "_" + call_date + "_" + call_time + "_" + str(
                            # datetime.strftime(datetime.now(), "%S")) + ".wav"
                            audio_file_name = from_phone + b'_' + call_date + b'_' + call_time + b'_' + call_second_ext + wav_file_ext
                            from_phone = ''
                            call_date = ''
                            call_time = ''
