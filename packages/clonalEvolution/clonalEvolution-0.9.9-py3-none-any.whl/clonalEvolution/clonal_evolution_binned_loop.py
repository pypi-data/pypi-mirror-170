...
def division(i, divide, m_list):
    if divide > 0:
        passanger_divide = []
        if m_list:
            uni = np.unique(i[5]).tolist() # select only unique mutations ids (two same mutations cannot happen in one cell)
            for x in range(divide):
                ...
                if mut >= len(uni): ## check if duplicate is greater than mutation list length
                    mut = len(uni)
                try:
                    new_muts = random.sample(uni, k=mut) ## select random mutations to duplicate
        ...
        if m_list:
            for y in range(death):
                ...
                if mut >= len(uni): ## check if mutation to erease is greater than mutation list length
                    mut = len(uni)
                try:
                    passenger_death = random.sample(uni, k=mut) ## select random mutation from mutation list
    ...
    for x in range(CLONE_ID, CLONE_ID + m_d):
        if m_list:
            ...
            if mut >= len(uni):
                mut = len(uni)
            try:
                passenger = random.sample(uni, k=mut) ## select random mutations to copy
            except IndexError as error:
