...
class FileStorage():
  def __init__(self, path='./finlab_db'):
    ...
    if not os.path.isdir(path):
      os.mkdir(path)
      with open(os.path.join(path, 'timestamp.pkl'), 'wb') as f:
        pickle.dump({}, f)
      with open(os.path.join(path, 'stock_names.pkl'), 'wb') as f:
        pickle.dump({}, f)
...
def get(dataset:str, save_to_storage:bool=True):
    ...
    if 'quota' in url_data:
        logger.warning(f'{dataset} -- Daily data usage: {url_data["quota"]:.1f} / {url_data["limit_size"]} MB')
    ...
    if 'date' in df:
        ...
        if table_name in ['monthly_revenue', 'rotc_monthly_revnue', 'financial_statement', 'fundamental_features',
                          'tw_total_pmi', 'tw_total_nmi', 'tw_industry_nmi', 'tw_industry_pmi']:
        ...
        if (df.columns.str.find(' ') != -1).all():
            ...
            if check_numeric_dtype:
                ...
                df = df.fillna(np.nan).transpose().groupby(level=0).last().transpose()
