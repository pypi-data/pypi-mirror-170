...
def get_file_frame_segments(mixdb: MixtureDatabase, mixid: MixtureID = ':') -> Dict[int, Segment]:
    """
...
def get_mixtures_from_mixid(mixdb: MixtureDatabase, mixid: MixtureID = None) -> Mixtures:
    ...
    return [deepcopy(mixdb.mixtures[i]) for i in convert_mixid_to_list(mixdb, mixid)]
...
def get_mixture_metadata(mixdb: MixtureDatabase, mixid: int) -> str:
    ...
    metadata = ''
    for ti in range(len(mixture.target_file_index)):
        metadata += f'target {ti} name: {mixdb.targets[ti].name}\n'
        metadata += f'target {ti} augmentation: {mixdb.target_augmentations[ti].to_dict()}\n'
        metadata += f'target {ti} target_gain: {mixture.target_gain[ti]}\n'
        ts = mixdb.targets[ti].truth_settings
        for tsi in range(len(ts)):
            metadata += f'target {ti} truth index {tsi}: {ts[tsi].index}\n'
    metadata += f'noise name: {mixdb.noises[mixture.noise_file_index].name}\n'
    metadata += f'noise augmentation: {mixdb.noise_augmentations[mixture.noise_augmentation_index].to_dict()}\n'
    metadata += f'snr: {mixture.snr}\n'
    metadata += f'random_snr: {mixture.random_snr}\n'
    metadata += f'samples: {mixture.samples}\n'
    metadata += f'target_snr_gain: {mixture.target_snr_gain}\n'
    metadata += f'noise_snr_gain: {mixture.noise_snr_gain}\n'
    metadata += f'class_count: {mixture.class_count}\n'
