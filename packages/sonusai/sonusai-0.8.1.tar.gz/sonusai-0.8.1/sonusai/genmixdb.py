...
def genmixdb(file: str = None,
    ...
    raw_target_audio_samples = sum([len(x) for x in raw_target_audio])
    if logging:
        logger.info('')
    ...
    noise_audio_samples = sum([len(xa) for x in noise_audios for xa in x])
    if logging:
        logger.info('')
    ...
    mixdb.mixid_padding = int(np.ceil(np.log10(len(mixdb.mixtures))))
...
def _process_mixture_nen(mixture: Mixture) -> Mixture:
    ...
    return process_mixture_nen(mixture=mixture,
                               mixdb=MP_DICT['mixdb'],
                               raw_target_audios=MP_DICT['raw_target_audio'],
                               augmented_noise_audios=MP_DICT['noise_audios'])
