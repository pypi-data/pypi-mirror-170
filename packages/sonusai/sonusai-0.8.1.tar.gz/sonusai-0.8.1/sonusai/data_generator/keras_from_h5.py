...
class KerasFromH5(Sequence):
    ...
    def __init__(self,
        ...
        for m in self.mixid:
            ...
            filename = join(self.location, mixid_padded_name + '_feature.h5')
            if not exists(filename):
                raise SonusAIError(f'Could not find {filename}')
            filename = join(self.location, mixid_padded_name + '_truth_f.h5')
            if not exists(filename):
                raise SonusAIError(f'Could not find {filename}')
    ...
    def __getitem__(self, batch_index: int) -> (np.ndarray, np.ndarray):
        ...
        result = p_map(self._get_ft, batch_params.mixid)
