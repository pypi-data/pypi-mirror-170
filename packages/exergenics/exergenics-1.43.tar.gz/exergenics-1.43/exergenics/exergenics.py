    ...
    ********************************
    ...
    def authenticate(self):
        ...
        if self.authorizationToken is not None:
            self.lastStatus, self.lastResponse = self.doRequest(self.getExchangeEndpoint(), self.ex__tokenPing, {})
            if self.lastStatus != 200:
                return False
            else:
                return True
    ...
    def getWeather(self, buildingCode, distance=20):
        activityUrl = "{}/{}/?distance={}".format(self.ex__getWeather, buildingCode, distance)
    ...
    def jobLog(self, jobId, level, message):
        activityUrl = "{}/{}/{}/?message={}".format(self.ex__jobLog, jobId, level, message)
        self.lastStatus, self.lastResponse = self.doRequest(self.getExchangeEndpoint(), activityUrl, {})
        if self.noResponse():
            return False
        return True
    ...
    def putFile(self, plantCode, urlToFile, category, name, equipCode=''):
        activityUrl = "{}/{}/?urlToFile={}&category={}&name={}&equipCode={}".format(self.ex__putFile, plantCode,
    ...
    # function  : getFiles
    # param     : plantCode
    def getFiles(self, plantCode, category=None, equipCode=""):
        if category is not None:
            activityUrl = "{}/{}?category={}&equipCode={}".format(self.ex__getFiles, plantCode, category, equipCode)
        else:
            activityUrl = "{}/{}?equipCode={}".format(self.ex__getFiles, plantCode, equipCode)
    ...
    def deleteFiles(self, plantCode, category, equipCode=""):
        if category is not None:
            activityUrl = "{}/{}?category={}&equipCode={}".format(self.ex__deleteFiles, plantCode, category, equipCode)
        else:
            activityUrl = "{}/{}?equipCode={}".format(self.ex__deleteFiles, plantCode, equipCode)
        self.lastStatus, self.lastResponse = self.doRequest(self.getExchangeEndpoint(), activityUrl, {})
        if self.noResponse():
            return False
        return True
    ...
    def getJobData(self, jobId, fieldName):
        activityUrl = "{}/{}/?fieldName={}".format(self.ex__getJobData, jobId, fieldName)
    ...
    def getAutoDataRefreshJobs(self, date):
        activityUrl = "{}?date={}".format(self.ex__getAutoDataRefreshJobs, date)
        self.lastStatus, self.lastResponse = self.doRequest(self.getExchangeEndpoint(), activityUrl, {})
        if self.noResponse():
            return False
        return True
    ...
    def portalChart_lineChart(self, plantCode, chartName, portalCategory, dataFrame, title="title",
        ...
        urlToChart = self.plotlyLineChart(dataFrame, title, xAxisTitle, yAxisTitle, outputType, legendTitle,
                                          ...
                                          titleColor)
        self.putFile(plantCode, urlToChart, portalCategory, chartName, equipCode)
        return urlToChart
    ...
    def plotlyBowlChart2D(self, data, title="title", xAxisTitle="Approach", yAxisTitle="Fans Energy",
                        yAxis2Title="Chiller Energy", legendTitle="Key", backgroundColor="white",
                        fontSize=12, fontFamily="TimesNewRoman", fontColor="black", legendFamily="Courier",
    ...
    def portalChart_bowlChart2D(self, plantCode, chartName, portalCategory, data, title="title", equipCode=""):
        urlToChart = self.plotlyBowlChart2D(data, title)
        self.putFile(plantCode, urlToChart, portalCategory, chartName, equipCode)
        return urlToChart

    def plotlyBowlChart3D(self, point, x, y, z, title="title", xAxisTitle="Approach",
                          yAxisTitle="CDW Speed", zAxisTitle="Total Energy", backgroundColor="white",
                          fontSize=12, fontFamily="TimesNewRoman", fontColor="black", titleFont="Arial", titleColor="black"):
    ...
    def portalChart_bowlChart3D(self, plantCode, chartName, portalCategory, point, x, y, z, title="title", equipCode=""):
        urlToChart = self.plotlyBowlChart3D(point, x, y, z, title)
        self.putFile(plantCode, urlToChart, portalCategory, chartName, equipCode)
        return urlToChart
