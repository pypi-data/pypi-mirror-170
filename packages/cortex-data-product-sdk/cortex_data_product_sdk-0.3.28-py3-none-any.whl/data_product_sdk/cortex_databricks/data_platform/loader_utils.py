...
from data_product_sdk.cortex_databricks.data_platform.profilling_utils import (
...
def build_table_path(bucket_name: str, data_pack_name: str, dataset_name: str):
    assert bucket_name in (
        ...
        SILVER_BUCKET,
        GOLD_BUCKET,
    ), f"bucket name must be one of {BRONZE_BUCKET}, {SILVER_BUCKET} or {GOLD_BUCKET}"
...
def build_checkpoint_path(
    ...
    assert bucket_name == BRONZE_BUCKET or (
        input_table_name and bucket_name in (SILVER_BUCKET, GOLD_BUCKET)
...
def write_table(
    ...
    if sort_fields and upsert_fields:
        if check_table_exists(spark=spark, db_name=db_name, table_name=table_name):
            print(
                f"TABELA EXISTE - upsert_fields: {upsert_fields} - sort_fields: {sort_fields}"
            )
            upsert_func = partial(
                __upsert_function,
                db_name=db_name,
...
def apply_profiling(df: DataFrame, data_pack_name: str, dataset_name: str):
    ...
    df.show(5)
    __apply_profiling(
        df=df, profiling_s3_bucket=BRONZE_BUCKET, profiling_s3_key=profiling_s3_key
    )
