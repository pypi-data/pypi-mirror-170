...
class SSH():
    def __init__(self, host:str, port:int=None, user:str=None, password:str=None, pkey:str=None) -> None:
        ...
        host = self.sshcfg.lookup(host)["hostname"]
        
        # 如果没有设置user
        if not user:
            # 读取配置文件的user
            if "user" in self.sshcfg.lookup(host):
                user = self.sshcfg.lookup(host)["user"]
            else:
                # 如果没有配置文件的user, 那么使用当前登录的用户
                user = Os.GetLoginUserName()
        
        if not port:
            if 'port' in self.sshcfg.lookup(host):
                port = self.sshcfg.lookup(host)["port"]
            else:
                port = 22
        ...
        if password:
            ...
            if not pkey:
                # 如果配置文件有指定就用配置文件的
                if "identityfile" in self.sshcfg.lookup(host):
                    pkey = self.sshcfg.lookup(host)['identityfile'][0]
                else:
                    # 如果有设置家目录就找默认的
                    if Os.Getenv("HOME"):
