...
@click.option(
    ...
    type=int,
    multiple=True,
    help="The GitLab user ID(s) to assign the created MR to as reviewer(s).",
)
...
@click.option(
    "--mr-exists",
    is_flag=True,
    help="If set, checks if a merge request exists for the given 'source' and 'target' branch and exits without creating it.",
)
def cli(
    private_token,
    source_branch,
    ...
    if mr_exists:
        does_mr_exists(project, source_branch, target_branch, dry_run=True)
...
def check_if_mr_is_valid(project, source_branch, target_branch):
    ...
    * Does MR already exist for this source branch, exits without an error.
    ...
    if does_mr_exists(project, source_branch, target_branch):
        print(
            f"Merge request already exists for this branch {source_branch} to {target_branch}, no new merge request opened."
        )
        sys.exit(0)


def does_mr_exists(project, source_branch, target_branch, dry_run=False):
    """Checks if an MR for the source branch (i.e. feature/abc) to the target branch (i.e. develop) already exists.
       If set to dry_run and the MR does not exist, will exit with a message.

    Args:
        project (gitlab.v4.objects.Project): The json response from API checking which MRs are currently open.
        source_branch (str): The source branch to merge into (i.e. feature/#67).
        target_branch (str): The target branch to merge onto (i.e. develop).
        dry_run (bool): Argument to enable dry run.
    ...
    existing_mr = [mr for mr in mrs if mr.source_branch == source_branch and mr.target_branch == target_branch]
    ...
    if existing_mr:
        ...
        if dry_run:
            print(
                f"Merge request does not exist for this branch {source_branch} to {target_branch}, run without flag '--mr-exists' to open merge request."
            )
            sys.exit(0)
        return False
