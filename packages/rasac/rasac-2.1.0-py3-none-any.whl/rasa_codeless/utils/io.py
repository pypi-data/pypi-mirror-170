...
from os import path
from typing import List, Optional, Text, OrderedDict, Dict, NoReturn, Union, Tuple, Any
...
from rasa_codeless.shared.exceptions.io import (
    ...
    DirectoryPathNotSpecifiedException,
...
def create_dir(dir_path: Text, force: bool = False) -> NoReturn:
    """
    ...
    Args:
        dir_path: directory path where the dir
            should be created
        force: whether to delete and create an
            existing dir or not
    """

    if not dir_path:
        logger.error("Directory name is not specified. rasac will "
                     "terminate the request.")
        raise DirectoryPathNotSpecifiedException()
...
def _create_cache_dir(cache_data_dir: Text = DEFAULT_CACHE_PATH) -> bool:
    ...
    if not cache_data_dir:
        logger.error("Cache dir not specified. rasac will use the default caching directory to store the cached files.")
...
def get_rasac_init_caches(
        dir_path: Text = DEFAULT_INIT_DEST_DIR_NAME,
) -> Optional[List]:
    """
    Returns all rasac cache directories in a
    ...
    return [cache_dir for cache_dir in os.listdir(dir_path)
            if str(cache_dir).startswith(".rasac_init_")]
...
def get_rasa_testing_data(
    ...
    try:
        ...
        if file_exists(file_path=testing_data_dir):
            ...
            testing_data_files = get_dir_file_list(dir_path=testing_data_dir, file_suffixes=file_ext)
...
def get_latest_model_name(models_path: Union[Text, List]) -> Text:
    """
    ...
    """
    if isinstance(models_path, Text):
        ...
        model_list = models_path

    models_dict = {
        os.path.split(model_path)[-1]:
            int(''.join(list(re.findall(RASA_MODEL_REGEX, os.path.split(model_path)[-1])[0])))
        for model_path in model_list
        if re.findall(RASA_MODEL_REGEX, os.path.split(model_path)[-1])
    }
...
def rasa_models_dir_exists(dir_path: Text) -> bool:
    ...
    models = get_dir_file_list(dir_path=dir_path, file_suffixes=RASA_MODEL_EXTENSIONS)
    if len(models) == 0:
        return False
...
def rasa_model_exists(models_path: Text, model_name: Text) -> bool:
    ...
    if not re.fullmatch(RASA_MODEL_REGEX, model_name):
        return False
...
def dir_cleanup(dir_path: Text):
    try:
        ...
        for path_ in pathlib.Path(DEFAULT_TENSORBOARD_LOGDIR).iterdir():
            if path_.is_file():
                path_.unlink()
            elif path_.is_dir():
                shutil.rmtree(path_)
...
def get_model_list(model_path=DEFAULT_MODEL_PATH, file_suffixes=RASA_MODEL_EXTENSIONS) -> List:
    file_list = get_dir_file_list(dir_path=model_path, file_suffixes=file_suffixes)
    return [os.path.split(model_name)[-1] for model_name in file_list]


def get_botstore_model_list(
        botstore_path: Text = BOTSTORE_PATH,
        pattern: Text = RASA_BOTSTORE_DIR_REGEX
) -> List:
    top_level_assets = get_existing_toplevel_file_list(dir_path=botstore_path)
    valid_model_dirs = [
        dir_+RASA_MODEL_EXTENSIONS[0] for dir_ in top_level_assets if re.match(pattern, str(dir_))] \
        if top_level_assets else []
...
def get_valid_model_list(
    ...
    botstore_model_list = get_botstore_model_list(botstore_path=botstore_path)
    model_list = get_model_list(model_path=models_path)
    valid_model_list = list(set(model_list).intersection(set(botstore_model_list)))
...
def write_yaml_file(
        yaml_file: Text,
        yaml_content: Any,
        mode: Text = FilePermission.WRITE_PLUS,
) -> NoReturn:
    with open(yaml_file, mode) as yaml_file_:
        yml.dump(yaml_content, yaml_file_)


def persist_model_data(
        model_name: Text,
        botstore_path: Text = BOTSTORE_PATH,
        assets: Dict = None
) -> NoReturn:
    try:
        model_timestamp = model_name.replace(RASA_MODEL_EXTENSIONS[0], "")
        os.makedirs(os.path.join(botstore_path, model_timestamp), exist_ok=True)
        assets_to_persist = BOTSTORE_ASSETS

        if assets:
            for asset_type in assets.keys():
                if asset_type in assets_to_persist:
                    assets_to_persist[asset_type] += assets[asset_type]

        for asset_type, assets_paths in assets_to_persist.items():
            for path_ in assets_paths:
                src_ = path_
                dst_ = os.path.join(botstore_path, model_timestamp, path_)
                if not os.path.exists(path=src_):
                    continue

                src_type = SourceType.DIRECTORY if os.path.isdir(src_) else SourceType.FILE

                if asset_type == AssetType.DUPLICATE:
                    shutil.copytree(src=src_, dst=dst_, dirs_exist_ok=True) \
                        if src_type == SourceType.DIRECTORY \
                        else shutil.copyfile(src=src_, dst=dst_)
                elif asset_type == AssetType.MOVE_DIR:
                    shutil.move(src=src_, dst=dst_)
                elif asset_type == AssetType.MOVE_DIR_CONTENT:
                    dir_content = get_existing_toplevel_file_list(src_)
                    for content in dir_content:
                        shutil.move(os.path.join(src_, content), os.path.join(dst_, content))
                else:
                    raise InvalidAssetTypeException()
    except InvalidAssetTypeException as e:
        raise InvalidAssetTypeException(e)
    except Exception as e:
        raise ModelPersistException(e)
