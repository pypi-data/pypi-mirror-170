...
def software_install_one_shot_task(task, verify_source_result):
    """
    This custom Nornir task takes the result of the function verify_source_result as an argument and executes
    ...
    # Get the host destination file from verify_source_result
...
def install_remove_inactive_task(task):
    """
...
def software_install_one_shot(nr_obj, verify_source_result, verbose=False):
    """
    This function takes the result of the function verify_source_result as an argument andruns the custom
    Nornir task software_install_one_shot_task to start the one-shot installation process of the desired
    ...
    # Run the custom Nornir task software_install_one_shot_task
    task = nr_obj.run(
        task=software_install_one_shot_task, verify_source_result=verify_source_result, on_failed=True
    )
...
def install_remove_inactive(nr_obj, verbose=False):
    """
    This function runs the Nornir task install_remove_inactive_task to to remove all not needed software
    ...
    # Run the custom Nornir task install_remove_inactive_task
    task = nr_obj.run(task=install_remove_inactive_task, on_failed=True)
