...
def transform_env(iterable, startswith="env_"):
    """
    This function loops over a nested dictionary and if the key startswith the specific string, it loads the
    environment variable specified by the value and replace the value with the environment variable.
    """
    for key, value in iterable.copy().items():
        if isinstance(value, dict):  # For DICT
            iterable[key] = transform_env(value, startswith)
        elif isinstance(value, list):  # For LIST
            iterable[key] = [transform_env(item, startswith) for item in value]
        elif key.startswith(startswith):  # Update Key-Value
...
def nr_transform_default_creds_from_env(nr_obj, verbose=False):
    ...
    try:
        ...
        if verbose:
            ...
            print(f"-> Default password: {nr_obj.inventory.defaults.username}")
