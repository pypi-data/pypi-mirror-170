...
def prepare_upgrade_data_task(task, local_upload=False):
    """
    ...
    try:
        ...
        # If local upload is true, get the source image file from the Nornir inventory
        if local_upload:
    ...
    # If local upload is false, Cisco cloud download will be used
    else:
        result_info = (
            f"{task_info(text=task_text, changed='False')}\n"
            + f"'{task_text}' -> OSResponse: <Success: True>\n"
...
def upload_file_task(task, host_dict):
    """
...
def prepare_upgrade_data(nr_obj, local_upload=False):
    ...
    task_result = nr_obj.run(task=prepare_upgrade_data_task, local_upload=local_upload)
...
def upload_file(nr_obj, host_dict):
    """
    ...
    print("\n")
    upload_result = nr_obj.run(task=upload_file_task, host_dict=host_dict)
