...
class RawDataForecasting(sections.Plot2dSection):
    ...
    def data_value_reformatting(self, value):
        ...
        if value is list:
            ...
            raw_values = data["data"][0]["values"]
            for i in raw_values:
                return float(i)

    @classmethod
    def _preprocessing_raw_data(cls, raw_data_df: pd.DataFrame) -> pd.DataFrame:
        raw_data_df["data"] = raw_data_df["data"].apply(cls.data_value_reformatting)
        df = raw_data_df.loc[:, ["data", 'observation_timestamp']]
    ...
    def prophet_method(cls, previous_data, values_column):
        ...
        data["ds"] = (data['ds'] + pd.Timedelta(days=1)).dt.tz_localize(None)
    ...
    def getting_forecasting_data(cls, raw_data: pd.DataFrame) -> pd.DataFrame:
        """Get forecasting data."""
        forecast_df = pd.DataFrame()
        forecast_df["observation_timestamp"] = raw_data['observation_timestamp']
