...
--update-env-variables=PENDING_SIGNALS_CHUNKS_COLLECTION=$pending_signal_chunks_collection_name,SIGNALS_QUEUE_COLLECTION=$signals_queue_collection,DATASET=$dataset,MODEL_SIGNALS_TOPIC_NAME=$model_signals_topic_name,DATA_LAKE=$data_lake_bucket_name,RESOURCES_BUCKET=$composer_resources_bucket,LOCATION=$location
