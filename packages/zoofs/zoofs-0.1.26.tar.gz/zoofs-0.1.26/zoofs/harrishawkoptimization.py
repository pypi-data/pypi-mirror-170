...
class HarrisHawkOptimization(BaseOptimizationAlgorithm):
    ...
    def _exploration_phase(self):
        ...
        self.individuals[q_lesser_indexes] = np.where(
            np.random.random(q_lesser_individuals.shape) < X_sub, 1, 0
        )
        ...
        X_rand = self.individuals[
            np.random.choice(np.arange(0, self.individuals.shape[0]), size=len(q_greater_indexes))
        ]
        ...
        self.individuals[q_greater_indexes] = np.where(
            np.random.random(q_greater_individuals.shape) < X_update, 1, 0
        )

    def _temporary_evaluate_fitness(
        self, model, x_train, y_train, x_valid, y_valid, target_individuals
    ):
    ...
    def _levy_walk(self, soft_besiege_with_dives_indexes):
        ...
        u = (
            np.random.random((len(soft_besiege_with_dives_indexes), self.individuals.shape[1]))
            * sigma
        )
    ...
    def _soft_besiege(self):
        soft_beseige_indexes = self.exploitation_individuals_indexes[
            np.where((np.abs(self.exploitation_energy) >= 0.5) & (self.r >= 0.5))[0]
        ]
        J = 2 * (1 - np.random.random(len(soft_beseige_indexes))).reshape(-1, 1)
        delta_X = self.best_dim - self.individuals[soft_beseige_indexes]
        soft_beseige_res = delta_X - np.repeat(
            self.e[soft_beseige_indexes], self.individuals.shape[1]
        ).reshape(len(soft_beseige_indexes), self.individuals.shape[1]) * np.abs(
            J * np.repeat(self.best_dim.reshape(1, -1), len(soft_beseige_indexes), axis=0)
            - self.individuals[soft_beseige_indexes]
        )
        soft_beseige_res = self.sigmoid(soft_beseige_res)
        self.individuals[soft_beseige_indexes] = np.where(
            np.random.random(self.individuals[soft_beseige_indexes].shape) < soft_beseige_res, 1, 0
        )

    def _hard_besiege(self):
        hard_besiege_indexes = self.exploitation_individuals_indexes[
            np.where((np.abs(self.exploitation_energy) < 0.5) & (self.r >= 0.5))[0]
        ]
        hard_besiege_res = self.best_dim - self.e[hard_besiege_indexes].reshape(-1, 1) * np.abs(
            self.best_dim - self.individuals[hard_besiege_indexes]
        )
        hard_besiege_res = self.sigmoid(hard_besiege_res)
        self.individuals[hard_besiege_indexes] = np.where(
            np.random.random(self.individuals[hard_besiege_indexes].shape) < hard_besiege_res, 1, 0
        )

    def _soft_besiege_with_dives(self, model, X_train, y_train, X_valid, y_valid):
        soft_besiege_with_dives_indexes = self.exploitation_individuals_indexes[
            np.where((np.abs(self.exploitation_energy) >= 0.5) & (self.r < 0.5))[0]
        ]
        ...
        soft_besiege_with_dives_res = self.best_dim - np.repeat(
            self.e[soft_besiege_with_dives_indexes], self.individuals.shape[1]
        ).reshape(len(soft_besiege_with_dives_indexes), self.individuals.shape[1]) * np.abs(
            J
            * np.repeat(self.best_dim.reshape(1, -1), len(soft_besiege_with_dives_indexes), axis=0)
            - self.individuals[soft_besiege_with_dives_indexes]
        )

        Y_soft_besiege_with_dives_res = self.sigmoid(soft_besiege_with_dives_res)
        Z_soft_besiege_with_dives_res = (
            Y_soft_besiege_with_dives_res
            + np.random.random(Y_soft_besiege_with_dives_res.shape) * LF
        )

        Y_soft_besiege_with_dives_res = np.where(
            np.random.random(Y_soft_besiege_with_dives_res.shape) < Y_soft_besiege_with_dives_res,
            1,
            0,
        )
        Z_soft_besiege_with_dives_res = np.where(
            np.random.random(Z_soft_besiege_with_dives_res.shape) < Z_soft_besiege_with_dives_res,
            1,
            0,
        )

        ind_fitness = self._temporary_evaluate_fitness(
            model, X_train, y_train, X_valid, y_valid, soft_besiege_individuals
        )
        Y_fitness = self._temporary_evaluate_fitness(
            model, X_train, y_train, X_valid, y_valid, Y_soft_besiege_with_dives_res
        )
        Z_fitness = self._temporary_evaluate_fitness(
            model, X_train, y_train, X_valid, y_valid, Z_soft_besiege_with_dives_res
        )

        self.individuals[soft_besiege_with_dives_indexes] = np.where(
            self._repeat(Y_fitness, Y_soft_besiege_with_dives_res)
            < self._repeat(Z_fitness, Z_soft_besiege_with_dives_res),
            np.where(
                self._repeat(Y_fitness, Y_soft_besiege_with_dives_res)
                < self._repeat(ind_fitness, soft_besiege_individuals),
                Y_soft_besiege_with_dives_res,
                soft_besiege_individuals,
            ),
            np.where(
                self._repeat(Z_fitness, Z_soft_besiege_with_dives_res)
                < self._repeat(ind_fitness, soft_besiege_individuals),
    ...
    def _hard_besiege_with_dives(self, model, X_train, y_train, X_valid, y_valid):
        hard_besiege_with_dives_indexes = self.exploitation_individuals_indexes[
            np.where((np.abs(self.exploitation_energy) < 0.5) & (self.r < 0.5))[0]
        ]
        ...
        hard_besiege_with_dives_res = self.best_dim - np.repeat(
            self.e[hard_besiege_with_dives_indexes], self.individuals.shape[1]
        ).reshape(len(hard_besiege_with_dives_indexes), self.individuals.shape[1]) * np.abs(
            J
            * np.repeat(self.best_dim.reshape(1, -1), len(hard_besiege_with_dives_indexes), axis=0)
        ...
        Z_hard_besiege_with_dives_res = (
            Y_hard_besiege_with_dives_res
            + np.random.random(Y_hard_besiege_with_dives_res.shape) * LF
        )

        Y_hard_besiege_with_dives_res = np.where(
            np.random.random(Y_hard_besiege_with_dives_res.shape) < Y_hard_besiege_with_dives_res,
            1,
            0,
        )
        Z_hard_besiege_with_dives_res = np.where(
            np.random.random(Z_hard_besiege_with_dives_res.shape) < Z_hard_besiege_with_dives_res,
            1,
            0,
        )

        ind_fitness = self._temporary_evaluate_fitness(
            model, X_train, y_train, X_valid, y_valid, hard_besiege_individuals
        )
        Y_fitness = self._temporary_evaluate_fitness(
            model, X_train, y_train, X_valid, y_valid, Y_hard_besiege_with_dives_res
        )
        Z_fitness = self._temporary_evaluate_fitness(
            model, X_train, y_train, X_valid, y_valid, Z_hard_besiege_with_dives_res
        )

        self.individuals[hard_besiege_with_dives_indexes] = np.where(
            self._repeat(Y_fitness, Y_hard_besiege_with_dives_res)
            < self._repeat(Z_fitness, Z_hard_besiege_with_dives_res),
            np.where(
                self._repeat(Y_fitness, Y_hard_besiege_with_dives_res)
                < self._repeat(ind_fitness, hard_besiege_individuals),
                Y_hard_besiege_with_dives_res,
                hard_besiege_individuals,
            ),
            np.where(
                self._repeat(Z_fitness, Z_hard_besiege_with_dives_res)
                < self._repeat(ind_fitness, hard_besiege_individuals),
