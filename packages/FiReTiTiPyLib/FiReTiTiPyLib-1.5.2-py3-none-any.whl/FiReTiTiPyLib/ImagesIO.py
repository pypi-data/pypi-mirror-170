...
def Write(Array, ChannelFirst: bool, Name: str, FloatEncoding: bool=False):
	...
	if ChannelFirst is True and 2 < len(Array.shape) and 1 < Array.shape[0]: # Multiple channels first, so move them to the end.
		Array = numpy.moveaxis(Array, 0, -1)
	
	if ChannelFirst is True and 1 == Array.shape[0]: # 2D image
		Array = Array[0]

	if ChannelFirst is False and len(Array.shape) == 3 and Array.shape[2] == 1:
		Array = Array.squeeze()
