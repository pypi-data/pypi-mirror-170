...
from bigeye_sdk.exceptions.exceptions import NoSourcesFoundException
...
class DatawatchClient(BaseApiClient, GeneratedDatawatchClient, ABC):
    ...
    def create_deltas_from_simple_conf(self, sdcl: List[SimpleDeltaConfiguration]) -> List[
        ...
        responses = []
        # TODO: Add the ability to use fq table names.
        for sdc in sdcl:
    ...
    def post_metric_suite(self, metric_suite: MetricSuite, apply: bool = False) -> MetricSuiteResponse:
        if apply:
            url = "/api/v1/metric-suites"
        else:
            url = "/api/v1/metric-suites/dry-run"
