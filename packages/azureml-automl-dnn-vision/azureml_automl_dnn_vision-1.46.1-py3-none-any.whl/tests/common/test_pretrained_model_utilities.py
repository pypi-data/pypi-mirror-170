...
class TestPretrainedModelFactory:
    ...
    def test_load_state_dict_from_url_with_retry_urlerror_exceptions(
        ...
        assert "Permissions for required network traffic may not be provided in Firewall/NSG in VNet associated " \
            "with compute cluster." in str(exc_info.value)

        mock_logger_warning.assert_any_call(
            "Failed to load pretrained model 4 times. Suspected cause: incorrect network configuration. Permissions "
            "for required network traffic may not be provided in Firewall/NSG in VNet associated with compute cluster."
        )
