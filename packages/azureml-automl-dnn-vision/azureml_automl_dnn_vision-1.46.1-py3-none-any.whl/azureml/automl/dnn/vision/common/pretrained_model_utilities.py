    ...
    def _load_state_dict_from_url_with_retry(url: str, progress: bool = True,
        ...
        for i in range(PretrainedSettings.DOWNLOAD_RETRY_COUNT - 1):
            try:
                ...
                logger.warning(
                    "Encountered {} error while loading pretrained model. Retries left: {}. Error details: {}"
                    .format(e.__class__.__name__, PretrainedSettings.DOWNLOAD_RETRY_COUNT - i - 1, e)
                )
                ...
                logger.info("Waiting {:.2f} seconds before attempting download.".format(wait_time))
        ...
        if num_runtime_errors == PretrainedSettings.DOWNLOAD_RETRY_COUNT - 1:
            ...
            suspected_cause = "incorrect network configuration. Permissions for required network traffic may not be " \
                              "provided in Firewall/NSG in VNet associated with compute cluster"
        warning_message = "Failed to load pretrained model {} times.{}".format(
            PretrainedSettings.DOWNLOAD_RETRY_COUNT - 1,
        ...
        if num_url_errors == PretrainedSettings.DOWNLOAD_RETRY_COUNT - 1:
            raise UserErrorException(warning_message)
