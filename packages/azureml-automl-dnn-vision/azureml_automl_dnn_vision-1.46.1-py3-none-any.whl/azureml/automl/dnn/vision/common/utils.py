...
def fix_boolean_settings(automl_settings: Dict[str, Any], boolean_parameters: List[str]) -> None:
    """ Fix boolean settings in automl settings dictionary.
    When boolean parameters are passed as part of conditional HP space, they can be in string format.
    e.g: "True". This function parses the string and converts it to a boolean.

    :param automl_settings: Dictionary containing automl settings
    :type automl_settings: dict
    :param boolean_parameters: List of boolean parameters the task supports
    :type boolean_parameters: List
    """
    for parameter in boolean_parameters:
        value = automl_settings.get(parameter, None)
        if value is not None:
            automl_settings[parameter] = bool(strtobool(str(value)))
