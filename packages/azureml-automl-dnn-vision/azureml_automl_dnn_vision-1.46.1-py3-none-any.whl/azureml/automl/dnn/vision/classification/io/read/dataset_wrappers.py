...
class AmlDatasetWrapper(_CommonImageDatasetWrapper, AmlDatasetBaseWrapper):
    ...
    def _validate_dataset(self, images_df, multilabel, ignore_data_errors):
        def is_multilabel_classnames_valid(class_names):
            if (isinstance(class_names, list) and
                    all((type(label) in [str, int]) for label in class_names)):
                return True
            return False
        ...
        # Make list of indexes to filter out according to whether this is a multilabel or a multiclass task.
        if multilabel:
            indices_to_drop = [
                index for index in images_df.index
                if not is_multilabel_classnames_valid(images_df[self._label_column_name][index])
            ]
            base_mesg = "label should be a list of strings or integers for multilabel. " \
                        "Found some datapoints with a single label. "
        else:
            indices_to_drop = [
                index for index in images_df.index if isinstance(images_df[self._label_column_name][index], list)
            ]
            base_mesg = "label should be a string for multiclass. Found some datapoints with a list label. "
