...
from .utils import (command, arg, canonical_path, escape_like, prefix_upper,
...
@command('verify', help='Verify the integrity of the files on a backup disk')
@arg_disk
    ...
    if args.errors:
        if args.errors.startswith('@'):
            with os.fdopen(int(args.errors[1:]), 'w', encoding='utf8') as fp:
                vf.verify(error_fp=fp, minhash=args.start)
        else:
            with open(args.errors, 'w', encoding='utf8') as fp:
                vf.verify(error_fp=fp, minhash=args.start)
    else:
        vf.verify(minhash=args.start)
...
@arg('-p', '--path', metavar='PATH', help='Path to data files')
