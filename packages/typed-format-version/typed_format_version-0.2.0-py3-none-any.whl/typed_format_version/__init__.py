...
def get_format_and_load(
    data: Dict[str, Any], dtype: Type[T], *, pop: bool = False, **kwargs: Any
) -> Tuple[Format, T]:
...
def determine_version_match(version: Version, schemas: Dict[Tuple[int, int], Any]) -> VersionMatch:
    ...
    try:
        highest_lower_minor = max(
            v_minor for (v_major, v_minor) in schemas.keys() if v_major == major and v_minor < minor
        )
