...
class pls:
    ...
    def fit(self, X, y):
        ...
        Xcentered = X - meanX
        ycentered = y - meany
        model = simpls(Xcentered, ycentered, self.nlv)
    ...
    def predict(self, Xnew):
        '''
        predict the spectra with the pls coefficient of 1~nlv components

        Parameters
        ----------
        Xnew : 2d array
            new spectra for prediction.

        Returns
        -------
        ynew_hat : 2d array
            predicted reference values with 1~nlv pls components.
    ...
    def crossValidation_predict(self, nfold = 10):
        X = self.X
        y = self.y
        yhat = np.zeros((y.shape[0],self.nlv))
        model = pls(nlv = self.nlv)
    ...
    def optPredict(self, Xnew, optLV = None):
        if optLV == None:
            try:
                optLV = self.optLV
            except:
                optLV = self.get_optLV()    

        B = self.model['B'][:,optLV-1]
        if Xnew.shape[1] != B.shape[0]-1:
            raise ValueError('The feature number of predictor is isconsistent with that of indepnentent.')
        Xnew = np.append(np.ones([Xnew.shape[0],1]), Xnew, axis=1)
        ynew_hat = np.dot(Xnew,B)
...
class plsda():
    ...
    def fit(self, X, y):
        ...
        plsModel = PLSRegression(n_components = self.nlv, scale=False)
...
def regresssionReport(ytrue,ypred):
    report = dict()
    report["rmse"] = mean_squared_error(ytrue, ypred, squared=True)
...
def simpls(X, y, nlv):
    ...
    V = np.zeros((n_variables, nlv))
    x_scores = np.zeros((n_samples, nlv))  # X scores (standardized)
    x_weights = np.zeros((n_variables, nlv))  # X weights
    x_loadings = np.zeros((n_variables, nlv))  # X loadings
    y_loadings = np.zeros((1, nlv))  # Y loadings
    y_scores = np.zeros((n_samples, nlv))  # Y scores
