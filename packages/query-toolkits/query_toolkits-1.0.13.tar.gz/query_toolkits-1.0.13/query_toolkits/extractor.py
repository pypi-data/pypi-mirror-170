        ...
        if not org_name:
            ...
            for elem in org:
                if len(elem[0]) >= 3:
                    ...
                    break
        if org_name:
            org_name = utils.max_backward_match_end(org_name, self.keyword)

        res = re.search('[〔\[\(［]?\s*([0-9]{4})\s*[〕\]\)］]?\s*[年]?\s*[第]?\s*([0-9]+)\s*号', query)
        if res:
            year = res.group(1)
            number = res.group(2)
        else:
            res = re.search('[第]?\s*([0-9]+)\s*号[文]?', query)
            if res:
                number = res.group(1)
        res = re.search('[a-zA-Z]+/[a-zA-Z]+\s*[0-9]+(\.[0-9]+)?[—-][0-9]{4}', query)
        if res:
            another_number = res.group(0)
        return org_name, year, number, another_number
...
if __name__ == '__main__':
    et = Extractor()
    cut_res = list(psg.cut("最热的"))
    print(et.extract_requirement( cut_res))
    cut_res = list(psg.cut("5g"))
    print(et.extract_number(cut_res))
    res1, res2, res3, res4 = et.extract_reference_no("银监三发")
