...
class Extractor:
    def __init__(self):
        self.unit_transform = utils._read_table()
        self.organization = utils._read_list()
    ...
    def extract_reference_no(self, text):
        ...
        res = re.search('[〔\[\(［]?\s*([0-9]{4})\s*[〕\]\)］]?\s*[年]?\s*[第]?([0-9]+)\s*号', query)
        if res:
            year = res.group(1)
            number = res.group(2)
...
if __name__ == '__main__':
    et = Extractor()
    res1, res2, res3, res4 = et.extract_reference_no("银发〔2020〕10号/JR/T 0173-2020")
