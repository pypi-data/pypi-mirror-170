...
use crate::rrfs::{save_tensor_rrfs, tensor_from_hash_prefix};
...
impl TensorAxisIndex {
    ...
    pub fn split_sections_for_concat(
        &self,
        sections: &Vec<usize>,
    ) -> Option<Vec<Option<TensorAxisIndex>>> {
        ...
        match self {
            TensorAxisIndex::Single(single) => {
                ...
                let outer_idx = starts
                    .iter()
                    .take_while(|start| **start <= *single as usize)
                    .count()
                    - 1;
                let inner_idx = *single as usize - starts[outer_idx];
                let mut result = vec![None; sections.len()];
                result[outer_idx] = Some(TensorAxisIndex::Single(inner_idx as i64));
                Some(result)
            }
            TensorAxisIndex::Slice(slice) => {
                let uslice: Option<USlice> = slice.clone().into();
                let uslice = uslice?;
                let mut result = vec![None; sections.len()];
            ...
            TensorAxisIndex::Tensor(_tensor) => None,
        }
    }
...
impl TensorIndex {
    ...
    pub fn repr_bijection(&self) -> String {
        let mut result = "[".to_owned();
        for (i, idx) in self.0.iter().enumerate() {
            result.push_str(&match idx {
                TensorAxisIndex::Single(i) => i.to_string(),
                ...
                TensorAxisIndex::Tensor(tensor) => {
                    save_tensor_rrfs(tensor.clone()).unwrap();
    ...
    pub fn from_bijection_string(string: &str) -> Result<Self, CircuitConstructionError> {
        let er = CircuitConstructionError::ParsingFail {
        ...
        Ok(TensorIndex(
            string
                ...
                .filter(|x| !x.is_empty())
                .map(|x| {
                    let num_er = CircuitConstructionError::ParsingNumberFail {
                        string: format!("{}", x),
                    };
                    ...
                    if let Some(no_t) = x.strip_prefix("t") {
                        return tensor_from_hash_prefix(no_t)
                            .map(|z| TensorAxisIndex::Tensor(z))
                            .map_err(|_e| CircuitConstructionError::TensorHashNotFound {
                                hash: no_t.to_owned(),
                            });
                    }
                    if let [start, end] = x.split(":").collect::<Vec<_>>()[..] {
                        return Ok(TensorAxisIndex::Slice(Slice {
                            start: if start.is_empty() {
                                None
                            } else {
                                Some(start.parse::<i64>().map_err(|_e| num_er.clone())?)
                            },
                            stop: if end.is_empty() {
                                None
                            } else {
                                Some(end.parse::<i64>().map_err(|_e| num_er.clone())?)
                            },
                        }));
                    }
                    return Err(er.clone());
                })
                .collect::<Result<Vec<_>, CircuitConstructionError>>()?,
        ))
    }
}
