...
from thompcoutils import config_utils, log_utils
import holidays
...
def main():
    """
    Used to test PeakTimes
    """
    config_file_name = 'poolboy.ini'
    config_mgr = config_utils.ConfigManager(file_name=config_file_name, create=False)
    peak_times = PeakTimes(config_mgr, 'peak_time')
    print('As a string:')
    print(peak_times)
    print('Pretty print:')
    print(peak_times.pretty_print())
    print('Check some times')
    print(peak_times.in_peak(datetime.datetime.now()))
    year = datetime.datetime.now().year
    # july 4
    dt = datetime.datetime(year=year, month=7, day=4, hour=1)
    print(peak_times.in_peak(dt))
    # sunday
    d = datetime.datetime(year, int(1), 7)
    offset = -d.weekday() - 1  # weekday = 0 means monday
    dt = d + datetime.timedelta(offset)
    print(peak_times.in_peak(dt))

    for season in peak_times.peak_seasons:
        print('times for {} are {}'.format(season, peak_times.time_range(season)))
        print('months for {} are {}'.format(season, peak_times.month_range(season)))
