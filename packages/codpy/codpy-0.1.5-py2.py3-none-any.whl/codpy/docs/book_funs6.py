...
def MNIST_clustring(codpy_param = set_global_chap6()):
    ...
    set_kernel = set_gaussian_kernel
    scenarios_list = [ (-1, 1000, 2**i,-1) for i in np.arange(7,9,1)]
    #scenarios_list = [ (-1, 1000, 2**i,1000) for i in np.arange(7,5,1)]
    ...
    multi_plot([y1[0],y1[1],y2[0],y2[1]],fun_plot = show_mnist_pictures,mp_ncols = 2, mp_max_items = 10)
...
def stocks_clustering(codpy_param = set_global_chap6()):
    scenarios_list = [(-1, -1, i,-1) for i in range(10, 21,10)]
    scenario_generator_ = scenario_generator()
