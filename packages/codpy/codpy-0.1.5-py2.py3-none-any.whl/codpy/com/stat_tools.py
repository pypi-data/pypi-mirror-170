...
def ks_testD(x,y):
    ...
    H0: x and y are identical distributions
    H1: x and y are not identical distributions
    We chose the confidence level of 0.95.
    If the p-value is lower than .05, then we reject the null hypothesis,
    that the distributions are identical in support of alternative.
    ...
    for i in range(0,dim):
        ...
        ks.append(z)
        thsld += [np.sqrt(-np.log(alpha/2)*(1+len(x)/len(z))/(2*len(z)))]
    df, thsld = pd.DataFrame(ks).T, pd.DataFrame(thsld).T
    alphas = pd.DataFrame(np.repeat(alpha, dim)).T
    df.columns, thsld.columns, alphas.columns = columns, columns, columns  
    return df, alphas
...
##########################################################################################################################################################
def df_summary(df: DataFrame, **kwargs) -> Tuple[DataFrame, DataFrame, DataFrame, DataFrame]:
    """
    author: SMI
    ...
    mean = df.mean(axis=0).T.to_frame() 
    var = df.var(axis=0).T.to_frame() 
    skew = df.skew(axis=0).T.to_frame() 
    kurtosis = df.kurtosis(axis=0).T.to_frame() 
    return mean.applymap(F.format), var.applymap(F.format), skew.applymap(F.format), kurtosis.applymap(F.format)

def stats_df(df: DataFrame, df2: DataFrame, xy: List = None, **kwargs) -> DataFrame:
    """
    author: SMI
    Function outputs a data frame of summary statistics and p-values of KS test.
    Optionally it also outputs KL-divergence and MMD.
    Dependencies: df_summary, ks_testD, compare_distance.
    """
    assert isinstance(df, pd.DataFrame) and isinstance(df2, pd.DataFrame), "Data is not pandas data frame."
    def transf(df1, df2):
        if not isinstance(df1, pd.DataFrame) or not isinstance(df2, pd.DataFrame):
            df1, df2 = pd.DataFrame(df1), pd.DataFrame(df2)
        df2 = ('(' + df2.astype(str) + ')')
        df = pd.concat([df1, df2], axis = 1)
        df['column'] = df.apply( lambda x: ' '.join(x.dropna().astype(str)), axis=1)
        return df['column']
    def distance(xy):
        dist = []
        for i in xy:
    ...
    mean, var, skew, kurtosis = df_summary(df, **kwargs)
    mean2, var2, skew2, kurtosis2 = df_summary(df2,**kwargs)
    ks_df, thrsld = ks_testD(df, df2)

    mean = transf(mean.round(5), mean2.round(5))
    var = transf(var.round(5), var2.round(5))
    skew = transf(skew.round(5), skew2.round(5))
    kurtosis = transf(kurtosis.round(5), kurtosis2.round(5))
    ks = transf(ks_df.loc['pvalue'].round(3), thrsld.T.round(3))
    stats = pd.concat([mean, var, skew, kurtosis, ks], axis = 1)
    stats.columns = ['Mean', 'Variance', 'Skewness', 'Kurtosis', 'KS test']
    if xy:
        dist_df = distance(xy)
        stats = pd.concat([stats, dist_df.round(5)], axis=1)
        stats.columns = ['Mean', 'Variance', 'Skewness', 'Kurtosis', 'KS test (p-value)', 
        'KL div.', 'MMD']
    return stats.T

def table(ls: List, ys: List, xy: List = None, **kwargs) -> DataFrame:
    """
    The function is a wrapper of stats_df that outputs the dataframe with 
    statistical summaries of two distributions f_x, f_z
    """
    f_names = kwargs.get("f_names",[])
    #assert f_names == [], 'f_names must be passed in kwargs'
    tbl = []
    for i,j in zip(ls, ys):
        df_x = pd.DataFrame(i)
        df_z = pd.DataFrame(j)
        tbl += [stats_df(df_x,df_z, **kwargs)]
    if np.shape(ls)[0] > 1:
        stats = pd.concat(dict(zip(f_names, tbl)), axis=1, names=["", ""])
    else:
        stats = pd.DataFrame(tbl[0])
        stats.columns = f_names
    return stats
