...
class ts_data:
    ...
    def get_sep(**kwargs): return kwargs.get("sep",";")
    def get_csv_file(**kwargs): return kwargs.get("csv_file",None)
    def get_begin_date(**kwargs): return kwargs.get("begin_date",None)
    def get_end_date(**kwargs): return kwargs.get("end_date",None)
    def get_date_format(**kwargs): return kwargs.get("csv_format",'%d/%m/%Y')
    def get_csv_date_format(**kwargs): return kwargs.get("csv_date_format",'%m/%d/%Y %H:%M')
    def get_time_col(**kwargs): return kwargs.get("time_col","Date")
    def get_select_columns(**kwargs): return kwargs.get("select_columns",None)
    ...
    def get_yf_ts_data(**kwargs):
        ...
        index = [get_date(x, date_format=csv_date_format).strftime(date_format) for x in data.index] 
        ...
        if begin_date is not None: 
            ...
            data = data.loc[ [ ( get_float(x,date_format=date_format) >= begin_date) for x in data.index ] ]
        if end_date is not None: 
            end_date = get_float(end_date, date_format=date_format)
            data = data.loc[ [ ( get_float(x,date_format=date_format) <= end_date) for x in data.index ] ]
    ...
    def get_csv_ts_data(**kwargs):
        ...
        if begin_date is not None: 
            ...
            data = data.loc[ [ ( get_float(x,date_format = csv_date_format) >= begin_date) for x in data.index ] ]
        if end_date is not None: 
            end_date = get_float(pd.to_datetime(end_date, format=date_format))
            data = data.loc[ [ ( get_float(x,date_format = csv_date_format) <= end_date) for x in data.index ] ]
    ...
    def interpolate(data,**kwargs): return interpolate_nulls(data,**kwargs)

class time_serie_generator(data_generator):

    def get_params(**kwargs) : return kwargs.get('time_serie_generator',{})
    def get_P(**kwargs): return int(time_serie_generator.get_params(**kwargs).get("P",1))
    def get_H(**kwargs): return int(time_serie_generator.get_params(**kwargs).get("H",1))
    def get_yf_params(**kwargs) : return time_serie_generator.get_params(**kwargs).get('yf_param',None)
    ...
    def get_date_column_name(self,**kwargs): return time_serie_generator.get_params(**kwargs).get("time_id","Date")
    def get_nb_features(self,x,**kwargs): return int(x.shape[1] / time_serie_generator.get_P(**kwargs))
...
class forecast_predictor_codpy(forecast_predictor):
    ...
    def one_step_predictor(self,x,y,z,fx,**kwargs):
        ...
        return op.projection(x = x,y = y,z = z, fx = fx,**kwargs)
