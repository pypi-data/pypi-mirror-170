...
class alg:
    ...
    def get_x_z(fx,M,**kwargs):
        seed = kwargs.get("seed",42)
        rand = kwargs.get("random_sequence",np.random.rand)
        ...
        shape[0] = M
        z = np.random.random_sample(shape)
        if grid_projection: 
            x = alg.grid_projection(x)
            z = alg.grid_projection(z)
        x,fx,permutation = alg.reordering(x,fx,**kwargs)
        return x,z

    def sampler(fx, M:int,x= None, z= None,  seed = 42, **kwargs):
        from reordering import reordering_plot 
        # test = np.random.randint(fx.shape[0],size = M)
        # return fx[test]
        if x is None or z is None :
            getxz = kwargs.get('getxz',alg.get_x_z)
            x,z = getxz(fx,M,**kwargs)
            return alg.sampler(fx=fx,x=x,z=z,M=M,seed=seed,**kwargs),x,z
        # xp,fx,permutation = alg.reordering(x,fx,**kwargs)
        # reordering_plot(x,fx,xp,fx,plot_fun = graph_plot)
        fz= op.projection(x = x,y = x,z = z, fx = fx,**kwargs)

        # fxreshape = fx.reshape(fx.shape + (1,))
        # nabla_inv = op.nabla_inv(x,x,x,fxreshape,**kwargs)
        # nabla = op.nabla(x = x,y = x,z = x, fx = nabla_inv,**kwargs)
        # Leray = fxreshape - nabla
        # nabla = op.nabla(x = x,y = x,z = z, fx = nabla_inv,**kwargs)
        # Leray = Leray.squeeze()
...
class FD:
    def nabla(values,fun,set_fun=None,**kwargs): 
        import numdifftools.nd_scipy as nd
        if isinstance(values,list): return [FD.nabla(x=values[n],fun=fun,**kwargs) for n in range(0,values.shape[0])]
        if set_fun is None: set_fun = kwargs.get("setter")
        if values.ndim == 2 : 
            return np.concatenate([FD.nabla(values=values[n],fun=fun,set_fun=set_fun,**kwargs) for n in range(0,values.shape[0])], axis = 0)
        def lambda_helper(x):
            k = set_fun(x,**kwargs)
            out = fun(**k)
            return out
        out = nd.Gradient(lambda_helper)(values)
        # print("grad:",get_matrix(out).T)
        return get_matrix(out).T

    def hessian(values,fun,set_fun=None,**kwargs): 
        import numdifftools.nd_scipy as nd
        # if fun == None: fun = option_param.price
        if isinstance(values,list): return [FD.hessian(values=values[n],fun=fun,**kwargs) for n in range(0,value.shape[0])]
        if values.ndim == 2 : 
            out = np.array([FD.hessian(values=values[n],fun=fun,**kwargs) for n in range(0,values.shape[0])])
            return out
        def hessian_helper(x):
            out = FD.nabla(values=x,fun=fun,set_fun=set_fun,**kwargs)
            out = np.squeeze(out)
            return out

        out = nd.Jacobian(hessian_helper)(values)
        # print("grad:",get_matrix(out).T)
        return out.T

    def nablas(fun, values,**kwargs): 
    ...
    def hessians(fun, values,**kwargs): 
        copy_kwargs = copy.deepcopy(kwargs)
        # copy_kwargs = kwargs.copy()
        def helper(v): return FD.hessian(fun = fun, values = v,**copy_kwargs) 
        if isinstance(values,list): out = [helper(v) for v in get_matrix(values)]
        elif isinstance(values,np.ndarray): 
            if values.ndim == 1: return helper(values)
            out = [helper(values[n]) for n in range(0,values.shape[0]) ]
        elif isinstance(values,pd.DataFrame): return FD.hessians(fun=fun,values= values.values,**copy_kwargs)
        out = np.array(out).reshape(values.shape[0],values.shape[1],values.shape[1])
        return out
