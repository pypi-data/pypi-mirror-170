...
class historical_path_generator(path_generator):
    ...
    def generate_distribution_from_samples_np(samples,sample_times,**kwargs):
        ...
        N = kwargs.get("N",samples.shape[0])
        D = samples.shape[1]
        T = samples.shape[2]
        mapped_samples=np.zeros((N,D,T-1))
        ...
        x,z = alg.get_x_z(fx=xm,M = N * len(payoff_times),distance = "norm22",seed= seed, grid_projection = grid_projection)
        ...
        linear_part = op.projection(x=x,y=x,z=x,fx=xm,set_codpy_kernel=linear_kernel,rescale=True)
        quadratic_part = xm - linear_part
        linear_part = op.projection(x=x,y=x,z=z,fx=xm,set_codpy_kernel=linear_kernel,rescale=True)
        ...
        # out = alg.sampler(fx= test, M = N * len(payoff_times), distance = 'norm22', **kwargs)
        ...
        diffusion_part = alg.sampler(fx= quadratic_part, x=x,z=z,M = N * len(payoff_times), set_codpy_kernel = set_codpy_kernel, rescale = True)
        ...
        return diffusion_part,linear_part,mapped_samples[0,1:,:].T
    ...
    def generate_from_samples(self,samples,sample_times,**kwargs):
        ...
        def generate_from_samples_np(samples,sample_times,**kwargs):
            diffusion_part,linear_part,fx = historical_path_generator.generate_distribution_from_samples_np(samples,sample_times,**kwargs)
            ...
            def helper(n) : f_z[n,1:,:] = out[:,n*T:(n+1)*T]
