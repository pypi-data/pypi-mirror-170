...
class data_random_generator(data_generator):
    ...
    def get_box(x):
        if isinstance(x,list):
            temp = [np.asarray(data_random_generator.get_box(y)) for y in x]
        ...
        x = get_matrix(x)
        if x.shape[1] == 1 : return [np.min(x),np.max(x)]
        out = [data_random_generator.get_box(x[:,d]) for d in range(0,x.shape[1])]
        return out
        ...
        def get_array(N,D,min,max,**kwargs):
            ...
            if type == "sto" or D>=5: 
                ...
                if minmax is not None :
                    out = np.zeros((N,D))
                    def helper(d): out[:,d] = np.random.uniform(minmax[d,0],minmax[d,1],size=N)
                    [helper(d) for d in range(0,D)]
                    return out
                return np.random.uniform(min, max, size = (N,D))
            v = [np.arange(start=min,stop=max+0.00001,step = (max-min) / ( int(N**(1/D) )) )]
...
class credit_card_fraud_data_generator(raw_data_generator):
    ...
    def get_raw_data(self,**kwargs):
        ...
        path_to_file = os.path.join(data_path, "creditcard.csv")
        ...
        if not exists(path_to_file): kaggle_api_file(data_set="mlg-ulb/creditcardfraud",path_to_file = data_path, unzip = True)
...
if __name__ == "__main__":
    credit_card_fraud_data_generator()
