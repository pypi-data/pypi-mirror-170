...
class basket_option_param_getter(option_param_getter):
    ...
    def get_spot(self,**kwargs): return [option_param_getter().get_spot(**k) for k in self.get_processes_param(**kwargs)]
    def set_spot(self,values,**kwargs): 
        new_spots = [option_param_getter().set_spot(**k,value = v) for k,v in zip(self.get_processes_param(**kwargs),values)]
...
class BasketOption(payoff):
    ...
    def basket_value(self,weights,values):
        import itertools
...
class BasketOptionClosedFormula(OptionClosedFormula):
    ...
    def get_spot_basket(self,x,**kwargs):
        ...
        if weights is None: weights = np.array(getter.get_weights(**kwargs))
        # x = kwargs.get("x")
        if isinstance(x,pd.DataFrame):x = x.loc[:,x.columns != "Date"]
        if x.ndim == 2:
            out = [self.get_spot_basket(x=x[:,n],weights= weights) for n in range(0,x.shape[1])]
            return np.array(out)
        if x.ndim == 3:
            out = np.zeros((x.shape[0],x.shape[2]))
            def helper(n) : out[n] = self.get_spot_basket(x=x[n],weights= weights)
    ...
    def nabla(self,**kwargs):
        ...
        theta = np.array([european_option.theta()])
    ...
    def nablas(self, values,**kwargs): 
        ...
        set_fun = kwargs.get("setter",self.getter())
