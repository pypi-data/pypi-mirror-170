...
def RegenerateHistory(**kwargs):
    ...
    f_z = historical_path_generator().generate_from_samples(samples[:,:,:],sample_times=timelist,initial_values = initial_values,time_start=time_start,**kwargs)

    return f_z,data,samples

def plot_trajectories(fz,fx,data):
    paths=pd.DataFrame(fz[:,2,:].T)
    paths.set_index(data.index,inplace=True)
    paths.plot(colormap='tab20b')
    paths['Real Google'] = fx[:,2,:].T
    # paths.plot(colormap='tab20b',y=[i for i in range(fz.shape[0]+1)])
    paths['Real Google'].plot(color='red',legend='Real Google',linewidth=3)
