...
global_param = {
    'begin_date':'01/06/2021',
    'end_date':'01/06/2022',
    'yf_begin_date': '2021-06-01',
    'yahoo_columns': ['Close'],
    'H' : 1, #Shift
    'P' : 1,
    'symbols' : ["^GSPC"],
...
def plot_train_test_variables(results,**kwargs):
    ...
    minmax = np.array(data_random_generator.get_box([generated_data,historical_data]))
...
def plot_confidence_levels(param,**kwargs):
    import itertools
    fun = kwargs['getter'].get_closed_formula().get_spot_basket
    training_set,test_set = get_matrix(param[0]),get_matrix(param[1])

    ax = get_ax_helper(**kwargs)

    minmax = np.array(data_random_generator.get_box([test_set,training_set]))
    ...
    xb2,yb2,xt2,yt2 = test_set[:,0], fun(x=test_set[:,1:].T,**kwargs),training_set[:,0], fun(x=training_set[:,1:].T,**kwargs)
