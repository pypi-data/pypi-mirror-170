...
@click.command('davpoint', epilog="Report bugs to <devel@incenp.org>.")
@click.option(
    '--config',
    '-c',
    'config_file',
    type=click.Path(exists=True),
    default='{}/.davfs2/sharepoint.conf'.format(os.getenv('HOME', default='.')),
...
@click.option(
    ...
    default=None,
    help="Write authentication reply to the specified file.",
)
@click.version_option(version=__version__, message=prog_notice)
@click.argument('share')
def main(config_file, debug_auth, share):
    ...
    cookies = authenticate(endpoint, username, password, debug_file=debug_auth)
    davfs2_options['add_header'] = (
        f'Cookie rtFa={cookies["rtFa"]};' f'FedAuth={cookies["FedAuth"]};'
    )
