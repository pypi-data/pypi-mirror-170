...
from .hyper_log_log import HyperLogLog # 自己寫的

# log time helper function
def _logtime(ts):
...
# construct iterator for memory saving
...
class Adhoc:
    ...
    async def _read_point(self, ts, series, columns=None, refresh=True):
        ...
        start = pd.Timestamp(ts).floor(freq_str) # force align
        end = start + delta
        table = await self._tenant.execute_adhoc(self.pid, _get_utc_timestamp(start), _get_utc_timestamp(end), series)
        kcol, mcol = _compute_kcol_mcol(self.pipeline) # move to constructor
...
class Dataset:
    ...
    async def patch(self, dts, overwrite=False):
        ...
        tasks = []
        tsidx = []
        # TODO: should support timestamp or iterable
        for ts in dts:
            start = ts.floor(freq_str)
