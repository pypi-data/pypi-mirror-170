...
GB("StreamReader", desc=json.dumps(desc_json)). \
    aggregate([],
              ...
              lambda a, r: a + r). \
    foreach(lambda x: tracker_log(f'{inspect.getsourcefile(transform)} {inspect.signature(transform)}', f'DEBUG - {job.get("name")} - ')). \
    map(lambda records: transform(
        records=records,
        number_of_rotated_keys=job.get('number_of_rotated_keys'),
        dimension=job.get('dimension')
    )). \
    foreach(lambda x: tracker_log(f'{inspect.getmodule(load_pageviews)} {inspect.signature(load_pageviews)}', f'DEBUG - {job.get("name")} - ')). \
