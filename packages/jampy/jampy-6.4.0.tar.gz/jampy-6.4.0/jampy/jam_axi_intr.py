"""
    Copyright (C) 2019-2022, Michele Cappellari
...
def integrand_cyl(u, R, z,
    ...
    cani = 1 - gamma[:, None, None] if np.ptp(gamma) else 1 - gamma[0]       # Anisotropy ratio c = (sig_phi/sig_R)**2
...
def intrinsic_moments_cyl(R, z,
    ...
    sig2R, sig2z, sig2phi, v2phi = \
        [quad1d(integrand_cyl, [0, 1], epsrel=epsrel, epsabs=0, args=args+[txt]).integ/nu
...
def intrinsic_moments_sph(R, z,
    ...
    v2phi += quad1d(integand_tan_dth_pot, xlim, epsrel=epsrel, epsabs=0, singular=0,
