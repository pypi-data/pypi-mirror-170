...
class URLUtils:
    ...
    def _file_mtime(self,
        ...
        """
        def regex_stat_error(stderr):
            search = re.search("\'(.*)\'", stderr)
            if search is None:
                return None
        ...
        def stat():
            ...
            times = [int(t) for t in ret.stdout.split('\n') if t != '']
            error = [regex_stat_error(e) for e in ret.stderr.split('\n')]

            return times, error, ret.returncode

        def pystat():
            ex = f"import os; files={files} " \
                 f"\nfor f in files: " \
                 f"\n\ttry: print(os.stat(f).st_mtime) " \
    ...
    def ls(self,
           ...
           as_list: bool = True,
           local: bool = None,
           raise_errors: bool = True) -> [CMD, list]:
        """
