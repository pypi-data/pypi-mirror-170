...
public:
  ...
  bool VpiIncludedFile(const std::string& data);

  const std::string& VpiIncludedFile() const;

  virtual unsigned int VpiType() const final { return vpiIncludeFileInfo; }

  virtual include_file_info* DeepClone(Serializer* serializer, ElaboratorListener* elaborator, BaseClass* parent) const override;
  ...
  virtual  UHDM_OBJECT_TYPE UhdmType() const final { return uhdminclude_file_info; }

protected:
  void DeepCopy(include_file_info* clone, Serializer* serializer,
...
private:
  ...
  SymbolFactory::ID vpiIncludedFile_ = 0;
};
