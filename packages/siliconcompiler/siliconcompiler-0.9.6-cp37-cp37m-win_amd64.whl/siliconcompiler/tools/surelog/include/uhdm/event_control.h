...
#ifndef UHDM_EVENT_CONTROL_H
#define UHDM_EVENT_CONTROL_H
...
class event_control final : public atomic_stmt {
  UHDM_IMPLEMENT_RTTI(event_control, atomic_stmt)
public:
  // Implicit constructor used to initialize all members,
  // comment: event_control();
  virtual ~event_control() final = default;
  ...
  bool VpiCondition(any* data) { if (!expr_sequence_inst_named_event_groupGroupCompliant(data)) return false; vpiCondition_ = data; return true; }

  const any* Stmt() const { return stmt_; }

  bool Stmt(any* data) { if (!stmtGroupCompliant(data)) return false; stmt_ = data; return true; }

  virtual unsigned int VpiType() const final { return vpiEventControl; }

  virtual event_control* DeepClone(Serializer* serializer, ElaboratorListener* elaborator, BaseClass* parent) const override;
  ...
  virtual  UHDM_OBJECT_TYPE UhdmType() const final { return uhdmevent_control; }

protected:
  void DeepCopy(event_control* clone, Serializer* serializer,
