...
class State:
    ...
    def __eq__(self, __o: object) -> bool:
        return self.features == __o.features
...
class MCTNode(NodeMixin):
    ...
    def __init__(
        self, state: State, tag: Any = None, parent=None, children=None
    ) -> None:
...
class MCTS():
    ...
    def __init__(
        self, root: MCTNode, c: Union[int, float], max_iter: int, max_height:int=None, debug=False
    ) -> None:
        super().__init__()
        self._root = root
        self._c = c
        ...
        try:
            if node.visits >= 1:
                if node.expanded == False:
                    node._expanded = True
                    new_states, tags = discover_func(node)
                    for index, state in enumerate(new_states):
                        new_node = MCTNode(state=state, tag=tags[index], parent=node)
                    if len(new_states) == 0:
                        if self.debug:
                            logger.info(
                                f"Node {self.current_chosen_node.state} is a leaf, can not be expanded."
                            )
                    else:
                        if self.debug:
                            logger.info(
                                f"Node {self.current_chosen_node.state} is expanded by {len(new_states)} nodes."
                            )
                else:
                    if self.debug:
                        logger.info(
                            f"Node {self.current_chosen_node.state} has been expanded previously."
                        )
    ...
    def get_reward(self, node: MCTNode, reward_func: Callable):
        ...
        try:
            if node.rewarded is False:
                ...
                if self.debug:
                    logger.info(
                        f"Node {node.tag} receives reward {node.reward}."
                    )
        except Exception as e:
            logger.error(e)
    ...
    def terminate(self) -> bool:
        ...
        if all_nodes_explored or reached_max_iterations or reached_max_heights:
            if self.debug:
