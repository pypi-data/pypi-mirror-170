...
class Commands(object):
    ...
    def gcmd(self, product, cmdtype):
        ...
        assert cmdtype in ["aoc", "lang", "moc", "pud"]
...
class Product(object):
    ...
    def detectb(self):
        ...
        self.__write(" ")
        self.wait(0.05)
        if len(self.s.read_all()) > 0:
            self.__write("v")
            self.wait(0.05)
            print(self.s.read_all())
            #FC350 /FC360
            print(self.getback("dtm moc readmem -l -a 0x0800E036"))
            #Aoc hardware version
            print(self.getback("dtm read_version -v 18"))
            #MCP hardware version
            print(self.getback("dtm moc readmem -s -a 0xE0042000"))
            #Power size
            print(self.getback("dtm moc readmem -b -d -a 0x0800E010"))
            print(self.getback("dtm moc readmem -b -d -a 0x0800800E"))
            return True
        else:
            return False
    ...
    @serialsetting
    def wait_redboot(self, timeout=10):
        rr = b""
        retry = 0
    ...
    def __ysend(self, file_path):
        ...
        def callback(total_packets, success_count, error_count):
            print("Total:{} success:{} error:{}".format(
