...
from ..connectors.mssql import get_conn_mssql
from ..connectors.postgres import get_conn_pg


class NPGBQ(object):
    def __init__(
        self, project_id, gcp_service_account_path: Optional[str] = None
    ):
    ...
    # ====================================== Private method ======================================
    def get_valid_colname(self, text: str):
        val = self.remove_double_space(text)
        val = val.replace(" ", "_").replace("-", "_").replace(".", "_")
        val = val.upper()
        val = re.sub("[^A-Za-z0-9_]+", "", val)
        if re.search(r"^\d", val):
            val = f"_{val}"
        val = self.remove_double_underscores(val)
        return val

    def remove_double_space(self, text: str) -> str:
        regex = re.compile(r"\s+")
        return re.sub(regex, " ", text)

    def remove_double_underscores(self, text: str) -> str:
    ...
    def __validate_col_type(self, col_type: str) -> bool:
        ...
        if col_type not in valid_col_type:
            raise ValueError(
                f"the input column datatype {  col_type} is not valid"
            )
        return True

    def __get_full_qualified_table_name(self, dataset_name, table_id):
        full_qualified_id = f"{self.client.project}.{dataset_name}.{table_id}"
    ...
    def create_backup_table(self, dataset_name, table_name):
    ...
    def merge_table(self, dataset_id, table_src, table_dst, col_key, columns):
        table_src = self.__get_full_qualified_table_name(
            dataset_name=dataset_id, table_id=table_src
        )
        table_dst = self.__get_full_qualified_table_name(
            dataset_name=dataset_id, table_id=table_dst
        )
    ...
    def convert_dtype(
        self, df: pd.DataFrame, schema_bq: List[bigquery.SchemaField]
    ):
        for sch in schema_bq:
            print(f"Converting: {sch.name}")
            data_type = sch.field_type
            if data_type in ("NUMERIC", "BIGNUMERIC"):
                df[sch.name] = df[sch.name].apply(self.convert_to_decimal)
                ...
                df[sch.name] = pd.to_datetime(
                    df[sch.name], errors="coerce"
                ).dt.date
            elif data_type in ("TIME"):
                df[sch.name] = df[sch.name].apply(self.convert_time)
    ...
    def create_external_table_from_gcs(
        self, table_id: str, schema, gcs_uri
