...
def stringTester(testString):
    matched = re.fullmatch('[a-z0-9]([-a-z0-9]*[a-z0-9])', testString)
    return(bool(matched))
...
#region Namespace creation
    ...
    try:
        ...
        logging.error(e, exc_info=True)
        printError(f"ERROR. Deployment failed. Attempt to create user namespace failed with error: {writeModels.text} {e}")
        return
#endregion
...
def main():
    ...
    parser.add_argument("--keep_alive", help="How many hours the model should remain active without any usage. Anything greater than 48 hours may incur additional cost. You will experience a cold start once this time period expires. Defaults to 48", required=False, default=48, type=int)
