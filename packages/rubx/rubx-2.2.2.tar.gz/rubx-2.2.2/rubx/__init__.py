...
class Robot:
    def __init__(self, app='rubx', phone_number=None, auth=None, device=defaultDevice, proxy={'http':'http://127.0.0.1:9050'}, your_name='rubx', city='tehran', logo=True) :
        self.app = app
        citys.append(city)
        self.proxy = proxy
        proxys.append(proxy)
        if logo != False:
            [(print(s, flush=True, end=''), sleep(0.01)) for s in f'\n\033[0m< \033[31mrubx \033[0m> \033[36m | \033[31mstarted in \033[0m{str(datetime.datetime.now())}\n']
        if your_name != None:
    ...
    def sendCode(phone_number: str, send_type='SMS', password=None) -> dict:
        ...
        tmp = Robot._tmpGeneration(); enc = encryption(tmp)
