...
def ingest(log, force=False):
    ...
    for item in log.parsed_output:
        # Parsing
        # https://github.com/networktocode/ntc-templates/tree/master/tests/cisco_ios/show_lldp_neighbors_detail
        device_o = log.discoverable.device
        discoverable_o = log.discoverable
        interface_name = item['local_interface']
        if item["neighbor_interface"]:
            remote_interface_name = item["neighbor_interface"]
        else:
            remote_interface_name = item["neighbor_port_id"]
        remote_address = item['management_ip']
        if item["neighbor"]:
            remote_device_name = item["neighbor"]
        else:
            remote_device_name = item["chassis_id"]
