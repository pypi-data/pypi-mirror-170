...
def ingest(log, force=False):
    ...
    for item in log.parsed_output:
        ...
        if item['master']:
            vrf_o = functions.set_get_vrf(name=item['master'], create_kwargs={})
        ...
        addresses = []
        addresses_text = []
        for index, ipaddr in enumerate(item["ip_addresses"]):
            addresses.append(ipaddress.IPv4Interface(f'{ipaddr}/{item["ip_masks"][index]}'))
            addresses_text.append(str(ipaddress.IPv4Interface(f'{ipaddr}/{item["ip_masks"][index]}')))
