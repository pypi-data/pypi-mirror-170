...
class ContainerLauncher:
    ...
    __SPOT_INSTANCE_TERMINATION_TIMEOUT_IN_MINUTES = 25
    ...
    def start_container(self, container_input):
        ...
        try:
            ...
            if container_input.get('instance_type'):
                ...
                instance_type = ec2.get_optimal_instance_type(get_optimal_instance_type_input)
                self.logger.debug(f'The optimal instance type is {instance_type}')
            ...
            ec2_iam_role_arn = self.__create_ec2_iam_role(iam_ec2_policy_documents)
            ...
            self.__add_role_to_instance_profile()
            ...
            self.__create_ecs_task_definition(ecs_task_definition_data, instance_type)
            ...
            logs = {
                ...
                'log_stream': f"{self.__ECS_TASK_LOG_STREAM_PREFIX}-{self.__options['name_suffix']}"
            }
    ...
    def __create_ec2_iam_role(self, policy_documents=None):
        name_suffix = self.__options['name_suffix']
        ...
        for managed_policy_arn in self.__IAM_EC2_MANAGED_POLICY_ARNS:
    ...
    def __destroy(self):
        ...
        try:
            ...
            if self.__resources.get('ec2_iam_role_name') and ec2_iam_role_policy_document_names and len(ec2_iam_role_policy_document_names) > 0:
                ...
                self.logger.debug(f"The IAM policies {str(self.__resources['ec2_iam_role_policy_document_names'])} has been deleted")
        except Exception as e:
        ...
        try:
            ...
            self.logger.warning(f"An error occurred during the {self.__resources['ec2_iam_role_name']} delete_role operation: {str(e)}")
        try:
        ...
        try:
            ...
            self.logger.warning(f"An error occurred during the {self.__resources.get('ecs_task_iam_role_name')} delete_role operation: {str(e)}")
        try:
            ...
            self.logger.warning(f"An error occurred during the {self.__resources.get('ecs_task_execution_iam_role_name')} delete_role operation: {str(e)}")
    ...
    def __add_role_to_instance_profile(self):
        self.logger.debug(
            f"Adding the {self.__resources['ec2_iam_role_name']} EC2 IAM role to the {self.__resources['instance_profile_name']} instance profile")
    ...
    def __create_ecs_task_definition(self, task_def_opts, instance_type):
        name_suffix = self.__options['name_suffix']
        ...
        response = self.clients['ecs'].register_task_definition(
            ...
            taskRoleArn=self.__resources['ecs_task_iam_role_name'],
            executionRoleArn=self.__resources['ecs_task_execution_iam_role_name'],
            networkMode='host',
            containerDefinitions=[
                {
                    ...
                    'logConfiguration': {
                        ...
                        'options': {
                            ...
                            'awslogs-stream-prefix': f'{self.__ECS_TASK_LOG_STREAM_PREFIX}-{name_suffix}'
                        },
                    },
    ...
    def __wait_for_instance_deploying(self, is_spot):
        ...
        self.logger.info('Waiting for an EC2 instance deploying')

        timeout_in_minutes = self.__SPOT_INSTANCE_TERMINATION_TIMEOUT_IN_MINUTES if is_spot else self.__INSTANCE_TERMINATION_TIMEOUT_IN_MINUTES
        ...
        while True:
            ...
            if diff_minutes > timeout_in_minutes:
                raise Exception('EC2 instance deploying timeout')
            if len(instances) > 0:
            ...
            time.sleep(10)
            instances = self.__get_asg_instances()
            self.logger.debug('Waiting for an EC2 instance deploying')
    ...
    def __create_launch_template(self, launch_template_opts):
        ...
        is_spot = launch_template_opts.get('is_spot', False)
...
yum install jq -y
mounts=($(lsblk --fs --json | jq -r '.blockdevices[] | select(.children == null and .fstype == null) | .name'))
for i in "${mounts[@]}" ; do echo -e "o\\nn\\np\\n1\\n\\n\\nw" | fdisk /dev/$i ; done
mounts_count=${#mounts[@]}
dev_mounts=""
for i in "${mounts[@]}" ; do dev_mounts="$dev_mounts /dev/$i" ; done
