...
def get_preferred_locale():
    ...
    The preferred locale has to be valid, i.e. it has to be configured in the
    application's configuration in ``LOCALES``.

    :return: The preferred locale. If no matching locale could be found,
        ``LOCALE_DEFAULT`` will be returned as configured in the application's
        configuration.
    """
    default_locale = current_app.config["LOCALE_DEFAULT"]
    locales = list(current_app.config["LOCALES"].keys())

    return request.accept_languages.best_match(locales, default=default_locale)
...
def download_bytes(data, download_name, mimetype=None, as_attachment=True):
    ...
    if mimetype is None:
        mimetype = guess_type(download_name)[0] or const.MIMETYPE_DEFAULT
...
def qparam(
    ...
    parse = parse if isinstance(parse, list) else [parse]
