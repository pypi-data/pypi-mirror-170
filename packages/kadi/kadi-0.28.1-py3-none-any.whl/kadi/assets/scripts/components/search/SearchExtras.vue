...
kinds of values based on the selected types. Multiple such queries can be combined with an 'AND' or an 'OR' operation \
in the form of '(Q1 AND Q2) OR (Q3 AND Q4)'. Exact matches for keys and string values can be required by using double \
quotes, e.g. "key".`
              ) }}
              <hr class="my-1">
              {{ $t(
                `Note that keys inside of nested metadata entries are indexed in the form of 'key_1.key_2'. In case of \
list entries, keys are replaced by the corresponding index in the list instead, starting at 1.`
              ) }}
            </template>
          </popover-toggle>
  ...
  data() {
    return {
      ...
      selectors: {
        ...
        dateRanges: {gt: $t('Later'), lt: $t('Before'), bt: $t('Between')},
        numRanges: {gt: $t('Greater'), lt: $t('Less'), bt: $t('Between')},
      },
    };
  },
  props: {
