...
export default {
  ...
  methods: {
    loadWorkbook() {
      ...
      axios.get(this.sheetUrl, {responseType: 'arraybuffer'})
        .then((response) => {
          this.workbook = read(response.data, {
            cellDates: true,
            cellFormula: false,
            cellHTML: false,
            cellText: false,
        ...
        .catch((error) => kadi.alerts.danger($t('Error loading workbook.'), {request: error.request}))
        .finally(() => this.loading = false);
    },
    loadSheet(name) {
      if (!this.sheets[name]) {
        this.sheets[name] = utils.sheet_to_json(this.workbook.Sheets[name], {
          blankrows: true,
