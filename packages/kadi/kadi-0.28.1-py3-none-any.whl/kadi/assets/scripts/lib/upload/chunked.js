...
import BaseUpload from 'scripts/lib/upload/core.js';
  ...
  async upload(upload) {
    ...
    try {
      ...
      if (error.request.status === 413) {
        ...
        kadi.alerts.warning(error.response.data.description);
      } else {
        kadi.alerts.danger($t('Error finishing upload.'), {request: error.request});
      }
    ...
    while (true) {
      ...
      while (true) {
        ...
        try {
          ...
          kadi.alerts.danger(`${errorMsg} ${timeoutMsg}`, {request: error.request, timeout});
      ...
      axios.get(upload.getStatusEndpoint)
        .then((response) => {
          ...
          if (data._meta.file) {
            ...
            kadi.alerts.danger(data._meta.error);
        ...
        .catch((error) => {
          kadi.alerts.danger($t('Error updating upload status.'), {request: error.request});
        });
    };
