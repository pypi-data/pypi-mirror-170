...
def view_information():
    ...
    num_users = User.query.filter(
        User.state == UserState.ACTIVE, User.new_user_id.is_(None)
    ).count()

    num_records = Record.query.filter(Record.state == RecordState.ACTIVE).count()
    ...
    num_groups = Group.query.filter(Group.state == GroupState.ACTIVE).count()
    ...
    if qparams["tab"] == "customization" and customization_form.validate_on_submit():
        ...
        if qparams["action"] == "test_email":
            if send_test_mail(
                ...
                success(_("A test email has been sent."))
            else:
                danger(_("Could not send test email."))
