...
def search_records(
    ...
    if extra_queries:
        ...
        # matter.
        for query_dict in extra_queries:
            es_query = _dict_to_query(query_dict)

            if es_query:
                if query_dict.get("link") == "or":
                    if and_relations:
...
def _make_extra_key_query(extra_type, extra_key):
    ...
    if is_quoted(extra_key):
        ...
        should_query.append(Query("match", **{f"extras_{extra_type}.key": extra_key}))

    should_query.append(
        Query("term", **{f"extras_{extra_type}.key.keyword": extra_key})
    )
...
def _make_nested_extra_key_query(extra_type, extra_key):
    ...
    if is_quoted(extra_key):
        ...
        should_query.append(
            Query(
                ...
                query=Query("match", **{f"extras_{extra_type}.key": extra_key}),
            )
        )
    ...
    should_query.append(
        Query(
            "nested",
            path=f"extras_{extra_type}",
            query=Query("term", **{f"extras_{extra_type}.key.keyword": extra_key}),
        )
    )
...
def _dict_to_query(query_dict):
    ...
    if extra_type == "str":
        ...
        if str_value:
            ...
            if is_quoted(str_value):
                ...
                should_query.append(Query("match", extras_str__value=str_value))

            should_query.append(Query("term", extras_str__value__keyword=str_value))
    ...
    if extra_type == "numeric":
        ...
        if min_value:
            int_query.append(Query("range", extras_int__value={"gt": min_value}))
            float_query.append(Query("range", extras_float__value={"gt": min_value}))

        if max_value:
            int_query.append(Query("range", extras_int__value={"lt": max_value}))
            float_query.append(Query("range", extras_float__value={"lt": max_value}))
    ...
    if extra_type == "date":
        ...
        if min_value:
            date_query.append(Query("range", extras_date__value={"gt": min_value}))

        if max_value:
            date_query.append(Query("range", extras_date__value={"lt": max_value}))
