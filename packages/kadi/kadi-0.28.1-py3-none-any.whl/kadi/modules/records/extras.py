...
class _ValidationSchema(KadiSchema):
    """Schema to represent the optional validation instructions of extras."""
    ...
    def _flatten_errors(self, errors):
        ...
        # always expect them to be a flat list of strings per top-level "extra
        # property".
    ...
    def _validates_schema(self, data, **kwargs):
        # We currently only need to validate/convert the options further, so skip this
        # validation if they were not given or invalid.
        ...
        for option in data["options"]:
            try:
                value = _validate_extra_value(option, self.value_type)
                options.add(value)

            except ValueError as e:
                raise ValidationError(
                    _(
                        "Not all values are a valid %(type)s.",
                        type=pretty_type_name(self.value_type),
                    ),
                    "options",
                ) from e
...
class ExtraSchema(KadiSchema):
    ...
    def _apply_validation(self, extra, errors, index):
        ...
        if validation.get("required", False) and not self.is_template and value is None:
            self._add_validation_error(errors, index, "value", _("Value is required."))

        # Handle the "options" validation. None values always pass it, if that is not
        # desirable, "required" can be used in addition.
        if (
    ...
    def _validates_schema(self, data, many, **kwargs):
        ...
        for index, extra in enumerate(data):
            # When the extra is completely empty, the input type was likely invalid, so
            # we skip our custom validation.
            if not extra:
                continue
            ...
            if value_type in ["str", "int", "float", "bool", "date", "dict", "list"]:
                if is_nested_type(value_type):
                    ...
                    try:
                        ...
                        validation_errors[index] = {"value": e.messages}
                ...
                self._add_validation_error(
                    validation_errors, index, "type", _("Invalid value.")
                )
            ...
            if key in prev_keys:
                self._add_validation_error(
                    validation_errors, index, "key", _("Duplicate value.")
                )
...
# Provide translations for all internal Marshmallow errors that can realistically occur
# when using the extras form field.
ERROR_TRANSLATIONS = {
        ...
        for index, extra in enumerate(extras):
            ...
            for key, value in errors.get(index, {}).items():
                ...
                if isinstance(value, list) and key in extra_formdata:
                    for error in value:
                        error = ERROR_TRANSLATIONS.get(error, error)
