    ...
    if request.method == "POST":
        if form.validate():
            ...
            if group:
                ...
                success(_("Group created successfully."))
                return redirect(url_for("groups.view_group", id=group.id))

        danger(_("Error creating group."))
        ...
        if role_rule:
            ...
            if rules_form.retroactive.data:
                if not start_apply_role_rules_task(role_rule=role_rule):
                    danger(_("Could not apply rule retroactively."))
...
def delete_group(id):
    ...
    success(_("Group deleted successfully."))
