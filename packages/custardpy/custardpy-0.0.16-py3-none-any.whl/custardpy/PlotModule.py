...
def drawHeatmapSquare(plt, matrix, resolution, *, tads="", loops="",
    ...
    if (isinstance(tads, pd.DataFrame)):
        for tad in tads.itertuples(name=None):
            ...
            if x1 > 0:
                plt.vlines(x1, x1, min(x2, e-s), linestyle='dashed', linewidth=0.3)
            if x2 < e-s:
                plt.vlines(x2, max(0, x1), x2, linestyle='dashed', linewidth=0.3)
            if x1 > 0:
                plt.hlines(x1, x1, min(x2, e-s), linestyle='dashed', linewidth=0.3)
            if x2 < e-s:
                plt.hlines(x2, max(0, x1), x2, linestyle='dashed', linewidth=0.3)
    plt.xlim(0,e-s)
    plt.ylim(0,e-s)
