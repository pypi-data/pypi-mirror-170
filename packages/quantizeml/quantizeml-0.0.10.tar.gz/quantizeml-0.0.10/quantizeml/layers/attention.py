...
@tf.keras.utils.register_keras_serializable()
class QuantizedAttention(Calibrable, Attention):

    def __init__(self, num_heads, softmax='shiftmax', quant_config={}, **kwargs):
        if softmax != 'shiftmax':
            raise ValueError(
        ...
        self.values_shift = CalibrableVariable()
    ...
    def call(self, inputs, training=None):
        ...
        # Separate 2D embeddings per head to obtain 3D inputs
        # Note: values is allowed to be quantized per-axis, so it must be aligned
        query = self.separate_heads(inputs[0])
        key = self.separate_heads(inputs[1])
        value, vshift = inputs[2].promote(self.buffer_bitwidth).align()
