...
def shiftmax(logits, axis=-1):
    ...
    p' = 2^y/sum(2^y) = 2^(logits-x0)/sum(2^(logits-x0))
       = (2^logits * 2^-x0)/(2^-x0 * sum(2^logits)) = 2^logits/sum(2^logits) = p
    ...
    Returns:
        :obj:`tf.Tensor` value of shiftmax function with the same type and
            shape as `logits`.
...
@tf.keras.utils.register_keras_serializable()
class QuantizedShiftmax(Calibrable, keras.layers.Layer):
    ...
    make it more hardware friendly, we also approximated the sum(2^x_k) to the
    closest power of two:

        shiftmax(x_i) = 2^x_i / 2^round(log2(sum(2^x_k)))
