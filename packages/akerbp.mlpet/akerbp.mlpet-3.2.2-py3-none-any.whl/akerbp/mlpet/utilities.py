    ...
    for item in form_or_group_or_system.unique():
        ...
        try:
            ...
            if dic["LEVEL"] == "FORMATION":
                ...
                if " GP" in dic["PARENT"]:
                    group = dic["PARENT"]
                if " SY" in dic["GRANDPARENT"]:
                    siistem = dic["GRANDPARENT"]
...
def normalize_group_formation_system(df_, add_systems=False):
    """
...
def get_vertical_depths(
    ...
    for well in well_names:
        ...
        well_df_discrete = client.sequences.data.retrieve_dataframe(
            id=well_data_cdf[0].id, start=0, end=None
        )
...
def get_cognite_client(
    ...
    if cognite_api_key is None:
        try:
            ...
            try:
                ...
                cognite_api_key = config.api_keys["data"]
            except ImportError:
                # akerbp.mlops is not installed so ignore
                pass
            except KeyError:
                # Could not find the data key in config try to use whichever
                # key is available
                v = config.api_keys.values()
                if v:
                    cognite_api_key = v[0]
    ...
    if cognite_api_key is None:
        raise ValueError(
            "No API key provided, none could be imported from akerbp.mlops and no environment variable "
            "COGNITE_API_KEY was found!"
        ) from e

    # Assuming COGNITE_API_KEY is set in the environment
    credentials = APIKey(cognite_api_key)
...
def run_CDF_function(
    ...
    in_data = {
        "input": [
            {
                "well": k,
                "input_logs": v.to_dict(orient="list"),
