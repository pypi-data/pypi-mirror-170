...
:ref:`blasr <about-blasr>` on the input BAM file, each one producing
...
Before running :ref:`blasr <about-blasr>` the program will try to find the
two aligned versions of the input file, *if it is* unaligned.
...
.. option:: -b <PATH>, --blasr-path <PATH>
   ...
   Path to the :ref:`blasr program <about-blasr>`. If not given, the program
   is expected to be found in :term:`PATH`.


.. option:: -p <PATH>, --pbindex-path <PATH>

   Path to the :ref:`pbindex program <about-pbindex>`. If not given, the
   program is expected to be found in :term:`PATH`.
...
.. option:: -c <PATH>, --ccs-path <PATH>
   ...
   Path to the :ref:`ccs program <about-ccs>`. If not given, the program is
   expected to be found in :term:`PATH`.
...
.. option:: --nprocs-blasr <INT>
   ...
   Number of worker processes that each instance of :ref:`blasr <about-blasr>`
